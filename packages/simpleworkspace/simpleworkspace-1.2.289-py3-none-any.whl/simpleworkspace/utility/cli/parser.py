...
class CLIParser:
    ...
    @classmethod
    def _MapNativeParser(cls, parser:_argparse.ArgumentParser, template:_Type, namespace:list[str]=None):
        subparsers = None
        #the arguments should be mapped directly on static class level
        for key,argument in template.__dict__.items():
            ...
            if(argument.template is not None): #subparser argument type
                ...
                cls.__SubGroupNamespaces__.append(KeyNS)
                cls._MapNativeParser(subParser, argument.template, namespace=KeyNS)
                continue
    ...
    def Parse(cls, arguments:_Sequence[str]=None, ignoreUnkownArguments=False, description:str=None, add_help=True) -> '_typing.Self':
        ...
        cls.__SubGroupNamespaces__ = []
        cls._MapNativeParser(nativeParser, cls)
        ...
        for keyNS in cls.__SubGroupNamespaces__:
