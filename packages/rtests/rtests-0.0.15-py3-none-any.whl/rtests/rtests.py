...
def r_wilcoxsign_test(groupA, groupB, print_result=False, __paired_groups=True):
    ...
    if __paired_groups:
        ...
        pattern_test1 = r"W\s*=\s*(\d+\.?\d*),\s*p-value\s*=\s*(\d+(\.\d+)?([eE][+-]?\d+)?)"
    ...
    report = f"${'W' if not __paired_groups else 'U'} = {v_or_w_value}, Z = {z_value}, p = {p_value}{effect_size}$"

    key = "w_statistics" if not __paired_groups else "u_statistics"

    return {key: v_or_w_value, 'z_statistics': z_value, 'p_value': p_value,
