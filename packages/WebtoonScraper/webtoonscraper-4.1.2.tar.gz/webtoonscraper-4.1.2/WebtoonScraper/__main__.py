...
from WebtoonScraper.processing import BatchMode, concat_webtoon
from WebtoonScraper.processing import _webtoon as webtoon
...
download_subparser.add_argument(
    ...
    help=(
        ...
        f"Supported platforms: {', '.join(webtoon.platforms)}"
    )
)
download_subparser.add_argument(
    "-m",
...
def _parse_concat_option(raw_input: Iterable[str]):
    match raw_input:
        case "a" | "all" | ["a" | "all"]:
            batch_mode = "all"
        case "c" | "count", value:
            batch_mode = "count", int(value)
        case "h" | "height", value:
            batch_mode = "height", int(value)
        case "r" | "ratio", value:
            batch_mode = "ratio", float(value)
        case other:
...
def parse_download(args: argparse.Namespace) -> None:
    for webtoon_id in args.webtoon_ids:
        ...
        if args.concat is None:
            ...
            concat = _parse_concat_option(args.concat)
...
def parse_concat(args: argparse.Namespace) -> None:
    # open directory selector if -s option specified
    if args.select:
    ...
    if args.all:
        ...
        raw_input = input("Enter batch mode: ")
        batch_mode = _parse_concat_option(raw_input.split())
...
def parse_shell(args: argparse.Namespace):
    from WebtoonScraper.processing._shell import main as enter_shell
    # EOFERROR나 KeyboardInterrupt의 traceback이 나오는 것을 방지함
    try:
        enter_shell()
