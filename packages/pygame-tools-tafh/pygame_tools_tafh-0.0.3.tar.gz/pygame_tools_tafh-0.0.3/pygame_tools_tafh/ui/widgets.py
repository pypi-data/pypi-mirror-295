...
class Widget(RawWidget):
    ...
    def draw(self, window: pygame.Surface):
        ...
        if border:
            ...
            bw = border.width
            pygame.draw.rect(window, border.color, (self.x - bw, self.y - bw, self.width + bw, self.height + bw), bw,
                             border.radius)
        elif color:
        ...
        if self.text:
            ...
            window.blit(txt, (self.x + (self.width - txt.get_width()) / 2,
                              self.y + (self.height - txt.get_height()) / 2) + txt.get_size())
