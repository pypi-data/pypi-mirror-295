...
from osbot_utils.utils.Misc import new_guid, random_guid
...
class S3_DB__Server_Requests(S3_DB__CBR):
    ...
    def s3_key(self, server=None, what=None, when=None, request_id=None):
        if not request_id:
            request_id = random_guid()

        path_elements = [self.s3_folder_server_requests()]
        if server    : path_elements.append(server      )
