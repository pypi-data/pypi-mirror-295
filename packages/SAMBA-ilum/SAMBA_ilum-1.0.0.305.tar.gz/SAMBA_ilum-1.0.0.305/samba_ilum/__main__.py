...
if (tarefa == 1):
   ...
   if (loop_ht == 1):
      ...
      n_Lattice = 2
      #-------------------------------------------------------------------------------
      # Verificando a existência do arquivo 'check_list_ht.txt' não vazio ------------
      #-------------------------------------------------------------------------------
      check_list_dir = dir_files + '/' + dir_o + '/check_list_ht.txt';  temp_check = 0
      ...
      for nloop in range(len(bilayer_materials)):
          ...
          if (temp_check == 1):
             for mnt in range(len(mat1)):
                 temp0_mat1 = mat1[mnt]; temp0_mat2 = mat2[mnt]
          ...
          #------------------------------------------------------------------------
          check_list = open(dir_files + '/' + dir_o + '/' 'check_list_loop.txt', 'a')
          t_Lattice1 = Lattice1.replace('.vasp', ' ');  t_Lattice2 = Lattice2.replace('.vasp', ' ')   
          if (n_Lattice == 2):
             check_list.write(f'{t_Lattice1} {t_Lattice2} \n')
          if (n_Lattice == 3):
             t_Lattice3 = Lattice3.replace('.vasp', ' ')
             check_list.write(f'{t_Lattice1} {t_Lattice2} {t_Lattice3} \n')
          check_list.close()
          #-----------------
