...
USERS_AUTH_API = getattr(settings, 'IMMUNITY_USERS_AUTH_API', True)
USERS_AUTH_THROTTLE_RATE = getattr(
    settings,
    'IMMUNITY_USERS_AUTH_THROTTLE_RATE',
    default_or_test(value='20/day', test=None),
)
AUTH_BACKEND_AUTO_PREFIXES = getattr(
    settings, 'IMMUNITY_USERS_AUTH_BACKEND_AUTO_PREFIXES', tuple()
)
...
USER_PASSWORD_EXPIRATION = getattr(
    settings, 'IMMUNITY_USERS_USER_PASSWORD_EXPIRATION', 0
)
STAFF_USER_PASSWORD_EXPIRATION = getattr(
    settings, 'IMMUNITY_USERS_STAFF_USER_PASSWORD_EXPIRATION', 0
)
