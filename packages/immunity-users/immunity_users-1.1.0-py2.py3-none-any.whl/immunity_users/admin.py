...
class UserCreationForm(UserFormMixin, BaseUserCreationForm):
    ...
    class Media:
        js = ('admin/js/jquery.init.js', 'immunity-users/js/addform.js')
...
class UserAdmin(MultitenantAdminMixin, BaseUserAdmin, BaseAdmin):
    ...
    change_form_template = 'admin/immunity_users/user/change_form.html'
    ...
    def queryset(self, request, queryset):
        if self.value():
            queryset = queryset.filter(
                immunity_users_organizationuser__organization=self.value()
            )
        return queryset
...
class OrganizationAdmin(
    ...
    class Media(UUIDAdmin.Media):
        css = {'all': ('immunity-users/css/admin.css',)}
...
# register immunity_users.Group proxy model
