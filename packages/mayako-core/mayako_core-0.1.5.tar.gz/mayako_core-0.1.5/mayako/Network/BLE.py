...
#TODO: callback disconnect => connected = false
    ...
    """def _notification_handler(self, sender: str, data: bytes):
        ...
        if len(data) < CPACKET.HEADER_SIZE:
            self._logger.debug("BLE: packet to small to be a valid mayako packet")
            return
        ...
        while index + CPACKET.HEADER_SIZE <= len(data):
            ...
            self._logger.debug(header_data)
            payload_start = index + CPACKET.HEADER_SIZE
            payload_stop =  payload_start + packet.get_payload_size() - 1

            if len(data) < payload_stop:
                self._logger.debug("BLE: data size too short for expected payload size")
                break
    ...
    def _process_complete_packet(self, packet: bytearray):
        ...
        try:
            ...
            self._logger.debug(header_data)
            self._logger.debug(payload_data)
            self._read_queue.put(pkt)
            self._logger.debug(self._read_queue.qsize())
