...
from ..src import postprocess as postprocess
...
def get_detection_class_thresholds(table: float = 0.5,
                                   table_rotated: float = 0.5, no_object: int = 10):
    """
    Get detection threshold in dictionary format.

    Arguments:
        table: float, table detection threshold = 0.5,
        table_rotated: float, table rotated detection threshold = 0.5,
        no_object: int, number of objects = 10
    """
    return {'table': table,
...
class TableExtractionPipeline(object):
    def __init__(self, det_device=None, str_device=None,
                 ...
                 easyocr_config: Dict = {"gpu": False, "lang_list": ['en','vi'], "quantize": False}):
        ...
        str_config = Config.structure_config
        ...
        self.reader = easyocr.Reader(**easyocr_config)
        print("OCR reader initialized.")


    def __call__(self, image_path, page_tokens=[]) -> Tuple[List, List, List]:
        try:
            image = Image.open(image_path).convert("RGB")
            # Detect tables in the page image and return cropped images and tokens
            tables = self.extract(image, page_tokens)
            table_objects = list(map(lambda x: x["objects"], tables))
            table_imgs = list(map(lambda x: x["image"], tables))
            table_cells_coordinates = list(map(lambda x: get_cell_coordinates_by_row(x), table_objects))
            table_cells_text = list(map(lambda x,y: apply_ocr(self.reader, x, y), table_imgs, table_cells_coordinates))
            return table_objects, table_cells_coordinates, table_cells_text
