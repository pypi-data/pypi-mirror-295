...
@register_model_view(Contact, name='contactsshkeys', path='public-keys')
class ContactSshKeys(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        assets = SshKey.objects.filter(contact=instance)
        table = SshKeyTable(assets)
        data = {
            "table": table,
        }
        return data
