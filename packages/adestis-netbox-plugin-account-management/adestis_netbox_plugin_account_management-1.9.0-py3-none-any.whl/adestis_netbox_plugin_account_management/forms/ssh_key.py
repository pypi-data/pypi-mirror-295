...
class SshKeyForm(NetBoxModelForm):
    ...
    def clean(self):
        ...
        if self.cleaned_data["raw_ssh_key"]:
            ...
            parsed_key_comment = matches.group(4).strip()
...
class SshKeyBulkEditForm(NetBoxModelBulkEditForm):
    ...
    ssh_key_status = forms.ChoiceField(
        required=False,
...
class SshKeyFilterForm(NetBoxModelFilterSetForm):
    ...
    fieldsets = (
        (None, ('q', 'index', 'tag', 'contact', 'raw_ssh_key', 'key_comment', 'key_type', 'encoded_key',
                'ssh_key_status')),
    )
    ...
    ssh_key_status = forms.MultipleChoiceField(
        choices=SshKeyStatusChoices,
        required=False
    )
