...
class SmartData:
    def __init__(self, df_list, llm = None, show_detail = config['SHOW_DETAIL'], memory_size = config['MEMORY_SIZE'], 
        ...
        if llm is None:
            chat_llm = ChatOpenAI(temperature=config['TEMP_CHAT'], model=config['CHAT_MODEL'], model_kwargs={"seed": seed})
            self.llm = chat_llm
        else:
            self.llm = llm
    ...
    def create_model(self, use_openai_llm = True, seed = 0):
        ...
        if use_openai_llm:
            self.llm = ChatOpenAI(temperature=config['TEMP_CHAT'], model=config['CHAT_MODEL'], model_kwargs={"seed": seed})
    ...
    def run_model(self, question):
        for i in range(10):
            prompt, agent_executor = self.create_model(use_openai_llm = True, seed = i)
            try:
                ...
                if len(code_list_datachange_with_add_on)>0:
                    ...
                    if len(self.df_change)>0:
                        ...
                        new_prompt, _ = self.create_model()
    ...
    def create_data_clean_summary(self, result):
        ...
        summary_model = ChatOpenAI(temperature=config['TEMP_CHAT'],model=config['CHAT_MODEL'])
