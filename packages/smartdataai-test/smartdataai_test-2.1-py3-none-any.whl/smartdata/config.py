...
class Config:
    ...
    PROMPT_CLEAN_DATA = """
        ...
        1. For categorical (object) columns, merge similar, redundant, and granular categories. Treat lowercase and uppercase values as equivalent, and combine abbreviations where appropriate. For example: merge 'US' and 'USA' into 'United States', or combine 'm', 'he', and 'male' into 'Male', and 'w', 'white', 'WHITE' into 'White'. For the 'Not Specified' values, attempt to map it to a more meaningful subcategory if possible. Make sure you don't leave anything not mapped.
    ...
    PROMPT_CREATE_DATA_CLEAN_SUMMARY = """
        Summarize the data cleaning result in around 125 words for non-technical audience. Make sure use a friendly tone, use bullet points (add string '\n' before every bullet), and without any titles. Here is the result:
        {result}
        """
    ...
    DEFAULT_PREFIX_SINGLE_DF = """
        ...
        - Each code snippet must be self-contained, runnable independently and include all necessary imports and data for the plots.
        ...
        - Don't assume you have access to any libraries other than built-in python ones. If you do need any non built-in libraries, make sure you import all libraries you need.
        - Each code snippet must be self-contained, runnable independently and include all necessary imports and data.
