...
def clean_dataframe(df):
    ...
    for col in df_update.select_dtypes(include=['object']).columns:
        ...
        if missing_percentage > 0.9:
            ...
            missing_count = df_update[col].isnull().sum()
            if missing_count > 0:
                df_update[col].fillna('unknown', inplace=True)
    ...
    summary_md = "**Summary of Data Cleaning Process:**\n\n"
    ...
    if summary['numeric_outliers_capped']:
        summary_md += "- Numeric columns where outliers were capped (based on the IQR method):\n  "
    ...
    if summary['categorical_columns_filled']:
        summary_md += "- Categorical columns with missing values filled with 'unknown':\n  "
