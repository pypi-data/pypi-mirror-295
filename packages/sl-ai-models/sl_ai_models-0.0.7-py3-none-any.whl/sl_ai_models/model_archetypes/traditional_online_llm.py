...
class TraditionalOnlineLlm(
    ...
    async def invoke(self, *args, **kwargs) -> Any:
        result = await self._invoke_with_request_cost_time_and_token_limits_and_retry(*args, **kwargs)
        return result
    ...
    async def _invoke_with_request_cost_time_and_token_limits_and_retry(self, *args, **kwargs) -> Any:
        logger.debug(f"Invoking model with args: {args} and kwargs: {kwargs}")
        direct_call_response = await self._mockable_direct_call_to_model(*args, **kwargs)
        response_to_log = direct_call_response[:1000] if isinstance(direct_call_response, str) else direct_call_response
