...
class PerplexityTextModel(OpenAiTextToTextModel, PricedPerRequest, ABC):
    ...
    def __init_subclass__(cls: type[PerplexityTextModel], **kwargs) -> None:
        ...
        if ABC not in cls.__bases__:
            if cls.PRICE_PER_TOKEN is NotImplemented:
                raise NotImplementedError('You forgot to define PRICE_PER_TOKEN')
    ...
    def input_to_tokens(self, prompt: str) -> int:
        messages: list[ChatCompletionMessageParam] = self._turn_model_input_into_messages(prompt)
        chat = ChatPerplexity(client=self._OPENAI_ASYNC_CLIENT, api_key=self.PERPLEXITY_API_KEY, timeout=self.TIMEOUT_TIME)
        langchain_messages = convert_to_messages(messages) #type: ignore
        tokens = chat.get_num_tokens_from_messages(langchain_messages)
        adjustment = -2 * len(messages) # Experimentally the actual tokens always seem to be 2 less more than the tokens caculated (it used to be 47 more)
        adjusted_tokens = tokens + adjustment
        if adjusted_tokens < 0:
            raise ValueError(f"adjusted_tokens is less than 0. tokens: {tokens}, adjustment: {adjustment}")
    ...
    def _get_mock_return_for_direct_call_to_model_using_cheap_input(cls) -> TextTokenCostResponse:
        ...
        total_cost = cls().caculate_cost_from_tokens(prompt_tokens, completion_tokens)
