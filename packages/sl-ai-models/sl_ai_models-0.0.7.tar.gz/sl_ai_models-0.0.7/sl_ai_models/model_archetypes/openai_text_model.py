...
class OpenAiTextToTextModel(TraditionalOnlineLlm, ABC):
    ...
    async def _mockable_direct_call_to_model(self, prompt: str) -> TextTokenCostResponse:
        ...
        response: TextTokenCostResponse = await self._call_online_model_using_api(messages, self.temperature)
        return response


    def _turn_model_input_into_messages(self, prompt: str) -> list[ChatCompletionMessageParam]:
        if self.system_prompt is None:
            return OpenAiUtils.put_single_user_message_in_list_using_prompt(prompt)
        else:
            return OpenAiUtils.create_sytem_and_user_message_from_prompt(prompt, self.system_prompt)


    async def _call_online_model_using_api(self, messages: list[ChatCompletionMessageParam], temperature: float, max_tokens: int | None = None) -> TextTokenCostResponse:
        ...
        if response.choices[0].message.content is None:
            raise RuntimeError("The model failed to give an answer. response.choices[0].message.content is None")
    ...
    def _get_mock_return_for_direct_call_to_model_using_cheap_input(cls) -> TextTokenCostResponse:
        ...
        completion_tokens = OpenAiUtils.text_to_tokens_direct(probable_output, cls.MODEL_NAME)
    ...
    def caculate_cost_from_tokens(self, prompt_tkns: int, completion_tkns: int) -> float:
        prompt_cost = get_openai_token_cost_for_model(self.MODEL_NAME, prompt_tkns, is_completion=False)
        completion_cost = get_openai_token_cost_for_model(self.MODEL_NAME, completion_tkns, is_completion=True)
