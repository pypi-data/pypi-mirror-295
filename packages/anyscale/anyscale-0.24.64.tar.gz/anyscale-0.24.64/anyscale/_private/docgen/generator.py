...
CLI_EXAMPLE_ENFORCEMENT = {
    "anyscale job",
    "anyscale schedule",
    "anyscale workspace_v2",
...
class MarkdownGenerator:
    ...
    def _generate_legacy_sdks(self, m: Module, legacy_sdks: List[LegacySDK]) -> str:
        if not m.legacy_sdk_commands:
            return ""
        output = f"## {m.title} SDK <span class='label-h2 label-legacy'>Legacy</span>\n"
    ...
    def _generate_legacy_models(
        ...
        if not m.legacy_sdk_models:
            return ""
        output = (
            f"## {m.title} Models <span class='label-h2 label-legacy'>Legacy</span>\n"
        )
    ...
    def _generate_clis(self, m: Module, is_legacy_cli: bool = False) -> str:
        ...
        if is_legacy_cli:
            output = (
                f'## {m.title} CLI <span class="label-h2 label-legacy">Legacy</span>\n'
            )
        else:
    ...
    def _gen_markdown_for_cli_command(
        ...
        is_cli_example_enforced = (
            not isinstance(c, LegacyAnyscaleCommand)
            and cli_prefix in CLI_EXAMPLE_ENFORCEMENT
        )
        has_cli_example = hasattr(c, "__doc_cli_example__")
        if is_cli_example_enforced or has_cli_example:
    ...
    def _gen_markdown_for_legacy_sdk_command(
        self, legacy_sdk: LegacySDK, new_sdk: Callable, sdk_prefix: str
    ) -> str:
