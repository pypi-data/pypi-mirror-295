...
class Slack:
    ...
    def get_all_dm_ids(self):
        try:
            ...
            logging.error(f"Error fetching DM IDs: {e}")
            return []
        
    def get_all_mpim_ids(self):
        try:
            mpim_ids = []
            cursor = None
            while True:
                result = self.client.conversations_list(types="mpim", cursor=cursor)
                mpims = result["channels"]
                mpim_ids.extend([mpim["id"] for mpim in mpims])
                cursor = result["response_metadata"].get("next_cursor")
                if not cursor:
                    break
            return mpim_ids
        except SlackApiError as e:
            logging.error(f"Error fetching MPIM IDs: {e}")
            return []
