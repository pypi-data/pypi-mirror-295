...
class MonthDayCondition(ConditionBase, metaclass=ABCMeta):
    """
    ...
    def __init__(self, values, what, date_format=DateFormat.ISO_FORMAT, qualifier=ConditionBase.Qualifier.ForAnyValue,
        ...
        if values and isinstance(values, list):
            for i in values:
                i_convert = self.convert_day_format_from_any(i)
                if i_convert:
...
class MonthDayConditionSchema(Schema):
    """
    ...
    date_format = fields.String(allow_none=False, load_default=MonthDayCondition.DateFormat.ISO_FORMAT, )
