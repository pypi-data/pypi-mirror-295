...
class Parser:
    ...
    def xPath(self, path: str, element: any=None, pos: int=None, data_type: str=None, prop: str=None, explode=None, condition=None, substring=None, transform=None, replace=None, join=None, timeout=99, is_error: bool=True) -> any:
        return self.scraper.xPath(path=path, element=element, pos=pos, data_type=data_type, prop=prop, explode=explode, condition=condition, substring=substring, transform=transform, replace=replace, join=join, timeout=timeout, is_error=is_error)
