...
class time_series(quantim):
    ...
    def clustering(self, tks, ref_curr='USD', ini_date=None, ncluster=None, cluster_method='graph', factor_tickers=None, verify=False):
        ...
        clusters, rets_summ, invalid_tickers, valid_dates, factors = pd.DataFrame(resp['clusters']), pd.DataFrame(resp['rets_summ']), resp['invalid_tickers'], resp['valid_dates'], pd.DataFrame(resp['factors'])
