...
def get_local_run_cmd(context: CertoraContext) -> str:
    ...
    """
    run_args = []
    if context.is_tac or Attrs.is_solana_app() or Attrs.is_wasm_app():
...
def run_typechecker(typechecker_name: str, with_typechecking: bool, args: List[str]) -> None:
    ...
    exit_code = Util.run_jar_cmd(typecheck_cmd, False, custom_error_message="Failed to run Certora Prover locally. "
...
def run_local_spec_check(with_typechecking: bool, context: CertoraContext) -> None:
    ...
    if Util.is_java_installed():
        ...
        raise Util.CertoraUserInputError("Cannot run local checks because of missing a suitable java installtion. "
