...
from ..params import (EmissionParameters, GrainParameters, MetParameters,
                      ...
                      save_parameters)
from ..queryreport import (print_text, print_title, query_change_value,
...
def print_emission_parameters(emission_params):
    ...
    for j, (lower, upper, Suzuki_k) in enumerate(
        zip(emission_params.lower, emission_params.upper, emission_params.Suzuki_k)
    ):
        print_text("  Emission profile for grain class {}".format(j))
        print_text("    Lower altitude = {} m".format(lower))
        print_text("    Upper altitude = {} m".format(upper))
        print_text("    Suzuki k = {}".format(Suzuki_k))
        print_text("    Suzuki peak = {} m".format(Suzuki_peak_from_k(Suzuki_k, lower, upper)))
    print_text("********************")
...
def set_emission_parameters(params_set, printout=True):
    ...
    while profiles_added < N:
        ...
        params_set.emission.add_profile(lower, upper, Suzuki_k)
        profiles_added += 1
...
def set_model_parameters(params, met, square=False):
    ...
    for j in range(params.grains.bins):
        ...
        xScale[j] = np.max(np.abs(sol.y[0,:])) + params.source.radius*3
        yScale[j] = np.max(np.abs(sol.y[1,:])) + params.source.radius*3
        if square:
