...
def copy_parameters(A):
    ...
    for j in range(A.emission.len()):
        new_A.emission.add_profile(
            A.emission.lower[j], A.emission.upper[j], A.emission.Suzuki_k[j]
        )
...
def update_parameters(
    ...
    if grains is not None:
        ...
        grains = sorted(grains, key=lambda p: p["class"])
        for grain in grains:
    ...
    if emissions is not None:
        if not isinstance(emissions, list):
            raise ValueError(
                "in update_parameters, emissions must be a list of dicts; "
                + "received {}".format(emissions)
            )
        if not all(
            ...
            raise ValueError(
                ...
                + " and either 'Suzuki_k' or 'Suzuki_peak'; received {}".format(emissions)
            )
        emissions = sorted(emissions, key=lambda p: p["class"], reverse=True)
        for profile in emissions:
            class_i = profile["class"] - 1
            lower = profile["lower"]
            upper = profile["upper"]
            if "Suzuki_peak" in profile:
                Suzuki_k = Suzuki_k_from_peak(profile["Suzuki_peak"],lower,upper)
            else:
                Suzuki_k = profile["Suzuki_k"]
            if class_i < A.emission.len():
                ...
                A.grains.add_grain(lower, upper, Suzuki_k)
...
def load_parameters(file):
    ...
    params.grains.fromlists(diameters, densities, props)
    ...
    params.emission.fromlists(lower, upper, Suzuki_k)
