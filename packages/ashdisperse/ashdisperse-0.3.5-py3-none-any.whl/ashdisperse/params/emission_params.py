...
class EmissionParameters:
    ...
    def fromvalue(self, lower, upper, Suzuki_k):
        self.add_profile(lower, upper, Suzuki_k)
        return

    def fromlists(self, lower, upper, Suzuki_k):
        ...
        for j, (l, u, k) in enumerate(zip(lower, upper, Suzuki_k)):
            self.add_profile(l, u, k)

        return

    def add_profile(self, lower, upper, Suzuki_k):
    ...
    def describe(self):
        ...
        for j in range(len(self.lower)):
            print("  Emission profile for grain class ", j + 1)
