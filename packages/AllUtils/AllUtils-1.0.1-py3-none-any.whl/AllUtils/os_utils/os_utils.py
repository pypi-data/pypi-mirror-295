...
import shutil
import pandas
from mytools.AllUtils.data_structure_utils.data_structure_utils import StringUtils
...
class OSUtils:
    ...
    @staticmethod
    def get_all_files_in_single_directory_name_include_string1(directory, string):
        """
    ...
    @staticmethod
    def get_all_files_in_single_directory_name_include_string2(directory, string):
        """
    ...
    @staticmethod
    def get_all_files_in_directory_name_include_string1(directory, string):
        """
        这个函数用来获取指定目录下所有包含指定字符串的文件的绝对路径
        """
        found_files = []
...
if __name__ == '__main__':
    path = OSUtils.get_all_subdirectory_in_directory(r'D:\Python\Python38\Lib\site-packages\mytools\AllUtils')[0]
    p = [pa for pa in path if '__' not in pa]
    for item in p:
        OSUtils.add_files_to_directory(item, r'D:\Python\Python38\Lib\site-packages\mytools\AllUtils\__init__.py')
