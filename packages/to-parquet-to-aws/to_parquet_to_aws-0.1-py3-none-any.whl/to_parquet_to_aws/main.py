...
import S3Connection as s3Connection
...
def upload_parquet_to_aws(
    ...
    if isinstance(data_to_upload, pd.DataFrame):
        ...
        #If file is csv
            ...
            raise Exception('Not appropriate data to upload')
    
    to_parquet(data_df, full_parquet_path)
    upload_to_aws(full_parquet_path, aws_access_key, aws_secret_key, aws_region, aws_directory, s3_folder)


def to_parquet(data_df: pd.DataFrame, file_path: str):
...
def upload_to_aws(file_path, aws_access_key, aws_secret_key, aws_region, aws_directory, s3_folder):       
    # Upload file to S3 bucket
    if path.exists(file_path):
        bucket_connection = s3Connection.S3Connection(access_key= aws_access_key, 
                                                        secret_key= aws_secret_key, 
                                                        session_token= '', 
                                                        region_name= aws_region)
        bucket_connection.create_session()
        bucket_connection.upload_file(local_file_path= file_path, 
                                        staging_directory= aws_directory, 
                                        folder_path= s3_folder + path.split(file_path)[1])
