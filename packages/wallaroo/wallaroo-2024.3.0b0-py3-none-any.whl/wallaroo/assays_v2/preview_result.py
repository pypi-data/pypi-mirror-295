...
class PreviewResult:
    ...
    def to_df_row(self) -> Dict[str, Any]:
        ...
        scores = {}
        for k, v in self.scores.additional_properties.items():
            score_data = cast(ScoreData, v)
            scores[f"{k}.score"] = score_data.score
            scores[f"{k}.scores"] = score_data.scores  # type: ignore[union-attr, assignment]
...
class PreviewResultList(List[PreviewResult]):
    ...
    def chart_df(
        ...
        float_only_df = df[f"{self.parent_assay._get_iopath()}.score"].map(
    ...
    def chart_iopaths(
        ...
        for i, iopath in enumerate(df["iopath"].unique()):
            if selected_labels is None or (
                ...
                self.chart_df(df_, label, nth_x_tick=nth_x_tick)
    ...
    def chart_scores(
        ...
        self.chart_df(df, title, nth_x_tick=nth_x_tick, start=start, end=end)
    ...
    def to_full_dataframe(self) -> pd.DataFrame:
        ...
        :return pd.DataFrame: A dataframe representation of the list of preview results.
        """
        return dict_list_to_dataframe([a.raw.to_dict() for a in self])
