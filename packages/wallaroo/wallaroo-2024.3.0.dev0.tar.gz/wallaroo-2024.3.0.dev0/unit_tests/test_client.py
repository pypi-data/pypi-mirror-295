...
from .reusable_responders import (
    add_insert_model_config_response, add_get_model_config_response, get_assay_by_id_responder,
    add_default_workspace_responder, add_get_pipeline_by_id_responder, add_get_workspace_by_id_responder,
    add_pipeline_variant_by_id_responder, add_pipeline_by_id_responder,
    add_get_model_by_id_responder, add_list_models_responder
)
...
class TestClientUrls(unittest.TestCase):
    ...
    def test_compute_urls_raises_error_when_passed_in_none(self):
        with self.assertRaises(ValueError):
            wallaroo.client.Client.get_urls(
    ...
    def test_compute_urls_get_auth_type_from_env_var(self):
        ...
        self.assertEqual(auth_type, "user_password")
        self.assertEqual(api_endpoint, self.api_endpoint)
    ...
    def test_compute_urls_get_api_endpoint_from_env_vars(self):
        ...
        self.assertEqual(auth_type, self.auth_type)
        self.assertEqual(api_endpoint, "http://my-new-api-endpoint:8080")

    @mock.patch.dict(os.environ, {"WALLAROO_SDK_AUTH_TYPE": "user_password", "WALLAROO_URL": "http://my-new-api-endpoint:8080"})
...
class TestClient(unittest.TestCase):
    ...
    def test_given_config(self):
        ...
        self.assertEqual("x86", _config["default_arch"])
    ...
    def test_list_models_filtered_by_workspace(self, respx_mock):
        ...
        add_list_models_responder(respx_mock, self.test_client.api_endpoint, {"workspace_id": 1, "workspace_name": None}, model_list_resp_1)
        models_1 = self.test_client.list_models()
        self.assertEqual(2, len(models_1))

        # Filtering by workspace_id
        add_list_models_responder(respx_mock, self.test_client.api_endpoint, {"workspace_id": 2, "workspace_name": None}, model_list_resp_2)
        models_2 = self.test_client.list_models(workspace_id=2)
        self.assertEqual(1, len(models_2))

        # Filtering only by workspace_name
        add_list_models_responder(respx_mock, self.test_client.api_endpoint,
                                  {"workspace_id": None, "workspace_name": "test-default-workspace-2"}, model_list_resp_2)
        models_2 = self.test_client.list_models(workspace_name="test-default-workspace-2")
        self.assertEqual(1, len(models_2))
    ...
    def test_register_mlflow_model(self):
        ...
        self.assertEqual(upload_call.request.body.encoding, 'utf-8')
        self.assertTrue('multipart/form-data' in upload_call.request.body.content_type)
    ...
    def test_pipelines_by_name(self):
        ...
        self.assertEqual(1, pipelines[0].workspace().id())

    @responses.activate
    @respx.mock(assert_all_mocked=True)
    def test_list_pipelines_none(self, respx_mock):
        respx_mock.post(f"{self.test_client.api_endpoint}/v1/api/pipelines/list_pipelines").mock(
    ...
    def test_list_pipelines(self, respx_mock):
        ...
        self.assertEqual(2, len(pipelines))
        self.assertEqual(pipelines[0].workspace().id(), 1)
        self.assertEqual(pipelines[0].workspace().name(), "test - default workspace 1")

        self.assertEqual(pipelines[1].workspace().id(), 2)
        self.assertEqual(pipelines[1].workspace().name(), "test - default workspace 2")
    ...
    def test_get_pipeline(self):
        ...
        self.assertIsInstance(test_pipeline, Pipeline)
        # Doesn't set a specific version to deploy
        self.assertIsNone(test_pipeline._pipeline_version_to_deploy)
    ...
    def test_get_pipeline_specific_version(self):
        ...
        test_pipeline = self.test_client.get_pipeline(name="test-pipeline", version="v1")
        self.assertIsInstance(test_pipeline, Pipeline)
        # Sets a specific version to deploy
        self.assertEqual("v1", test_pipeline._pipeline_version_to_deploy.name())
    ...
    def test_get_pipeline_when_pipeline_does_not_exist(self):
        ...
        self.assertEqual(expected_string, str(error.exception))
    ...
    def test_get_model(self, respx_mock):
        ...
        self.assertIsInstance(test_model_version, ModelVersion)
        # Default version is the latest
        self.assertEqual(test_model_version.version(), "v2")
        self.assertEqual(test_model_version.workspace().id(), 1)
    ...
    def test_get_model_name_does_not_exist(self, respx_mock):
        ...
        with self.assertRaises(Exception):
            test_model_version = self.test_client.get_model(name="test-model-does-not-exist")
    ...
    def test_generate_model_query_with_all_params(self):
        ...
        assert query.translate(
            string.whitespace
        ) == expected_query.translate(string.whitespace)
    ...
    def test_generate_pipelines_query_with_workspace_filtering(self):
        ...
        expected_params = {"search_term": "", "workspace_id": 1, "workspace_name": "test_workspace"}
        client = self.test_client
        (query, params) = client._generate_search_pipeline_query(workspace_id=1, workspace_name="test_workspace")
    ...
    def test_jupyter_client(self):
        with patch.dict("os.environ",
            ...
            client = wallaroo.Client(auth_type="test_auth", config={"default_arch": "x86"})
            assert client._interactive == True

            client = wallaroo.Client(interactive=True, auth_type="test_auth", config={"default_arch": "x86"})
            assert client._interactive == True

            client = wallaroo.Client(interactive=False, auth_type="test_auth",config={"default_arch": "x86"})
    ...
    def test_list_assays(self, respx_mock):
        ...
        self.assertEqual(2, assays[0]._id)
        self.assertEqual("Assay 965409", assays[0]._name)
        self.assertEqual(15, assays[0]._pipeline_id)
    ...
    def test_get_assay_results(self, respx_mock):
        resp = respx_mock.post(f"{self.test_client.api_endpoint}/v1/api/assays/get_assay_results").mock(
            return_value=httpx.Response(200, json=SAMPLE_GET_ASSAYS_RESULTS)
        )
        respx_mock.post(f"{self.test_client.api_endpoint}/v1/api/assays/get_assay_by_id").mock(
            return_value=httpx.Response(200, json={
            'id': 27,
            ...
            'baseline': {
                'static': {
                    ...
                    'edges': [236238.65625, 311515.125, 437177.84375, 513583.125, 684577.1875, 1412215.25, None],
        ...
        self.assertDictEqual({'assay_id': 27, 'start': start_date_utc.isoformat(), 'end': end_date_utc.isoformat(), 'workspace_id': None, 'workspace_name': None},
                             response_params)
        ...
        self.assertEqual(assay_results[0].id, 41)
        assay_results_df = assay_results.to_dataframe()
        self.assertEqual(assay_results_df["assay_name"].iloc[0], "getassayinfotest")
        self.assertEqual(assay_results_df["pipeline_name"].iloc[0], "assay-demonstration-tutorial-5")
        self.assertEqual(assay_results_df["iopath"].iloc[0], "output variable 0")
    ...
    def test_set_assay_active(self, respx_mock):
        ...
        respx_mock.post(
            f"{self.test_client.api_endpoint}/v1/api/assays/set_active", json=turn_on_payload
        ).mock(return_value=httpx.Response(200, json=turn_on_payload))

        respx_mock.post(
            f"{self.test_client.api_endpoint}/v1/api/assays/set_active", json=turn_off_payload
    ...
    def test_get_assay_info_for_assay_built_with_uploaded_file(self, respx_mock):
        respx_mock.post(
            f"{self.test_client.api_endpoint}/v1/api/assays/get_assay_by_id", json={"id": 1, "workspace_id": None, "workspace_name": None}
        ...
        self.assertEqual(assay_info.iloc[0]["id"], 1)
        self.assertEqual(assay_info.iloc[0]["name"], "getassayinfotest")
        expected_assay_info = ['id', 'name', 'active', 'status', 'pipeline_name', 'last_run', 'next_run',
                               'alert_threshold', 'workspace_id', 'workspace_name',
                               'baseline', 'iopath', 'metric', 'num_bins', 'bin_weights', 'bin_mode']
        self.assertListEqual(assay_info.columns.tolist(), expected_assay_info)
        self.assertEqual(assay_info.iloc[0]["baseline"], "Uploaded File")
    ...
    def test_get_assay_info_for_assay_built_with_baseline_dates(self, respx_mock):
        ...
        expected_assay_info = ['id', 'name', 'active', 'status', 'pipeline_name', 'last_run', 'next_run',
                               ...
                               'baseline', 'iopath', 'metric', 'num_bins', 'bin_weights', 'bin_mode']
        self.assertListEqual(assay_info.columns.tolist(), expected_assay_info)
        self.assertEqual(assay_info.iloc[0]["baseline"], "Start:2024-02-26T02:56:37.290182+00:00, End:2024-02-27T02:56:37.290182+00:00")
