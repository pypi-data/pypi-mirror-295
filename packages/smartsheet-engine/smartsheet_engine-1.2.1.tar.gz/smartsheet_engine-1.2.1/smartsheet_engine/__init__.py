...
class SmartsheetEngine:
	...
	def get_sheet(
		...
		:param force_refresh: Fetch the newest data from Smartsheet, instead of the data cached in the GridRepository
		...
		if not grid:
			...
			if not grid:
				...
				return
			if grid.sheet_df and len(grid.sheet_df.index) == 0:
				logger.warning(f'{sheet_name} ({grid.sheet_id}): Found Sheet with 0 rows after refreshing index of available Sheets.')
