...
class SmartsheetEngine:
	...
	def set_column_dropdown(
		...
		if restrict_values:
			...
			restrict_values_msg = ' and restricted column to allow dropdown values only'
		else:
			restrict_values_msg = ''
		
		resp = self._set_column_property(grid, column_name, settings)
		if resp:
			logger.info(f'{grid.sheet_name} ({grid.sheet_id}): {column_name}: Set the dropdown values to {dropdown_options}{restrict_values_msg}')
