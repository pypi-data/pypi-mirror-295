...
class m:
    ...
    def set_first_password(self, invoking_function_name, request, sqlite_db_path, gmail_bot_preset_name, telegram_bot_preset_name):
        ...
        try:
            ...
            # Check if the email and temp_password match with the records in the user_pending_validation table
            cursor.execute(
                """SELECT temp_password FROM user_pending_validation WHERE email = ?""",
                (email,)
            )
            record = cursor.fetchone()
