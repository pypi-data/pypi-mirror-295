...
class CachedDataGenerator(Sequence):
    ...
    def as_XYw(self):
        ...
        sample = self.get_sample(self.df.iloc[0])

        def get_a(sample):
            shape = sample.shape
            if not shape:
                shape = (1,)
            full_shape = [n_samples] + list(shape)
            dtype = sample.dtype
            itemsize = dtype.itemsize
            mem_size = reduce(operator.mul, full_shape, itemsize)
            print(f'mem={mem_size/1000000:.1f}Mb')
            X = np.zeros(full_shape, dtype=dtype)
            return X

        X = [get_a(s) for s in sample.inputs]
        Y = [get_a(s) for s in sample.outputs] if sample.outputs is not None else []
        w = [np.zeros(n_samples)] if sample.weight is not None else []

        num_batches = len(self)
        for batch_num in range(num_batches):
            Xb, Yb, wb = self[batch_num]
            start_index = batch_num * self.batch_size

            batch_size = Xb[0].shape[0]

            for A, Ab in [(X, Xb), (Y, Yb), (w, wb)]:
                if Ab is None:
                    continue

                for idx in range(len(A)):
                    A[idx][start_index:start_index+batch_size] = Ab[idx]

        Y = Y if len(Y) else None
        w = w[0] if len(w) else None
        return X, Y, w
