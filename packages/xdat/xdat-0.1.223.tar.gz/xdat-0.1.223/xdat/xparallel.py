...
def x_on_iter(llist, calc_func, total=None, clear_nones=False, flatten=False, backend=None, with_tqdm=True, n_jobs=-1):
    if total is None:
        try:
            total = len(llist)
        except TypeError:
    ...
    if n_jobs == 1:
        if with_tqdm:
            xtqdm = tqdm
        all_items = [calc_func(i) for i in xtqdm(llist, total=total)]

    else:
        if with_tqdm:
            xtqdm = x_tqdm

        xtqdm2 = xtqdm(llist, total=total, n_jobs=n_jobs)
        try:
            all_items = Parallel(backend=backend, n_jobs=n_jobs)(delayed(calc_func)(i) for i in xtqdm2)
        except PicklingError:
            if backend != 'threading':
                print("NOTE: PicklingError might get resolved by switching to 'threading' backend")
            raise
