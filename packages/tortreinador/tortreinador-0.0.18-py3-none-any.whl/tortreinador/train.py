...
class TorchTrainer:
    ...
    def fit(self, t_l, v_l, **kwargs):
        ...
        AUTO_COUNT = 1
        if 'checkpoint_path' not in kwargs.keys():
            CHECK_POINT_PATH = '{}check_point_{}.pth'.format(kwargs['m_p'], self.file_time)

        else:
            CHECK_POINT_PATH = kwargs['checkpoint_path']
        ...
        # Initial PTH file
        if 'checkpoint_path' not in kwargs.keys():
            kwargs['checkpoint_path'] = CHECK_POINT_PATH
            torch.save({
...
def config_generator(model_save_path: str, warmup_epochs: int = None, lr_milestones: list = None,
    ...
    Parameters:
        ...
        auto_save(int): Epoch interval of saving training information.
