...
def send_sms(payload: Dict[str, str], account_sid: str, auth_token: str) -> Dict[str, Any]:
    ...
    # Append the result to event stream (assumes event_stream is global)
    event_stream.append({"event": "api_call", "api_name": "send_sms", "response": response})
    return response

def find_in_event_stream(key: str, event_stream: List[Dict[str, Any]]) -> Optional[str]:
    """
...
def resolve_and_execute(event_stream: List[Dict[str, Any]], metadata: Dict[str, Any], account_sid: str, auth_token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    for key in metadata[api_name]["sample_payload"]:
        value = find_in_event_stream(key, event_stream)
        if not value:
