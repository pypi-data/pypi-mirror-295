...
class TestAdaptavist:
    ...
    def test_get_users(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._jira_url}/rest/api/2/user/search?username=.&startAt=0&maxResults=200",
            text=load_fixture("get_users.json"),
        )
    ...
    def test_get_environments(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/environments?projectKey=JQA", text=load_fixture("get_environments.json")
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_create_environment(self, requests_mock: Mocker):
        ...
        environment = adaptavist.create_environment(
            project_key="TEST", environment_name="Test environment", description="Cool new environment for testing."
        )
    ...
    def test_get_folders(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._jira_url}/rest/tests/1.0/project/10000/foldertree/testcase?startAt=0&maxResults=200",
    ...
    def test_get_test_cases(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testcase/search?query=folder+%3C%3D+%22%2F%22&startAt=0",
            text=load_fixture("get_test_cases.json"),
        )
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testcase/search?query=folder+%3C%3D+%22%2F%22&startAt=1", text="[]"
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_create_test_case(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.create_folder"):
            test_case = adaptavist.create_test_case(
                project_key="JQA", test_case_name="Ensure the axial-flow pump is enabled", folder="Test folder"
            )
            assert test_case == "JQA-T123"

        # Test that folder is submitted as null if the root folder is chosen
        with patch("adaptavist.Adaptavist.create_folder"), patch("adaptavist.Adaptavist._post") as post:
    ...
    def test_edit_test_case(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.get_test_case", return_value={"name": "Test case", "projectKey": "JQA"}), patch(
            "adaptavist.Adaptavist.create_folder"
        ):
        ...
        with patch(
            ...
            return_value={"name": "Test case", "projectKey": "JQA", "labels": ["automated"]},
        ...
        with patch(
            "adaptavist.Adaptavist.get_test_case",
            return_value={"name": "Test case", "projectKey": "JQA", "ci_server_url": ["mock://jenkins"]},
        ), patch("adaptavist.Adaptavist.create_folder"), patch("adaptavist.Adaptavist._put") as put:
    ...
    def test_get_test_case_links(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/issuelink/JQA-1234/testcases", text=load_fixture("get_test_case_links.json")
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_link_test_cases(self, requests_mock: Mocker):
        ...
        with patch(
            "adaptavist.Adaptavist.get_test_case",
            return_value={"name": "Test case", "projectKey": "JQA", "issueLinks": ["JQA-123"]},
        ), patch("adaptavist.Adaptavist._put") as put:
            assert adaptavist.link_test_cases(issue_key="JQA-123", test_case_keys=["JQA-T123"])
    ...
    def test_unlink_test_cases(self, requests_mock: Mocker):
        ...
        with patch(
            "adaptavist.Adaptavist.get_test_case",
            return_value={"name": "Test case", "projectKey": "JQA", "issueLinks": ["JQA-123", "JQA-123"]},
        ), patch("adaptavist.Adaptavist._put") as put:
        ...
        with patch(
            "adaptavist.Adaptavist.get_test_case",
            return_value={"name": "Test case", "projectKey": "JQA", "issueLinks": ["JQA-124"]},
        ), patch("adaptavist.Adaptavist._put") as put:
            assert adaptavist.unlink_test_cases(issue_key="JQA-123", test_case_keys=["JQA-T123"])
            assert put.assert_not_called
    ...
    def test_get_test_plans(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testplan/search?query=folder+%3C%3D+%22%2F%22&startAt=0",
            text=load_fixture("get_test_plans.json"),
        )
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testplan/search?query=folder+%3C%3D+%22%2F%22&startAt=1", text="[]"
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_create_test_plan(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.create_folder"):
            test_plan = adaptavist.create_test_plan(
                project_key="JQA", test_plan_name="Plan for a new version", folder="Test folder"
            )
    ...
    def test_edit_test_plan(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.get_test_plan", return_value={"name": "Test plan", "projectKey": "JQA"}), patch(
            "adaptavist.Adaptavist.create_folder"
        ):
        ...
        # Test that existing labels are removed, if the list starts with "-"
        with patch(
            "adaptavist.Adaptavist.get_test_plan",
            return_value={"name": "Test plan", "projectKey": "JQA", "labels": ["automated"]},
        ), patch("adaptavist.Adaptavist.create_folder"), patch("adaptavist.Adaptavist._put") as put:
    ...
    def test_get_test_runs(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testrun/search?query=folder+%3D+%22%2F%22&startAt=0",
            text=load_fixture("get_test_runs.json"),
        )
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testrun/search?query=folder+%3D+%22%2F%22&startAt=1", text="[]"
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_create_test_run(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.create_folder"):
            test_run = adaptavist.create_test_run(
                project_key="JQA", test_run_name="Run for a new version", folder="Test folder"
            )
    ...
    def test_clone_test_run(self):
        ...
        with patch("adaptavist.Adaptavist.get_test_run", return_value=json.loads(load_fixture("get_test_run.json"))), patch(
            "adaptavist.Adaptavist.create_test_run", return_value="JQA-R124"
        ) as create_test_run, patch(
            "adaptavist.Adaptavist.get_test_plans", return_value=json.loads(load_fixture("get_test_plans.json"))
        ), patch(
            "adaptavist.Adaptavist.edit_test_plan"
        ) as edit_test_plan:
        ...
        with patch("adaptavist.Adaptavist.get_test_run", return_value=json.loads(load_fixture("get_test_run.json"))), patch(
            "adaptavist.Adaptavist.create_test_run", return_value="JQA-R124"
        ) as create_test_run, patch("adaptavist.Adaptavist.get_test_plans", return_value=[]), patch(
            "adaptavist.Adaptavist.edit_test_plan"
        ) as edit_test_plan:
    ...
    def test_get_test_execution_results(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._jira_url}/rest/tests/1.0/reports/testresults?startAt=0&maxResults=10000",
            text=load_fixture("get_test_execution_results.json"),
        )
        requests_mock.get(
            f"{TestAdaptavist._jira_url}/rest/tests/1.0/reports/testresults?startAt=1&maxResults=10000", text='{"results":[]}'
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_create_test_results(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist.get_test_run", return_value=json.loads(load_fixture("get_test_run.json"))):
            assert adaptavist.create_test_results(
                test_run_key="JQA-R123", results=[{"status": "Fail", "testCaseKey": "JQA-T5678"}]
            )
        ...
        with patch("adaptavist.Adaptavist.get_test_run", return_value=json.loads(load_fixture("get_test_run.json"))), patch(
            ...
            adaptavist.create_test_results(
                test_run_key="JQA-R123", results=[{"status": "Fail", "testCaseKey": "JQA-T5678"}], assignee="", executor=""
            )
    ...
    def test_create_test_result(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist._post") as post:
            adaptavist.create_test_result(
                test_run_key="JQA-R123", test_case_key="JQA-T123", status=STATUS_PASS, assignee="", executor=""
            )
        ...
        with patch("adaptavist.Adaptavist._post") as post:
            adaptavist.create_test_result(
                test_run_key="JQA-R123", test_case_key="JQA-T123", status=STATUS_PASS, execute_time=3, issue_links=["JQA-123"]
            )
    ...
    def test_edit_test_result_status(self, requests_mock: Mocker):
        ...
        with patch("adaptavist.Adaptavist._put") as put:
            adaptavist.edit_test_result_status(
                test_run_key="JQA-R123", test_case_key="JQA-T123", status=STATUS_FAIL, assignee="", executor=""
            )
    ...
    def test_get_test_result_attachement(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testresult/123/attachments",
            text=load_fixture("get_test_result_attachments.json"),
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_add_test_result_attachment(self):
        ...
        with patch("adaptavist.Adaptavist.get_test_result", return_value={"id": 123}), raises(SyntaxError):
            assert adaptavist.add_test_result_attachment(
                test_run_key="JQA-R123", test_case_key="JQA-T123", attachment="testfile"
            )
    ...
    def test_edit_test_script_status(self, requests_mock: Mocker):
        ...
        with patch(
            ...
            return_value={"id": 123, "status": STATUS_FAIL, "scriptResults": [{"index": 0, "status": STATUS_FAIL}]},
        ):
            assert adaptavist.edit_test_script_status(
                test_run_key="JQA-R123", test_case_key="JQA-T123", step=1, status=STATUS_PASS
            )

        # Test that executor and assignee are submitted as null if empty string is given
        with patch("adaptavist.Adaptavist.get_test_result", return_value={"id": 123, "status": STATUS_FAIL}), patch(
            "adaptavist.Adaptavist._put"
        ) as put:
            adaptavist.edit_test_script_status(
                test_run_key="JQA-R123", test_case_key="JQA-T123", step=1, status=STATUS_PASS, assignee="", executor=""
            )
            assert put.call_args_list[0][0][1]["assignedTo"] is None
            assert put.call_args_list[0][0][1]["executedBy"] is None
    ...
    def test_get_test_script_attachment(self, requests_mock: Mocker):
        ...
        requests_mock.get(
            f"{TestAdaptavist._adaptavist_api_url}/testresult/123/step/0/attachments",
            text=load_fixture("get_test_result_attachments.json"),
        )
        adaptavist = Adaptavist(jira_server=TestAdaptavist._jira_url, jira_username="User", jira_password="Password")
    ...
    def test_add_test_script_attachment(self):
        ...
        with patch("adaptavist.Adaptavist.get_test_result", return_value={"id": 123}), raises(SyntaxError):
            assert adaptavist.add_test_script_attachment(
                test_run_key="JQA-R123", test_case_key="JQA-T123", step=1, attachment="testfile"
            )
        ...
        with patch("adaptavist.Adaptavist.get_test_result", return_value={"id": 123}), patch(
            ...
            attachment.name = "testdata.txt"
            assert adaptavist.add_test_script_attachment(
                test_run_key="JQA-R123", test_case_key="JQA-T123", step=1, attachment=attachment
            )
