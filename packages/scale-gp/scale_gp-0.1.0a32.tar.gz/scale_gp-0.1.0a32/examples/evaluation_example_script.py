...
assert (
    api_key is not None
), "You need to provide your API key - see https://egp.dashboard.scale.com/admin/api-key"
...
bundle_config = VendorConfigurationBundleConfig(
    image="gemini-pro", registry="aws-registry", tag="latest"
)
...
uploaded_test_cases = client.evaluation_datasets.test_cases.batch(
    ...
    items=[
        {"account_id": account_id, "schema_type": "GENERATION", "test_case_data": item}
        for item in DATASET
    ],
)
...
while True:
    ...
    evaluation_results = client.evaluations.retrieve(
        evaluation_id=evaluation.id, view="TestCaseResults"
    )
