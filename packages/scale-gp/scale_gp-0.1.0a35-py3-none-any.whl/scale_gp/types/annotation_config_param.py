# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

from typing import List, Iterable
from typing_extensions import Literal, Required, TypedDict

__all__ = ["AnnotationConfigParam", "Component", "ComponentItem", "ComponentComparison"]


class ComponentItem(TypedDict, total=False):
    data_loc: Required[List[str]]

    label: str


class ComponentComparison(TypedDict, total=False):
    data_loc: Required[List[str]]

    label: str


class Component(TypedDict, total=False):
    item: Required[ComponentItem]

    comparison: ComponentComparison


class AnnotationConfigParam(TypedDict, total=False):
    components: Required[Iterable[Component]]

    annotation_config_type: Literal["flexible", "summarization", "multiturn"]
    """An enumeration."""

    display: Literal["col", "row"]
    """An enumeration."""
