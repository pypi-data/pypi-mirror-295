...
def addExifLayerToMap(gdf, name, m, layer_id, mapElementsDict):
    ...
    def style_function(feature):
        ...
        return {
            'className': f"year-{year}",
            'fillOpacity': 0.1
        }
    geojson = folium.GeoJson(data=mapCopy, name=name, show=show, class_name='animateVisibility ' + layer_id, popup=popup, popup_keep_highlighted=True, style_function=style_function, highlight_function=lambda feature: {'fillOpacity': 0.8})
