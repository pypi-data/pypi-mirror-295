    ...
    def schedule(self, event_handler, path, recursive=False, event_filter=None):
        with self._lock:
            ...
            if self._emitter_for_watch.get(watch) is None:
                emitter = self._emitter_class(event_queue=self.event_queue, watch=watch, event_handler=event_handler, previous_snapshot_file=self._filename, timeout=self.timeout,
                                              event_filter=event_filter, on_snapshot=self.on_snapshot)
                if self.is_alive():
                    emitter.start()
