    ...
    def process(self, exifDf):
        ...
        if not valid_df.empty:
            ...
            aggfuncs = {
                ...
                'CreateDate': lambda x: x.min() if x.nunique() == 1 else f"{x.min()} to {x.max()}"
            }
            logging.info(pointsDf['geometry'].dtype)
            ...
            polygonsDf = polygonsDf[[*groupByListGL, 'Image Count', 'CreateDate', 'geometry']]
...
def setDfColumns(df):
    ...
    if 'CreateDate' not in df.columns:
        # Add the CreateDate column with  as default value
        df['CreateDate'] = '1700:01:01'
        logging.warning("Added CreateDate column with default value '1700:01:01'")
