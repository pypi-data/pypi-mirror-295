...
from .models import (
    SlurpitImportedDevice, SlurpitPlanning, SlurpitSetting, SlurpitIPAddress, SlurpitInterface, SlurpitPrefix, SlurpitVLAN
)
...
from nautobot.ipam.models import Namespace, VRF, IPAddress, RIR, Prefix, VLANGroup, VLAN
...
class SlurpitPrefixForm(NautobotModelForm, TenancyForm, PrefixFieldMixin):
    ...
    ignore_date_allocated = forms.BooleanField(
        required=False,
        label=_('Ignore value for updating data')
    )
    ignore_tenant = forms.BooleanField(
...
class SlurpitVLANForm(NautobotModelForm, TenancyForm):
...
class SlurpitVLANBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(queryset=SlurpitVLAN.objects.all(), widget=forms.MultipleHiddenInput())
    vlan_group = DynamicModelChoiceField(
        queryset=VLANGroup.objects.all(),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["status"] = DynamicModelChoiceField(
            required=True,
            queryset=Status.objects.all(),
            query_params={"content_types": VLAN._meta.label_lower},
        )
