...
from .models import SlurpitImportedDevice, SlurpitLog, SlurpitIPAddress, SlurpitInterface, SlurpitPrefix, SlurpitVLAN
...
from nautobot.ipam.tables import PrefixTable, VLANTable
...
{% tree_hierarchy_ui_representation record.ancestors.count|as_range table.hide_hierarchy_ui%}
<span class="hover_copy">
  <a id="vlan_{{ record.pk }}" pk={{record.pk}} class="reconcile-detail-btn">{{ record.name }}</a>
</span>
"""
...
class SlurpitVLANTable(VLANTable):
    last_updated = tables.Column(
        verbose_name = _('Last updated')
    )

    name = tables.TemplateColumn(
        template_code=VLAN_LINK, attrs={"td": {"class": "text-nowrap"}}, order_by=("name", "vid")
    )
    edit = tables.TemplateColumn(
        template_code=EDIT_LINK,
        verbose_name=_('')
