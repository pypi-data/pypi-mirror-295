...
from ..validator import (
    device_validator, ipam_validator, interface_validator, prefix_validator, vlan_validator
)
...
from nautobot.ipam.models import (
    IPAddress, Namespace, get_default_namespace, VRF, Prefix, VLAN, VLANGroup
)
            ...
            if enable_reconcile:
                ...
                for item in total_data:
                    ...
                    if slurpit_interface_item:
                        ...
                        fields = {'label', 'status', 'type', 'vrf', 'mode', 'description'}
                        not_null_fields = {'label', 'type', 'vrf', 'mode', 'description'}
    ...
    def create(self, request):
        ...
        try:
            ...
            if initial_obj:
                ...
                initial_prefix_values['date_allocated'] = '2024-06-19 12:00:00'
            ...
            if enable_reconcile:
                ...
                for item in total_data:                    
                    ...
                    if slurpit_prefix_item:
                        ...
                        allowed_fields_with_none = {'status'}
                        allowed_fields = {'type', 'role', 'date_allocated', 'tenant', 'description'}
                        ...
                        fields = {'status', 'type', 'role', 'date_allocated', 'tenant', 'description'}
                        not_null_fields = {'type', 'role', 'date_allocated', 'tenant', 'description'}
                        new_prefix = {}
    ...
    def create(self, request):
        ...
        try:
            ...
            if initial_obj:
                ...
                if initial_vlan_values['status'] is not None:
                    status = Status.objects.get(pk=initial_vlan_values['status'])
                ...
                initial_vlan_values['status'] = status
            ...
            if enable_reconcile:
                ...
                for item in total_data:
                    ...
                    slurpit_vlan_item = SlurpitVLAN.objects.filter(name=item['name'], vlan_group=item['hostname'])
                    if not slurpit_vlan_item:
                        slurpit_vlan_item = SlurpitVLAN.objects.filter(vid=item['vid'], vlan_group=item['hostname'])
                    ...
                    if slurpit_vlan_item:
                        ...
                        obj = VLAN.objects.filter(name=item['name'], vlan_group__name=item['hostname'])
                        if not obj:
                            obj = VLAN.objects.filter(vid=item['vid'], vlan_group__name=item['hostname'])
                ...
                while offset < count:
                    ...
                    for vlan_item in batch_qs:
                        ...
                        if not 'vlan_group' in filtered_vlan_item:
                            vlan_group = VLANGroup.objects.create(name=filtered_vlan_item['hostname'])
                            filtered_vlan_item['vlan_group'] = vlan_group
                ...
                batch_update_qs = []
                for update_item in update_data:
                    if not 'vlan_group' in update_item:
                        vlan_group = VLANGroup.objects.create(name=update_item['hostname'])
                        update_item['vlan_group'] = vlan_group

                    item = VLAN.objects.get(name=update_item['name'], vlan_group=update_item['vlan_group'])
                    if not item:
                        item = VLAN.objects.get(vid=update_item['vid'], vlan_group=update_item['vlan_group'])
                    # Update
                    allowed_fields_with_none = {'status'}
                    allowed_fields = {'role', 'tenant', 'description'}
