...
from pinecone_plugins.assistant.data.core.client.model.search_completions import (
    SearchCompletions as ChatRequest,
)
from pinecone_plugins.assistant.control.core.client.models import (
    Assistant as OpenAIAssistantModel,
)
from pinecone_plugins.assistant.data.core.client.model.search_completions_messages import (
    SearchCompletionsMessages as ChatContext,
)
...
class AssistantModel:
    ...
    def upload_file(
        ...
        try:
            with open(file_path, "rb") as file:
                upload_resp = self._assistant_data_api.upload_file(
                        assistant_name=self.assistant.name, file=file, metadata=json.dumps(metadata)
                    ) if metadata else self._assistant_data_api.upload_file(
    ...
    def list_files(self, filter: Optional[dict[str, any]] = None) -> List[FileModel]:
        ...
        >>> km.list_files()
          ...
          'updated_on': '2024-06-02T19:48:00Z'}, ...]
        """
        files_resp = self._assistant_data_api.list_files(self.name, filter=json.dumps(filter)) if filter else self._assistant_data_api.list_files(self.name)
    ...
    def chat_completions(
        ...
        stream: bool = False,
    ) -> Union[ChatResultModel, Iterable[StreamingChatResultModel]]:
        """
        ...
        """
        if stream:
            return self._chat_completions_streaming(messages=messages, filter=filter)
        else:
            return self._chat_completions_single(messages=messages, filter=filter)

    def _chat_completions_single(
        self, messages: List[Message], filter: dict[str, any] = None
    ) -> ChatResultModel:
        messages = [ChatContext(role=ctx.role, content=ctx.content) for ctx in messages]
        context = ChatRequest(messages=messages, filter=filter) if filter else ChatRequest(messages=messages)
    ...
    def _chat_completions_streaming(
        self, messages: List[Message], filter: Optional[dict[str, any]] = None
    ) -> Iterable[StreamingChatResultModel]:
        ...
        content = {"messages": messages, "stream": True}
        if filter:
