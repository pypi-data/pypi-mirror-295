...
class AssistantModel:
            ...
            with open(file_path, "rb") as file:
                if metadata:
                    upload_resp = self._assistant_data_api.upload_file(
                        assistant_name=self.assistant.name, file=file, metadata=metadata
                    )
                else:
                    upload_resp = self._assistant_data_api.upload_file(
    ...
    def list_files(self, filter: Optional[str] = None) -> List[FileModel]:
        """
        ...
        """
        if filter:
            files_resp = self._assistant_data_api.list_files(self.name, filter=filter)
        else:
    ...
    def _chat_completions_single(
        self, messages: List[Message], filter: dict[str, any] = {}
    ) -> ChatResultModel:
        messages = [ChatContext(role=ctx.role, content=ctx.content) for ctx in messages]
        context = ChatRequest(messages=messages, filter=filter)
    ...
    def _chat_completions_streaming(
        ...
        content = {"messages": messages, "filter": filter, "stream": True}
