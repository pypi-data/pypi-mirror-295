...
def extract_docstring_from_object(obj: str | ModuleType | Type  | None) -> dict[str, str]:
    ...
    try:
        ...
        print(f"Only have {sys.modules.keys()}")
        raise ImportError(f"Could not import {obj}: {e}") from e
    module = obj
    try:
        module = importlib._bootstrap.module_from_spec(importlib.util.find_spec(module))
        return extract_docstrings_from_cls(module)
    except (ImportError, AttributeError) as e:
        traceback.print_exc()  # noqa: F821
        print(f"Only have {sys.modules.keys()}")
        raise ImportError(f"Could not import {module}: {e}") from e


if __name__ == "__main__":

    print(extract_docstring_from_object("pydantic"))