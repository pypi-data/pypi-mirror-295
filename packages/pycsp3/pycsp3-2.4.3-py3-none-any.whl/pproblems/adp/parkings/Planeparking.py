...
def table_rewards(flight):
    ...
    for i, parking in enumerate(parkings):
        val = data["strategies"][flight['Stratégie code dep']][target_parkings.index(parking)] if parking in target_parkings else 0
        table.add((i, int(val) if not math.isnan(val) else 0))
    return table
