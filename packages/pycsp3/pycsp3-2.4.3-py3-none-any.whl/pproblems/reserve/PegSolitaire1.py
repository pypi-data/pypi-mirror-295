...
python ppycsp3/pproblems/reserve/PegSolitaire1.py -data=[3,3,0] -variant=english-hybrid -keephybrid

wrt PegSolitaire0, inversion i,j,t in t,i,j (but search -pra=0 -ela=4) n'est plus efficace)
MAIS ok pour :
# java ace PegSolitaire2-english-hybrid-3-3-0.xml -varh=FraOnDom -jl=10000
# java ace PegSolitaire2-english-hybrid-4-4-0.xml -jl=10000 -valh=AsgsE
"""
...
assert variant() in {"english", "french", "3x3", "4x4", "test1", "test2"} and subvariant() in {"dec", "decb", "hybrid"}
...
x = VarArray(size=[nMoves + 1, n, m], dom={0, 1})
...
if subvariant() in ("dec", "decb"):

    def unchanged(i, j, t):
        valid = [k for k, tr in enumerate(transitions) if (i, j) in (tr[0:2], tr[2:4], tr[4:6])]
        if len(valid) == 0:
            return None
        return iff(y[t] not in valid if subvariant("dec") else conjunction(y[t] != k for k in valid), x[t, i, j] == x[t + 1, i, j])
    ...
    def to0(i, j, t):
        ...
        if len(valid) == 0:
            return None
        return iff(y[t] in valid if subvariant("dec") else disjunction(y[t] == k for k in valid), (x[t, i, j] == 1) & (x[t + 1, i, j] == 0))
    ...
    def to1(i, j, t):
        ...
        if len(valid) == 0:
            return None
        return iff(y[t] in valid if subvariant("dec") else disjunction(y[t] == k for k in valid), (x[t, i, j] == 0) & (x[t + 1, i, j] == 1))
    ...
    def table():
        ...
        for k, tr in enumerate(transitions):
            ...
            # secondly, x[t+1,i,j]
            t += [ANY if init_board[i][j] is None else 0 if tr[0:2] == (i, j) or tr[2:4] == (i, j) else 1 if tr[4:6] == (i, j) else col(i * m + j)
                  for i in range(n) for j in range(m)]
