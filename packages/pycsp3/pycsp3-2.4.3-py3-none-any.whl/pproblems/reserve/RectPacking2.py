...
boxes = list(reversed(data.boxes))
nBoxes = len(boxes)

# x[i] is the x-coordinate where is put the ith rectangle
x = VarArray(size=nBoxes, dom=lambda i: range(width - boxes[i].width + 1))

# y[i] is the y-coordinate where is put the ith rectangle
y = VarArray(size=nBoxes, dom=lambda i: range(height - boxes[i].height + 1))
