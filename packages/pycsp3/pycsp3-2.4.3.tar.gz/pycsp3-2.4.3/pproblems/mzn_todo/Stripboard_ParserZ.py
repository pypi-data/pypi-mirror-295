...
next_line()
l = next_line()
data['nets'] = l[l.index('{') + 1:l.rindex('}')].split(', ')
l = next_line()
...
data['components'] = [(s, footprint_w[i][1], footprint_h[i][1]) for i, s in enumerate(components)]
next_line()
pins_dx = []
...
for i in range(len(components)):
    ...
    assert all(pins[i][j] == tt[j][0] for j in range(len(pins[i])))
    pins_net.append(tt)
data['pins'] = [[(s, pins_dx[i][j][1], pins_dy[i][j][1], pins_net[i][j][1]) for j, s in enumerate(t)] for i, t in enumerate(pins)]
