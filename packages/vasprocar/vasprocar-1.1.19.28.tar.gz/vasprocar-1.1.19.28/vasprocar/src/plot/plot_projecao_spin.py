...
for l in range (1,(3+1)):
    ...
    if (l == 1):
       for i in range(len(Si_t)):
           Si_sum = abs(Si_u[i]) + abs(Si_d[i])
           pSx_u = Si_t*0.0;  pSx_d = Si_t*0.0
           if (Si_sum > 0.0): 
              pSx_u[i] = abs(Si_u[i])/(Si_sum);  pSx_d[i] = abs(Si_d[i])/(Si_sum)
           if (Si_sum == 0.0): 
              pSx_u[i] = 0.0;  pSx_d[i] = 0.0
    if (l == 2):
       for i in range(len(Si_t)):
           Si_sum = abs(Si_u[i]) + abs(Si_d[i])
           pSy_u = Si_t*0.0;  pSy_d = Si_t*0.0
           if (Si_sum > 0.0): 
              pSy_u[i] = abs(Si_u[i])/(Si_sum);  pSy_d[i] = abs(Si_d[i])/(Si_sum)
           if (Si_sum == 0.0): 
              pSy_u[i] = 0.0;  pSy_d[i] = 0.0
    if (l == 3):
       for i in range(len(Si_t)):
           Si_sum = abs(Si_u[i]) + abs(Si_d[i])
           pSz_u = Si_t*0.0;  pSz_d = Si_t*0.0
           if (Si_sum > 0.0): 
              pSz_u[i] = abs(Si_u[i])/(Si_sum);  pSz_d[i] = abs(Si_d[i])/(Si_sum)
           if (Si_sum == 0.0): 
              pSz_u[i] = 0.0;  pSz_d[i] = 0.0
    #-------------------------------------------
