...
Sx_ud = rx*0.0;  Sy_ud = rx*0.0;  Sz_ud = rx*0.0
...
for l in range (1,(3+1)):
    ...
    for i in range(len(rx)):
        ...
        #------------------------------------
        if (l == 1):
           Sx_ud[i] = abs(Si_u[i]) + abs(Si_d[i])
           if (Sx_ud[i] > 0.0): 
              pSx_u[i] = abs(Si_u[i])/(Sx_ud[i])
              pSx_d[i] = abs(Si_d[i])/(Sx_ud[i])
        if (l == 2):
           Sy_ud[i] = abs(Si_u[i]) + abs(Si_d[i])
           if (Sy_ud[i] > 0.0): 
              pSy_u[i] = abs(Si_u[i])/(Sy_ud[i])
              pSy_d[i] = abs(Si_d[i])/(Sy_ud[i])
        if (l == 3):
           Sz_ud[i] = abs(Si_u[i]) + abs(Si_d[i])
           if (Sz_ud[i] > 0.0): 
              pSz_u[i] = abs(Si_u[i])/(Sz_ud[i])
              pSz_d[i] = abs(Si_d[i])/(Sz_ud[i])
    #-------------------------------------------
