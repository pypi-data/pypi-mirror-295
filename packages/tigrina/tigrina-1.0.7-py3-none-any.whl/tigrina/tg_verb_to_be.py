...
def fabricate(text):
    ...
    for index, item in enumerate(t_sentence_array):
        ...
        if item in tigrina_pronoun:
            ...
            t_sentence_others.append({'text': item, 'index': index, 'type': 'others'})
    

    # if len(statments_and_questions_verb_to_be) > 0:
    #     for item in statments_and_questions_verb_to_be:
    #         t_sentence_excludes.append(item.index)

    # if len(tigrina_pronoun_collection) > 0:
    #     for item in tigrina_pronoun_collection:
    #         t_sentence_excludes.append(item.index)
