...
def stem_futurePresentPastCoder(code, lastLetter, lastLetterCode, secondLetter, secondLetterCode, sentenceType):
    secondCodeCheckup = lastLetterCode + lastLetter
    thridTriesCodeCheckup = secondLetterCode + lastLetter
    fourthTriesCodeCheckup = secondLetterCode + secondLetter
...
def ends_with_self_explaining_or_stem_expression(info):
    ...
    for item in info['combinationsCollection']:
        if info['lastLetter']=='ን' or info['lastLetter']=='ም' or info['lastLetter']=='ና':
            item = info['secondFromLastLetterOrderIs'] + item
        else:
            item = info['lastLetterOrderIs'] + item
    ...
    results = {}
    guesPronounFromCode = stem_futurePresentPastCoder(info['code'], info['lastLetter'], info['lastLetterOrderIs'], info['secondFromLastLetter'], info['secondFromLastLetterOrderIs'], info['sentenceType'])
    ...
    tobeReplaced = ['1', '2', '3', '4', '5', '6', '7']
    for key in range(len(tobeReplaced)):
        if tobeReplaced[key] == cutoff[0]:
            mynewcut = cutoff.replace(tobeReplaced[key], '')
            cutoff = mynewcut
...
def end_word_additions_cut_off(text):
    ...
    if sliced_word:
        ...
        if not founds:
            ...
            mycode = str(last_letter_order_is) + text['frontErases2'] if text['frontErases2'] else str(last_letter_order_is)
...
def generate_multiple_choices_of_word(text):
    ...
    print("letters_to_be_converted:",letters_to_be_converted)
    letters_to_be_converted_first_letter_index_from_last = alphabet_info['tigrina_alphabets'].index(letters_to_be_converted[0])
    letters_to_be_converted_second_letter_index_from_last = alphabet_info['tigrina_alphabets'].index(letters_to_be_converted[1])
    
    words = [[] for _ in range(24)]
    
    first_letter_on_none_conv = word[0]
    first_letter_on_none_conv_index = alphabet_info['tigrina_alphabets'].index(first_letter_on_none_conv)
    
    
    second_letter_on_none_conv = word[1]
    second_letter_on_none_conv_index = alphabet_info['tigrina_alphabets'].index(second_letter_on_none_conv)
    
    def append_results(result, indices):
        # print("resultt:",result)
        for i in indices:
            key = i % 7
            words[i].append(result.get(key, 'default_value'))
            # print("words:",words)
    word1=[]
    word2=[]
    word3=[]
    ...
    if len(list(text)) == 2:
        ...
        # append_results(result, [0, 1, 2, 3, 5, 6])
        result_2 = get_alphabet_info.get_alphabet(second_letter_on_none_conv)
        word1.append(result_2['first'])
        ...
        # append_results(result, list(range(24)))
        result_2 = get_alphabet_info.get_alphabet(second_letter_on_none_conv)
        ...
        # append_results(result, list(range(15)))
        result_2 = get_alphabet_info.get_alphabet(second_letter_on_none_conv)
    ...
    # choices_collection = [''.join(word) for word in words if word]
    choices = {}
    choices_collection = []
...
class VerbInfo:
    ...
    @staticmethod
    def make_verb_from_stem(text, situation, person, third_person, negativities_or_front_additions, using_ley_letter):
        # try:
