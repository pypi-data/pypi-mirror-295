...
from polylith_cli.polylith import check, distributions, libs
...
def with_third_party_libs_from_lock_file(project_data: dict) -> dict:
    ...
    merged = {**project_data, **{'deps': {'items': third_party_libs, 'source': filename}}}
    return merged

def check_libs_versions(projects_data: List[dict], all_projects_data: List[dict], options: dict) -> bool:
    is_strict = options['strict']
    is_quiet = options['quiet']
    if not is_strict:
        return True
    development_data = next((p for p in all_projects_data if p['type'] == 'development'))
    libraries = libs.report.libs_with_different_versions(development_data, projects_data)
    if not is_quiet:
        libs.report.print_libs_with_different_versions(libraries, development_data, projects_data, options)
...
def run(root: Path, ns: str, project_data: dict, options: dict) -> bool:
    ...
    collected_libs = distributions.known_aliases_and_sub_dependencies(deps, alias, options)
