...
from polylith_cli.polylith import configuration, deps, diff, info, repo
...
def flatten_dependent_bricks(changed_bricks: Set[str], bases: Set[str], components: Set[str], import_data: dict) -> Set[str]:
    matrix = [deps.report.sorted_used_by(brick, bases, components, import_data) for brick in changed_bricks]
    flattened: List[str] = sum(matrix, [])
    filtered = set(flattened).difference(changed_bricks)
    return filtered

def calculate_dependent_bricks(root: Path, ns: str, projects_data: List[dict], changed_bricks: Set[str]) -> dict:
    ...
    dependent_bricks = flatten_dependent_bricks(changed_bricks, bases, components, import_data)
...
def print_views(root: Path, tag: str, options: dict) -> None:
    ...
    if short and (not only_bricks):
        affected_projects = diff.collect.get_projects_affected_by_changes(projects_data, changed_projects, changed_bases, changed_components)
        diff.report.print_projects_affected_by_changes(affected_projects, short)
        return
    dependent_bricks = calculate_dependent_bricks(root, ns, projects_data, changed_bricks) if with_deps else {}
    diff.report.print_detected_changes_in_bricks(changed_bases, changed_components, dependent_bricks, options)
