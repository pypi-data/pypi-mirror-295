...
class TimingCalibration:
    ...
    def _find_all_raising_edge_xings(self, event: dict, channel: int, threshold: int):
        """Returns all raising edge xings in an event.
        ...
        Returns:
            List of all raising crossings between event data and threshold
        """
    ...
    def _distances_between_two_peaks(
        ...
        for idx, evt in enumerate(events):
            if "data" not in evt.keys():
                continue
            xings = self._find_all_raising_edge_xings(evt, channel, threshold)
            if len(xings) == 0:
                continue
