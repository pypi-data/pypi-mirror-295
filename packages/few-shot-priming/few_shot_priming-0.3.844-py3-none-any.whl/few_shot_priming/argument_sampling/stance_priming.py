...
def generate_most_similar_stance_arguments(experiment, different_topics, debug):
    ...
    if different_topics:
        for _, test_record in tqdm(dataset["test"].iterrows()):
            ...
            training_instances = list(df_training.itertuples(index=False))
            training_instances.sort(key=lambda x: x.score, reverse=True, inplace=True)
            for k in ks:
                topic_set = set()
                id_set = set()
                samples= []
                ...
                while len(samples) < k:
                    for training_record in training_instances:
                        if exists_enough_topics and training_record.topic not in topic_set and training_record.id not in id_set:
                            ...
                            topic_set.add(training_record.topic)
                        if len(samples) == k:
                            samples = [instance for instance in training_instances if instance.id in id_set]
                            all_samples.extend(samples)
                            break
                    if len(samples) < k:
                        exists_enough_topics = False
        df_samples = pd.DataFrame.from_records(all_samples, columns=all_samples[0]._fields)
