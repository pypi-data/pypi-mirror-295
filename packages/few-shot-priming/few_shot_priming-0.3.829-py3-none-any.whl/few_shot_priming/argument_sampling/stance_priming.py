...
def generate_most_similar_stance_arguments(experiment, different_topics, debug):
    ...
    for _, test_record in dataset["test"].iterrows():
        instances_scores = similarities[test_record["id"]]
        instances_scores = zip(instances_scores.key(), instances_scores.values())
        instances_scores.sort(key = lambda x: x[1])
        if experiment == "vast":
            ks = [3, 6, 12, 24, 48, 96]
        else:
            ks in [2, 4, 8, 16, 32, 64]

        for k in ks:
            top_indices, top_scores = zip(*instances_scores[:k])
            samples = df_training["id"].isin(top_indices)
    ...
    df_samples.to_sv(path_samples, sep="\t", encoding="utf-8", columns=["topic", "stance", "text", "id", "score", "test-id", "k"])
