...
def convert_to_nearest_preferred_unit(
    ...
    for unit in preferred_units:
        converted_quantity = interface.changeQuantityUnit(
            quantity,
            unit,
            formatter=mockedQuantityChangeUnitFormatter
        )
    ...
    # Ordenate data by integer size
    sorted_int = sorted(data, key = lambda entry: entry['integer_size'])
    nonzero_sorted_int = []
    for item in sorted_int:
        # Delete items with zero-size integer part
        if item['integer_size'] != 0:
            nonzero_sorted_int.append(item)
    
    if len(nonzero_sorted_int) == 0:
        # All items have zero-size integer part. We need to sort them by decimal part now.
        selected_quantity = quantity
        
        # Ordenate data by decimal size
        sorted_decimal = sorted(data, key = lambda entry: entry['decimal_size'])
        nonzero_sorted_decimal = sorted_decimal
        for item in sorted_decimal:
            # Delete items with zero-size decimal part
            if item['decimal_size'] != 0:
                nonzero_sorted_decimal.append(item)

        if len(nonzero_sorted_decimal) == 0:
            # All items have zero-size decimal part, do nothing.
            selected_quantity = quantity
        else:
            # The chosen quantity will be the one with the smallest decimal part.
            selected_quantity = nonzero_sorted_decimal[0]['converted_quantity']
    else:
        # The chosen quantity will be the one with the smallest integer part.
