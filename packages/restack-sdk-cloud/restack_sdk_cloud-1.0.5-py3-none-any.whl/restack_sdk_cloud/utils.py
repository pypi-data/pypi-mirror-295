...
def pretty_print_deploy_results(stacks: list[DeployedStack]) -> None:
    for stack in stacks:
        stack_name = getattr(stack, 'name', 'Unknown')
        print(Style.BRIGHT + f'\n=== Stack: {Fore.BLUE}{stack_name}{Style.RESET_ALL} ===')

        stack_error = getattr(stack, 'error', None)
        if stack_error:
...
def get_application_with_github_app_error(deployed_stacks: list[DeployedStack]) -> DeployedApplication | None:
    for stack in deployed_stacks:
        if not hasattr(stack, 'applications'):
            continue
        app = next((app for app in stack.applications if hasattr(app, 'type') and app.type == errorCodes.GITHUB_APP_INSTALLATION_REQUIRED), None)
        if app:
            return app
    return None
