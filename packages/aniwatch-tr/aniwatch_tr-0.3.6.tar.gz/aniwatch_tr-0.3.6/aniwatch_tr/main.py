...
from aniwatch_tr.fetch import fetch_data
from aniwatch_tr.watch import watch_anime
...
def main():
    ...
    animeapp = GetAnimeApp(use_vlc=args.vlc)
    ...
    selected_name, selected_id = animeapp.select_anime(anime_data)
    
    animeapp.episodes = fetch_dt.fetch_anime_eps(selected_id=selected_id)
    if animeapp.episodes:
        animeapp.current_anime_name = selected_name
        animeapp.current_episode_index = 0
        
        # Menü döngüsünü başlat
        while True:
            animeapp.clear_screen()  # Ekranı temizle
            option = animeapp.display_menu()
            animeapp.handle_menu_option(option)
    else:
        print("Bölüm bilgileri bulunamadı.")
