...
if sys.version_info < (3, 9):
    err, s = (RuntimeError, "To use TensePy library, consider having Python 3.9 or newer. For versions 3.5 - 3.8 consider using function 'tense.revitalize.older()'")
    raise err(s)
...
class Tense(NennaiAbroads, NennaiStringz, NennaiRandomize, Time, Math, tc.Positive[str], tc.Negative[str], tc.Invertible[str]):
    ...
    @_cm
    def clear(self, *seqs: __tc.MutableSequence[_T]):
        """
        \\@since 0.3.27a4
        ```ts \\
        "class method" in class Tense
