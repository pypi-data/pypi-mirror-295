...
_AbroadInitializer = tc.AbroadInitializer[int]
...
def _reckon_prepend_init(*countables: _ReckonTypePre[_T]):
    import typing as tp, io, collections as ct, tense.types_collection as __tc, tkinter as tk
    i = 0
    for e in countables:
        if isinstance(e, __tc.ReckonOperable): i += e.__reckon__()
        if isinstance(e, array): i += e.__len__()
        elif isinstance(e, (io.TextIOWrapper, io.FileIO, io.BufferedWriter, io.BufferedRandom, io.BufferedReader, tp.IO, tp.TextIO, tp.BinaryIO, io.BufferedIOBase, io.BufferedRWPair, io.BytesIO, io.StringIO)):
            for r in e.read():
           ...
           for _ in e.get():
               i += 1
        elif isinstance(e, (str, enumerate, tp.Sequence, tp.MutableSequence, ct.deque, list, tuple, ct.Counter, ct.defaultdict, dict, set, frozenset, range, bytes, bytearray, mmap, memoryview,
                            tp.Mapping, tp.MutableMapping, ct.OrderedDict, ct.ChainMap, tp.MutableSet, tp.AbstractSet, tp.Iterable, tp.AsyncIterable)):
            for _ in e:
                i += 1
        else:
            err = TypeError
...
def _abroad_prepend_init(value1: _AbroadValue1Pre[_T1], /, value2: _AbroadValue2Pre[_T2] = None, modifier: _AbroadModifierPre[_T3] = None):
    import math, tense.types_collection as __tc
    conv: list[int] = []
    [v1, v2, m] = [0, 0, 0]
    # v1 (value1)
...
class Math:
    ...
    def triangular(self, n: int, /):
        ...
        if not isinstance(n, int) or (isinstance(n, int) and n < 1):
            err, s = (TypeError, "Expected a positive integer")
            raise err(s)
        return int((n * (n + 1)) / 2)
    @classmethod
...
def reckon(*countables: _ReckonType[_T]):
    ...
    `frozenset`, `collections.Counter`, `defaultdict` (also counts pairs), \\
    ...
    since 0.3.26b3 - `AsyncIterable` and since 0.3.26c1 - objects supporting the \\
    magic method `__reckon__`.
    """
    i = 0
    for e in countables:
...
class YamiTk:
    ...
    """
    import tkinter as __tk, tense.types_collection as __tc
    __loc_tk = None
    __loc_frame = None
    __loc_label = None
    __loc_button = None
...
__author__ = "Aveyzan, John \"Aveyzan\" Mammoth <p1k4zx@gmail.com>"
"\\@since 0.3.26rc3"
__license__ = "MIT"
"\\@since 0.r3.26c3"