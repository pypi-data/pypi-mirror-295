...
def validate_torch_version(actual_version):
    if Version(MIN_TORCH_BACKEND_VERSION_FOR_RULE_ID_API) not in SimpleSpec(f'<={actual_version}'):
        raise TorchSdkException(f'This API is not supported for Torch version {actual_version}.')
...
def calculate_months_before_time(months_before):
    """
    Calculate the time range starting before the given number of months from the beginning of the current month.
...
def calculate_time_range(filter):
    if filter.period is not None:
        ...
        if filter.period == ExecutionPeriod.Last15minutes:
            started_after_time, finished_before_time = calculate_minutes_before_time(15)
        elif filter.period == ExecutionPeriod.Last30minutes:
            started_after_time, finished_before_time = calculate_minutes_before_time(30)
        elif filter.period == ExecutionPeriod.Last1hour:
            started_after_time, finished_before_time = calculate_hours_before_time(1)
        elif filter.period == ExecutionPeriod.Last3hours:
            started_after_time, finished_before_time = calculate_hours_before_time(3)
        elif filter.period == ExecutionPeriod.Last6hours:
            started_after_time, finished_before_time = calculate_hours_before_time(6)
        elif filter.period == ExecutionPeriod.Last12hours:
            started_after_time, finished_before_time = calculate_hours_before_time(12)
        elif filter.period == ExecutionPeriod.Last24hours:
            started_after_time, finished_before_time = calculate_hours_before_time(24)
            ...
            started_after_time, finished_before_time = calculate_days_before_time(1)
        elif filter.period == ExecutionPeriod.Last7days:
            started_after_time, finished_before_time = calculate_days_before_time(7)
            ...
            started_after_time, finished_before_time = calculate_months_before_time(1)
        elif filter.period == ExecutionPeriod.Last3month:
            started_after_time, finished_before_time = calculate_months_before_time(3)
...
class TorchHttpClient(TorchClientInterFace, ABC):
    ...
    def get_policy(self, identifier) -> RuleResource:
        ...
        actual_version = extract_actual_version(torch_version)
        validate_torch_version(actual_version)
    ...
    def get_all_rules(self, filter: PolicyFilter, page=0, size=25, withLatestExecution=True,
        ...
        if filter.period is not None:
            finished_before_time, started_after_time = calculate_time_range(filter)
