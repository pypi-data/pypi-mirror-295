...
class MetaArray(object):
    ...
    MetaArray is also meant to be used in situations where datasets are periodically appended to along one axis:

        image_data = np.zeros((n_frames:=10, rows:=512, cols:=512))
        ma = MetaArray(image_data, info=[
            {'name': 'Time', 'values': np.linspace(0, 1, n_frames), 'position': np.zeros((n_frames, 3)},
    """
