...
class BeepNotify:
    ...
    def get_auth_token(self, beep_code, acode):
        """
        Returns auth token.
        :param beep_code:
        :param acode:
    ...
    def authorize_user(self, token):
        response = requests.post(f"https://beep.api.worldz.tech/api/auth/{self.project}/auth/", data={
            'token': token,
        })
    ...
    def check_authorization(self, token):
        response = requests.get(f'https://vpn.back.worldz.tech/api/management/check_auth/', headers={
            'BEEP-ATOKEN': token
        })
