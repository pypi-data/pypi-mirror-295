...
def compute_detected_signal(source: Source, detector: Detector, scatterer_distribution: ScattererDistribution, granularity: float = 1.0, A: float = 1.5, n: float = 2.0) -> float:
    ...
    pms_scatterer = Sphere(
        diameter=scatterer_distribution.sizes.magnitude,
        index=scatterer_distribution.refractive_index,
        medium_index=1.0,
        source=pms_source
    )
    ...
    coupling = experiment.get(measure=measure.coupling, export_as_numpy=True)

    return coupling.squeeze() * detector.responsitivity * ureg.watt
