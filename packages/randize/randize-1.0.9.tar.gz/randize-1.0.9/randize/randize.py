...
class Randize:
    ...
    @staticmethod
    def random_text(language='en', word_count=50):
        """
        Generates random text in English and translates it into a given language.

        Parameters:
        - language (str): The language code to translate the text into (default is 'en').
        - word_count (int): The number of words to generate (default is 50).

        Returns:
        - str: Randomly generated text in the specified language.
        """
        # Generate the exact number of words using the `lorem` library
        text = ' '.join(lorem.text().split()[:word_count])  # Generate text and slice to word count

        # If the language is English, no translation is needed
        if language == 'en':
            return text

        # Attempt to translate the text into the specified language
        try:
            translated_text = GoogleTranslator(source='it', target=language).translate(text)
