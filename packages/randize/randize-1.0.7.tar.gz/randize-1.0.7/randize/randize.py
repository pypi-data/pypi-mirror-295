...
class Randize:
    ...
    def random_text(language='en', word_count=50):
        """
        Генерирует случайный текст на английском языке и переводит его на заданный язык.
        :param language: Язык текста ('en', 'ru', 'es', etc.)
        :param word_count: Количество слов в тексте
        :return: Строка случайного текста на выбранном языке
        """
        # Генерируем случайный текст на английском
        text = ' '.join(lorem.sentence().split()[:word_count])

        # Если выбран английский, возвращаем текст без перевода
        if language == 'en':
            return text
