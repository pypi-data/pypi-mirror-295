...
class client:
    ...
    def post(self, url:str, headers:dict={}, data=Optional[Union[str, dict]], method:str='POST'):
        ...
        if self.running:
            return 'create a new client for each thread\nexample: https://github.com/xsxo/fiberhttp/examples/threads.py'

        elif host not in self.hosts:
            self.hosts[host] = new_connection(host, parsed_url.port or (80 if parsed_url.scheme == 'http' else 443), time_out=self.time_out)
    ...
    def connect(self, host:str, ssl_verify:bool=True):
        if host not in self.hosts:
            self.hosts[host] = new_connection(host, (443 if ssl_verify else 80), time_out=self.time_out)
