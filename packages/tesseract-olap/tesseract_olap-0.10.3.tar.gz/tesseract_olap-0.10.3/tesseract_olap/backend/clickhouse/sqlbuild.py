...
def dataquery_sql(query: DataQuery) -> Tuple[Selectable, ParamManager]:
                    ...
                    if table_dim is table_fact:
                        ...
                        qb = qb.where(
                            field_time.isin(
                                ...
                                .limit(lvlfi.time_restriction.amount)
                            )
                        )
    ...
    def dataquery_tgroup_sql(tcore: Selectable) -> Selectable:
        ...
        for msrfi in query.fields_quantitative:
            ...
            if msrfi.constraint == NullityOperator.ISNULL:
                criterion = column.isnull()
            elif msrfi.constraint == NullityOperator.ISNOTNULL:
                criterion = column.isnotnull()
            else:
                criterion = _get_filter_comparison(column, msrfi.constraint[0])
                # add second constraint to criterion if defined
                if len(msrfi.constraint) == 3:
                    criterion2 = _get_filter_comparison(column, msrfi.constraint[2])
...
def _get_filter_comparison(field: Field, constr: NumericConstraint):
