...
class NullityOperator(str, Enum):
...
class RestrictionScale(str, Enum):
    ...
    @classmethod
    def from_str(cls, string: str):
        assert string, "Invalid RestrictionScale: no value provided"
        key = string.strip().upper()
        value = SYMBOL_MAP.get(key)
        if not isinstance(value, cls):
            raise ValueError(f"Invalid RestrictionScale value: {string}")
        return value
...
SYMBOL_MAP = {
    ...
    "YEAR": RestrictionScale.YEAR,
    "QUARTER": RestrictionScale.QUARTER,
    "MONTH": RestrictionScale.MONTH,
    "WEEK": RestrictionScale.WEEK,
    "DAY": RestrictionScale.DAY,
}
