...
def _filter_parse(value: str) -> Tuple[str, FilterCondition]:
    field, condition = value.split(".", 1)

    if condition.lower() in ("isnull", "isnotnull"):
        return field, NullityOperator.from_str(condition)
...
"""
    ...
    return dict(_filter_parse(item) for item in filters.split(",") if item != "")
