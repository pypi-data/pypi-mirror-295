...
class TesseractModule(ll.LogicLayerModule):
    ...
    def debug_flush(self, token: Optional[ll.AuthToken] = Depends(auth_token)):
        if "sysadmin" in self.auth.get_roles(token):
    ...
    def debug_sql(
        ...
        # TODO: make it use the Accept HTTP header to answer in json or html
        return {"request": repr(params), "query": repr(query), "sql": sql}
