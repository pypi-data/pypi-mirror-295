...
def router(rank_base_folder='./rank/', check_token=None, int_rank_limit_days=30, int_rank_limit_number=30):
    ...
    async def set_rank(item: ScoreItem):
        try:
            ...
            lst_cache = get_rank_json(item.str_game_name, item.str_rank_file)
            #print('get cache file')
            new_ranking = update_ranking(lst_cache, item.str_user, item.str_name,
    ...
    def get_rank(str_user: str = '190041-s090504',
        ...
        lst_cache = get_rank_json(str_game_name, str_rank_file)
        if lst_cache:
            #print('找到排行榜')
            return lst_cache
        print('找不到排行檔案，改用預設值')
    ...
    def get_rank_json(str_game_name, str_ranking_file):
