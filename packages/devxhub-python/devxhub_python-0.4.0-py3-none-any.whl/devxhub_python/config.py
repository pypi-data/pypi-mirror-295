...
from devxhub_python.exceptions import ConfigDoesNotExistException, InvalidConfiguration
...
USER_CONFIG_PATH = os.path.expanduser('~/.devxhub_pythonrc')
...
DEFAULT_CONFIG = {
    'devxhub_pythons_dir': os.path.expanduser('~/.devxhub_pythons/'),
    'replay_dir': os.path.expanduser('~/.devxhub_python_replay/'),
...
def get_config(config_path: Path | str) -> dict[str, Any]:
    ...
    raw_cookies_dir = config_dict['devxhub_pythons_dir']
    config_dict['devxhub_pythons_dir'] = _expand_path(raw_cookies_dir)
...
def get_user_config(
    ...
    Otherwise look up the config file path in the ``devxhub_python_CONFIG``
    ...
    try:
        ...
        env_config_file = os.environ['devxhub_python_CONFIG']
    except KeyError:
