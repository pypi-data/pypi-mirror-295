...
from devxhub_python import __version__
from devxhub_python.config import get_user_config
from devxhub_python.exceptions import (
...
from devxhub_python.log import configure_logger
from devxhub_python.main import devxhub_python


def version_msg() -> str:
    """Return the devxhub_python version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"devxhub_python {__version__} from {location} (Python {python_version})"
...
def list_installed_templates(
    ...
    """List installed (locally cloned) templates. Use devxhub_python --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    devxhub_python_folder: str = config['devxhub_pythons_dir']
    if not os.path.exists(devxhub_python_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {devxhub_python_folder}"
        )
        sys.exit(-1)

    template_names = [
        folder
        for folder in os.listdir(devxhub_python_folder)
        if os.path.exists(
            os.path.join(devxhub_python_folder, folder, 'devxhub_python.json')
        )
    ]
...
@click.option(
    '--no-input',
    is_flag=True,
    help='Do not prompt for parameters and only use devxhub_python.json file content. '
...
@click.option(
    '--directory',
    help='Directory within repo that holds devxhub_python.json file '
...
def main(
    ...
    """Create a project from a devxhub_python project template (TEMPLATE).
    ...
    try:
        devxhub_python(
            ...
            password=os.environ.get('devxhub_python_REPO_PASSWORD'),
            directory=directory,
