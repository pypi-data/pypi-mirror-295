...
from devxhub_python.find import find_template
from devxhub_python.hooks import run_hook_from_repo_dir
from devxhub_python.prompt import YesNoPrompt
from devxhub_python.utils import (
...
def is_copy_only_path(path: str, context: dict[str, Any]) -> bool:
    ...
    :param context: devxhub_python context.
    """
    try:
        for dont_render in context['devxhub_python']['_copy_without_render']:
        ...
        if isinstance(context_value, list):
            ...
            if isinstance(overwrite, list):
                ...
                if overwrite in context_value:
                    ...
                    # see ``devxhub_python.prompt.prompt_choice_for_config``
...
def generate_context(
    context_file: str = 'devxhub_python.json',
    default_context: dict[str, Any] | None = None,
    extra_context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Generate the context for a devxhub_python project template.

    Loads the JSON file as a Python object, with key being the JSON filename.

    :param context_file: JSON file containing key/value pairs for populating
        the devxhub_python's variables.
...
def generate_file(
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the devxhub_python's variables.
    ...
    if context['devxhub_python'].get('_new_lines', False):
        # Use `_new_lines` from context, if configured.
        newline = context['devxhub_python']['_new_lines']
...
def _run_hook_from_repo_dir(
    ...
    :param context: devxhub_python project context.
    :param delete_project_on_failure: Delete the project directory on hook
        failure?
    """
    warnings.warn(
        "The '_run_hook_from_repo_dir' function is deprecated, "
        "use 'devxhub_python.hooks.run_hook_from_repo_dir' instead",
        DeprecationWarning,
        2,
    )
