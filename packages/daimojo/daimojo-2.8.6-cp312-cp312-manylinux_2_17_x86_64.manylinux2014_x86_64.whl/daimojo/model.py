...
class model:
    def __init__(self, filename):
        ...
        self.cppmodel = daimojo.cppmojo.model(
            self.modelfile, daimojo.__path__[0])
        ...
        SupportedOps = namedtuple('SupportedOps', [
    ...
    def schema(self, pred_contribs=False, pred_contribs_original=False, interval=False):
        return self.cppmodel.schema(
            pred_contribs, pred_contribs_original, interval)

    def predict(self, pydt, pred_contribs=False, pred_contribs_original=False, interval=False, debug=False):
        ...
        if missing_cols:
            ...
            for c in missing_cols:
                missing_col_info += c + \
                    '(' + \
                    self.feature_types[self.feature_names.index(c)] + '); '
