...
export const useOrderFormStore = defineStore('OrderForm', () => {
  ...
  const mapForm = () => {
    let mappedForm: IMappedOrder = {
      ...
      aircrafts: formModel.aircraft?.id ? [{ aircraft_id: formModel.aircraft?.id }] : [],
      fuel_aircrafts: formModel.aircraft?.id ? [{
        aircraft_id: formModel.aircraft,
        fuel_quantity: formModel.fuel_order!.fuel_quantity!,
        fuel_uom: formModel.fuel_order!.fuel_uom!.id!,
      }] : [],
