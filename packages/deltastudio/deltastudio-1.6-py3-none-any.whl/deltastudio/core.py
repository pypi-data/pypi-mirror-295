...
class app:
    ...
    def table_query(self, script: str):
        ...
        with engine.connect() as conn:
            sql = f'''
                ...
                where "type" = 'table' and du.username = '{self.username}' and dou.access_level like '%r%'
            '''
        ...
        for item in output:
            if f' {item[0]} ' in script:
                script = script.replace(f' {item[0]} ', f' delta_scan("{item[1]}") ')
        script = normalize_script(script)
    ...
    def add_source_to_datawarehouse(self, source_name: str, query: str):
        try:
            tempdir_path = Path('/home/delta/.temp')
            tempdw_path = tempdir_path.joinpath('tempdw.db')
            tempfile_path = tempdir_path.joinpath(f'{source_name}')
        
            if not tempdir_path.exists():
                tempdir_path.mkdir(parents=True, exist_ok=True)
                
            # write the query to parquet
            self.sql(query).write.mode('overwrite').parquet( tempfile_path.__str__() )
        
            # connect to tempdb and create a tables based on the file
            conn = duckdb.connect( tempdw_path.__str__() )
            conn.sql(f'''
                create or replace table {source_name} as
                select * from read_parquet('{tempfile_path}/*.parquet')
            ''')
