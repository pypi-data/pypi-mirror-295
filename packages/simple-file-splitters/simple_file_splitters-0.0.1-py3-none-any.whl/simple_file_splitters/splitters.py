...
def split_Docx_Recursive(path: str, separators=["\n\nTable_Start\n", "\n\n\n", "\n\n", "\n", " ", ""], chunk_size=1024, chunk_overlap=750) -> list[dict]:
    ...
    NOTE: Since the docx parser can separate text and table sections, these are separated by a '\\n\\nTable_Start\\n' and a\
        subsequent '\\n\\n\\n' from the text blocks. To keep the tables together as a block when splitting (since otherwise\
        a lot of context is lost), the list of separators should have those expressions as highest priority. This is the default,\
        but when overriding the separators, this must be done manually.
