...
skip_dbt_option = click.option(
    "--no-dbt",
    "skip_dbt",
    required=False,
    help="Will skip building Hashboard models defined in dbt.",
    is_flag=True,
    default=False,
)
...
dbt_select_option = click.option(
    "--dbt-select",
    "dbt_select_string",
    required=False,
    help="A list of arguments to pass into the dbt select flag and determine which dbt defined Hashboard models should be built. If not provided, all dbt models will be built. If your build is not configured to read from your dbt project, this flag will be ignored.",
    type=click.STRING,
)
...
def build_create(
    ...
    dbt_manifest_path, dbt_metadata = handle_dbt_args(
        dbt_artifacts_path, skip_dbt, dbt_select_string, dbt_state, dbt_force_prod
    )
