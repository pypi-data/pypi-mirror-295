...
# Define the callback to save the trace
def save_trace(prof):
    # Specify the directory where traces will be saved
    user_home_dir = os.path.expanduser("~")
    trace_dir = user_home_dir + "/.stgraph/logs/profiler"
    # Save the trace
    prof.export_chrome_trace(f"{trace_dir}/trace_{prof.step_num}.json")
...
def train(
    ...
    with open(output_file_path, "w") as f:
        ...
        # nan_table_before = DataTable("NaN and Inf Truth Table Before Training", ["(index, k)", "y_hat", "edge_weight", "hidden_state"])
        # nan_table_after = DataTable("NaN and Inf Truth Table After Training", ["(index, k)", "y_out", "y_hat", "hidden_state"])
