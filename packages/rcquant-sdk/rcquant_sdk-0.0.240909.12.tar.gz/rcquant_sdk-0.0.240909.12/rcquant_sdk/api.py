...
def fin_read_instruments(exchange_id: List[str] = [], instrument_id: List[str] = [], unicode: List[str] = [], **kwargs):
    ...
    '''
    base_path = '' if 'base_path' not in kwargs.keys()else kwargs.get('base_path')
    return FinDataClient.instance().findata_handle().fin_read_instruments(ReadInstrumentParamData(exchange_id=exchange_id, instrument_id=instrument_id, unicode=unicode, base_path=base_path))
...
def fin_save_instruments(df, **kwargs):
    ...
    '''
    base_path = '' if 'base_path' not in kwargs.keys()else kwargs.get('base_path')
    return FinDataClient.instance().findata_handle().fin_save_instruments(df=df, base_path=base_path)
