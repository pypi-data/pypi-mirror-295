...
def display(name = ""):
    available = {'bfs      ' : "Breadth First Search",
                 ...
                 'sudoku   ' : "Solving Sudoku(Simulated Annealing)",
...
def ghost(key = None, what = ""):
    if key and isinstance(key,str) and key == str(x.now().time())[:8]:
        available = {'sudoku   ' : "Solving Sudoku(Loading Bar)"}
        try:
            match what.lower():
                case "sudoku"   :   print(sudoku_lb)
                case _:
                    print("Invalid Value! Refer Below Table")
                    for k, v in available.items():
                        print(k,v,sep = " : ")
