...
from zhixin import __version__, exception, fs, proc
...
from zhixin.project.helpers import get_default_projects_dir
...
DEFAULT_SETTINGS = {
    "check_zhixin_interval": {
        "description": "Check for the new ZhiXin Core interval (days)",
        "value": 7,
    },
    ...
    "enable_telemetry": {
        "description": ("Telemetry service (Yes/No)"),
        "value": True,
    },
    "force_verbose": {
    ...
    "projects_dir": {
        "description": "Default location for ZhiXin projects (ZhiXin Home)",
...
def resolve_state_path(conf_option_dir, file_name, ensure_dir_exists=True):
    state_dir = ProjectConfig.get_instance().get("zhixin", conf_option_dir)
...
def get_setting(name):
    _env_name = "ZHIXIN_SETTING_%s" % name.upper()
...
def is_disabled_progressbar():
    return os.getenv("ZHIXIN_DISABLE_PROGRESSBAR") == "true"
