...
class NbSite(NetboxBase):
    def __init__(self, komora_obj):
        ...
        self.latitude = float(komora_obj.latitude[0:9]) if komora_obj.latitude else None
        self.longitude = (
            round(float(komora_obj.longitude[0:10]), 6)
            if komora_obj.longitude
            else None
        )

        self.komora_id = komora_obj.id
        self.komora_url = (
            f"{get_config()['komora']['KOMORA_URL']}/app/site/{self.komora_id}"
        )
        ...
        try:
            if komora_obj.address:
                ...
                if municipality_komora_id:
                    self.region = self.nb.dcim.regions.get(
        ...
        for location in self.site_locations:
            parent = (
                next(
                    (
                        parent
                        for parent in self.site_locations
                        if parent.id == location.parentId
                    ),
                    None,
                )
