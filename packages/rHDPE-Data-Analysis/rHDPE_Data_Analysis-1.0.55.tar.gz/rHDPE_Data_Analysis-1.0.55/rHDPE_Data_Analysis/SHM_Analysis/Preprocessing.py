...
def read_raw_data_file_1( filename, f, resin_data, file_data, data ):
    ...
    with open( filename, 'r' ) as file:
        ...
        for line in lines:
            ...
            if line.rstrip():
                ...
                column_data[0].append( np.array( float( a_list[3] ) ) )
                column_data[1].append( np.array( float( a_list[5] ) ) )
    ...
    data[2].append( column_data[0] * column_data[0] - 1 / column_data[0] )
...
def write_csv( output_directory, file_data, data ):
    ...
    for i, f in enumerate( file_data ):
        ...
        np.savetxt( output_directory + "SHM/Condensed_Data/Resin{}_{}_.csv".format( f[0], f[1] ), array, delimiter = ",", fmt = "%.4f" )

    array = np.array( file_data )

    np.savetxt( output_directory + "SHM/File_Data/file_data.csv", array, delimiter = ",", fmt = "%s" )
...
def read_csv( directory, output_directory, merge_groups ):
    ...
    df = pd.read_csv( output_directory + "SHM/File_Data/file_data.csv", sep = ",", header = None )
    ...
    filenames = sorted( [os.path.basename( path ) for path in glob.glob( output_directory + "SHM/Condensed_Data/*" )], key = gu.sort_raw_files_3 )
