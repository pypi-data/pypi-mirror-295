...
def compute_derivatives( data, width = 1 ):
    ...
    for i in range( len( data[5] ) ):
        ...
        derivative_data[4].append( gu.derivative( data[5][i], data[4][i], width ) )
...
def extract_TT_features( output_directory, file_data, data, first_derivative_data, second_derivative_data ):
    ...
    for i in range( len( file_data ) ):
        ...
        array = np.where( np.array( data[4][i] ) > 5 )[0]

        feature_1.append( np.array( data[5][i] )[array[-1]] ) # Changed definition of sab to be the largest strain at which the stress is greater than 5.
    ...
    for i in range( len( file_data ) ):
        ...
        feature_1.append( np.array( data[4][i] ).max() )
