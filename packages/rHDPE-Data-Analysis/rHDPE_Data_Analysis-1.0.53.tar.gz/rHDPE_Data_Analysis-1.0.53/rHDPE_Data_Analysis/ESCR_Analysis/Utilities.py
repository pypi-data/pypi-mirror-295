...
def extract_ESCR_features( output_directory, file_data, data ):
    ...
    for i in range( len( file_data ) ):
        ...
        mask = np.where( np.array( data[1][i] ) <= 10 )[0]
        ...
        if file_data[i][0] == 3 or file_data[i][0] == 9:
            ...
            if np.array( data[0] )[mask[0]] == 48:

                hours_1, hours_2 = 24, 48

            else:

                hours_1, hours_2 = np.array( data[0] )[mask[0] - 1], np.array( data[0] )[mask[0]]

        else:

            hours_1, hours_2 = np.array( data[0] )[mask[0] - 1], np.array( data[0] )[mask[0]]

        percentage_1, percentage_2 = np.array( data[1][i] )[mask[0] - 1], np.array( data[1][i] )[mask[0]]
    ...
    for i in range( len( file_data ) ):
        ...
        mask = np.where( np.array( data[1][i] ) <= 25 )[0]
    ...
    for i in range( len( file_data ) ):
        ...
        mask = np.where( np.array( data[1][i] ) <= 50 )[0]
