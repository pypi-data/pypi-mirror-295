...
def unzip_file(
    ...
    if destination is None:
        unzip_file = tempfile.TemporaryDirectory(prefix="tmp_unzipped_", dir=Path.cwd().absolute().as_posix())
    else:
        unzip_file = tempfile.TemporaryDirectory(prefix="tmp_unzipped_", dir=destination.as_posix())
    ...
    with zipfile.ZipFile(file, "r") as zip_ref:
        ...
        for file_ in files_to_extract:
            zip_ref.extract(file_, path=Path(unzip_file.name))    
    return unzip_file
