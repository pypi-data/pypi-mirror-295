    ...
    def plot(
        ...
        **kwargs,
        ...
        if kwargs.get("preprocess", True):
            ...
            data = self.open_dataset()
        data["points"] = (
            self.range_resolution * np.arange(1, data.dims["points"] + 1) / 1e3
        )
        figs, outputpaths = [], []
        for channel_ in data.channels.values:
            ...
            colors = color_list(
                data.dims["time"], cmap=PLOT_INFO["wavelength2cmaps"][wavelength]
            )
            raw_colors = color_list(data.dims["time"], cmap="Greys")
...
class SCC_elpp(SCC_product):
    ...
    def plot(
        ...
        for product_ in elpp.channel.values:
            ...
            wavelength = int(string_list[0].split("_")[0][:-3])
            colors = color_list(
                elpp.dims["time"], cmap=PLOT_INFO["wavelength2cmaps"][wavelength]
            )
