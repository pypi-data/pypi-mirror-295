...
class TensorBoardOutput(AsyncOutput):
  ...
  def _write(self, summaries):
    ...
    if not self._writer or reset:
      ...
      self._writer = self._retry(lambda: tf.summary.create_file_writer(
  ...
  def _retry(fn, *args, attempts=3, delay=(3, 10), etyp=Exception):
    for retry in range(attempts):
      try:
        ...
        if retry == attempts - 1:
          raise
        if delay:
          time.sleep(float(np.random.uniform(*delay)))
