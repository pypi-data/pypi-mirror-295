...
def scan(path: str, hash: str, depth_lowest: bool, 
    ...
    '''
    scan will look at all of the files, recursively, in the given path
    and computeds a hash for each file.  A dictionary is then created
    with the hash as the ID and value a list of files with the same hash
    if the list has length > 1, they are duplicates.  The results of the
    scan are written to the current directory in a file called duple.scan
    '''
