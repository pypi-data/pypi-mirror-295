...
class DataExplorer(QtWidgets.QMainWindow):
    ...
    def __init__(self, dfRaw, ClassQueries, pdAttr):
        ...
        self.ButExportXls.clicked.connect(self.ButExportXLS_Click)
    ...
    def ButNorm_Click(self):
        ...
        self.NormGui = NormGui(dSel.reset_index(drop=True))
        self.NormGui.show()
    ...
    def ButExportXLS_Click(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getSaveFileName(self, "Save XLS file", "", "All Files (*);; (*.xlsx)", options=options)
        if fileName:
            dSel = self.GetSelection()
            dSel.to_excel(fileName)
