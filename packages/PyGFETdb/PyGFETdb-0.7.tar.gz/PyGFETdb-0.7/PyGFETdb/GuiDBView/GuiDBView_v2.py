...
import PyGFETdb.GuiDBView.ClassQueries as GenClQ
...
class DBViewApp(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ButImport.clicked.connect(self.ImportData)
        self.RadElectDC.clicked.connect(self.GenACElectricalPars)
        self.RadElectAC.clicked.connect(self.GenACElectricalPars)
        self.RadElectCustom.clicked.connect(self.GenACElectricalPars)
        self.CheckForceVgs.clicked.connect(self.UpdateForceVgs)
        self.SpinVgs.valueChanged.connect(self.GenACElectricalPars)
        self.CehckNorm.clicked.connect(self.GenACElectricalPars)
    ...
    def ImportData(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "Load PKl file", "", "pickle file (*.pkl) ;; All Files (*)", options=options)
        if fileName:
            dfDat = pd.read_pickle(fileName)
    ...
    def ButEditEleAC_Click(self):
        ...
        self.pdAttr = GenClQ.GenpdAttrs(self.ClassQueries)

    def GenACElectricalPars(self):
        if self.RadElectDC.isChecked():
            self.ClassQueries = copy.deepcopy(GenClQ.ClassQueriesDC)
        elif self.RadElectAC.isChecked():
            self.ClassQueries = copy.deepcopy(GenClQ.ClassQueriesSimple)
        elif self.RadElectCustom.isChecked():
            self.ClassQueries = copy.deepcopy(GenClQ.ClassQueries)

        if self.CheckForceVgs.isChecked():
            self.ClassQueries = GenClQ.UpdateVgs(self.ClassQueries,
                                                 Vgs=self.SpinVgs.value()*pq.V,
                                                 Ud0Norm=self.CehckNorm.isChecked())

        self.pdAttr = GenClQ.GenpdAttrs(self.ClassQueries)

    def UpdateForceVgs(self):
        if self.RadElectCustom.isChecked():
            self.SpinVgs.setEnabled(True)
            self.CehckNorm.setEnabled(True)
        else:
            self.SpinVgs.setEnabled(False)
            self.CehckNorm.setEnabled(False)
