...
Use the package manager [pip](https://test.pypi.org/simple/Micro-GC-Fusion-API) to install Micro GC Fusion API.

```bash
pip install https://test.pypi.org/simple/Micro-GC-Fusion-API
```

## Usage

```python
from Micro_GC_Fusion_API import Fusion
...
#Close the HTTP request session with Micro GC Fusion:
f.close()
...
#Subclass of Fusion that provides access to the user methods on the system
    ...
    #Create a default method based on the system part number. These methods were created by INFICON GC experts and are a better starting point
...
Reach out to us via email with questions
emal: fusion.syr@inficon.com
