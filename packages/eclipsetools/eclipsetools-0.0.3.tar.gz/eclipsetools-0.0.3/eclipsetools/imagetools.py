    ...
    for i in range(len(files) - 1):
        for file in files[i]:
            try:
                ...
                print_et(f"Could not remove file: [{file}]. Error: {e}")
...
def crop_image(image_path: str, width: int, height: int, save_path: str = None):
    ...
    print_et(f"Cropped image saved as {save_path}")
    return save_path
...
def resize_image(image_path: str, width: int, height: int, keep_aspect_ratio: bool = True, save_path: str = None):
    ...
    print_et(f"Resized image saved as {save_path}")
    return save_path
...
def rotate_flip_image(image_path: str, rotation_degrees: int = 0, flip_axis: str = None, save_path: str = None):
    ...
    print_et(f"Rotated/Flipped image saved as {save_path}")
    return save_path
...
def adjust_hsb(image_path: str, brightness=1.0, saturation=1.0, hue=0, save_path: str = None):
    ...
    if image is None:
        print_et("Error: Cannot open the image file.")
        return None
    ...
    print_et(f"HSB-adjusted image saved as {save_path}")
    return save_path
...
def adjust_contrast_gamma(image_path: str, contrast=1.0, gamma=1.0, save_path: str = None):
    ...
    # Save the adjusted image
    cv2.imwrite(save_path, adjusted_image)
    print_et(f"Contrast and gamma-adjusted image saved as {save_path}")
    return save_path
...
def blur_sharpen_image(image_path: str, blur_strength=0, sharpen_strength=0, save_path: str = None):
    ...
    print_et(f"Blurred/Sharpened image saved as {save_path}")
    return save_path
...
def zoom_pan_image(image_path: str, zoom_factor: float, pan_factor_x: int, pan_factor_y: int, save_path: str = None):
    ...
    print_et(f"Zoomed/Panned image saved as {save_path}")
    return save_path
...
def concatenate_images(image_paths: list, save_path: str = None):
    ...
    """
    if not image_paths:
        print_et("Error: No images provided for concatenation.")
        return None
    ...
    if first_image is None:
        print_et("Error: Cannot open the first image file.")
        return None
    ...
    for image_path in image_paths:
        image = cv2.imread(image_path)
        if image is None:
            print_et(f"Error: Cannot open image file: {
    ...
    out.release()
    print_et(f"Concatenated video saved as {save_path}")
    return save_path
...
def save_array_as_image(image_array: np.ndarray, save_path: str = None):
    ...
    if image_array is None or image_array.size == 0:
        print_et("Error: Invalid image array.")
        return None

    # Save the array as an image
    success = cv2.imwrite(save_path, image_array)
    if not success:
        print_et("Error: Unable to save the image file.")
        return None

    print_et(f"Image saved as {save_path}")
    return save_path
