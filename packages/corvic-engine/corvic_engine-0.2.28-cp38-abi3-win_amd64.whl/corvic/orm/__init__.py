...
class Room(BelongsToOrgMixin, SoftDeleteMixin, Base):
    ...
    agents: sa_orm.Mapped[dict[str, Agent]] = sa_orm.relationship(
        collection_class=attribute_mapped_collection("agent_key"),
        cascade="all",
        init=False,
        default_factory=dict,
    )
...
class Agent(SoftDeleteMixin, BelongsToOrgMixin, Base):
    ...
    room: sa_orm.Mapped[Room] = sa_orm.relationship(
        back_populates="agents", init=True, default=None
    )
