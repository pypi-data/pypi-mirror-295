        ...
        if channels == 1:
            if header.source == SOURCE_PHASE:
                ...
                for y in iter_unpack("<h", file_content):
                    value = float("nan") if y[0] > 8192 else 180.0 * cast(int, y[0]) / 8192.0
...
def Daq_SetChannel(
    ...
    if enabled and not isinstance(voltage_range, DaqRange):
        raise TypeError("voltage_range must be an instance of DaqRange IntEnum")
...
def Daq_GetChannel(channel: DaqChannel) -> Dict[str, Union[bool, DaqRange, DaqNCTerm, None]]:
    """
...
def Daq_SetTimeBase(
    sampling_rate: DaqSamplingClk = DaqSamplingClk.SCLK_150MHZ, points_number: int = 0x10000000
) -> None:
    """
    ...
    if not isinstance(sampling_rate, DaqSamplingClk):
        raise TypeError("sampling_rate must be an instance of DaqSamplingClk IntEnum")
    _check_limits(c_uint32, points_number, "points_number")
    CTS3Exception._check_error(
        _MPuLib.Daq_SetTimeBase(c_uint8(sampling_rate), c_uint32(points_number))
    )
...
def Daq_SetTrigger(
    ...
    if not isinstance(trigger_source, DaqTrigSource):
        raise TypeError("trigger_source must be an instance of DaqTrigSource IntEnum")
    ...
    CTS3Exception._check_error(
        _MPuLib.Daq_SetTrigger(
            c_uint8(trigger_source), c_int16(level_mV), c_uint8(direction), c_int32(delay)
        )
    )
...
def Daq_StartStopAcq(
    ...
    if not isinstance(download_mode, DaqDownloadMode):
        raise TypeError("download_mode must be an instance of DaqAcqMode IntEnum")
    ...
    CTS3Exception._check_error(
        func_pointer(c_uint32(acq_mode), c_uint32(download_mode), c_uint32(0), file)
    )
...
def Daq_GetInfo() -> str:
    ...
    rfu = c_uint8()
    CTS3Exception._check_error(
        _MPuLib.Daq_GetInfo(byref(year), byref(version), byref(revision), byref(rfu))
    )
...
def Daq_SetFilter(filter: DaqFilter, enabled: bool) -> None:
    ...
    CTS3Exception._check_error(_MPuLib.Daq_SetFilter(c_uint32(filter), c_bool(enabled)))
...
def Daq_ProbeCompensation(channel: int, label: Optional[str]) -> None:
    ...
    if label is None:
        ...
        CTS3Exception._check_error(
            _MPuLib.Daq_ProbeCompensation(c_uint8(channel), c_uint32(0), label.encode("ascii"))
        )
...
def Daq_LoadProbe(label: Optional[str], channel: int) -> None:
    ...
    _check_limits(c_uint8, channel, "channel")
    if label is None:
        CTS3Exception._check_error(_MPuLib.Daq_LoadProbe(None, c_uint8(channel)))
    else:
        CTS3Exception._check_error(_MPuLib.Daq_LoadProbe(label.encode("ascii"), c_uint8(channel)))
...
def MPS_DaqAutoTest(test_id: DaqAutotestId = DaqAutotestId.TEST_DAQ_ALL) -> List[List[str]]:
    """
    ...
    result = c_char_p()
    ret = CTS3ErrorCode(
        _MPuLib.MPS_DaqAutoTest(c_uint32(test_id), c_bool(True), c_uint32(0), byref(result))
    )
    if (
...
def Daq_FlashFirmware(partIndex: int, call_back: Optional[Callable[[int], int]] = None) -> None:
    """
    ...
    if call_back:
        ...
        CTS3Exception._check_error(_MPuLib.Daq_FlashFirmware(c_uint8(partIndex), None))
