...
def MPC_Set14443AInitParameters(
    atqa: bytes, uid: bytes, sak: Union[bytes, int], ats: Optional[bytes]
) -> None:
    """
    ...
    if ats is None:
        CTS3Exception._check_error(
            _MPuLib.MPC_Set14443AInitParameters(
                c_uint8(0),
                atqa,
                c_uint32(len(uid)),
                uid,
...
def MPC_Set14443BInitParameters(atqb: bytes) -> None:
    ...
    CTS3Exception._check_error(
        _MPuLib.MPC_Set14443BInitParameters(c_uint8(0), c_uint32(len(atqb)), atqb)
    )


def MPC_Set15693InitParameters(
    data_rate: VicinityDataRate, sub_carrier: VicinitySubCarrier
) -> None:
    """
    ...
    CTS3Exception._check_error(
        _MPuLib.MPC_Set15693InitParameters(c_uint8(0), c_uint8(data_rate), c_uint8(sub_carrier))
    )
...
def MPC_SetNFCInitParameters(
    ...
    if masked_events is not None and not isinstance(masked_events, NfcSimulatorEvent):
        raise TypeError("masked_events must be an instance of NfcSimulatorEvent IntFlag")
...
def MPC_SetSParameterInit(
    pcd_to_picc_bitrate: Union[bytes, int],
    ...
    if isinstance(pcd_to_picc_bitrate, bytes):
        if len(pcd_to_picc_bitrate) != 1:
            raise TypeError("pcd_to_picc_bitrate must be an instance of 1 byte")
        ...
        raise TypeError("pcd_to_picc_bitrate must be an instance of int or 1 byte")
    if isinstance(picc_to_pcd_bitrate, bytes):
        if len(picc_to_pcd_bitrate) != 1:
            raise TypeError("picc_to_pcd_bitrate must be an instance of 1 byte")
        ...
        raise TypeError("picc_to_pcd_bitrate must be an instance of int or 1 byte")
    if isinstance(framing_option_picc_to_pcd, bytes):
        if len(framing_option_picc_to_pcd) != 1:
            raise TypeError("framing_option_picc_to_pcd must be an instance of 1 byte")
        framing = framing_option_picc_to_pcd[0]
    elif isinstance(framing_option_picc_to_pcd, int):
        _check_limits(c_uint8, framing_option_picc_to_pcd, "framing_option_picc_to_pcd")
        framing = framing_option_picc_to_pcd
    else:
        raise TypeError("framing_option_picc_to_pcd must be an instance of int or 1 byte")

    CTS3Exception._check_error(
        _MPuLib.MPC_SetSParameterInit(
            c_uint8(0),
            c_uint8(pcd_to_picc),
            c_uint8(picc_to_pcd),
            c_uint8(framing),
...
def MPC_SetT2TInitParameters(
    ...
    Args:
        ...
        nfc_id: NFCID1 to answer
    """
    if masked_events is not None and not isinstance(masked_events, Type2TagSimulatorEvent):
        raise TypeError("masked_events must be an instance of Type2TagSimulatorEvent IntFlag")
    if not isinstance(sens_res, bytes) or len(sens_res) != 2:
        raise TypeError("sens_res must be an instance of 2 bytes")
    if isinstance(sel_res, bytes):
        if len(sel_res) != 1:
    ...
    if masked_events is not None:
        CTS3Exception._check_error(
            _MPuLib.MPC_SetT2TInitParameters(
                c_uint8(0),
...
class _TypeARATSStruct(Structure):
    ...
    _fields_ = [("sb", c_uint8), ("param", c_uint8), ("crc_1", c_uint8), ("crc_2", c_uint8)]
...
def MPC_GetBufferedRawFrame() -> Optional[Dict[str, Union[bytes, TechnologyType]]]:
    """
    Gets received frame
    ...
    rx_size = c_uint32()
    rx_type = c_int32()
    ret = _MPuLib.MPC_GetBufferedRawFrame(c_uint8(0), byref(rx_type), data, byref(rx_size))
    if ret == CTS3ErrorCode.ERRSIM_NO_FRAME_AVAILABLE.value:
        return None
    CTS3Exception._check_error(ret)
    return {"rx_frame": data[: rx_size.value], "rx_type": TechnologyType(rx_type.value)}
...
def MPC_GetRawFrame() -> Optional[Dict[str, Union[bytes, TechnologyType]]]:
    ...
    rx_size = c_uint32()
    rx_type = c_int32()
    ret = _MPuLib.MPC_GetRawFrame(c_uint8(0), byref(rx_type), data, byref(rx_size))
    if ret == CTS3ErrorCode.ERRSIM_NO_FRAME_AVAILABLE.value:
        return None
    CTS3Exception._check_error(ret)
    return {"rx_frame": data[: rx_size.value], "rx_type": TechnologyType(rx_type.value)}
...
def MPS_GetAPDU2() -> Optional[Dict[str, bytes]]:
    ...
    length = c_uint32()
    apdu_len = c_uint32()
    ret = _MPuLib.MPS_GetAPDU2(c_uint8(0), byref(header), data, byref(length), byref(apdu_len))
...
class _TypeNFC_PSL_REQ(Structure):
    ...
    _fields_ = [("did", c_uint8), ("brs", c_uint8), ("fsl", c_uint8)]
...
class _TypeNFC_WUP_REQ(Structure):
    ...
    _fields_ = [("nfc_id3i", c_uint8 * 10), ("didi", c_uint8)]
...
def MPS_SendRAPDU(apdu: Optional[bytes], status: Union[int, bytes]) -> None:
    ...
    if apdu is None:
        CTS3Exception._check_error(
            _MPuLib.MPS_SendRAPDU(c_uint8(0), None, c_uint32(0), c_uint16(status_word))
        )
    else:
        if not isinstance(apdu, bytes):
            raise TypeError("apdu must be an instance of bytes")
        CTS3Exception._check_error(
            _MPuLib.MPS_SendRAPDU(c_uint8(0), apdu, c_uint32(len(apdu)), c_uint16(status_word))
        )


def MPC_SendNFC_PSL_RES(did: Union[int, bytes] = 0xFF, crc: Union[int, bytes] = 0) -> None:
    """
    ...
    CTS3Exception._check_error(
        _MPuLib.MPC_SendNFC_PSL_RES(
            c_uint8(0), byref(c_uint8(did_value)), byref(c_uint16(crc_value))
        )
    )


def MPC_SendNFC_DSL_RES(did: Union[int, bytes] = 0xFF, crc: Union[int, bytes] = 0) -> None:
    """
    ...
    if isinstance(crc, bytes):
        ...
        raise TypeError("crc must be an instance of int or 2 bytes")
    CTS3Exception._check_error(
        _MPuLib.MPC_SendNFC_DSL_RES(
            c_uint8(0), byref(c_uint8(did_value)), byref(c_uint16(crc_value))
        )
    )


def MPC_SendNFC_RLS_RES(did: Union[int, bytes] = 0xFF, crc: Union[int, bytes] = 0) -> None:
    """
    ...
    if isinstance(crc, bytes):
        ...
        raise TypeError("crc must be an instance of int or 2 bytes")
    CTS3Exception._check_error(
        _MPuLib.MPC_SendNFC_RLS_RES(
            c_uint8(0), byref(c_uint8(did_value)), byref(c_uint16(crc_value))
        )
    )


def MPC_SendNFC_WUP_RES(did: Union[int, bytes] = 0xFF, crc: Union[int, bytes] = 0) -> None:
    """
...
def MPC_SendNFCTimeoutExtensionRequest(rtox: Union[int, bytes]) -> None:
    ...
    CTS3Exception._check_error(
        _MPuLib.MPC_SendNFCTimeoutExtensionRequest(c_uint8(0), c_uint8(rtox_value))
    )
...
def MPC_SendNFC_RUserData(data: bytes) -> None:
    ...
    if not isinstance(data, bytes):
        raise TypeError("data must be an instance of bytes")
    CTS3Exception._check_error(_MPuLib.MPC_SendNFC_RUserData(c_uint8(0), c_uint32(len(data)), data))
...
def MPC_SendPPSResponse(pps: Optional[bytes] = None) -> None:
    ...
    """
    if pps is None:
        CTS3Exception._check_error(_MPuLib.MPC_SendPPSResponse(c_uint8(0), None))
    else:
        ...
        CTS3Exception._check_error(_MPuLib.MPC_SendPPSResponse(c_uint8(0), byref(pps_struct)))
...
def MPS_SimWaitNStart(
    ...
    if not isinstance(mode, CardEmulationMode):
        raise TypeError("mode must be an instance of CardEmulationMode IntEnum")
...
def MPS_WaitSimEvent(
    ...
    ret = _MPuLib.MPS_WaitSimEvent(
        c_uint8(0), c_uint32(timeout_ms), c_uint32(0), byref(event), byref(protocol)
    )
...
def MPS_ChangeSimParameters(
    ...
    if not isinstance(parameter_type, SimulatorParameter):
        raise TypeError("parameter_type must be an instance of SimulatorParameter IntEnum")

    # Events mask parameter
    if parameter_type == SimulatorParameter.SIM_EVENTS_MASK:
        if (
            not isinstance(parameter_value, IsoSimulatorEvent)
        ...
        CTS3Exception._check_error(
            _MPuLib.MPS_ChangeSimParameters(
                c_uint8(0), c_uint32(parameter_type), byref(c_uint32(parameter_value)), c_uint32(4)
            )
        )
    ...
    # IsoSimulatorEvent parameter
        ...
        if parameter_value is None:
            CTS3Exception._check_error(
                _MPuLib.MPS_ChangeSimParameters(
                    c_uint8(0), c_uint32(parameter_type), byref(c_uint32(0)), c_uint32(4)
                )
            )
        else:
            if not isinstance(parameter_value, IsoSimulatorEvent):
                raise TypeError("event must be an instance of IsoSimulatorEvent IntFlag")
            CTS3Exception._check_error(
                _MPuLib.MPS_ChangeSimParameters(
                    c_uint8(0),
    ...
    # Boolean parameter
        ...
        parameter_type == SimulatorParameter.SIM_AUTO_WTX
    ...
    # int or 2-byte parameter
        ...
        parameter_type == SimulatorParameter.SIM_ATS_CRC
        ...
        if parameter_value is None:
            CTS3Exception._check_error(
                _MPuLib.MPS_ChangeSimParameters(
                    c_uint8(0), c_uint32(parameter_type), None, c_uint32(0)
                )
            )
        else:
            if isinstance(parameter_value, bytes):
                if len(parameter_value) != 2:
                    raise TypeError("parameter_value must be an instance of 2 bytes")
                ...
                raise TypeError("parameter_value must be an instance of int or 2 bytes")
            CTS3Exception._check_error(
                _MPuLib.MPS_ChangeSimParameters(
                    c_uint8(0), c_uint32(parameter_type), byref(c_uint16(value)), c_uint32(2)
                )
            )
    ...
    # int parameter
        ...
        CTS3Exception._check_error(
            _MPuLib.MPS_ChangeSimParameters(
                c_uint8(0), c_uint32(parameter_type), byref(c_uint8(parameter_value)), c_uint32(1)
            )
        )
    elif (
        parameter_type == SimulatorParameter.SIM_NFC_MAX_NAK
...
def MPS_GetSimParameters(
    ...
    if parameter_type == SimulatorParameter.SIM_VICINITY_COLLISIONS:
        val = bytes(255)
        CTS3Exception._check_error(
            _MPuLib.MPS_GetSimParameters(
                c_uint8(0), c_uint32(parameter_type), val, c_uint32(255), byref(param_size)
            )
        )
...
class IoClCrcFilter_pattern:
    ...
    def __init__(
        self, crc: Union[int, bytes], pattern: bytes, mask: Optional[bytes] = None, index: int = 1
    ):
        """
...
class IoClFrameSuppFilter_pattern:
    ...
    def __init__(self, pattern: bytes, mask: Optional[bytes] = None, index: int = 1):
        """
...
def MPS_AddFilter(
    ...
    if isinstance(filter, IoClCrcFilter_index):
        ...
        temp = bytearray(filter.pattern + b"\x00" * (256 - len(filter.pattern)))
...
class _ActionConditionDataPattern(Structure):
    ...
    _fields_ = [("length", c_uint32), ("mask", c_uint8 * 256), ("pattern", c_uint8 * 256)]
...
def MPS_SimAddRule(
    ...
    if pattern_condition is None:
        ...
        temp = bytearray(pattern_condition.pattern + b"\x00" * (256 - len(pattern_condition.mask)))
        pattern = (c_uint8 * 256).from_buffer(temp)
        temp = bytearray(pattern_condition.mask + b"\x00" * (256 - len(pattern_condition.pattern)))
        mask = (c_uint8 * 256).from_buffer(temp)
        ...
        raise TypeError("pattern_condition must be an instance of ActionConditionDataPattern")
