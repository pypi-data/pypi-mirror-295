...
from VisualShape3D.geomutils import SegmentXPolygon, PointInPolygon,PointInSegment
...
from VisualShape3D.shapes    import transform,isDefaultShape, flip, displace
...
class Shape(Polygon):
    ...
    def transform(self, reference = None, to = None, angles = (0.0,0.0)):
        return self.move(reference, to, angles)

    def move(self, reference = None, to = None, by = (0.0,0.0)):
        angles = list(by)
        angles[0] = self.angles[0] + angles[0] 
        angles[1] = self.angles[1] + angles[1]

        if reference is None : reference = self.P0
        if to is None : to = self.P0
        
        vertices = transform(self.R, reference, to, by)

        # newPolygon = Shape(shape='Polygon',**{'vertices':vertices})
        # newPolygon.P0 = to
        # newPolygon.angles = angles

        return self.copy(vertices=vertices, P0=self.P0, angles=angles)
    ...
    def displace(self, by):
        ...
        self.P0 = displace(self.P0, by = by)
        return self.copy()

    def rotate(self, by=(0.0, 0)):
        return self.transform(angles = by)

    def copy(self, vertices = None, P0=None, angles=None):
        ...
        if angles is None : 
            newShape.angles = self.angles 
        else : 
            newShape.angles = angles
