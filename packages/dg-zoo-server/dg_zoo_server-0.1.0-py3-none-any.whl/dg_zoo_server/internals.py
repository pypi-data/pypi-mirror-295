...
class ZooManager:
    ...
    async def download_model(
        ...
        async with self._lock.reader:
            ...
            temp_model_path = pathlib.Path(tempfile.gettempdir()) / str(uuid4())
    ...
    async def _temp_link_get(
        self,
        path: pathlib.Path,
    ) -> pathlib.Path:
        """Create and return temporary (hard link) file path for requested model file

        - `path`: file path to copy

        Returns full model file temporary path for requested model
        """
        tmp_path = pathlib.Path(tempfile.gettempdir()) / str(uuid4())
        await aiofiles.os.link(path, tmp_path)
