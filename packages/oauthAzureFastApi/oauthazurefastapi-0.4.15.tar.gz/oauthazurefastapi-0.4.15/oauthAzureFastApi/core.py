...
class OAuthApp:
    def __init__(self, client_id: str, client_secret: str, tenant_id: str, redirect_uri: str,frontend_uri:str):
    ...
    def _define_routes(self):
        ...
        async def auth(request: Request):
            ...
            payload = self.verify_token(token["id_token"])
            user_info = {
            ...
            user_info_encoded = urllib.parse.quote(str(user_info).replace("'", '"'))
            # return {"access_token": token["id_token"], "refresh_token": token["refresh_token"],"session_id": session_id,"user_info":user_info}
            redirect_url = f"{self.frontend_uri}#access_token={token['id_token']}&refresh_token={token['refresh_token']}&session_id={session_id}&user_info={user_info_encoded}"
