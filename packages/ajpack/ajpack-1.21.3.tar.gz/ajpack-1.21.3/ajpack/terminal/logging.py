...
class Logger:
    ...
    def __init__(self) -> None:
        ...
        self.log_file_format: str = "(L: {line})  {level}  |  {timestamp}  ({message})"
    ...
    def log(self, level: str, message: str, **kwargs: str) -> None:
        ...
        if self.print_debug and level in ["INFO", "DEBUG", "SUCCESS"]: return
        ...
        formatted_file_message: str = self.log_file_format.format(line=line, level=level, timestamp=timestamp, message=message, **kwargs)
        ...
        if self.async_logging and self.executor:
            self.executor.submit(self._write_log, formatted_message)
