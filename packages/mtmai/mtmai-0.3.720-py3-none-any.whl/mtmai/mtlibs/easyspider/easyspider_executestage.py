...
from .constants import DataWriteMode, GraphOption, WriteMode
...
class BrowserThread(Thread):
    def __init__(
        self, browser_t, id, service, version, event, saveName, config, option
    ):
        ...
        try:
            if service.get("startFromExit", 0) == 1:
                with open(
                    "Data/Task_" + str(self.id) + "/" + self.saveName + "_steps.txt",
                    "r",
                    encoding="utf-8-sig",
                ) as file_obj:
        ...
        stealth_path = (
            driver_path[: driver_path.find("chromedriver")] + "stealth.min.js"
        )
        ...
        if (
            self.task_version >= "0.3.1"
        ):  # 0.3.1及以上版本以上的EasySpider兼容从0.3.1版本开始的所有版本
            pass
        elif (
            self.task_version != version
        ):  # 0.3.1以下版本的EasySpider不兼容0.3.1及以上版本的EasySpider
            self.print_and_log(
                f"版本不一致，请使用{self.task_version}版本的EasySpider运行该任务！"
            )
            self.print_and_log(
                f"Version not match, please use EasySpider {self.task_version} to run this task!"
            )
            self.browser.quit()
            sys.exit()
        ...
        if service_links:
            self.links = list(
                filter(isnotnull, service_links.split("\n"))
            )  # 要执行的link的列表
        else:
        ...
        if self.outputFormat in ["csv", "txt", "xlsx"]:
            if not os.path.exists(
                f"Data/Task_{str(self.id)}/{self.saveName}.{self.outputFormat}"
            ):
        ...
        for param in service["outputParameters"]:  # 初始化输出参数
            if param["name"] not in self.outputParameters.keys():
                ...
                self.outputParametersRecord.append(
                    bool(param.get("recordASField", True))
                )
                # 文件叠加的时候不添加表头
                if (
                    self.outputFormat in ["csv", "txt", "xlsx"]
                    and self.writeMode == WriteMode.Create.value
                ):
    ...
    def preprocess(self):
        for index_node, node in enumerate(self.procedure):
            ...
            parameters["iframe"] = (
                False if not iframe else parameters.get("iframe", False)
            )
            ...
            if parameters.get("waitElementIframeIndex"):
                parameters["waitElementIframeIndex"] = int(
                    parameters["waitElementIframeIndex"]
                )
            else:
            ...
            if option == GraphOption.Get.value:  # 打开网页操作
                ...
                for param in params:
                    ...
                    param["splitLine"] = (
                        0 if not param.get("splitLine") else param.get("splitLine")
                    )
                ...
                parameters["exitElement"] = (
                    "//body"
                    if not parameters.get("exitElement")
                    or parameters.get("exitElement") == ""
                ...
                if (
                    ...
                    and (
                        int(node["parameters"]["loopType"]) == 1
                        or int(node["parameters"]["loopType"]) == 2
                    )
                ):
                    params = (
                        self.procedure[node["sequence"][0]]
                        .get("parameters")
                        .get("params")
                    )
                    if not params:
                    ...
                    waitElement = self.procedure[node["sequence"][0]]["parameters"].get(
                        "waitElement", ""
                    )

                    if parameters["iframe"]:
                        parameters["quickExtractable"] = (
                            False  # 如果是iframe，那么不可以快速提取
                        )
                    else:
                    ...
                    if parameters["skipCount"] > 0:
                        parameters["quickExtractable"] = (
                            False  # 如果有跳过的元素，那么不可以快速提取
                        )

                    for param in params:
                        optimizable = detect_optimizable(
                            param, ignoreWaitElement=False, waitElement=waitElement
                        )
                        param["iframe"] = param.get("iframe", False)
                        if (
                            param["iframe"] and not param["relative"]
                        ):  # 如果是iframe，那么不可以快速提取
                            optimizable = False
                        if (
                            not optimizable
                        ):  # 如果有一个不满足优化条件，那么就不能快速提取
                    ...
                    if parameters["quickExtractable"]:
                        ...
                        self.print_and_log(
                            f"Loop operation <{node['title']}> can extract data quickly"
                        )
                        parameters["clear"] = self.procedure[node["sequence"][0]][
                            "parameters"
                        ].get("clear", 0)
                        parameters["newLine"] = self.procedure[node["sequence"][0]][
                            "parameters"
                        ].get("newLine", 1)

                        if int(node["parameters"]["loopType"]) == 1:  # 不固定元素列表
                            node["parameters"]["baseXPath"] = node["parameters"][
                                "xpath"
                            ]
    ...
    def readFromExcel(self):
        ...
        try:
            ...
            self.print_and_log(
                "读取Excel失败，将会使用默认参数执行任务，请检查文件路径是否正确：",
    ...
    def run(self):
        ...
        try:
            ...
            pass
        self.print_and_log(
            "正在清理临时用户目录……|Cleaning up temporary user directory..."
        )
        try:
        ...
        self.print_and_log(
            "您现在可以安全的关闭此窗口了。|You can safely close this window now."
        )
    ...
    def saveData(self, exit=False):
        ...
        if exit == True or len(self.OUTPUT) >= self.save_threshold:
            ...
            if self.log:
                with open(
                    "Data/Task_" + str(self.id) + "/" + self.saveName + ".log",
                    "a",
                    encoding="utf-8-sig",
                ) as file_obj:
                ...
                if scrollType == 1 or scrollType == 2:
                    ...
                    while True:
                        newBodyText = self.browser.find_element(
                            By.CSS_SELECTOR, "body", iframe=False
                        ).text
    ...
    def execute_code(self, codeMode, code, max_wait_time, element=None, iframe=False):
        ...
        if int(codeMode) == 0:
            ...
            try:
                ...
                self.print_and_log(
                    "Error executing the following code:" + code, ", error is:", e
                )
        elif int(codeMode) == 6:
            try:
                code = readCode(code)
                ...
                self.print_and_log(
                    "获得下面的代码返回值时出错:" + code, "，错误为：", e
                )
                self.print_and_log(
                    "Error executing and getting return value the following code:"
                    + code,
                    ", error is:",
                    e,
                )
    ...
    def customOperation(self, node, loopValue, loopPath, index):
        ...
        if codeMode == 2:  # 使用循环的情况下，传入的clickPath就是实际的xpath
            ...
            self.print_and_log(
                "根据设置的自定义操作，任务已刷新页面|Task refreshed page according to custom operation"
            )
            ...
            line = new_line(
                self.outputParameters, self.maxViewLength, self.outputParametersRecord
            )
            self.OUTPUT.append(line)
        elif codeMode == 12:  # 退出程序
            self.print_and_log(
                "根据设置的自定义操作，任务已退出|Task exited according to custom operation"
            )
            self.saveData(exit=True)
            self.browser.quit()
            self.print_and_log(
    ...
    def switchSelect(self, param, loopValue):
        ...
        try:
            ...
            dropdown = Select(
                self.browser.find_element(By.XPATH, xpath, iframe=param["iframe"])
            )
            try:
                if optionMode == 0:
                    # 获取当前选中的选项索引
                ...
                self.print_and_log(
                    "Failed to change drop-down box option:",
                    xpath,
                    param["optionMode"],
                    param["optionValue"],
                )
    ...
    def moveToElement(self, param, loopElement=None, loopPath="", index=0):
        ...
        if param["useLoop"]:  # 使用循环的情况下，传入的clickPath就是实际的xpath
            if xpath == "":
                ...
                index = (
                    0  # 如果是相对循环内元素的点击，在定位到元素后，index应该重置为0
                )
            # element = loopElement
        else:
            index = 0
    ...
    def executeNode(self, nodeId, loopValue="", loopPath="", index=0):
        ...
        try:
            ...
            self.print_and_log(
                "执行节点<" + node["title"] + ">时出错，将继续执行，错误为：", e
            )
            self.print_and_log(
                "Error executing node <"
                + node["title"]
                + ">, will continue to execute, error is:",
                e,
            )

        # 执行完之后进行等待
        if (
            node["option"] != 0 and node["option"] != 2
        ):  # 点击元素操作单独定义等待时间操作
            ...
            if tType == 0:  # 什么条件都没有
                ...
                try:
                    ...
                    if self.browser.find_element(
                        By.XPATH, xpath, iframe=cnode["parameters"]["iframe"]
                    ):
                        executeBranchId = i
                        break
    ...
    def handleHistory(
        ...
        if (
            ...
            difference = (
                thisHistoryLength - self.history["index"]
            )  # 计算历史记录变化差值
            self.browser.execute_script(
                "history.go(" + str(difference) + ")"
            )  # 回退历史记录
    ...
    def loopExecute(self, node, loopValue, clickPath="", index=0):
        ...
        if node["parameters"]["quickExtractable"]:
            ...
            if int(node["parameters"]["loopType"]) == 1:  # 不固定元素列表
                baseXPath = replace_field_values(
                    node["parameters"]["baseXPath"], self.outputParameters, self
                )
            ...
            for row in rows:
                ...
                for param in node["parameters"]["quickParams"]:
                    ...
                    try:
                        content = " ".join(
                            result.strip() for result in content if result.strip()
                        )
            ...
            while True:  # do while循环
                try:
                    ...
                    if not finished:
                        self.print_and_log("\n\n-------Retrying-------\n\n")
                        self.print_and_log(
                            "-------Retrying-------: ", node["parameters"]["xpath"]
                        )
                        for i in node[
                            "sequence"
                        ]:  # 不带点击元素的把剩余的如提取数据的操作执行一遍
                ...
                if (
                    node["parameters"]["exitCount"] == count
                ):  # 如果达到设置的退出循环条件的话
                    break
                if (
                    int(node["parameters"]["breakMode"]) > 0
                ):  # 如果设置了退出循环的脚本条件
                    output = self.execute_code(
            ...
            try:
                ...
                while index < len(elements):
                    ...
                    try:
                        ...
                        self.print_and_log(
                            "元素已失效，重新获取元素|Element has expired, reacquiring element"
                        )
                        elements = self.browser.find_elements(
                            By.XPATH, xpath, iframe=node["parameters"]["iframe"]
                        )
            ...
            if len(textList) == 1:  # 如果固定文本列表只有一行，现在就可以替换变量
                textList = replace_field_values(
                    node["parameters"]["textList"], self.outputParameters, self
                ).split("\n")
            skipCount = node["parameters"]["skipCount"]
            index = 0
            for text in textList:
                ...
                if (
                    ...
                    if code <= 0:
                        break
                index, _ = self.handleHistory(
                    node, "", thisHandle, thisHistoryURL, thisHistoryLength, index
                )
    ...
    def inputInfo(self, param, loopValue):
        ...
        try:
            ...
            self.print_and_log(
                "找不到输入框元素:" + xpath + "，请尝试在执行此操作前设置等待时间"
            )
    ...
    def clickElement(self, param, loopElement=None, clickPath="", index=0):
        ...
        try:
            ...
            self.print_and_log(
                "找不到要点击的元素:" + path + "，请尝试在执行此操作前设置等待时间"
            )
        ...
        try:
            if (
                ...
                # except Exception as e:
                script = (
                    "document.elementFromPoint(" + str(x) + "," + str(y) + ").click();"
                )
                ...
                try:
                    ...
                    if newTab == 1:  # 在新标签页打开
                        if sys.platform == "darwin":  # Mac
                            actions.key_down(Keys.COMMAND).click(element).key_up(
                                Keys.COMMAND
                            ).perform()
                        else:
                            # Ctrl + Click
                            actions.key_down(Keys.CONTROL).click(element).key_up(
                                Keys.CONTROL
                            ).perform()
                    else:
                    ...
                    try:
                        ...
                        self.print_and_log(
                            f"Selenium点击元素{path}失败，将尝试使用JavaScript点击"
                        )
                        self.print_and_log(
                ...
                try:
                    ...
                    try:
                        ...
                        self.print_and_log(
                            f"Selenium双击元素{path}失败，将尝试使用JavaScript双击"
                        )
                        self.print_and_log(
    ...
    def get_content(self, p, element):
        ...
        if p["contentType"] == 0:
            ...
            if p["nodeType"] == 2:
                ...
                if downloadPic == 1:
                    download_image(
                        self,
                        content,
                        "Data/Task_" + str(self.id) + "/" + self.saveName + "/images",
                        element,
                    )
            else:  # 普通节点
                if p["splitLine"] == 1:
            ...
            if p["nodeType"] == 2:
                ...
                return str;'
                content = (
                    self.browser.execute_script(command, element)
                    .replace("\n", "")
                    .replace("\\s+", " ")
                )
            ...
            try:
                ...
                location = (
                    "Data/Task_" + str(self.id) + "/" + self.saveName + "/" + temp_name
                )
                image.save(location)
    ...
    def getData(self, param, loopElement, isInLoop=True, parentPath="", index=0):
        ...
        for p in param["params"]:
            if not p["optimizable"]:
                ...
                if not (
                    p["contentType"] == 5 or p["contentType"] == 6
                ):  # 如果不是页面标题或URL，去找元素
                    try:
                        # relativeXPath = relativeXPath.lower()
                        # relativeXPath = lowercase_tags_in_xpath(relativeXPath)
                        if p["relative"]:  # 是否相对xpath
                            if (
                                ...
                                if relativeXPath.find("//") >= 0:
                                    ...
                                    element = loopElement.find_element(
                                        By.XPATH, relativeXPath[1:]
                                    )
                        else:
                            element = self.browser.find_element(
                ...
                self.outputParameters[p["name"]] = content
                self.execute_code(
                    2, p["afterJS"], p["afterJSWaitTime"], element, iframe=p["iframe"]
                )  # 执行后置JS
...
if __name__ == "__main__":
    ...
    if sys.platform == "darwin" and platform.architecture()[0] == "64bit":
        ...
        if sys.platform == "win32" and platform.architecture()[0] == "32bit":
            ...
            driver_path = os.path.join(
                os.getcwd(),
                "EasySpider/resources/app/chrome_win32/chromedriver_win32.exe",
            )
            ...
            driver_path = os.path.join(
                os.getcwd(),
                "EasySpider/resources/app/chrome_win64/chromedriver_win64.exe",
            )
            options.add_extension("EasySpider/resources/app/XPathHelper.crx")
        ...
        options.binary_location = (
            "../ElectronJS/chrome_win64/chrome.exe"  # 指定chrome位置
        )
    ...
    for id in c.ids:
        tmp_options.append(
            {"options": copy.deepcopy(options), "tmp_user_data_folder": ""}
        )
        ...
        for i in range(len(c.ids)):
            ...
            if os.path.exists(tmp_user_data_folder):
                try:
                    ...
                    pass
            print(
                f"Copying user data folder to: {tmp_user_data_folder}, please wait..."
            )
            ...
            if os.path.exists(absolute_user_data_folder):
                try:
                    ...
                    print(
                        "用户信息目录复制成功，如果程序在运行过程中被手动退出，请手动删除临时用户信息目录。"
                    )
                except:
            ...
            options.add_argument(
                f"--user-data-dir={tmp_user_data_folder}"
            )  # TMALL 反扒
    ...
    for i in range(len(c.ids)):
        ...
        try:
            ...
            print(
                f"未找到id为{str(id)}的任务，请检查'execution_instances'文件夹中是否存在{str(id)}.json文件。"
            )
            continue
        try:
        ...
        if cloudflare == 0:
            ...
            path = os.path.join(
                os.path.abspath("./"), "Data", "Task_" + str(id), "files"
            )
            ...
            if browser == "chrome":
                if c.docker_driver == "":
                    ...
                    browser_t = MyChrome(
                        service=selenium_service, options=options, mode="local_driver"
                    )
                else:
    ...
    try:
        ...
        if c.keyboard:
            with Listener(
                on_press=on_press_creator(press_time, event),
