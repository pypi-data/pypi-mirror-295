...
from mtmai.api.deps import MqDep, SessionDep
from mtmai.core.config import settings
from mtmai.mtlibs.mq.mq import send_message
from mtmai.mtlibs.mtmcrawler.mtmcrawler import MTMCrawlerSiteParams, crawl_site
...
def mtmcrawler_scrapgraph_demo(req: ScrapeGraphAiReq):
...
async def crawl_site_api(db: SessionDep, mq: MqDep):
    """
    用爬虫对特定网址进行抓取，并建立索引
    """
    ...
    # 开发阶段，特定参数固定
    site_id = "1"
    url = "https://top.aibase.com/tool/screenshot-to-code"
    params = MTMCrawlerSiteParams(site_id=site_id, entry_urls=[url])

    await crawl_site(db, params)

    send_message(mq, queue="mtmcrawl_queue", message=json.dumps(params))
