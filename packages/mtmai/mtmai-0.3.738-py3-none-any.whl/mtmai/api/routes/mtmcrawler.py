...
from mtmai.mtlibs.mtmcrawler.mtmcrawler import crawl_page
...
@router.post("/mtmcrawler_crawl_page")
async def mtmcrawler_crawl_page(db: SessionDep):
    """
    用爬虫对特定网址进行抓取，并建立索引
    """
    ...
    html = await crawl_page(db, site_id, url)
    return {
        "ok": True,
        "site_id": site_id,
        "url": url,
