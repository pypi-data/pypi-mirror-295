    ...
    def multibanlance(self):
        ...
        for orderrow in hisposition.keys():
            ...
            if pos_long_his != 0 and pos_short_his != 0:
                quote = self.api.get_quote(orderrow)
                order_1 = self.api.insert_order(symbol=orderrow, direction='SELL', offset="CLOSE",
                                                limit_price=quote['pre_close'],
                                                volume=vol_)
                order_2 = self.api.insert_order(symbol=orderrow, direction='BUY', offset="CLOSE",
                                                limit_price=quote['pre_close'],
                                                volume=vol_)
                orderlist.append(order_1)
                orderlist.append(order_2)

        def order_is_finished(orderlist):
            order_is_finished_number = 0
            for _order_one in orderlist:
                if _order_one.status == 'FINISHED':
                    order_is_finished_number += 1
            if order_is_finished_number == len(orderlist):
                return True
            return False

        while True:
            self.api.wait_update()
            if order_is_finished(orderlist):
                print('平仓订单全部成交，退出')
                break
            print('双平订单还没有全部成交')
