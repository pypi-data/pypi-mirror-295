...
def test_rescaling4d():
    ...
    resampled_image = rescale(image, scale_z=1.0, scale_y=1.0, scale_x=1.0)
    assert np.array_equal(resampled_image.shape, image.shape)

    resampled_image = rescale(image, scale_z=0.5, scale_y=0.5, scale_x=0.5)
    assert np.array_equal(resampled_image.shape[1:], (np.asarray(image.shape) / 2)[1:])
    ...
    resampled_image1 = rescale(image, scale_z=1.0, scale_y=1.0, scale_x=0.5).squeeze()
    resampled_image2 = rescale(image, scale_z=1.0, scale_y=0.5, scale_x=1.0).squeeze()
    resampled_image3 = rescale(image, scale_z=0.5, scale_y=1.0, scale_x=1.0).squeeze()
