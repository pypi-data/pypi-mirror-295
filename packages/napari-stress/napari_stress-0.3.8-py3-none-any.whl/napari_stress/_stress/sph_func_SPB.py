...
def Proj_Into_SPH_Charts_At_Quad_Pts(func_quad_vals, Proj_Deg, lbdv):
    # In chart A
    sph_func_A = Faster_Double_Proj(
        func_quad_vals, Proj_Deg, lbdv
    ...
    for quad_pt in range(lbdv.lbdv_quad_pts):
        quad_pt_vals_B[quad_pt] = func_quad_vals[
            lbdv.Eval_Inv_Rot_Lbdv_Quad_vals(quad_pt)
        ]

    sph_func_B = Faster_Double_Proj(
        quad_pt_vals_B, Proj_Deg, lbdv
    )  # using class structure
...
def Lp_Rel_Error_At_Quad(approx_f_vals, f_vals, lbdv, p):  # Assumes f NOT 0
    ...
    return (Lp_Err / Lp_f) ** (1.0 / p)  # ||f_approx - f||_p / || f ||_p
...
class spherical_harmonics_function(
    object
):  # create class for Spherical Harmonics fn in our basis
    def __init__(self, SPH_Coef, SPH_Deg):
        self.sph_coef = SPH_Coef  # Array Representation in SPH Basis
        self.sph_deg = (
            SPH_Deg  # maximum degree of SPH Basis (formely mislabeled as order)
        )
    ...
    def Eval_SPH(self, Theta, Phi):
        ...
        for L_Coef in range(0, self.sph_deg + 1):
            for M_Coef in range(0, L_Coef + 1):
                SPH_Val += self.sph_coef[L_Coef - M_Coef][L_Coef] * Eval_SPH_Basis(
                    M_Coef, L_Coef, Theta, Phi
                )

                if L_Coef > 0 and M_Coef > 0:
                    SPH_Val += self.sph_coef[L_Coef][
                        L_Coef + (-1 * M_Coef)
                    ] * Eval_SPH_Basis(-1 * M_Coef, L_Coef, Theta, Phi)
    ...
    def Eval_SPH_Der_Phi(self, Theta, Phi):
        ...
        for L_Coef in range(0, self.sph_deg + 1):
            for M_Coef in range(0, L_Coef + 1):
                Der_SPH_Val += self.sph_coef[L_Coef - M_Coef][
                    L_Coef
                ] * Der_Phi_Basis_Fn(M_Coef, L_Coef, Theta, Phi)

                if L_Coef > 0 and M_Coef > 0:
                    Der_SPH_Val += self.sph_coef[L_Coef][
                        L_Coef + (-1 * M_Coef)
                    ] * Der_Phi_Basis_Fn(-1 * M_Coef, L_Coef, Theta, Phi)
    ...
    def Eval_SPH_Der_Phi_Phi(self, Theta, Phi):
        ...
        for L_Coef in range(0, self.sph_deg + 1):
            for M_Coef in range(0, L_Coef + 1):
                Sec_Der_SPH_Val += self.sph_coef[L_Coef - M_Coef][
                    L_Coef
                ] * Der_Phi_Phi_Basis_Fn(M_Coef, L_Coef, Theta, Phi)

                if L_Coef > 0 and M_Coef > 0:
                    Sec_Der_SPH_Val += self.sph_coef[L_Coef][
                        L_Coef + (-1 * M_Coef)
                    ] * Der_Phi_Phi_Basis_Fn(-1 * M_Coef, L_Coef, Theta, Phi)
    ...
    def Quick_Theta_Der(self):
        ...
        for n_coef in range(self.sph_deg + 1):
            for m_coef in range(1, n_coef + 1):  # Theta Der of Y^0_n = 0
                ...
                Der_Coef[n_coef - m_coef][n_coef] = (
                    m_coef * self.sph_coef[n_coef][n_coef - m_coef]
                )

                # D_theta Z^m_n = m*X^m_n
                Der_Coef[n_coef][n_coef - m_coef] = (
                    -1 * m_coef * self.sph_coef[n_coef - m_coef][n_coef]
                )
    ...
    def Lp_Rel_Error_in_Chart(self, f, lbdv, p):  # Assumes f NOT 0
        ...
        for quad_pt in range(lbdv.lbdv_quad_pts):
            ...
            if Chart_Min_Polar < phi_pt and phi_pt < Chart_Max_Polar:
                Lp_Err_Pt = (
                    abs(self.Eval_SPH_Coef_Mat(quad_pt, lbdv) - f(theta_pt, phi_pt))
                    ** p
                ) * weight_pt
        ...
        return (Lp_Err / Lp_f) ** (1.0 / p)  # ||self - f||_p / || f ||_p
    ...
    def Lp_Rel_Error_in_S2(self, f, lbdv, p):  # Assumes f NOT 0
        ...
        for quad_pt in range(lbdv.lbdv_quad_pts):
            ...
            Lp_Err_Pt = (
                abs(self.Eval_SPH_Coef_Mat(quad_pt, lbdv) - f(theta_pt, phi_pt)) ** p
            ) * weight_pt
    ...
    def plus_sph(self, other_SPH):
        return spherical_harmonics_function(
            self.sph_coef + other_SPH.sph_coef, self.sph_deg
        )
    ...
    def print_spherical_harmonics_function(self):
        print(
            "spherical_harmonics_function has degree: "
            + str(self.sph_deg)
            + " and has coef mat: "
            + "\n"
            + str(self.sph_coef)
