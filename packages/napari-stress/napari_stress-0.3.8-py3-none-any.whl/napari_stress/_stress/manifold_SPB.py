...
PICKLE_Manny_DIR = os.path.join(
    MY_DIR, "Pickled_Manny_Inv_Mat_Files"
)  # subfolder of manny_inv_matrix pickled files
...
def Non_Radial_Manifold_X_Def(theta, phi, r_0, Manny_Name):
    if Manny_Name == "Cusp_Bowl":
        return 2.0 * np.cos(theta) * np.sin(phi)

    elif Manny_Name == "UFO":
        return np.cos(theta) * (
            2.0 * np.sin(phi) + r_0 * np.sin(phi) * np.cos(phi) ** 3
        )
        ...
        phi_1 = (
            np.pi * (np.pi * R / 2.0) / (r_0 + np.pi * R)
        )  # normalzied arclen of top hem
        phi_2 = (
            np.pi * (r_0 + np.pi * R / 2.0) / (r_0 + np.pi * R)
        )  # normalized arcen up to bottom hemi
        ...
        if np.isscalar(phi):
            ...
            return np.where(
                phi < phi_1,
                R * np.sin(phi / (R * omega)) * np.cos(theta),
                np.where(
                    phi < phi_2,
                    R * np.cos(theta),
                    R * np.cos((phi - phi_2) / (R * omega)) * np.cos(theta),
                ),
            )
    else:
        print(
...
def Non_Radial_Manifold_Y_Def(theta, phi, r_0, Manny_Name):
    if Manny_Name == "Cusp_Bowl":
        return 2.0 * np.sin(theta) * np.sin(phi)
        ...
        if np.isscalar(phi):
            ...
            return np.where(
                phi < phi_1,
                R * np.sin(phi / (R * omega)) * np.sin(theta),
                np.where(
                    phi < phi_2,
                    R * np.sin(theta),
                    R * np.cos((phi - phi_2) / (R * omega)) * np.sin(theta),
                ),
            )
    else:
        print(
...
def Non_Radial_Manifold_Z_Def(theta, phi, r_0, Manny_Name):
    if Manny_Name == "Cusp_Bowl":
        return np.cos(phi) - r_0 * (np.cos(2.0 * phi) ** 2 - 1.0)

    elif Manny_Name == "UFO":
        return (np.cos(phi) - r_0 * np.cos(3.0 * phi)) / (1.0 - r_0)
    elif Manny_Name == "Ellipsoid_r0":
        return (1.0 + 3.0 * r_0) * np.cos(phi)
    elif Manny_Name == "Gen_Pill_nr":
        return (1.0 + r_0) * np.cos(phi)
    elif Manny_Name == "Cone_Head":
        return np.cos(phi) * (1.0 + r_0 * np.exp(-1.0 * 4.0 * phi**2))
    elif Manny_Name == "Cone_Head5x":
        ...
        if np.isscalar(phi):
            if phi < phi_1:
                return R * np.cos(phi / (R * omega)) + r_0 / 2.0
            elif phi < phi_2:
            ...
            return np.where(
                phi < phi_1,
                R * np.cos(phi / (R * omega)) + r_0 / 2.0,
                np.where(
                    phi < phi_2,
                    r_0 / 2.0 - (phi - phi_1) / omega,
                    -1.0 * r_0 / 2.0 - R * np.sin((phi - phi_2) / (R * omega)),
                ),
            )
    else:
        print(
...
def Man_Name_Radial(Man_Name):
    ...
    if (
        Man_Name == "Cusp_Bowl"
        or Man_Name == "UFO"
        or Man_Name == "Ellipsoid_r0"
        or Man_Name == "Gen_Pill_nr"
        or Man_Name == "Cone_Head"
        or Man_Name == "Cone_Head_Logis"
        or Man_Name == "Cone_Head5x"
        or Man_Name == "Fission_Yeast_R2"
        or Man_Name == "Fission_Yeast_R1pt2"
    ):
        IsRadial = False
    return IsRadial
...
def Radial_Manifold_R_Def(theta, phi, r_0, Manny_Name):
    if Manny_Name == "S2":  # Unit Sphere
        ...
        return 2.0 + np.cos(phi)

    elif Manny_Name == "Pill":  # Ellipsoid: x^2 + y^2 + z^2/4 = 1
        return 2.0 / np.sqrt(1.0 + 3.0 * np.sin(phi) ** 2)

    elif Manny_Name == "Gen_Pill":
        return (1.0 + r_0) / np.sqrt(
            1.0 + ((1.0 + r_0) ** 2 - 1.0) * np.sin(phi) ** 2
        )  # Ellipsoid: x^2 + y^2 + z^2/((1+r_0)^2) = 1

    elif Manny_Name == "Oblate_Gen_Pill":
        return (1.0 + r_0) / np.sqrt(
            ((1.0 + r_0) ** 2) * (np.cos(phi) ** 2) + np.sin(phi) ** 2
        )  # Ellipsoid: (x^2 + y^2)/((1+r_0)^2) + z^2 = 1

    elif Manny_Name == "Gen_S2":
        return 1.0 + r_0  # radius: 1+r_0

    elif Manny_Name == "Chew_Toy":
        return 1.0 + r_0 * np.sin(3 * phi) * np.cos(theta)

    elif Manny_Name == "Little_Five_Points":
        return 1.0 + r_0 * np.sin(5 * phi) * np.cos(theta)
        ...
        d_0_sq = (X_S2 - 0.0) ** 2 + (Y_S2 - 0.0) ** 2 + (1.0 - Z_S2) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_0_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_0_sq)),
            1.0,
        )

        d_1_sq = (X_S2 - 1.0) ** 2 + (Y_S2 - 0.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_1_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_1_sq)),
            R_shape,
        )

        d_2_sq = (X_S2 - 0.0) ** 2 + (Y_S2 - 1.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_2_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_2_sq)),
            R_shape,
        )

        d_3_sq = (X_S2 - 0.0) ** 2 + (Y_S2 - 0.0) ** 2 + (-1.0 - Z_S2) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_3_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_3_sq)),
            R_shape,
        )

        d_4_sq = (X_S2 - -1.0) ** 2 + (Y_S2 - 0.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_4_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_4_sq)),
            R_shape,
        )
        ...
        d_3_sq = (X_S2 - 0.0) ** 2 + (Y_S2 + 1.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_3_sq) > tol_shape,
            (1.0 + r_0 * np.exp(-1.0 * k_shape * d_3_sq)),
            R_shape,
        ...
        # r0 =  distortion magnitude
        k_shape = 5.0  # inv. length-scale of distortion decay
        tol_shape = 1.0e-3
        ...
        d_1_sq = (X_S2 - R_S2) ** 2 + (Y_S2 - 0.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_1_sq) > tol_shape,
            (R_S2 + r_0 * np.exp(-1.0 * k_shape * d_1_sq)),
            R_S2,
        )

        d_2_sq = (X_S2 - 0.0) ** 2 + (Y_S2 - R_S2) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_2_sq) > tol_shape,
            (R_S2 + r_0 * np.exp(-1.0 * k_shape * d_2_sq)),
            R_shape,
        )

        d_3_sq = (X_S2 - 0.0) ** 2 + (Y_S2 + R_S2) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_3_sq) > tol_shape,
            (R_S2 + r_0 * np.exp(-1.0 * k_shape * d_3_sq)),
            R_shape,
        )

        d_4_sq = (X_S2 + R_S2) ** 2 + (Y_S2 - 0.0) ** 2 + (Z_S2 - 0.0) ** 2
        R_shape = np.where(
            r_0 * np.exp(-1.0 * k_shape * d_4_sq) > tol_shape,
            (R_S2 + r_0 * np.exp(-1.0 * k_shape * d_4_sq)),
            R_shape,
        )
        ...
        # r0 =  distortion magnitude
        k_shape = 2.0  # inv. length-scale of distortion decay
        tol_shape = 1.0e-3
        ...
        return 1.0 + r_0 * np.exp(-10.0 * phi**2)
        ...
        phi_1 = np.arctan(R / (r_0 / 2.0))
        phi_2 = np.pi - np.arctan(R / (r_0 / 2.0))

        if np.isscalar(phi):
            if phi < phi_1:
                return (r_0 / 2.0) * np.cos(phi) + np.sqrt(
                    R**2 - ((r_0 / 2) * np.sin(phi)) ** 2
                )
            elif phi < phi_2:
                return R / np.sin(phi)
            else:
                return (r_0 / 2.0) * np.cos(np.pi - phi) + np.sqrt(
                    R**2 - ((r_0 / 2) * np.sin(np.pi - phi)) ** 2
                )
        else:
            return np.where(
                phi < phi_1,
                (r_0 / 2.0) * np.cos(phi)
                + np.sqrt(R**2 - ((r_0 / 2) * np.sin(phi)) ** 2),
                np.where(
                    phi < phi_2,
                    R / np.sin(phi),
                    (r_0 / 2.0) * np.cos(np.pi - phi)
                    + np.sqrt(R**2 - ((r_0 / 2) * np.sin(np.pi - phi)) ** 2),
                ),
            )
        ...
        if np.isscalar(phi):
            if np.cos(phi) > 0.9:
                return 1.0 - r_0 * np.exp(
                    -1.0 * (0.19) / (np.cos(phi) ** 2 - (0.9) ** 2) + 1.0
                )
        ...
        return 1.0 - (6.0 * r_0) * np.exp(
            -1.0 / (1.0 - np.cos(phi) ** 2)
...
# Takes an array of R0 Values and returns string format
# for Rayleigh Dissipation Studies (!!!!VTU NOT PICKLE!!!!)
...
class manifold(object):
    ...
    def __init__(
        ...
        if self.Use_Man_Name is True:
            ...
            self.Man_Shape_Name = self.Man_Shape_Dict[
                "Maniold_Shape_Name"
            ]  # name of manifold shape (i.e. spehre, ellipsoid, etc)

            R_0_str = str(self.Man_R0_Val).replace(".", "pt")
            self.Man_Official_Name = (
                self.Man_Shape_Name
                + "_R_0_"
                + R_0_str
                + "_Pdeg_"
                + str(self.Man_SPH_Deg)
                + "_Q"
                + str(self.num_quad_pts)
            )  # str for picking, with R0, num_pts, deg of basis, etc

            self.X_A_Pts = Manny_Fn_Def_X(
                lbdv.theta_pts,
                lbdv.phi_pts,
                self.Man_R0_Val,
                self.Man_Shape_Name,
        ...
        self.Y, self.Y_Bar = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
            self.Y_A_Pts, self.Man_SPH_Deg, lbdv
        )
        self.Z, self.Z_Bar = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
            self.Z_A_Pts, self.Man_SPH_Deg, lbdv
        )

        # BJG: Lists of Quad Pt Vals, so we can evaluate,
        ...
        self.Metric_Factor_A_pts = np.sqrt(
            (
                self.X_theta_A_Pts * self.Y_phi_A_Pts
                - self.Y_theta_A_Pts * self.X_phi_A_Pts
            )
            ** 2
            + (
                self.X_theta_A_Pts * self.Z_phi_A_Pts
                - self.Z_theta_A_Pts * self.X_phi_A_Pts
            )
            ** 2
            + (
                self.Y_theta_A_Pts * self.Z_phi_A_Pts
                - self.Z_theta_A_Pts * self.Y_phi_A_Pts
            )
            ** 2
        )
        self.Metric_Factor_B_pts = np.sqrt(
            (
                self.X_theta_B_Pts * self.Y_phi_B_Pts
                - self.Y_theta_B_Pts * self.X_phi_B_Pts
            )
            ** 2
            + (
                self.X_theta_B_Pts * self.Z_phi_B_Pts
                - self.Z_theta_B_Pts * self.X_phi_B_Pts
            )
            ** 2
            + (
                self.Y_theta_B_Pts * self.Z_phi_B_Pts
                - self.Z_theta_B_Pts * self.Y_phi_B_Pts
            )
            ** 2
        )

        # For error metric on Manny:
        self.Metric_Factor_A_over_sin_phi_pts = np.where(
            np.sin(lbdv.phi_pts) > self.Tol,
            self.Metric_Factor_A_pts / np.sin(lbdv.phi_pts),
            0,
        )
        self.Metric_Factor_B_over_sin_phi_bar_pts = np.where(
            np.sin(lbdv.phi_pts) > self.Tol,
            self.Metric_Factor_B_pts / np.sin(lbdv.phi_pts),
            0,
        )
        ...
        self.F_A_pts = (
            self.X_theta_A_Pts * self.X_phi_A_Pts
            + self.Y_theta_A_Pts * self.Y_phi_A_Pts
            + self.Z_theta_A_Pts * self.Z_phi_A_Pts
        )
        self.F_B_pts = (
            self.X_theta_B_Pts * self.X_phi_B_Pts
            + self.Y_theta_B_Pts * self.Y_phi_B_Pts
        ...
        self.E_theta_A_pts = (
            ...
            + 2.0 * self.Y_theta_A_Pts * self.Y_theta_theta_A_Pts
            + 2.0 * self.Z_theta_A_Pts * self.Z_theta_theta_A_Pts
        )
        self.E_phi_A_pts = (
            2.0 * self.X_theta_A_Pts * self.X_theta_phi_A_Pts
            + 2.0 * self.Y_theta_A_Pts * self.Y_theta_phi_A_Pts
            + 2.0 * self.Z_theta_A_Pts * self.Z_theta_phi_A_Pts
        )

        self.E_theta_B_pts = (
            2.0 * self.X_theta_B_Pts * self.X_theta_theta_B_Pts
            + 2.0 * self.Y_theta_B_Pts * self.Y_theta_theta_B_Pts
            + 2.0 * self.Z_theta_B_Pts * self.Z_theta_theta_B_Pts
        )
        self.E_phi_B_pts = (
            2.0 * self.X_theta_B_Pts * self.X_theta_phi_B_Pts
            + 2.0 * self.Y_theta_B_Pts * self.Y_theta_phi_B_Pts
            + 2.0 * self.Z_theta_B_Pts * self.Z_theta_phi_B_Pts
        )

        self.F_theta_A_pts = (
            (
                self.X_theta_A_Pts * self.X_theta_phi_A_Pts
                + self.X_phi_A_Pts * self.X_theta_theta_A_Pts
            )
            + (
                self.Y_theta_A_Pts * self.Y_theta_phi_A_Pts
                + self.Y_phi_A_Pts * self.Y_theta_theta_A_Pts
            )
            + (
                self.Z_theta_A_Pts * self.Z_theta_phi_A_Pts
                + self.Z_phi_A_Pts * self.Z_theta_theta_A_Pts
            )
        )
        self.F_phi_A_pts = (
            (
                self.X_theta_A_Pts * self.X_phi_phi_A_Pts
                + self.X_phi_A_Pts * self.X_theta_phi_A_Pts
            )
            + (
                self.Y_theta_A_Pts * self.Y_phi_phi_A_Pts
                + self.Y_phi_A_Pts * self.Y_theta_phi_A_Pts
            )
            + (
                self.Z_theta_A_Pts * self.Z_phi_phi_A_Pts
                + self.Z_phi_A_Pts * self.Z_theta_phi_A_Pts
            )
        )

        self.F_theta_B_pts = (
            (
                self.X_theta_B_Pts * self.X_theta_phi_B_Pts
                + self.X_phi_B_Pts * self.X_theta_theta_B_Pts
            )
            + (
                self.Y_theta_B_Pts * self.Y_theta_phi_B_Pts
                + self.Y_phi_B_Pts * self.Y_theta_theta_B_Pts
            )
            + (
                self.Z_theta_B_Pts * self.Z_theta_phi_B_Pts
                + self.Z_phi_B_Pts * self.Z_theta_theta_B_Pts
            )
        )
        self.F_phi_B_pts = (
            (
                self.X_theta_B_Pts * self.X_phi_phi_B_Pts
                + self.X_phi_B_Pts * self.X_theta_phi_B_Pts
            )
            + (
                self.Y_theta_B_Pts * self.Y_phi_phi_B_Pts
                + self.Y_phi_B_Pts * self.Y_theta_phi_B_Pts
            )
            + (
                self.Z_theta_B_Pts * self.Z_phi_phi_B_Pts
                + self.Z_phi_B_Pts * self.Z_theta_phi_B_Pts
            )
        )

        self.G_theta_A_pts = (
            2.0 * self.X_phi_A_Pts * self.X_theta_phi_A_Pts
            + 2.0 * self.Y_phi_A_Pts * self.Y_theta_phi_A_Pts
            + 2.0 * self.Z_phi_A_Pts * self.Z_theta_phi_A_Pts
        )
        self.G_phi_A_pts = (
            2.0 * self.X_phi_A_Pts * self.X_phi_phi_A_Pts
            + 2.0 * self.Y_phi_A_Pts * self.Y_phi_phi_A_Pts
            + 2.0 * self.Z_phi_A_Pts * self.Z_phi_phi_A_Pts
        )

        self.G_theta_B_pts = (
            2.0 * self.X_phi_B_Pts * self.X_theta_phi_B_Pts
            + 2.0 * self.Y_phi_B_Pts * self.Y_theta_phi_B_Pts
            + 2.0 * self.Z_phi_B_Pts * self.Z_theta_phi_B_Pts
        )
        self.G_phi_B_pts = (
            2.0 * self.X_phi_B_Pts * self.X_phi_phi_B_Pts
            + 2.0 * self.Y_phi_B_Pts * self.Y_phi_phi_B_Pts
            + 2.0 * self.Z_phi_B_Pts * self.Z_phi_phi_B_Pts
        )
        ...
        # d_i(sqrt(g)):
        self.Metric_Factor_dTheta_A_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (0.5)
            * (
                self.E_theta_A_pts * self.G_A_pts
                + self.E_A_pts * self.G_theta_A_pts
                - 2 * self.F_A_pts * self.F_theta_A_pts
            )
            / self.Metric_Factor_A_pts,
            0,
        )
        self.Metric_Factor_dPhi_A_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (0.5)
            * (
                self.E_phi_A_pts * self.G_A_pts
                + self.E_A_pts * self.G_phi_A_pts
                - 2 * self.F_A_pts * self.F_phi_A_pts
            )
            / self.Metric_Factor_A_pts,
            0,
        )

        self.Metric_Factor_dTheta_B_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (0.5)
            * (
                self.E_theta_B_pts * self.G_B_pts
                + self.E_B_pts * self.G_theta_B_pts
                - 2 * self.F_B_pts * self.F_theta_B_pts
            )
            / self.Metric_Factor_B_pts,
            0,
        )
        self.Metric_Factor_dPhi_B_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (0.5)
            * (
                self.E_phi_B_pts * self.G_B_pts
                + self.E_B_pts * self.G_phi_B_pts
                - 2 * self.F_B_pts * self.F_phi_B_pts
            )
            / self.Metric_Factor_B_pts,
            0,
        )
        ...
        # d_i(|g|):
        self.Metric_Factor_Squared_dTheta_A_pts = (
            self.E_theta_A_pts * self.G_A_pts
            + self.E_A_pts * self.G_theta_A_pts
            - 2 * self.F_A_pts * self.F_theta_A_pts
        )
        self.Metric_Factor_Squared_dPhi_A_pts = (
            self.E_phi_A_pts * self.G_A_pts
            + self.E_A_pts * self.G_phi_A_pts
            - 2 * self.F_A_pts * self.F_phi_A_pts
        )

        self.Metric_Factor_Squared_dTheta_B_pts = (
            self.E_theta_B_pts * self.G_B_pts
            + self.E_B_pts * self.G_theta_B_pts
            - 2 * self.F_B_pts * self.F_theta_B_pts
        )
        self.Metric_Factor_Squared_dPhi_B_pts = (
            self.E_phi_B_pts * self.G_B_pts
            + self.E_B_pts * self.G_phi_B_pts
            - 2 * self.F_B_pts * self.F_phi_B_pts
        )
        ...
        self.Metric_Factor_dTheta_over_Metric_Factor_A_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Metric_Factor_dTheta_A_pts / self.Metric_Factor_A_pts,
            0,
        )
        self.Metric_Factor_dPhi_over_Metric_Factor_A_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Metric_Factor_dPhi_A_pts / self.Metric_Factor_A_pts,
            0,
        )

        self.Metric_Factor_dTheta_over_Metric_Factor_B_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Metric_Factor_dTheta_B_pts / self.Metric_Factor_B_pts,
            0,
        )
        self.Metric_Factor_dPhi_over_Metric_Factor_B_pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Metric_Factor_dPhi_B_pts / self.Metric_Factor_B_pts,
            0,
        )

        self.F_over_Metric_Factor_dTheta_A = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_A_pts * self.F_theta_A_pts
                - self.F_A_pts * self.Metric_Factor_dTheta_A_pts
            )
            / self.Metric_Factor_Squared_A,
            0,
        )
        self.G_over_Metric_Factor_dTheta_A = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_A_pts * self.G_theta_A_pts
                - self.G_A_pts * self.Metric_Factor_dTheta_A_pts
            )
            / self.Metric_Factor_Squared_A,
            0,
        )

        self.E_over_Metric_Factor_dPhi_A = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_A_pts * self.E_phi_A_pts
                - self.E_A_pts * self.Metric_Factor_dPhi_A_pts
            )
            / self.Metric_Factor_Squared_A,
            0,
        )
        self.F_over_Metric_Factor_dPhi_A = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_A_pts * self.F_phi_A_pts
                - self.F_A_pts * self.Metric_Factor_dPhi_A_pts
            )
            / self.Metric_Factor_Squared_A,
            0,
        )

        self.F_over_Metric_Factor_dTheta_B = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_B_pts * self.F_theta_B_pts
                - self.F_B_pts * self.Metric_Factor_dTheta_B_pts
            )
            / self.Metric_Factor_Squared_B,
            0,
        )
        self.G_over_Metric_Factor_dTheta_B = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_B_pts * self.G_theta_B_pts
                - self.G_B_pts * self.Metric_Factor_dTheta_B_pts
            )
            / self.Metric_Factor_Squared_B,
            0,
        )

        self.E_over_Metric_Factor_dPhi_B = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_B_pts * self.E_phi_B_pts
                - self.E_B_pts * self.Metric_Factor_dPhi_B_pts
            )
            / self.Metric_Factor_Squared_B,
            0,
        )
        self.F_over_Metric_Factor_dPhi_B = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (
                self.Metric_Factor_B_pts * self.F_phi_B_pts
                - self.F_B_pts * self.Metric_Factor_dPhi_B_pts
            )
            / self.Metric_Factor_Squared_B,
            0,
        )
        ...
        self.Sigma_Theta_A_Pts = np.hstack(
            (self.X_theta_A_Pts, self.Y_theta_A_Pts, self.Z_theta_A_Pts)
        )
        self.Sigma_Theta_B_Pts = np.hstack(
            (self.X_theta_B_Pts, self.Y_theta_B_Pts, self.Z_theta_B_Pts)
        )

        self.Sigma_Phi_A_Pts = np.hstack(
            (self.X_phi_A_Pts, self.Y_phi_A_Pts, self.Z_phi_A_Pts)
        )
        self.Sigma_Phi_B_Pts = np.hstack(
            (self.X_phi_B_Pts, self.Y_phi_B_Pts, self.Z_phi_B_Pts)
        )

        # BJG: derivative vectors are easy in this framework:
        self.Sigma_Theta_Theta_A_Pts = np.hstack(
            (
                self.X_theta_theta_A_Pts,
                self.Y_theta_theta_A_Pts,
                self.Z_theta_theta_A_Pts,
            )
        )
        self.Sigma_Theta_Theta_B_Pts = np.hstack(
            (
                self.X_theta_theta_B_Pts,
                self.Y_theta_theta_B_Pts,
                self.Z_theta_theta_B_Pts,
            )
        )
        ...
        self.Sigma_Phi_Phi_A_Pts = np.hstack(
            (self.X_phi_phi_A_Pts, self.Y_phi_phi_A_Pts, self.Z_phi_phi_A_Pts)
        )
        self.Sigma_Phi_Phi_B_Pts = np.hstack(
            (self.X_phi_phi_B_Pts, self.Y_phi_phi_B_Pts, self.Z_phi_phi_B_Pts)
        )

        # Normal Vectors and II can be computed here as well
        # (within Chart, for pickled fields below):
        ...
        self.Normal_Dir_Y_A_Pts = -1.0 * (
            self.X_theta_A_Pts * self.Z_phi_A_Pts
            - self.Z_theta_A_Pts * self.X_phi_A_Pts
        )
        self.Normal_Dir_Y_B_Pts = -1.0 * (
            self.X_theta_B_Pts * self.Z_phi_B_Pts
            - self.Z_theta_B_Pts * self.X_phi_B_Pts
        )
        ...
        self.Normal_Vec_X_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_X_A_Pts / self.Metric_Factor_A_pts,
            0,
        )
        self.Normal_Vec_X_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_X_B_Pts / self.Metric_Factor_B_pts,
            0,
        )

        self.Normal_Vec_Y_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Y_A_Pts / self.Metric_Factor_A_pts,
            0,
        )
        self.Normal_Vec_Y_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Y_B_Pts / self.Metric_Factor_B_pts,
            0,
        )

        self.Normal_Vec_Z_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Z_A_Pts / self.Metric_Factor_A_pts,
            0,
        )
        self.Normal_Vec_Z_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Z_B_Pts / self.Metric_Factor_B_pts,
            0,
        )
        ...
        self.L_A_Pts = (
            self.Normal_Vec_X_A_Pts * self.X_theta_theta_A_Pts
            + self.Normal_Vec_Y_A_Pts * self.Y_theta_theta_A_Pts
            + self.Normal_Vec_Z_A_Pts * self.Z_theta_theta_A_Pts
        )
        self.L_B_Pts = (
            self.Normal_Vec_X_B_Pts * self.X_theta_theta_B_Pts
            + self.Normal_Vec_Y_B_Pts * self.Y_theta_theta_B_Pts
            + self.Normal_Vec_Z_B_Pts * self.Z_theta_theta_B_Pts
        )

        self.M_A_Pts = (
            self.Normal_Vec_X_A_Pts * self.X_theta_phi_A_Pts
            + self.Normal_Vec_Y_A_Pts * self.Y_theta_phi_A_Pts
            + self.Normal_Vec_Z_A_Pts * self.Z_theta_phi_A_Pts
        )
        self.M_B_Pts = (
            self.Normal_Vec_X_B_Pts * self.X_theta_phi_B_Pts
            + self.Normal_Vec_Y_B_Pts * self.Y_theta_phi_B_Pts
            + self.Normal_Vec_Z_B_Pts * self.Z_theta_phi_B_Pts
        )

        self.N_A_Pts = (
            self.Normal_Vec_X_A_Pts * self.X_phi_phi_A_Pts
            + self.Normal_Vec_Y_A_Pts * self.Y_phi_phi_A_Pts
            + self.Normal_Vec_Z_A_Pts * self.Z_phi_phi_A_Pts
        )
        self.N_B_Pts = (
            self.Normal_Vec_X_B_Pts * self.X_phi_phi_B_Pts
            + self.Normal_Vec_Y_B_Pts * self.Y_phi_phi_B_Pts
            + self.Normal_Vec_Z_B_Pts * self.Z_phi_phi_B_Pts
        )

        # We can use this to compute entries of the Weingarten Map Directly,
        # where W = [[W_11, W_12], [W_21, W_22]]:
        self.Wein_11_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.L_A_Pts * self.G_A_pts - self.M_A_Pts * self.F_A_pts)
            / self.Metric_Factor_Squared_A,
            0,
        )
        self.Wein_12_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.M_A_Pts * self.G_A_pts - self.N_A_Pts * self.F_A_pts)
            / self.Metric_Factor_Squared_A,
            0,
        )
        self.Wein_21_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.M_A_Pts * self.E_A_pts - self.L_A_Pts * self.F_A_pts)
            / self.Metric_Factor_Squared_A,
            0,
        )
        self.Wein_22_A_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.N_A_Pts * self.E_A_pts - self.M_A_Pts * self.F_A_pts)
            / self.Metric_Factor_Squared_A,
            0,
        )

        self.Wein_11_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.L_B_Pts * self.G_B_pts - self.M_B_Pts * self.F_B_pts)
            / self.Metric_Factor_Squared_B,
            0,
        )
        self.Wein_12_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.M_B_Pts * self.G_B_pts - self.N_B_Pts * self.F_B_pts)
            / self.Metric_Factor_Squared_B,
            0,
        )
        self.Wein_21_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.M_B_Pts * self.E_B_pts - self.L_B_Pts * self.F_B_pts)
            / self.Metric_Factor_Squared_B,
            0,
        )
        self.Wein_22_B_Pts = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            (self.N_B_Pts * self.E_B_pts - self.M_B_Pts * self.F_B_pts)
            / self.Metric_Factor_Squared_B,
            0,
        )
        ...
        Denom_A = np.multiply(
            (self.R_sq_A_Pts), np.sqrt(self.X_A_Pts**2 + self.Y_A_Pts**2)
        )

        self.dx_dy_A_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Z_A_Pts / self.Metric_Factor_Squared_A,
            0,
        )
        self.dx_dy_A_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, -self.Z_A_Pts / Denom_A, 0
        )
        self.dx_dz_A_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            -1.0 * self.Normal_Dir_Y_A_Pts / self.Metric_Factor_Squared_A,
            0,
        )
        self.dx_dz_A_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, self.Y_A_Pts / Denom_A, 0
        )
        self.dy_dz_A_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_X_A_Pts / self.Metric_Factor_Squared_A,
            0,
        )
        self.dy_dz_A_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, -self.X_A_Pts / Denom_A, 0
        )

        Denom_B = np.multiply(
            (self.R_sq_B_Pts), np.sqrt(self.Y_B_Pts**2 + self.Z_B_Pts**2)
        )

        self.dx_dy_B_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_Z_B_Pts / self.Metric_Factor_Squared_B,
            0,
        )
        self.dx_dy_B_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, -self.Z_B_Pts / Denom_B, 0
        )
        self.dx_dz_B_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            -1.0 * self.Normal_Dir_Y_B_Pts / self.Metric_Factor_Squared_B,
            0,
        )
        self.dx_dz_B_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, self.Y_B_Pts / Denom_B, 0
        )
        self.dy_dz_B_Vals_From_Polar_NEW = np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            self.Normal_Dir_X_B_Pts / self.Metric_Factor_Squared_B,
            0,
        )
        self.dy_dz_B_Vals_From_Polar = np.where(
            lbdv.Chart_of_Quad_Pts > 0, -self.X_B_Pts / Denom_B, 0
        )
        ...
        if (
            ...
            self.xi_theta_A_Mats = (
                zero_vector_of_basis_mats()
            )  # xi_i = -1*G*(A^-1)*(d_i[A^-1])*(A^-1)
            ...
            self.K_A_pts = np.where(
                lbdv.Chart_of_Quad_Pts > 0,
                (self.L_A_Pts * self.N_A_Pts - self.M_A_Pts**2)
                / self.Metric_Factor_Squared_A,
                0,
            )  # zeros_like(lbdv.X) #Note these are Vectors
            self.K_B_pts = np.where(
                lbdv.Chart_of_Quad_Pts > 0,
                (self.L_B_Pts * self.N_B_Pts - self.M_B_Pts**2)
                / self.Metric_Factor_Squared_B,
                0,
            )  # zeros_like(lbdv.X)

            # Calculate H at quad pts, for Droplets:
            self.H_A_pts = np.where(
                lbdv.Chart_of_Quad_Pts > 0,
                (
                    self.L_A_Pts * self.G_A_pts
                    - 2.0 * self.M_A_Pts * self.F_A_pts
                    + self.N_A_Pts * self.E_A_pts
                )
                / (2.0 * self.Metric_Factor_Squared_A),
                0,
            )
            self.H_B_pts = np.where(
                lbdv.Chart_of_Quad_Pts > 0,
                (
                    self.L_B_Pts * self.G_B_pts
                    - 2.0 * self.M_B_Pts * self.F_B_pts
                    + self.N_B_Pts * self.E_B_pts
                )
                / (2.0 * self.Metric_Factor_Squared_B),
                0,
            )
            ...
            for quad_pt in range(self.num_quad_pts):
                if lbdv.Chart_of_Quad_Pts[quad_pt] > 0:
                    ...
                    # use these to compute matrcies for flat,
                    # and derivative of cotangent components
                    ...
                    xi_theta_B_pt = np.linalg.solve(
                        B_Mat_pt.T, np.dot(-1 * B_theta_Mat_pt.T, rho_B_pt.T)
                    ).T

                    xi_phi_A_pt = np.linalg.solve(
                        A_Mat_pt.T, np.dot(-1 * A_phi_Mat_pt.T, rho_A_pt.T)
                    ).T
                    xi_phi_B_pt = np.linalg.solve(
                        B_Mat_pt.T, np.dot(-1 * B_phi_Mat_pt.T, rho_B_pt.T)
                    ).T
        ...
        # If we have already pickled the above matricies, we load them:
            ...
            (
                ...
                self.rho_theta_B_Mats,
                self.rho_phi_A_Mats,
                self.rho_phi_B_Mats,
                self.xi_theta_A_Mats,
                self.xi_theta_B_Mats,
                self.xi_phi_A_Mats,
                self.xi_phi_B_Mats,
            ) = np.squeeze(np.split(Pickled_inv_Manny_Mats, 10, 3))
    ...
    def Normal_Dir(
        self, quad_pt, Chart
    ):  # for 2-form conversion, we need un-normalized:
        if Chart == "A":
    ...
    def I_Mat(self, quad_pt, Chart):
        return np.array(
            [
                ...
                [self.F(quad_pt, Chart), self.G(quad_pt, Chart)],
            ]
        )
    ...
    def I_theta_Mat(self, quad_pt, Chart):
        return np.array(
            [
                ...
                [self.F_theta(quad_pt, Chart), self.G_theta(quad_pt, Chart)],
            ]
        )
    ...
    def I_phi_Mat(self, quad_pt, Chart):
        return np.array(
            [
                ...
                [self.F_phi(quad_pt, Chart), self.G_phi(quad_pt, Chart)],
            ]
        )
    ...
    def Wein_Map(self, quad_pt, Chart):
        return np.array(
            [
                ...
                [self.W_21(quad_pt, Chart), self.W_22(quad_pt, Chart)],
            ]
        )
    ...
    def Normal_vec_dtheta(self, quad_pt, Chart):
        ...
        # to output vector of solutions at each point:
            ...
            return -1.0 * (
                W_map[0, 0].reshape(len(quad_pt), 1) * sigma_theta_C_pt
                + W_map[1, 0].reshape(len(quad_pt), 1) * sigma_phi_C_pt
            )
    ...
    def Normal_vec_dphi(self, quad_pt, Chart):
        ...
        # sumation to output vector of solutions at each point:
            ...
            return -1.0 * (
                W_map[0, 1].reshape(len(quad_pt), 1) * sigma_theta_C_pt
                + W_map[1, 1].reshape(len(quad_pt), 1) * sigma_phi_C_pt
            )

    # Matrix Used to Convert to Polar
    def Change_Basis_Mat(self, quad_pt, Chart):
        Basis_mat = np.zeros((3, 3))
