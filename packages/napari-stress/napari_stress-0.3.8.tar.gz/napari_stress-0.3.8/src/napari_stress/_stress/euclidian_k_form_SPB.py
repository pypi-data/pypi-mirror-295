...
def get_quadrature_points_from_sh_function(SPH_Func, lbdv, Chart):
    ...
    if Chart == "B":  # The list needs to be in
        Extracted_Quad_vals = SPH_Func.Eval_SPH_Coef_Mat(
            lbdv.Eval_Rot_Lbdv_Quad_vals(quad_pts), lbdv
        )
...
def Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(sph_func, lbdv, Chart):
    ...
    if Chart == "A":
        return np.where(
            ...
            sph_func.Eval_SPH_Der_Phi_Coef(quad_pts, lbdv),
            0,
        )
    ...
    if Chart == "B":
        ...
        return np.where(
            lbdv.Chart_of_Quad_Pts[quad_pts_rot] > 0,
            sph_func.Eval_SPH_Der_Phi_Coef(quad_pts_rot, lbdv),
            0,
        )
...
def Extract_dPhi_Phi_Quad_Pt_Vals_From_SPH_Fn(sph_func, lbdv, Chart):
    ...
    if Chart == "A":
        return np.where(
            lbdv.Chart_of_Quad_Pts > 0,
            sph_func.Eval_SPH_Der_Phi_Phi_Coef(quad_pts, lbdv),
            0,
        )
    ...
    if Chart == "B":
        ...
        return np.where(
            lbdv.Chart_of_Quad_Pts[quad_pts_rot] > 0,
            sph_func.Eval_SPH_Der_Phi_Phi_Coef(quad_pts_rot, lbdv),
            0,
        )
...
def Combine_Chart_Quad_Vals(Quad_Vals_A, Quad_Vals_B, lbdv):
    ...
    return np.where(
        lbdv.Chart_of_Quad_Pts > 0,
        Quad_Vals_A,
        Quad_Vals_B[lbdv.Eval_Rot_Lbdv_Quad_vals(quad_pts)],
    )
...
def Combine_Manny_Gauss_Curvatures(Manny, lbdv, verbose=False):
    ...
    if verbose is True:
        print(
            ...
            + str(
                (Integral_on_Manny(K_cominbed_A_pts, Manny, lbdv) - 4 * np.pi)
                / (4 * np.pi)
            )
            + " (Rel Error)"
        )
...
def Sharp(dtheta_A_vals, dphi_A_vals, dtheta_B_vals, dphi_B_vals, p_deg, lbdv, Manny):
    ...
    g_inv_theta_theta_A_pts = (
        Manny.g_Inv_Theta_Theta_A_Pts
    )  # Manny.g_inv_theta_theta(theta_pts, phi_pts, 'A')
    g_inv_theta_phi_A_pts = (
        Manny.g_Inv_Theta_Phi_A_Pts
    )  # Manny.g_inv_theta_phi(theta_pts, phi_pts, 'A')
    g_inv_phi_phi_A_pts = (
        Manny.g_Inv_Phi_Phi_A_Pts
    )  # Manny.g_inv_phi_phi(theta_pts, phi_pts, 'A')

    g_inv_theta_theta_B_pts = (
        Manny.g_Inv_Theta_Theta_B_Pts
    )  # Manny.g_inv_theta_theta(theta_pts, phi_pts, 'B')
    g_inv_theta_phi_B_pts = (
        Manny.g_Inv_Theta_Phi_B_Pts
    )  # Manny.g_inv_theta_phi(theta_pts, phi_pts, 'B')
    g_inv_phi_phi_B_pts = (
        Manny.g_Inv_Phi_Phi_B_Pts
    )  # Manny.g_inv_phi_phi(theta_pts, phi_pts, 'B')

    sigma_theta_A_pts = (
        Manny.Sigma_Theta_A_Pts
    )  # np.squeeze(Manny.sigma_theta(theta_pts, phi_pts, 'A'))
    sigma_theta_B_pts = (
        Manny.Sigma_Theta_B_Pts
    )  # np.squeeze(Manny.sigma_theta(theta_pts, phi_pts, 'B') )

    sigma_phi_A_pts = (
        Manny.Sigma_Phi_A_Pts
    )  # np.squeeze(Manny.sigma_phi(theta_pts, phi_pts, 'A'))
    sigma_phi_B_pts = (
        Manny.Sigma_Phi_B_Pts
    )  # np.squeeze(Manny.sigma_phi(theta_pts, phi_pts, 'B'))

    # Sharp in (theta, phi) Coors:
    alpha_sharp_theta_A_pts = np.multiply(
        d_theta_pts_A, g_inv_theta_theta_A_pts
    ) + np.multiply(d_phi_pts_A, g_inv_theta_phi_A_pts)
    alpha_sharp_phi_A_pts = np.multiply(
        d_theta_pts_A, g_inv_theta_phi_A_pts
    ) + np.multiply(d_phi_pts_A, g_inv_phi_phi_A_pts)

    alpha_sharp_theta_B_pts = np.multiply(
        d_theta_pts_B, g_inv_theta_theta_B_pts
    ) + np.multiply(d_phi_pts_B, g_inv_theta_phi_B_pts)
    alpha_sharp_phi_B_pts = np.multiply(
        d_theta_pts_B, g_inv_theta_phi_B_pts
    ) + np.multiply(d_phi_pts_B, g_inv_phi_phi_B_pts)

    # Convert Sharped Result to Euclidean Coors:
    alpha_sharp_A_pts = np.multiply(
        alpha_sharp_theta_A_pts, sigma_theta_A_pts
    ) + np.multiply(alpha_sharp_phi_A_pts, sigma_phi_A_pts)
    alpha_sharp_B_pts = np.multiply(
        alpha_sharp_theta_B_pts, sigma_theta_B_pts
    ) + np.multiply(alpha_sharp_phi_B_pts, sigma_phi_B_pts)

    sharped_x_vals_pts_A, sharped_y_vals_pts_A, sharped_z_vals_pts_A = np.hsplit(
        alpha_sharp_A_pts, 3
    )
    sharped_x_vals_pts_B, sharped_y_vals_pts_B, sharped_z_vals_pts_B = np.hsplit(
        alpha_sharp_B_pts, 3
    )

    sharped_x_vals_pts_Used = Combine_Chart_Quad_Vals(
        sharped_x_vals_pts_A, sharped_x_vals_pts_B, lbdv
    )
    sharped_y_vals_pts_Used = Combine_Chart_Quad_Vals(
        sharped_y_vals_pts_A, sharped_y_vals_pts_B, lbdv
    )
    sharped_z_vals_pts_Used = Combine_Chart_Quad_Vals(
        sharped_z_vals_pts_A, sharped_z_vals_pts_B, lbdv
    )

    # For 1-form constuction
    Sharped_Quad_Vals = np.hstack(
        (sharped_x_vals_pts_Used, sharped_y_vals_pts_Used, sharped_z_vals_pts_Used)
    )
...
def Gen_Curl_1(
    theta_C_A_vals, phi_C_A_vals, theta_C_B_vals, phi_C_B_vals, p_deg, lbdv, Manny
):
    ...
    dPhi_theta_C_A_quad_pt_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
        theta_C_A_SPH_Fn, lbdv, "A"
    )
    dPhi_theta_C_B_quad_pt_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
        theta_C_B_SPH_Fn, lbdv, "A"
    )
    ...
    inv_met_fac_B_pts = np.where(
        lbdv.Chart_of_Quad_Pts > 0, 1.0 / Manny.Metric_Factor_B_pts, 0
    )

    # Combine fields to produce gen_curl_1 in each chart:
    gen_curl_1_A_pts = np.multiply(
        dPhi_theta_C_A_quad_pt_vals - dTheta_phi_C_A_quad_pt_vals, inv_met_fac_A_pts
    )
    gen_curl_1_B_pts = np.multiply(
        dPhi_theta_C_B_quad_pt_vals - dTheta_phi_C_B_quad_pt_vals, inv_met_fac_B_pts
    )

    gen_curl_1_Used_pts = Combine_Chart_Quad_Vals(
        gen_curl_1_A_pts, gen_curl_1_B_pts, lbdv
    )
...
def Integral_on_Manny_Eta_Z(vals_at_quad_pts, Manny, lbdv):  # Orriginal Version
    ...
    Chart_A_integrand = np.multiply(
        np.multiply(met_fac_over_sin_phi_pts_A, eta_Z_Chart_A), vals_at_quad_pts
    )
    Chart_B_integrand = np.multiply(
        np.multiply(met_fac_over_sin_phi_pts_B, one_minus_eta_Z_Chart_B),
        rotated_vals_at_quad_pts,
    )

    Int_On_Manny = sph_f.S2_Integral(Chart_A_integrand, lbdv) + sph_f.S2_Integral(
        Chart_B_integrand, lbdv
    )
...
def Integral_on_Manny(vals_at_quad_pts, Manny, lbdv):  # New Version
    vals_at_quad_pts = vals_at_quad_pts.reshape(
        len(vals_at_quad_pts), 1
    )  # ASSERT THIS IS THE RIGHT SIZE
...
def lebedev_quad_adj_Manny(Manny, lbdv):
    met_fac_over_sin_phi_pts_A = Manny.Metric_Factor_A_over_sin_phi_pts
    met_fac_over_sin_phi_pts_B = Manny.Metric_Factor_B_over_sin_phi_bar_pts
    return Combine_Chart_Quad_Vals(
        met_fac_over_sin_phi_pts_A, met_fac_over_sin_phi_pts_B, lbdv
    )
...
def Lp_Rel_Error_At_Quad_On_Manny(
    ...
    return (Lp_Err_Manny / Lp_f_Manny) ** (1.0 / p)  # ||f_approx - f||_p / || f ||_p
...
def Tangent_Projection(Vectors_at_Quad, lbdv, Manny):
    ...
    for quad_pt in range(num_quad_pts):
        ...
        if lbdv.Chart_of_Quad_Pts[quad_pt] > 0:
            Normal_Vec_pt = np.cross(
                sigma_theta_A_pts[quad_pt, :].flatten(),
                sigma_phi_A_pts[quad_pt, :].flatten(),
            )
        ...
        # Chart B:
            ...
            quad_pt_rot = lbdv.Eval_Rot_Lbdv_Quad_vals(quad_pt)
            Normal_Vec_pt = np.cross(
                sigma_theta_B_pts[quad_pt_rot, :].flatten(),
                sigma_phi_B_pts[quad_pt_rot, :].flatten(),
            )

            dot_theta_B = np.dot(
                sigma_theta_B_pts[quad_pt_rot, :].flatten(), Normal_Vec_pt
            )
        ...
        Vec_pt_Tangent = Vec_pt - Normal_Vec_pt * (
            np.dot(Vec_pt, Normal_Vec_pt) / np.dot(Normal_Vec_pt, Normal_Vec_pt)
        )
...
def Riemann_L2_Inner_Product_One_Form(One_Form_pts_1, One_Form_pts_2, lbdv, manny):
    One_Form_1 = euc_k_form(
        1, lbdv.lbdv_quad_pts, manny.Man_SPH_Deg, manny, One_Form_pts_1
    )
    One_Form_2 = euc_k_form(
        1, lbdv.lbdv_quad_pts, manny.Man_SPH_Deg, manny, One_Form_pts_2
    )
...
def Riemann_L2_Norm_One_Form(One_Form_pts, lbdv, manny):
    return np.sqrt(
        Riemann_L2_Inner_Product_One_Form(One_Form_pts, One_Form_pts, lbdv, manny)
    )
...
def Riemann_L2_Norm_Zero_Form(Zero_Form_pts, lbdv, manny):
    return np.sqrt(
        Integral_on_Manny(np.multiply(Zero_Form_pts, Zero_Form_pts), manny, lbdv)
    )
...
class euc_k_form(object):
    ...
    def linear_comb(self, euc_k_form_2, Const_1, Const_2):
        return euc_k_form(
            ...
            self.P_degree,
            self.Manifold,
            Const_1 * self.quad_pt_array + Const_2 * euc_k_form_2.quad_pt_array,
        )
    ...
    def times_fn(self, fn_vals_at_quad_pts):
        ...
        product_array = np.einsum(
            "i, ij -> ij", np.squeeze(fn_vals_at_quad_pts), self.quad_pt_array
        )

        return euc_k_form(
            self.k_value, self.Q_value, self.P_degree, self.Manifold, product_array
        )
    ...
    def Flat(self, G_deriv, A_inv_deriv, lbdv):
        ...
        for quad_pt in range(self.Q_value):
            ...
            if lbdv.Chart_of_Quad_Pts[quad_pt] > 0:
                ...
                if G_deriv == "No_G":
                    one_form_comps_A_pt = np.linalg.solve(
                        Flat_A_mat, alpha_sharp_X_A_pt
                    )
                    one_form_comps_B_pt = np.linalg.solve(
                        Flat_B_mat, alpha_sharp_X_B_pt
                    )
        ...
        return (
            One_Form_Theta_Comp_A_vals,
            One_Form_Theta_Comp_B_vals,
            One_Form_Phi_Comp_A_vals,
            One_Form_Phi_Comp_B_vals,
        )
    ...
    def Ext_Der(self, lbdv):
        if self.k_value == 0:
            ...
            f_SPH_A, f_SPH_B = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                self.quad_pt_array, self.P_degree, lbdv
            )
            ...
            f_dtheta_A_quad_vals = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0,
                f_SPH_A_dtheta.Eval_SPH_Coef_Mat(quad_pts, lbdv),
                0,
            )
            f_dphi_A_quad_vals = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0,
                f_SPH_A.Eval_SPH_Der_Phi_Coef(quad_pts, lbdv),
                0,
            )

            f_dtheta_B_quad_vals = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0,
                f_SPH_B_dtheta.Eval_SPH_Coef_Mat(quad_pts, lbdv),
                0,
            )
            f_dphi_B_quad_vals = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0,
                f_SPH_B.Eval_SPH_Der_Phi_Coef(quad_pts, lbdv),
                0,
            )
        ...
        if self.k_value == 1:
            ...
            (
                alpha_sharp_x_quad_vals,
                alpha_sharp_y_quad_vals,
                alpha_sharp_z_quad_vals,
            ) = np.hsplit(self.quad_pt_array, 3)

            # Project euclidean comps of alpha_sharp into basis of each chart
            (
                alpha_sharp_x_SPH_A,
                alpha_sharp_x_SPH_B,
            ) = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                alpha_sharp_x_quad_vals, self.P_degree, lbdv
            )
            (
                alpha_sharp_y_SPH_A,
                alpha_sharp_y_SPH_B,
            ) = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                alpha_sharp_y_quad_vals, self.P_degree, lbdv
            )
            (
                alpha_sharp_z_SPH_A,
                alpha_sharp_z_SPH_B,
            ) = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                alpha_sharp_z_quad_vals, self.P_degree, lbdv
            )
            ...
            alpha_sharp_x_B_dphi_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
                alpha_sharp_x_SPH_B, lbdv, "B"
            )

            alpha_sharp_y_A_dphi_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
                alpha_sharp_y_SPH_A, lbdv, "A"
            )
            alpha_sharp_y_B_dphi_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
                alpha_sharp_y_SPH_B, lbdv, "B"
            )

            alpha_sharp_z_A_dphi_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
                alpha_sharp_z_SPH_A, lbdv, "A"
            )
            alpha_sharp_z_B_dphi_vals = Extract_dPhi_Quad_Pt_Vals_From_SPH_Fn(
                alpha_sharp_z_SPH_B, lbdv, "B"
            )

            # Create quad val vector Arrays:
            alpha_sharp_dTheta_A_quad_array = np.hstack(
                (
                    alpha_sharp_x_A_dtheta_vals,
                    alpha_sharp_y_A_dtheta_vals,
                    alpha_sharp_z_A_dtheta_vals,
                )
            )
            alpha_sharp_dPhi_A_quad_array = np.hstack(
                (
                    alpha_sharp_x_A_dphi_vals,
                    alpha_sharp_y_A_dphi_vals,
                    alpha_sharp_z_A_dphi_vals,
                )
            )

            alpha_sharp_dTheta_B_quad_array = np.hstack(
                (
                    alpha_sharp_x_B_dtheta_vals,
                    alpha_sharp_y_B_dtheta_vals,
                    alpha_sharp_z_B_dtheta_vals,
                )
            )
            alpha_sharp_dPhi_B_quad_array = np.hstack(
                (
                    alpha_sharp_x_B_dphi_vals,
                    alpha_sharp_y_B_dphi_vals,
                    alpha_sharp_z_B_dphi_vals,
                )
            )

            alpha_vec_dTheta_A = euc_k_form(
                1,
                self.Q_value,
                self.P_degree,
                self.Manifold,
                alpha_sharp_dTheta_A_quad_array,
            )
            alpha_vec_dPhi_A = euc_k_form(
                1,
                self.Q_value,
                self.P_degree,
                self.Manifold,
                alpha_sharp_dPhi_A_quad_array,
            )

            alpha_vec_dTheta_B = euc_k_form(
                1,
                self.Q_value,
                self.P_degree,
                self.Manifold,
                alpha_sharp_dTheta_B_quad_array,
            )
            alpha_vec_dPhi_B = euc_k_form(
                1,
                self.Q_value,
                self.P_degree,
                self.Manifold,
                alpha_sharp_dPhi_B_quad_array,
            )
            ...
            def Y_mat(q_pt, Chart, deriv):
                ...
                if deriv == "theta":
                    dBasis_Mat_deriv = self.Manifold.dChange_Basis_Mat_theta(
                        q_pt, Chart
                    )
            ...
            d_theta_comp_A_dphi_vals = (
                dG_phi_comps[0]
                + d_Basis_mat_phi_comps[0]
                + d_alpha_sharp_X_phi_A_comps[0]
            )
            d_phi_comp_A_dtheta_vals = (
                dG_theta_comps[2]
                + d_Basis_mat_theta_comps[2]
                + d_alpha_sharp_X_theta_A_comps[2]
            )

            d_theta_comp_B_dphi_vals = (
                dG_phi_comps[1]
                + d_Basis_mat_phi_comps[1]
                + d_alpha_sharp_X_phi_B_comps[1]
            )
            d_phi_comp_B_dtheta_vals = (
                dG_theta_comps[3]
                + d_Basis_mat_theta_comps[3]
                + d_alpha_sharp_X_theta_B_comps[3]
            )
            ...
            # We use quad pts within each chart
            d_alpha_A_Polar_pts = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0, d_alpha_A_Polar_vals, 0
            )
            d_alpha_B_Polar_pts = np.where(
                lbdv.Chart_of_Quad_Pts[quad_pts] > 0, d_alpha_B_Polar_vals, 0
            )
            ...
            d_alpha_Used_dx_dy_pts = Combine_Chart_Quad_Vals(
                d_alpha_dx_dy_A_pts, d_alpha_dx_dy_B_pts, lbdv
            )
            d_alpha_Used_dx_dz_pts = Combine_Chart_Quad_Vals(
                d_alpha_dx_dz_A_pts, d_alpha_dx_dz_B_pts, lbdv
            )
            d_alpha_Used_dy_dz_pts = Combine_Chart_Quad_Vals(
                d_alpha_dy_dz_A_pts, d_alpha_dy_dz_B_pts, lbdv
            )

            # print("d_1: combined into euc solution")

            # Create array of 2-form values
            New_Quad_Pt_Array = np.hstack(
                (d_alpha_Used_dx_dy_pts, d_alpha_Used_dx_dz_pts, d_alpha_Used_dy_dz_pts)
            )

            return euc_k_form(
                2, self.Q_value, self.P_degree, self.Manifold, New_Quad_Pt_Array
            )
    ...
    def Hodge_Star(self, lbdv):
        if self.k_value == 0:
            ...
            # Combine Values from Charts
            star_f_Used_dx_dy_pts = Combine_Chart_Quad_Vals(
                star_f_A_dx_dy_pts, star_f_B_dx_dy_pts, lbdv
            )
            star_f_Used_dx_dz_pts = Combine_Chart_Quad_Vals(
                star_f_A_dx_dz_pts, star_f_B_dx_dz_pts, lbdv
            )
            star_f_Used_dy_dz_pts = Combine_Chart_Quad_Vals(
                star_f_A_dy_dz_pts, star_f_B_dy_dz_pts, lbdv
            )

            # Create array of 2-form values
            New_Quad_Pt_Array = np.hstack(
                (star_f_Used_dx_dy_pts, star_f_Used_dx_dz_pts, star_f_Used_dy_dz_pts)
            )
        ...
        if self.k_value == 1:
            ...
            star_alpha_A_theta_pts = np.multiply(
                -1 * met_fac_A_pts,
                (
                    np.multiply(g_inv_theta_phi_A_pts, alpha_theta_comp_A_vals)
                    + np.multiply(g_inv_phi_phi_A_pts, alpha_phi_comp_A_vals)
                ),
            )

            star_alpha_A_phi_pts = np.multiply(
                met_fac_A_pts,
                (
                    np.multiply(g_inv_theta_theta_A_pts, alpha_theta_comp_A_vals)
                    + np.multiply(g_inv_theta_phi_A_pts, alpha_phi_comp_A_vals)
                ),
            )

            star_alpha_B_theta_pts = np.multiply(
                -1 * met_fac_B_pts,
                (
                    np.multiply(g_inv_theta_phi_B_pts, alpha_theta_comp_B_vals)
                    + np.multiply(g_inv_phi_phi_B_pts, alpha_phi_comp_B_vals)
                ),
            )

            star_alpha_B_phi_pts = np.multiply(
                met_fac_B_pts,
                (
                    np.multiply(g_inv_theta_theta_B_pts, alpha_theta_comp_B_vals)
                    + np.multiply(g_inv_theta_phi_B_pts, alpha_phi_comp_B_vals)
                ),
            )

            return Sharp(
                star_alpha_A_theta_pts,
        ...
        if self.k_value == 2:
            ...
            beta_dx_dy_B_pts, beta_dx_dz_B_pts, beta_dy_dz_B_pts = np.hsplit(
                self.quad_pt_array[quad_pts_inv_rot], 3
            )

            # 2-forms to cartesian conversions:
            (
                dx_dy_to_dtheta_dphi_A_pts,
                dx_dz_to_dtheta_dphi_A_pts,
                dy_dz_to_dtheta_dphi_A_pts,
            ) = Two_Form_Conv_to_Polar_pt(quad_pts, lbdv, self.Manifold, "A")
            (
                dx_dy_to_dtheta_dphi_B_pts,
                dx_dz_to_dtheta_dphi_B_pts,
                dy_dz_to_dtheta_dphi_B_pts,
            ) = Two_Form_Conv_to_Polar_pt(quad_pts, lbdv, self.Manifold, "B")

            # Do Hodge Star:
            beta_dtheta_dphi_A_pts = (
                np.multiply(beta_dx_dy_A_pts, dx_dy_to_dtheta_dphi_A_pts)
                + np.multiply(beta_dy_dz_A_pts, dy_dz_to_dtheta_dphi_A_pts)
                + np.multiply(beta_dx_dz_A_pts, dx_dz_to_dtheta_dphi_A_pts)
            )

            beta_dtheta_dphi_B_pts = (
                np.multiply(beta_dx_dy_B_pts, dx_dy_to_dtheta_dphi_B_pts)
                + np.multiply(beta_dy_dz_B_pts, dy_dz_to_dtheta_dphi_B_pts)
                + np.multiply(beta_dx_dz_B_pts, dx_dz_to_dtheta_dphi_B_pts)
            )

            star_beta_from_A_pts = np.multiply(
                beta_dtheta_dphi_A_pts, inv_met_fac_A_pts
            )
            star_beta_from_B_pts = np.multiply(
                beta_dtheta_dphi_B_pts, inv_met_fac_B_pts
            )

            # Combine Values from Charts
            star_beta_Used_pts = Combine_Chart_Quad_Vals(
                star_beta_from_A_pts, star_beta_from_B_pts, lbdv
            )

            return euc_k_form(
                0, self.Q_value, self.P_degree, self.Manifold, star_beta_Used_pts
            )
    ...
    def Gen_Curl_0(self, lbdv):
        if self.k_value == 0:
            ...
            # Take Hodge-Star of 1-Form:
            neg_star_df_A_theta_pts = (-1) * np.multiply(
                -1 * met_fac_A_pts,
                (
                    np.multiply(g_inv_theta_phi_A_pts, f_dtheta_A_quad_vals)
                    + np.multiply(g_inv_phi_phi_A_pts, f_dphi_A_quad_vals)
                ),
            )

            neg_star_df_A_phi_pts = (-1) * np.multiply(
                met_fac_A_pts,
                (
                    np.multiply(g_inv_theta_theta_A_pts, f_dtheta_A_quad_vals)
                    + np.multiply(g_inv_theta_phi_A_pts, f_dphi_A_quad_vals)
                ),
            )

            neg_star_df_B_theta_pts = (-1) * np.multiply(
                -1 * met_fac_B_pts,
                (
                    np.multiply(g_inv_theta_phi_B_pts, f_dtheta_B_quad_vals)
                    + np.multiply(g_inv_phi_phi_B_pts, f_dphi_B_quad_vals)
                ),
            )

            neg_star_df_B_phi_pts = (-1) * np.multiply(
                met_fac_B_pts,
                (
                    np.multiply(g_inv_theta_theta_B_pts, f_dtheta_B_quad_vals)
                    + np.multiply(g_inv_theta_phi_B_pts, f_dphi_B_quad_vals)
                ),
            )

            return (
                neg_star_df_A_theta_pts,
                neg_star_df_A_phi_pts,
    ...
    def Gen_Curl_to_K_Form(self, lbdv):
        if self.k_value == 0:
            ...
            C0_A_pts = np.multiply(
                np.multiply(f_dphi_A_quad_vals, Sigma_Theta_A_pts)
                - np.multiply(f_dtheta_A_quad_vals, Sigma_Phi_A_pts),
                inv_met_fac_A_pts,
            )
            C0_B_pts = np.multiply(
                np.multiply(f_dphi_B_quad_vals, Sigma_Theta_B_pts)
                - np.multiply(f_dtheta_B_quad_vals, Sigma_Phi_B_pts),
                inv_met_fac_B_pts,
            )
            ...
            return euc_k_form(
                1, self.Q_value, self.P_degree, self.Manifold, C0_Used_pts
            )
    ...
    def Divergence_1_Form(self, lbdv, debug_mode=False):
        if self.k_value == 1:
            ...
            Vec_X_SPH_Fn_A, Vec_X_SPH_Fn_B = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                self.quad_pt_array[:, 0], self.P_degree, lbdv
            )
            ...
            norm_err_Vec_X_A = sph_f.Lp_Rel_Error_At_Quad(
                Vec_X_SPH_Fn_A_quad_pts_from_SPH, self.quad_pt_array[:, 0], lbdv, 2
            )
            norm_err_Vec_X_B = sph_f.Lp_Rel_Error_At_Quad(
                Vec_X_SPH_Fn_B_quad_pts_from_SPH,
                self.quad_pt_array[quad_pts_inv_rot, 0],
                lbdv,
                2,
            )
            if (
                ...
                print(
                    "norm_err_Vec_X_B test failed, norm_err_Vec_X_B = "
                    + str(norm_err_Vec_X_B)
                )
            ...
            Vec_Y_SPH_Fn_A, Vec_Y_SPH_Fn_B = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                self.quad_pt_array[:, 1], self.P_degree, lbdv
            )
            ...
            norm_err_Vec_Y_A = sph_f.Lp_Rel_Error_At_Quad(
                Vec_Y_SPH_Fn_A_quad_pts_from_SPH, self.quad_pt_array[:, 1], lbdv, 2
            )
            norm_err_Vec_Y_B = sph_f.Lp_Rel_Error_At_Quad(
                Vec_Y_SPH_Fn_B_quad_pts_from_SPH,
                self.quad_pt_array[quad_pts_inv_rot, 1],
                lbdv,
                2,
            )
            if (
                ...
                print(
                    "norm_err_Vec_Y_B test failed, norm_err_Vec_Y_B = "
                    + str(norm_err_Vec_Y_B)
                )

            # get Z derivatives at quad pts:
            Vec_Z_SPH_Fn_A, Vec_Z_SPH_Fn_B = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
                self.quad_pt_array[:, 2], self.P_degree, lbdv
            )
            ...
            norm_err_Vec_Z_A = sph_f.Lp_Rel_Error_At_Quad(
                Vec_Z_SPH_Fn_A_quad_pts_from_SPH, self.quad_pt_array[:, 2], lbdv, 2
            )
            norm_err_Vec_Z_B = sph_f.Lp_Rel_Error_At_Quad(
                Vec_Z_SPH_Fn_B_quad_pts_from_SPH,
                self.quad_pt_array[quad_pts_inv_rot, 2],
                lbdv,
                2,
            )
            if (
                ...
                print(
                    "norm_err_Vec_Z_B test failed, norm_err_Vec_Z_B = "
                    + str(norm_err_Vec_Z_B)
                )

            # combine vector dirs
            Vec_dTheta_A_pts = np.hstack(
                (
                    Vec_X_A_dTheta_quad_vals,
                    Vec_Y_A_dTheta_quad_vals,
                    Vec_Z_A_dTheta_quad_vals,
                )
            )
            ...
            Vec_dTheta_B_pts = np.hstack(
                (
                    Vec_X_B_dTheta_quad_vals,
                    Vec_Y_B_dTheta_quad_vals,
                    Vec_Z_B_dTheta_quad_vals,
                )
            )
            ...
            dMet_Fac_dTheta_over_Met_Fac_A_pts = (
                self.Manifold.Metric_Factor_dTheta_over_Metric_Factor_A_pts
            )
            dMet_Fac_dPhi_over_Met_Fac_A_pts = (
                self.Manifold.Metric_Factor_dPhi_over_Metric_Factor_A_pts
            )

            dMet_Fac_dTheta_over_Met_Fac_B_pts = (
                self.Manifold.Metric_Factor_dTheta_over_Metric_Factor_B_pts
            )
            dMet_Fac_dPhi_over_Met_Fac_B_pts = (
                self.Manifold.Metric_Factor_dPhi_over_Metric_Factor_B_pts
            )
            ...
            for quad_pt in range(self.Manifold.num_quad_pts):
                if lbdv.Chart_of_Quad_Pts[quad_pt] > 0:  ### !!!! USE METRIC FACTOR !!!!
                    ...
                    dTheta_Vec_X_local_coors_A_pt = np.dot(
                        A_inv_from_G_pt, dTheta_Vec_X_A_pt
                    )  # np.linalg.solve(A_Mat_pt, dTheta_Vec_X_A_pt)
                    dPhi_Vec_X_local_coors_A_pt = np.dot(
                    ...
                    )  # -1.*np.linalg.solve(A_Mat_pt, A_theta_Mat_pt)
                    neg_A_inv_A_phi = np.dot(
                        A_inv_from_G_pt, A_phi_Mat_pt
                    )  # -1.*np.linalg.solve(A_Mat_pt, A_phi_Mat_pt)

                    V_local_coors_theta_dtheta_Amat_A[quad_pt, 0] = np.dot(
                        neg_A_inv_A_theta, Vec_X_local_coors_A_pt
                    )[0]
                    V_local_coors_phi_dphi_Amat_A[quad_pt, 0] = np.dot(
                        neg_A_inv_A_phi, Vec_X_local_coors_A_pt
                    )[1]
                    ...
                    dTheta_Vec_X_B_pt = Vec_dTheta_B_pts[
                        quad_pt, :
                    ].T  # these are already in B-Coors
                    ...
                    Vec_X_local_coors_B_pt = np.dot(
                        B_inv_from_G_pt, Vec_X_B_pt
                    )  # np.linalg.solve(B_Mat_pt, Vec_X_B_pt)

                    V_local_coors_theta_B[quad_pt, 0] = Vec_X_local_coors_B_pt[0]
                    V_local_coors_phi_B[quad_pt, 0] = Vec_X_local_coors_B_pt[1]

                    dTheta_Vec_X_local_coors_B_pt = np.linalg.solve(
                        B_Mat_pt, dTheta_Vec_X_B_pt
                    )
                    dPhi_Vec_X_local_coors_B_pt = np.linalg.solve(
                    ...
                    V_local_coors_theta_dtheta_Bmat_B[quad_pt, 0] = np.dot(
                        neg_B_inv_B_theta, Vec_X_local_coors_B_pt
                    )[0]
                    V_local_coors_phi_dphi_Bmat_B[quad_pt, 0] = np.dot(
                        neg_B_inv_B_phi, Vec_X_local_coors_B_pt
                    )[1]

            div_V_A_pts = (
                dMet_Fac_dTheta_over_Met_Fac_A_pts * V_local_coors_theta_A
                + dMet_Fac_dPhi_over_Met_Fac_A_pts * V_local_coors_phi_A
                + V_local_coors_theta_dtheta_Amat_A
                + V_local_coors_phi_dphi_Amat_A
                + V_local_coors_theta_dtheta_V_A
                + V_local_coors_phi_dphi_V_A
            )

            div_V_B_pts = (
                dMet_Fac_dTheta_over_Met_Fac_B_pts * V_local_coors_theta_B
                + dMet_Fac_dPhi_over_Met_Fac_B_pts * V_local_coors_phi_B
                + V_local_coors_theta_dtheta_Bmat_B
                + V_local_coors_phi_dphi_Bmat_B
            ...
            Debug_Dict["dTheta_V_local_coors_theta_A"] = (
                V_local_coors_theta_dtheta_Amat_A + V_local_coors_theta_dtheta_V_A
            )
            Debug_Dict["dphi_V_local_coors_phi_A"] = (
                V_local_coors_phi_dphi_Amat_A + V_local_coors_phi_dphi_V_A
            )
            Debug_Dict["dTheta_V_local_coors_theta_B"] = (
                V_local_coors_theta_dtheta_Bmat_B + V_local_coors_theta_dtheta_V_B
            )
            Debug_Dict["dphi_V_local_coors_phi_B"] = (
                V_local_coors_phi_dphi_Bmat_B + V_local_coors_phi_dphi_V_B
            )

            Debug_Dict["der_met_fac_comps_A_pts"] = (
                dMet_Fac_dTheta_over_Met_Fac_A_pts * V_local_coors_theta_A
                + dMet_Fac_dPhi_over_Met_Fac_A_pts * V_local_coors_phi_A
            )
            Debug_Dict["der_met_fac_comps_B_pts"] = (
                dMet_Fac_dTheta_over_Met_Fac_B_pts * V_local_coors_theta_B
                + dMet_Fac_dPhi_over_Met_Fac_B_pts * V_local_coors_phi_B
            )
            ...
            if debug_mode is True:
                ...
                return euc_k_form(
                    0, self.Q_value, self.P_degree, self.Manifold, div_V_quad_pts
                )
    ...
    def Explicit_LB(self, lbdv):
        Y_mn_SPH_Fn_A, Y_mn_SPH_Fn_B = sph_f.Proj_Into_SPH_Charts_At_Quad_Pts(
            self.quad_pt_array, self.P_degree, lbdv
        )
        ...
        LB_Basis_Fn_A_pts = (
            ...
            * (
                self.Manifold.G_over_Metric_Factor_A_pts
                * Y_mn_A_dTheta_dTheta_quad_vals
                - 2
                * self.Manifold.F_over_Metric_Factor_A_pts
                * Y_mn_A_dTheta_dPhi_quad_vals
                + self.Manifold.E_over_Metric_Factor_A_pts * Y_mn_A_dPhi_dPhi_quad_vals
                + self.Manifold.G_over_Metric_Factor_dTheta_A * Y_mn_A_dTheta_quad_vals
                - self.Manifold.F_over_Metric_Factor_dTheta_A * Y_mn_A_dPhi_quad_vals
                - self.Manifold.F_over_Metric_Factor_dPhi_A * Y_mn_A_dTheta_quad_vals
                + self.Manifold.E_over_Metric_Factor_dPhi_A * Y_mn_A_dPhi_quad_vals
            )
        )
        ...
        LB_Basis_Fn_B_pts = (
            ...
            * (
                self.Manifold.G_over_Metric_Factor_B_pts
                * Y_mn_B_dTheta_dTheta_quad_vals
                - 2
                * self.Manifold.F_over_Metric_Factor_B_pts
                * Y_mn_B_dTheta_dPhi_quad_vals
                + self.Manifold.E_over_Metric_Factor_B_pts * Y_mn_B_dPhi_dPhi_quad_vals
                + self.Manifold.G_over_Metric_Factor_dTheta_B * Y_mn_B_dTheta_quad_vals
                - self.Manifold.F_over_Metric_Factor_dTheta_B * Y_mn_B_dPhi_quad_vals
                - self.Manifold.F_over_Metric_Factor_dPhi_B * Y_mn_B_dTheta_quad_vals
                + self.Manifold.E_over_Metric_Factor_dPhi_B * Y_mn_B_dPhi_quad_vals
            )
        )

        LB_quad_pts = Combine_Chart_Quad_Vals(
            LB_Basis_Fn_A_pts, LB_Basis_Fn_B_pts, lbdv
        )
    ...
    def LB_Zero_Form_From_Curl(self, lbdv):
        if self.k_value != 0:
            ...
            # (-1)*LB = [(-*d)_1][(-*d)_0]:
            (
                gen_curl0_f_A_theta_pts,
                gen_curl0_f_A_phi_pts,
                gen_curl0_f_B_theta_pts,
                gen_curl0_f_B_phi_pts,
            ) = self.Gen_Curl_0(lbdv)
            neg_LB_f_0_Form = Gen_Curl_1(
                gen_curl0_f_A_theta_pts,
                gen_curl0_f_A_phi_pts,
                gen_curl0_f_B_theta_pts,
                gen_curl0_f_B_phi_pts,
    ...
    def Riemann_Self_Inner_Prouct_Two_Form(self, lbdv):
        ...
        inv_mf_beta_sq_from_A_pts = (
            np.multiply(beta_dtheta_dphi_A_pts, inv_met_fac_A_pts) ** 2
        )
        inv_mf_beta_sq_from_B_pts = (
            np.multiply(beta_dtheta_dphi_B_pts, inv_met_fac_B_pts) ** 2
        )

        # Combine Values from Charts
        return Combine_Chart_Quad_Vals(
            inv_mf_beta_sq_from_A_pts, inv_mf_beta_sq_from_B_pts, lbdv
        )
        ...
        if self.k_value != 1:
            ...
            for quad_pt in range(self.Q_value):
                ...
                if lbdv.Chart_of_Quad_Pts[quad_pt] > 0:
                    ...
                    push_forward_vec_A_pt = (
                        alpha_sharp_theta_A_pt * pf_sigma_theta_A_pt
                        + alpha_sharp_phi_A_pt * pf_sigma_phi_A_pt
                    )
                    ...
                    push_forward_vec_B_pt = (
                        alpha_sharp_theta_B_pt * pf_sigma_theta_B_pt
                        + alpha_sharp_phi_B_pt * pf_sigma_phi_B_pt
                    )
            ...
            return euc_k_form(
                1, self.Q_value, self.P_degree, Manny_2, push_forward_quad_vals
            )
