...
def cartesian_to_elliptical(
    ...
    for pt_numb in range(n_points):
        y_tilde_pt = np.linalg.solve(
            R_inverse, points[pt_numb, :].reshape(3, 1) - center.reshape(3, 1)
        )
        ...
        U_pt = np.arctan2(yt_1 * lengths[0], yt_0 * lengths[1])
        ...
        V_pt = np.arctan2(cylinder_r * lengths[2], yt_2 * cyl_r_exp)
