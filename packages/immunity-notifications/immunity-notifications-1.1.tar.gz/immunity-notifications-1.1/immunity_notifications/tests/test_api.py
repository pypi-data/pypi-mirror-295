...
Organization = swapper_load_model('immunity_users', 'Organization')
...
class TestNotificationApi(
    ...
    @patch('immunity_notifications.tasks.delete_ignore_object_notification.apply_async')
    ...
    def test_list_ignore_obj_notification_api(self, mocked_task):
        ...
        content_type = ContentType.objects.filter(
            app_label='immunity_users', model='user'
        ).first()
    ...
    def test_deleted_notification_type(self, *args):
        notify.send(sender=self.admin, type='default', target=self.admin)
        with patch('immunity_notifications.types.NOTIFICATION_TYPES', {}):
            url = self._get_path('notifications_list')
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(len(response.data['results']), 0)
