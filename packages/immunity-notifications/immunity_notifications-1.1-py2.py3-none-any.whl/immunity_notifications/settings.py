...
HOST = getattr(settings, 'IMMUNITY_NOTIFICATIONS_HOST', None)

CACHE_TIMEOUT = getattr(
    settings, 'IMMUNITY_NOTIFICATIONS_CACHE_TIMEOUT', 2 * 24 * 60 * 60
)
...
POPULATE_PREFERENCES_ON_MIGRATE = getattr(
    settings, 'IMMUNITY_NOTIFICATIONS_POPULATE_PREFERENCES_ON_MIGRATE', True
)
NOTIFICATION_STORM_PREVENTION = getattr(
    settings,
    'IMMUNITY_NOTIFICATIONS_NOTIFICATION_STORM_PREVENTION',
    {
...
def get_config():
    user_config = getattr(settings, 'IMMUNITY_NOTIFICATIONS_CONFIG', {})
