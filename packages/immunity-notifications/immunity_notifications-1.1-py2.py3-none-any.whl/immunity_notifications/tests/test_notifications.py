...
from immunity_notifications import tasks
from immunity_notifications.exceptions import NotificationRenderException
...
from immunity_notifications.tests.test_helpers import (
...
from immunity_notifications.utils import _get_absolute_url
...
Group = swapper_load_model('immunity_users', 'Group')
...
class TestNotifications(TestOrganizationMixin, TransactionTestCase):
    ...
    def test_generic_notification_type(self):
        ...
        expected_output = (
            ...
            user_path=reverse('admin:immunity_users_user_change', args=[self.admin.pk])
        )
    ...
    def test_notification_type_message_template(self):
        ...
        with self.subTest('Registering type with message template'):
            message_template.update(
                {'message_template': 'immunity_notifications/default_message.md'}
            )
            register_notification_type('message_template', message_template)
        ...
        with self.subTest('Links in notification message'):
            url = _get_absolute_url(
                reverse('admin:immunity_users_user_change', args=(self.admin.pk,))
            )
            message = (
    ...
    def test_notification_type_related_object_url(self):
        ...
        with self.subTest('Test related object callable URL'):
            # Update the notification type configuration
            unregister_notification_type('test_type')
            url_generator = 'immunity_notifications.tests.test_helpers.notification_related_object_url'
    ...
    @capture_any_output()
    @mock_notification_types
    @patch('immunity_notifications.tasks.delete_notification.delay')
    ...
    @patch(
        'immunity_notifications.tasks.ns_register_unregister_notification_type.delay',
    ...
    @patch('immunity_notifications.types.NOTIFICATION_ASSOCIATED_MODELS', set())
    @patch('immunity_notifications.tasks.delete_obsolete_objects.delay')
