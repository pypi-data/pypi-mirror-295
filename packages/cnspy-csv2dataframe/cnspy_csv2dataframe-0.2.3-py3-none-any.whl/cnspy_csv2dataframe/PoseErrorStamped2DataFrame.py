...
class PoseErrorStamped2DataFrame(CSV2DataFrame):
    ...
    def from_DataFrame(data_frame):
        ...
        if version_info[0] < 3:
            ...
            nu_vec = data_frame.as_matrix(['nu_x', 'nu_y', 'nu_z'])
            theta_vec = data_frame.as_matrix(['theta_x', 'theta_y', 'theta_z'])
            ...
            nu_vec = data_frame[['nu_x', 'nu_y', 'nu_z']].to_numpy()
            theta_vec = data_frame[['theta_x', 'theta_y', 'theta_z']].to_numpy()
            est_err_type_vec = data_frame[['est_err_type']].to_numpy()
            err_rep_vec = data_frame[['err_representation']].to_numpy()

        return t_vec, nu_vec, theta_vec, est_err_type_vec, err_rep_vec

    @staticmethod
    def to_DataFrame(t_vec, nu_vec, theta_vec, est_err_type_vec, err_rep_vec):
        t_rows, t_cols = t_vec.shape  # does not work in Python 3
        assert (len(est_err_type_vec) == t_rows)
        assert (len(err_rep_vec) == t_rows)
        p_rows, p_cols = nu_vec.shape
        q_rows, q_cols = theta_vec.shape
        ...
        df1 = pandas.DataFrame(
            {'t': t_vec[:, 0], 'nu_x': nu_vec[:, 0], 'nu_y': nu_vec[:, 1], 'nu_z': nu_vec[:, 2],
             'theta_x': theta_vec[:, 0], 'theta_y': theta_vec[:, 1], 'theta_z': theta_vec[:, 2]})
