...
"""
Library Copied From: https://github.com/dldevinc/jinja2-simple-tags
"""
...
class BaseTemplateTag(Extension):
    ...
    def render_wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            message = str(e).replace('render()', '').strip()
            raise Exception("TAG: {% " + self.tag_name + " %} " + message)
...
class StandaloneTag(BaseTemplateTag, ABC):
    def output(self, parser, block_call, target, tag_name, lineno):
        if target:
...
class ContainerTag(BaseTemplateTag, ABC):
    def output(self, parser, block_call, target, tag_name, lineno):
