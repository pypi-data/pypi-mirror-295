...
def get_domain_type(fact: AbstractFact, pretrained_types: dict[str, str] | None, nerc_pretrained: bool = True) -> AbstractDomainType:
    """
    ...
    if isinstance(fact, MentionFact):
        return get_domain_type(fact.value, pretrained_types)
...
def build_domain(gold_docs: tuple[TalismanDocument, ...], pretrained_types: dict[str, str] | None, nerc_pretrained: bool = True) -> Domain:
    """
    ...
    """
    return Domain(
        get_domain_type(fact, pretrained_types, nerc_pretrained) for fact in chain.from_iterable(doc.get_facts() for doc in gold_docs)
    )


def replace_with_domain(doc: TalismanDocument, domain: Domain, pretrained_types: dict[str, str] | None, nerc_pretrained: bool = True):
    """
    ...
    def _replace_with_domain(fact: AbstractFact):
        if isinstance(fact, Union[PropertyFact | RelationFact]):
            fact = replace(fact, type_id=get_domain_type(fact, pretrained_types, nerc_pretrained))
