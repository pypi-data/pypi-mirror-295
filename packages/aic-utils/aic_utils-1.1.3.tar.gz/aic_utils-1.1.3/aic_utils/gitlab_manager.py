...
class GitLabManager:
    def __init__(self, aic_instance, gitlab_token, gitlab_namespace=None, gitlab_base_url='https://git.autodatacorp.org/api/v4', use_hash_comparison=True):
        self.aic = aic_instance
        self.gitlab_base_url = gitlab_base_url
        if not gitlab_namespace:
            self.gitlab_namespace = 'pin/pin-analytics/pin-fusion-2.0/pipelines'
        else:
    ...
    def push_file_to_repo(self, repo_name, file_name, file_content):
        ...
        repo_path = f"{self.subgroup_id}/{repo_name.replace(' ', '_')}"  # Adjust repo path based on subgroup
    ...
    def ensure_correct_subgroup(self, workspace):
        """Ensure the correct subgroup path based on the workspace."""
