    ...
    async def score_agent_async(
        ...
        async def evaluate_question(question):
            ...
            try:
                ...
                print(f"Error: {e}")
                return False, 0, agent
            return success, dollars, agent

        batch_size = 10
        successes_with_dollars = []
        for i in range(0, len(questions), batch_size):
            batch = questions[i:i+batch_size]
            if min(batch_size, len(questions)) >= 5:
                batch_results = await tqdm_asyncio.gather(
                    *[evaluate_question(q) for q in batch],
                    desc=f"Batch {i//batch_size + 1}/{math.ceil(len(questions)/batch_size)}"
                )
            else:
                batch_results = await asyncio.gather(*[evaluate_question(q) for q in batch])
            successes_with_dollars.extend(batch_results)

        successes = [s for s, d in successes_with_dollars]
        dollars = sum([d for s, d in successes_with_dollars])
        return sum(successes) / len(successes), dollars, agent
