...
def score_agent_sync(
    ...
    t0 = time.time()
    agent_performance, agent_cost, _ = agent_benchmark.score_agent_sync(agent, split="train", indices=indices, verbose=False, use_persistent_container=False)
...
async def score_agent_async(
    ...
    t0 = time.time()
    agent_performance, agent_cost, _ = await agent_benchmark.score_agent_async(agent, split="train", indices=indices, verbose=False)
    t1 = time.time()
    print(f"Time taken: {t1-t0} seconds")
    print(f"Score for {model_name}: " + str(agent_performance))
    print(f"Cost for {model_name}: " + str(agent_cost))
...
if __name__ == "__main__":
    ...
    #score_agent_sync(model_name="deepseek-chat",indices=[i for i in range(0,50)], contexts_for_agent=contexts)
    asyncio.run(score_agent_async(model_name="claude-3-5-sonnet-20240620",indices=[i for i in range(0,50)], contexts_for_agent=contexts))
