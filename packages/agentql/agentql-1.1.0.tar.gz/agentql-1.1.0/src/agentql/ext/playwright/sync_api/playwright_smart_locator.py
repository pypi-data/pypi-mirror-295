...
def _goto(
    ...
    *,
    ...
    return self._original_goto(url=url, timeout=timeout, wait_until=wait_until, referer=referer)
...
def _wait_for_page_ready_state(self, wait_for_network_idle: bool = DEFAULT_WAIT_FOR_NETWORK_IDLE):
    ...
    add_event_listeners_for_page_monitor_shared(self, self._page_monitor)
    ...
    # Remove the event listeners to prevent overwhelming the event loop
    remove_event_listeners_for_page_monitor_shared(self, self._page_monitor)
