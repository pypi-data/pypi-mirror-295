...
def _get_edge_color(s, t, g):
    if g.get_class_value_by_node(s) == g.get_class_value_by_node(t):
        if g.get_class_value_by_node(s) == const.MINORITY_VALUE:
...
def _plot_graph(g, ax, pos, **kwargs):
    ...
    nodes, node_colors = zip(
        *[(node, viz.COLOR_MAJORITY if data[g.graph['class_attribute']] == maj else viz.COLOR_MINORITY)
          for node, data in g.node_list(data=True)])
...
def plot_samples(originals: List[netin.Graph], samples: List[List[netin.Graph]], ignore_singletons: bool = False, fn: str = None, seed=None, **kwargs):
    ...
    for col, g in enumerate(originals):
        ...
        if ignore_singletons:
            g = g.copy()
            to_remove = [n for n in g.node_list() if g.degree(n) == 0]
            if len(to_remove) > 0:
        ...
        for row, sample_set in enumerate(samples):    
            ...
            if ignore_singletons:
                s = s.copy()
                to_remove = [n for n in s.node_list() if s.degree(n) == 0]
                if len(to_remove) > 0:
