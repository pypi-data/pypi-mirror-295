...
class GeneticAlgorithmSampler(BaseSampler):
    ...
    def __init__(self,
        ...
        if self.ligand.cnfrs_ is not None:
            ...
            for i in range(3):
                ...
                _range = [self.box_center[i] - 10.0,
                          self.box_center[i] + 10.0]

                _range_init = [self.box_center[i] - 5.0,
                          self.box_center[i] + 5.0]
                xyz_ranges.append(_range)
                xyz_ranges_init.append(_range_init)
            ...
            #for crossdocking
            self.bound += [[-1 * np.pi, np.pi] for x in range(3)]
            self.bound += [[-1 * np.pi, 1*np.pi] for x in range(self.n_var - 6)]
            ...
            self.bound_init = xyz_ranges_init
            # self.bound_init = xyz_ranges
            # self.bound += [[-1.0, 1.0] for x in range(self.n_var - 3)]
            # for crossdocking
            self.bound_init += [[-1 * np.pi, np.pi] for x in range(3)]
            self.bound_init += [[-1 * np.pi, 1 * np.pi] for x in range(self.n_var - 6)]
        else:
        ...
        for i in range(self.n_pop - 1):
            ...
            def make_chrom():
                ...
                cnfrs_,_= self._variables2cnfrs(np.array([random.uniform(self.bound_init[i][0], self.bound_init[i][1]) for i in range(len(self.bound_init))]))
                #print("cnfrs_",cnfrs_)
                init_variables = self._cnfrs2variables(cnfrs_,self.receptor.cnfrs_)
                init_chrom = list(self.encode2chrom(init_variables))
                _chrom = [x for x in init_chrom]
    ...
    def sampling(self, n_gen=None, verbose=True, output=None):
        ...
        for sn_gen in range(n_gen):
            ...
            for sn_chrom, chrom in enumerate(self.chrom_pop2):
                ...
                #print("muate before lcnfrs_", lcnfrs_)

                lcnfrs_, rcnfrs_ = self._mutate(lcnfrs_, rcnfrs_,
                                                5.0, 0.1,
                                                minimize=True)

                #print("muate later lcnfrs_", lcnfrs_)
    ...
    def crossover_type0(self, p1, p2, p_c=None):
        """
