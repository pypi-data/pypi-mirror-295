...
class Cochleagram(TimeHistory):
    """
    This class generates a time history using the gamma tone filter bank to represent the frequency conversion. These
    filters are defined for the common frequencies and shapes for human hearing.
    """

    def __init__(self, a: Waveform, frames: FrameBuilder = None, integration_time: float = 0.25):
        """
        This constructs the object and uses the information from the arguments to define what will be sent through
        the filter bank in the calculation of the cochleagram.
        :param a: The audio waveform
        :type a: Waveform
        :param frames: The method to segregate the waveform in to the elements that will be processed by the filter bank
        :type frames: FrankBuilder
        :param integration_time: If the FrameBuilder is not specified, this parameter will define the methods to
        divide the input waveform into different subsets for the processing of the time-frequency transform.
    ...
    def _calculate_spectrogram(self):
        """
        This function constructs the cochleagram filter bank and divides the waveform into the elements that are used
        for the temporal segregation of the waveform for the cochleagram.
        :return:
        :rtype:
        """
