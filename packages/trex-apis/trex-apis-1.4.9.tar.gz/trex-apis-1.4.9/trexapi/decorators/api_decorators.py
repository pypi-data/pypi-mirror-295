...
def user_auth_token_required_and_check_duplicated_session(check_duplicated_session=True):
    ...
    def decorator(f):
        return _user_auth_token_required_decorated_function(f, check_duplicated_session=check_duplicated_session)
        '''
        @wraps(f)
        def decorated_function(*args, **kwargs):
...
def _user_auth_token_required_decorated_function(f, check_duplicated_session=False):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        user_auth_token     = request.headers.get('x-auth-token')
        user_reference_code = request.headers.get('x-reference-code')
...
def user_auth_token_required_pass_reference_code(f):
    return _user_auth_token_required_decorated_function(f, check_duplicated_session=False)
