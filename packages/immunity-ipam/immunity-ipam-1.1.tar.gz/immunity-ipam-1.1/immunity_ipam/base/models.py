...
class AbstractSubnet(ShareableOrgMixin, TimeStampedEditableModel):
    ...
    def request_ip(self, options=None):
        ...
        if not ip:
            return None
        ip_address = load_model('immunity_ipam', 'IpAddress')(
    ...
    def _read_subnet_data(self, reader):
        subnet_model = load_model('immunity_ipam', 'Subnet')
    ...
    def _read_ipaddress_data(self, reader, subnet):
        ipaddress_model = load_model('immunity_ipam', 'IpAddress')
        ipaddress_list = []
        for row in reader:
    ...
    def export_csv(self, subnet_id, writer):
        ipaddress_model = load_model('immunity_ipam', 'IpAddress')
        subnet = load_model('immunity_ipam', 'Subnet').objects.get(pk=subnet_id)
...
class AbstractIpAddress(TimeStampedEditableModel):
    subnet = models.ForeignKey(
        get_model_name('immunity_ipam', 'Subnet'), on_delete=models.CASCADE
    )
    ...
    def clean(self):
        ...
        addresses = (
            load_model('immunity_ipam', 'IpAddress')
