...
from .environment_parameters import BaseAccount, DefaultAccount, EnvironmentParameters
...
class ConfigProvider(Generic[TSettings, TAccount]):
    ...
    def __new__(cls, environments_parameters: dict, settings: TSettings, account: TAccount = DefaultAccount, relative_path: str = '../'):
        if cls._instance is None:
            cls._instance = super(ConfigProvider, cls).__new__(cls)
            cls._instance.__init_once(environments_parameters, settings, account, relative_path)
        return cls._instance

    def __init_once(self, environments_parameters: dict, settings: TSettings, account:TAccount, relative_path: str):

        env = EnvironmentParameters[account].model_validate(environments_parameters)
        ...
        self.envs_params = environments_parameters
        self.env_params: TAccount = getattr(env.accounts, self.env_lower)  # Fetching the current environment
