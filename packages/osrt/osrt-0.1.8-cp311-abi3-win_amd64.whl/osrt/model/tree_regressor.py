...
class TreeRegressor:
    ...
    def __initialize_training_loss__(self, X, y, metric, weights):
        """
        ...
        for i in range(n):
            ...
            label = y.values[i, -1]
            if metric == "L1":
                node["loss"] += weights[i] * abs(label - node["prediction"])
            elif metric == "L2":
                node["loss"] += weights[i] * (label - node["prediction"])**2
            else:
    ...
    def score(self, X, y, metric="L2", weight=None):
        """
        Parameters
        --- 
        X : matrix-like, shape = [n_samples by m_features]
        ...
        if metric == "L2":
            return mean_squared_error(y, y_hat, sample_weight=weight)
        elif metric == "L1":
            return mean_absolute_error(y, y_hat, sample_weight=weight)
        else:
            print(f'Metric {metric} is not supported.')
            return 0
                ...
                for group in self.__groups__(subtree):
                    ...
                    if node["relation"] == "==":
                        ...
                        if condition_result == "true":
                            ...
                            raise "OptimalSparseRegressionTree: Malformatted source {}".format(node)
                    elif node["relation"] == ">=":
                        rule["type"] = "Numerical"
                        if "max" not in rule:
                            rule["max"] = float("INF")
