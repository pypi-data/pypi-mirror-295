...
#
def create_table(schema, layer, table_name, yml=None, partition_fields=[], anonimized_fields=[], comment={}, comment_squad={}, field_comments={}):
    log(__name__).info(f"Running create_table method for {layer}.{table_name}")
    if layer != "bronze":
        ct = CreateTableSilver(schema, table_name, yml, partition_fields, anonimized_fields, comment, comment_squad, field_comments).execute()        
        CreateView(schema, layer, table_name, ct.yml, ct.tbl_comment.parsed_comment, anonimized_fields, field_comments).execute()
    else:
        CreateTableBronze(schema, table_name, yml, partition_fields).execute()
...
#       yml[yml object, string or None] = yml file path (it is not required to create a bronze table). Default is none.
#
def create_table_bronze(schema, table_name, yml=None, partition_fields=[]):
...
#       yml[yml object, string or None] = yml file path (it is not required to create a bronze table). Default is none.
#
def create_table_silver(schema, table_name, yml=None, partition_fields=[], anonimized_fields=[], comment={}, comment_squad={}, field_comments=[]):
    log(__name__).info(f"Running create_table_silver method for silver.{table_name}")
    ct = CreateTableSilver(schema, table_name, yml, partition_fields, anonimized_fields, comment, comment_squad, field_comments).execute()    
    CreateView(schema, "silver", table_name, ct.yml, ct.tbl_comment.parsed_comment, anonimized_fields, field_comments).execute()
