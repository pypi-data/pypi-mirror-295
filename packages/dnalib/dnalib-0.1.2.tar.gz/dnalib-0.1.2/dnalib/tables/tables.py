...
from dnalib.writer import WriteModes, TableWriter
...
class LayerTable:    
    """ 
        ...
        Attributes:
            ...
            writer (TableWriter): intância interna do TableWriter para persistir o dataframe target_df.
    ...
    def __init__(self, layer, table_name, source_df=None, include_checksum=False):                
        ...
        self.writer = TableWriter(self.table_name, layer)
        self.target_df = None      
    ...
    def persist(self, mode=WriteModes.OVERWRITE, partition_fields=[], optimize=True, source_df_name="source", update_df_name="update", merge_condition=None):             
        """ 
            Método wrapper do TableWriter.persist, automaticamente detecta se no dataframe foi adicionado o checksum e preeenche o campo has_checksum_field. 
        """
