...
class GirderPlugin(plugin.GirderPlugin):
    ...
    def get_folder_path(self, folder_id):
        def _get_path(folder):
            if not folder:
                return []
            if folder['parentCollection'] == 'user':
                return [folder['name']]
            parent = Folder().load(folder['parentId'], force=True)
            return _get_path(parent) + [folder['name']]
    ...
    def _extract_zip(self, event):
        ...
        try:
            ...
            if creator is None:
                ...
                return
            print(f"ZIP EXTRACTOR: zip file is saved here:\n {parent_folder}\n\n")
            print(f"ZIP EXTRACTOR: Path to the folder is: {self.get_folder_path(parent_folder['_id'])}")
            
            with tempfile.TemporaryDirectory() as tmpdirname:
                print(f"ZIP EXTRACTOR: Created temporary directory: {tmpdirname}")
                with zipfile.ZipFile(file_path, 'r') as zip_ref:
                    zip_ref.extractall(self.get_folder_path(parent_folder['_id']))
                    print(f"ZIP EXTRACTOR: Extracted zip contents to: {tmpdirname}")
                    # print(f"ZIP EXTRACTOR: Contents of tmp_dir: {os.listdir(tmpdirname)}")
            ...
            #     print(f"ZIP EXTRACTOR: Starting to process extracted files")
            #     for root, dirs, files in os.walk(tmpdirname):
            #         print(f"ZIP EXTRACTOR: Processing directory: {root}")
            #         print(f"ZIP EXTRACTOR: Files in this directory: {files}")
            #         for name in files:
            #             print(f"ZIP EXTRACTOR: Processing file: {name}")
            #             if name.startswith('__MACOSX') or name == '.DS_Store':
            #                 print(f"ZIP EXTRACTOR: Skipping file: {name}")
            #                 continue
            ...
            #                         print(f"ZIP EXTRACTOR: Parent folder path: {part} in {self.get_folder_path(current_folder['_id'])}")
            ...
            #                         parent=item,
            #                         user=creator,
            #                         # assetstore=file['assetstoreId'] if 'assetstoreId' in file else None
            #                     )
            ...
            #     print(f"ZIP EXTRACTOR: Finished processing all extracted files")
