...
def pytest_sessionstart(session):
    logger("DEBUG")
    session.config.option.last_json_report_path = f"{session.fspath}/report/json/last_test_result.json"
...
def pytest_report_teststatus(report, config):
    ...
    case_py = os.path.basename(report.nodeid.split('::')[0].replace(".py", ""))
    ...
    if not data["test_cases"]:
        data["test_cases"].append(py_case_log_text)
    else:
        for item in data["test_cases"]:
            if case_py == item["case_py"]:
                break
        else:
            data["test_cases"].append(py_case_log_text)
    ...
    for item in data["test_cases"]:
        if case_py == item["case_py"]:
            if not item["case_fun_set"]:
                item["case_fun_set"][case_fun] = {}
                break
            else:
                if case_fun in item["case_fun_set"]:
                    break
                else:
                    item["case_fun_set"][case_fun] = {}
    for item in data["test_cases"]:
        if case_py == item["case_py"]:
            item["case_fun_set"][case_fun][case_when] = case_outcome
            if case_when == "setup" and case_outcome == "skipped":
                item["case_fun_set"][case_fun]["call"] = "skipped"
            elif case_when == "setup" and case_outcome == "failed":
                item["case_fun_set"][case_fun]["call"] = "failed"

            if case_when == "teardown":
                setup_result = item["case_fun_set"][case_fun]["setup"]
                call_result = item["case_fun_set"][case_fun]["call"]
                teardown_result = item["case_fun_set"][case_fun]["teardown"]
                ...
                if teardown_result == "failed":
                    if call_result == "passed":
                        fun_outcome = "passed"
                item["case_fun_set"][case_fun]["fun_outcome"] = fun_outcome
            break
...
def pytest_collection_modifyitems(session, config, items):
    ...
    if has_interrupt_continue:
        ...
        try:
            fun_list = data["test_cases"][-1]["case_fun_set"]
            last_case = list(fun_list.keys())[-1]
        except IndexError:
            last_case = None
        if last_case:
...
def pytest_sessionfinish(session):
    ...
    for case_py in data["test_cases"]:
        ...
        if "failed" not in outcomes and "skipped" in outcomes:
            case_py["py_outcome"] = "skipped"
        if len(set(outcomes)) == 1 and "passed" in set(outcomes):
            case_py["py_outcome"] = "passed"

    py_test_results = [py_result["py_outcome"] for py_result in data["test_cases"]]
    fun_test_results = [fun_result["fun_outcome"] for py in data["test_cases"] for fun_result in
