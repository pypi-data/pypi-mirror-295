...
class Remote:
    ...
    def install_client_env(self, user, _ip, password):
        ...
        if return_code != 0:
            remote.remote_sudo_run('apt update', timeout=600)
            remote.remote_sudo_run('apt install python3-pip -y', timeout=120)
            remote.remote_run(f'pip3 install -U pip -i {setting.PYPI_MIRROR} --break-system-packages')
        remote.remote_run(f"pip3 install -U youqu3 -i {setting.PYPI_MIRROR} --break-system-packages")
        stdout, return_code = remote.remote_run(
