...
class SplintObjectsWithDeletedManager(models.Manager):
    ...
    def get_queryset(self, *args, **kwargs):
        return SplintQuerySet(model=self.model, using=self._db, hints=self._hints)
...
class SplintModel(models.Model):
    ...
    _deleted_at = models.DateTimeField("Data de deleção", null=True, blank=True)
        ...
        if log_activity:
            log = {
                ...
                "object": model_to_dict(
                    self, exclude=getattr(self, "exclude_log", None)
                ),
            }
