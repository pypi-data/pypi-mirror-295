...
class Client:
    def __init__(self, table_name: str, index_name:str, extension: str|None=None, credentials: dict|None=None,
    ...
    def read_entry(self, index: tuple = (None, None), column: str = '*', indices: list = None, page: int = 1, page_size: int = 10):
        """
        ...
        if indices:
            for col, val in indices:
                if filter_expression:
                    filter_expression &= Attr(col).eq(val)
                else:
                    filter_expression = Attr(col).eq(val)
        else:
            if index[0] and index[1]:
                filter_expression = Attr(index[0]).eq(index[1])
