...
class Shell:
    def __init__(
        self, executable: str = "", wait_time: int = 0.01, *, track_env: bool = False
    ) -> None:
        if not executable:
            executable = os.environ.get("SHELL", "bash")
    ...
    def _send(self, cmd: str, *, silent: bool = True) -> Result:
        uuid = str(uuid4())
        std_in = cmd
        for mode in VALID_MODES:
            if mode in cmd:
                ...
                warn(
                    ...
                    "`modes` argument of the `send`/`send_all` call",
                    stacklevel=1,
                )
    ...
    def _send_error_mode(self, cmds: list[str], *, silent: bool = True) -> list[Result]:
        results = []
        for cmd in cmds:
            result = self._send(cmd, silent=silent)
            results.append(result)
            if result.return_code != 0:
                break
        return results

    def send(self, *cmds: str, modes: list[Mode] | None = None) -> Result:
        return self._combine_results(self.send_all(list(cmds), modes))
    ...
    def send_all(
        ...
        if "-e" in modes:
            return self._send_error_mode(cmds, silent=silent)
        return [self._send(cmd, silent=silent) for cmd in cmds]
