...
class ParalexSite(BasePlugin[ParalexConfig]):
    ...
    def on_files(self, files, *, config):
        ...
        for name in self.p.resource_names:
            resource = self.p.get_resource(name)
            new_files = csv_to_html_table(name, self.p, self.tmp_dir.name,
...
def table_to_json(name, package, infos=None, with_paradigms=False):
...
def csv_to_html_table(name, package, out_dir, with_paradigms, compressed=False):
    files = []
    template = env.get_template("table.md")

    table_data = table_to_json(name, package, with_paradigms=with_paradigms)
