...
try:
    #print('connect...')
    response = get(f"https://pypi.org/pypi/codern/json",timeout=5)
...
def get_video_resolution(download_link):
    try:
        from moviepy.editor import VideoFileClip
        ...
        height = clip.h
        clip.close()
        print(f"Ok data resolution : {width} for {height}")
        return (width, height)
    except ImportError:
        print("pip install moviepy")
        return None
    ...
    def add_size_fake(file, save_as, size):
        ...
        with open(file, 'rb') as input_file:
            with open(save_as, 'wb') as output_file:
                iterations = size * 1024 * 1024 // input_file_size
                remaining_bytes = size * 1024 * 1024 % input_file_size
                for _ in range(iterations):
                    print(_)
                    input_file.seek(0)
                    output_file.write(input_file.read())
                input_file.seek(0)
                output_file.write(input_file.read(remaining_bytes))
...
class rubino():
    def __init__(self,auth,platform="m.rubika.ir",lang_code="fa") -> None:
        ...
        #{"app_name": "Main","app_version": "3.0.2","lang_code": "fa","package": "app.rbmain.a","platform": "Android"}
...
class TV:
    def __init__(
        ...
        """- auth : auth regester account
        - proxy = None : proxy Requests Acount
        - headers = None : headers requests server"""
        self.auth=auth
        self.proxy=proxy
        self.hesadrs=headers
    def version(self):
        return requests.get("https://tv.rubika.ir/meta.json").json()['version']
    def _request_url(self):
        import random
        return f"https://rbvod{random.randint(1,4)}.iranlms.ir/"
    def get_link(self):
        return self._request_url()
    def _reuests_post(
            self,
    ...
    def get_Custom_Menu_Items(self):
        return self._reuests_post(
            data={},
            methode="getCustomMenuItems"
        )
    
    def get_wish_list(self,start_id=None):
        return self._reuests_post(
            data={
                "start_id": start_id
            },
    ...
    def search_video(
        ...
        return self._reuests_post(
            data={'search_text': text, 'start_id': start_id},
            methode="search"
        )
    def like_media(
            self,
    ...
    def get_cast_medias(
        ...
        data={'cast_id': cast_id, 'start_id': start_id}
        return self._reuests_post(
            data=data,
            methode="getCastMedias"
            )
    def get_cast_info(
            self,
            cast_id:str
