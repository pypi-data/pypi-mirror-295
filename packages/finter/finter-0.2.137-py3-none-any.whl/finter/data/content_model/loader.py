...
class ContentFactory:
    ...
    def get_df(self, item_name, category=None, freq="1d", **kwargs):
        ...
        if self.use_cache and ContentFactory._global_cache is not None:
            for key in ContentFactory._global_cache.keys():
                if key[0] == cm_name:
                    cached_params = dict(key[1])
                    if cached_params != param:
                        df = BaseAlpha.get_cm(cm_name).get_df(**param)
                        ContentFactory._global_cache[key] = df
                        return df
                    else:
                        return ContentFactory._global_cache[key]
