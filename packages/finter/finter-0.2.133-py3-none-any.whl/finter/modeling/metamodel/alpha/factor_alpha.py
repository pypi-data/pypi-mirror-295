...
class UsFactorLongOnlyAlpha(BaseMetaAlpha):
    ...
    class Parameters(BaseMetaAlpha.Parameters):
        ...
        high: float = Field(
            ...,
            description="A float value between 0 and 1, where represents the high n percent value. For example, 0.2 corresponds to the top 20%. 0 means no selection. The high, low, and middle are used in union.",
        )  # 0~1
        low: float = Field(
            0,
            description="A float value between 0 and 1, where represents the low n percent value. For example, 0.2 corresponds to the bottom 20%. 0 means no selection. Default is 0. The high, low, and middle are used in union.",
        )
        middle: Tuple[float, float] = Field(
            (0, 0),
    ...
    def get_signal(self, factor, price):
        ...
        selected = rank[
            (rank >= 1 - self.high)
            | (rank <= self.low)
            | ((rank >= self.middle[0]) & (rank <= self.middle[1]))
        ]
