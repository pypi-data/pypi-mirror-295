...
class together with optimizer
interfaces etc. The module aims to contain all the logic behind SA-OO-VQE
solution, which is not directly connected to
...
class SAOOVQE:
    ...
    def __init__(
        ...
        # self._active_hamiltonian_evaluator = FastOperatorEvaluator(
        # self._problem.qubit_active_hamiltonian,
        #                                                          self._problem.fermionic_mapper)
        ...
        self._s_squared_evaluator = HermitianOperatorEvaluator(
            self._problem.qubit_s_squared, self._estimator
        )
        # self._s_squared_evaluator = FastOperatorEvaluator(
        # self._problem.qubit_s_squared, self._problem.fermionic_mapper)
        ...
        # self._ham_s_squared_sum_evaluator = FastOperatorEvaluator(
        # self._problem.qubit_active_hamiltonian +
        # self._problem.qubit_s_squared, self._problem.fermionic_mapper)
    ...
    def eval_nac(self, atom_idx: int) -> np.array:
        ...
        if self._cir_multipliers_trans is None:
            self._orb_multipliers_mat_trans, self._cir_multipliers_trans = (
        ...
        if self._rdm1_trans is None:
            self._rdm1_trans, self._rdm2_trans = (
    ...
    def eval_eng_gradient(self, state_idx: int, atom_moved: int) -> np.ndarray:
        ...
        geometry),
                           w.r.t. whose position the gradient is evaluated.
        ...
        # Check if necessary SAOOVQE propertienacs are initialized
        ...
        if self._cir_orb_hessians is None:
            self._cir_orb_hessians, self._cir_orb_hessian_avg = (
        ...
        if self._orb_hessian_avg_filter is None:
            self._orb_hessian_avg_filter, self._cir_orb_hess_avg_filter = (
        ...
        if self._circ_multipliers is None:
            self._orb_multipliers_mat, self._circ_multipliers = (
                    ...
                    for r in range(self._n_mo_optim - 1):
                        for s in range(r + 1, self._n_mo_optim):
                            if not self._is_param_pair_redundant(r, s):
                                ...
                                hess_filter[idx_pq_filter, idx_rs_filter] = (
            ...
            for p in range(q + 1, self._n_mo_optim):
                if not self._is_param_pair_redundant(p, q):
                    ...
                    for i in range(self._ansatz.num_parameters):
                        circuit_orbital_hess_filtered[i, ind_pq_filtered] = (
    ...
    def _weights_attribution(self, choice) -> list[float]:
        ...
        n_states = self._initial_circuits.n_states
