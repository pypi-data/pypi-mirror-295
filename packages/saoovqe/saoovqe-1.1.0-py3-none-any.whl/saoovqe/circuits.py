...
representing parts of initial to-be-optimized
state vectors.
"""
...
class OperatorEvaluatorBase(ABC):
    ...
    def __init__(self, operator, estimator: BaseEstimator) -> None:
        """
        Constructor
...
class HermitianOperatorEvaluator(OperatorEvaluatorBase):
    ...
    def get_evaluation_func(self, circuit: QuantumCircuit) -> Callable:
        ...
        def evaluation_func(circ_params: list[float] | np.ndarray) -> float | complex:
            ...
            return (
                self._estimator.run(
                    circuit.assign_parameters(param_binding), self._operator
                )
                .result()
                .values[0]
            )
