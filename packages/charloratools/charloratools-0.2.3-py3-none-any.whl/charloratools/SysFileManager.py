...
class ImgManager:
  ...
  def __init__(self,path:str,hashtype:str='sha256'):
    ...
    if not path.suffix.endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
  ...
  def to_html_base64(self,no_html_tag=False):
    ...
    try:
      ...
      raise errors.ImgOperationError(f"Failed to convert image to base64: {str(e)}")
...
class GalleryManager:
  ...
  def __init__(self,path:str,hashtype:str='sha256',show_tqdm:bool=False):
    if isinstance(path,str):
      ...
      raise errors.InvalidPathError("path must be string or path-like object")
...
class TmpManager(GalleryManager):
  ...
  def __init__(self, hashtype: str, save_content_on_deletion: bool = False, output_dir: str | None = None):
    ...
    self.output_dir = Path(output_dir).resolve() if output_dir else self.tmp_path
    self.is_open = False
  def __str__(self):
    return f"Temporary Directory;Name:{self.tmp_path};Opened:{self.is_open}"
  def __repr__(self):
