...
class FaceRecognizer:
  ...
  def filter_images_without_face(self,output_dir:str,min_face_size:int=20,
    ...
    """
    #Check dir
    utils.dirisvalid(path=output_dir,create_if_not_found=True)
    #Initializing model
          ...
          if boxes is not None:
            if prob_threshold is not None:
              if np.any(probs<prob_threshold):
                ...
                imgs_with_face+=1
                img_file.save(os.path.join(output_dir,os.path.basename(img)))
          ...
          if prob_threshold is not None:
            if np.any(probs<prob_threshold):
              ...
              if not same_face:
                ...
                cv2.imwrite(os.path.join(output_dir, os.path.basename(img_path)), rgb_img)
