...
def find_conf_file(conf_file_path, base_dir="docs/build/confluence"):
    """
    Recursively searches for a .conf file within the given base directory.

    :param conf_file_path: The name of the .conf file to search for (without the .conf extension).
    :type conf_file_path: str
    :param base_dir: The base directory to start the search. Defaults to 'docs/build/confluence'.
    :type base_dir: str
    :return: The full path to the .conf file if found, otherwise None.
    :rtype: str or None
    """
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file == f"{conf_file_path}.conf":
...
def update_confluence_page_if_changed(page_id, conf_file_path, confluence_domain=os.getenv("JiraDomain"),
    ...
    :type page_id: str
    :param conf_file_path: The name of the .conf file containing the new content for the page (without the .conf extension).
    :type conf_file_path: str
    ...
    # Search for the .conf file recursively in 'docs/build/confluence'
    conf_file_full_path = find_conf_file(conf_file_path)
    if not conf_file_full_path:
        raise FileNotFoundError(
            f"The .conf file '{conf_file_path}.conf' was not found in any subdirectories of 'docs/build/confluence/'.")

    # Get the contents of the found .conf file
    conf_content = testKitUtils.get_contents(conf_file_full_path)
...
def update_confluence_page(page_id: str,
    ...
    if response.status_code != 200:
        # Raise an exception with the error message
        logger.debug(
            f' Page: {page_id}. Error updating Confluence page: {filepath}')
        raise Exception('Error updating Confluence page: ' + response.text)
