...
class FNIRTCoefficients(object):
    ...
    def __init__(self, coeffs, src, ref, constrain_jac=False):
        if isinstance(coeffs, (str, Path)):
            ...
            assert isinstance(coeffs, (Nifti1Image, FSLImage))
            if isinstance(coeffs, FSLImage):
                coeffs = coeffs.nibImage
...
def get_field(coeff1, ref, coeff2=None, mid=None, post=None, jmin=None, jmax=None):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        coeff1.to_filename(w1)
        # coeff_copy = nibabel.Nifti1Image(
        #     coeff1.get_fdata(), affine=coeff1.affine, header=coeff1.header
        # )
        # coeff_copy.to_filename(w1)
