...
class NonLinearRegistration(Transform):
    ...
    def inverse(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            # Hack to break the break the link to the filesystem
            # Otherwise we get errors because the original file
            # is deleted when the temporary directory is cleaned up
            newcoeffs = nibabel.Nifti1Image(
                newcoeffs.get_fdata(),
                affine=newcoeffs.affine,
                header=newcoeffs.header,
            )
