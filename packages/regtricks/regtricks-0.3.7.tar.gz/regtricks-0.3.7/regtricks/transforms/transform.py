...
class Transform(object):
    ...
    def apply_to_array(
        ...
        **kwargs,
        ...
        # Pad data on each spatial axis to avoid potentially losing slices
        # if transform is trying to warp voxels from outside the image
        data = np.pad(
            data, [(1, 1)] * 3 + [(0, 0)] * (len(data.shape) - 3), mode="edge"
        )
        src = src.resize([-1, -1, -1], data.shape[:3])
        ...
        if mask and (order > 0):
            ...
            mvol = np.stack(
                [
                    binary_closing(
                        binary_fill_holes(mvol[..., idx]), iterations=3, border_value=1
                    )
                    for idx in range(mvol.shape[3])
                ],
                axis=-1,
            )
            ...
            mres = np.stack(
                [
                    binary_closing(
                        binary_fill_holes(mres[..., idx]), iterations=3, border_value=1
                    )
                    for idx in range(mres.shape[3])
                ],
                axis=-1,
            )
