...
class ImageSpace(object):
    ...
    def vox2FSL(self):
        ...
        if len(self.size) < 3:
            raise RuntimeError(
                "Volume has less than 3 dimensions, " "cannot resolve space"
            )
    ...
    def __repr__(self):
        from .application_helpers import aff_trans

        xyz_min = aff_trans(self.vox2world, -0.5 * np.ones((1, 3))).squeeze()
        xyz_max = aff_trans(self.vox2world, (self.size[None, :] - 0.5)).squeeze()
