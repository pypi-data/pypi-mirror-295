...
def get_classification_path(image_path, sample_interval, n_clusters, random_state):
    ...
    """
    return Path(os.path.join(image_path.parent, "thematic", image_path.stem + ".kmeans++-" + str(sample_interval) + "-" + str(n_clusters) + "-" + str(random_state) + ".tif"))
...
def get_centroid_path(image_path, sample_interval, n_clusters, random_state):
    """
       ...
       """
    return Path(os.path.join(image_path.parent, "thematic", image_path.stem + ".kmeans++-centroid-" + str(sample_interval) + "-" + str(n_clusters) + "-" + str(random_state) + ".csv"))


def compute_kmeans_score(image1_path, image2_path, sample_interval=10, n_clusters=10, random_state=42):
    """
    ...
    if image_classif_path.exists() is False:
        image_classif = compute_kmeans(image_path, image_classif_path, centroid_path, n_clusters=n_clusters, sample_interval=sample_interval, random_state=random_state)
    else:
    ...
    if decompressed_classif_path.exists() is False:
        decompressed_classif = compute_kmeans(decompressed_path, decompressed_classif_path, centroid_path, n_clusters=n_clusters, sample_interval=sample_interval, random_state=random_state)
    else:
