...
class Regnskab:
    ...
    def __EBITDA(self) -> float:
        if self.__result_dict['EBIT'] and self.__result_dict['afskrivninger']:
            return sum(self.__result_dict["EBIT"], abs(self.__result_dict["afskrivninger"]))
        return self.__result_dict["EBIT"]

    @property
    def __balance_test(self) -> float:
        hensaettelser = self.__result_dict["hensaettelser"] if abs(self.__result_dict["hensaettelser"]) > 0 else 0
        if all(self.__samlet_gaeld_test, self.__result_dict["egenkapital"], hensaettelser):
            return sum([self.__samlet_gaeld_test, self.__result_dict["egenkapital"], hensaettelser])
        return 0

    @property
    def __hensaettelser(self) -> float:
        if not self.__result_dict["hensaettelser"]:
    ...
    def __ebit_temp(self) -> float:
        # EBITDA beregnes udelukkende hvis bruttoresultatet er angivet og bruttoresultat kan beregnes (__brutto_test)
        if not self.__result_dict["bruttoResultat"] or self.__result_dict["bruttoResultat"] != self.__brutto_test:
            return None
        ...
        udgifter_foer_EBIT = sum([self.__result_dict[res] for res in resultatUdg_EBIT_beregning if res in self.__result_dict])
