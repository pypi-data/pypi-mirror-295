...
class CVROwner:
    ...
    def __extract_org_info(self, org_dict: dict) -> dict:
        ...
        gyldig_til_datoer = []
        for org in org_dict:
            medlemsdata = org_dict["medlemsData"][0]["attributter"]
            roller.extend(rolle["vaerdier"][0]["vaerdi"].lower() for rolle in medlemsdata)
            gyldig_til_datoer.extend([m["vaerdier"][0]["periode"]["gyldigTil"] for m in medlemsdata])
        reel_ejer = 0
        for rolle in roller:
    ...
    def __deltager_relationer(self) -> list[dict]:
        return [deltager for deltager in self.response["hits"]["hits"][0]["_source"]["Vrvirksomhed"]["deltagerRelation"] if deltager["deltager"]] 