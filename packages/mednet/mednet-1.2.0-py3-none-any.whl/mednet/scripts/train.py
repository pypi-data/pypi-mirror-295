...
from ..scripts.click import ConfigCommand, PathOrURL
...
def reusable_options(f):
    ...
    @click.option(
        ...
        "-B/-N",
        help="""If set, balances the loss term to take into consideration the
        occurence of each target in the training and validation splits.""",
        required=True,
        show_default=True,
        default=True,
        cls=ResourceOption,
    ...
    @click.option(
        ...
        corresponding to the architecture set with `--model`.  Optionally, you
        may also pass a directory containing the result of a training session,
        in which case either the best (lowest validation) or latest model will
        be loaded. Note these weights will be overwritten when re-starting
        training from a checkpoint. This parameter should only be used as a set
        of initial weights (when starting a new training session).""",
        required=False,
        cls=ResourceOption,
        type=PathOrURL(
            exists=True,
...
def train(
    ...
    **_,
    ...
    # resets the number of outputs in a classification model, if necessary
    reset_num_outputs_on_model(datamodule.unshuffled_train_dataloader(), model)
