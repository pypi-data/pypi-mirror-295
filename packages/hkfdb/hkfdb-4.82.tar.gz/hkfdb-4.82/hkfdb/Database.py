...
class Database:
    ...
    def get_hk_stock_ohlc(self, code, start_date, end_date, freq, price_adj=False, vol_adj=False, anchor_adj=True):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                if price_adj == True:
                    ...
                    qfq_df = qfq_df.reindex()

                    for i in range(1, len(qfq_df)):
                        pre_qfq_factor = qfq_df.at[i - 1, 'qfq_factor']
                        qfq_factor = qfq_df.at[i, 'qfq_factor']
                        if pre_qfq_factor < 1 and qfq_factor == 1:
                            qfq_df.at[i, 'qfq_factor'] = pre_qfq_factor

                    if anchor_adj:
                        qfq_df = qfq_df[qfq_df['date'] <= end_date]
                        first_qfq_factor = qfq_df.at[qfq_df.index[0], 'qfq_factor']
                        qfq_df['qfq_factor'] = qfq_df['qfq_factor'] / first_qfq_factor
    ...
    def get_ccass_holding_rank(self, code, start_date, end_date):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                res_obj = json.loads(response.content).replace("LING\'S", "LINGS").replace("LEE'S", "LEES")
                res_obj = res_obj.replace("'", "\"")

                result = json.loads(res_obj)
    ...
    def get_ccass_by_id(self, ccass_id, start_date, end_date):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                def correct_negative_values(value):
                    if value < 0:
                        return np.int64(np.uint32(value))
                    return value

                df['share'] = df['share'].apply(correct_negative_values)
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                if response_ok == True:
                    ...
                    df['share_first'] = df['share_first'].astype(np.int64)
                    df['share_last'] = df['share_last'].astype(np.int64)
                    df['share_chg'] = df['share_chg'].astype(float)

                    df['share_first'] = df['share_first'].apply(correct_negative_values)
                    df['share_last'] = df['share_last'].apply(correct_negative_values)
