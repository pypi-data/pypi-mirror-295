...
class UniLayerConv2D(UniLayer, _UniLayerBaseConv2D):
	op: ClassVar = "Conv2D"
	valid_input_range: ClassVar = range(2, 4)
	input_names: ClassVar = ["input", "weights", "bias"]
	output_names: ClassVar = [""]

	def __eq__(self, other):
		if self.name != other.name or self.k_h != other.k_h or self.k_w != other.k_w or self.s_h != other.s_h or self.s_w != other.s_w or self.p_h_before != other.p_h_before or self.p_h_after != other.p_h_after or self.p_w_before != other.p_w_before or self.p_w_after != other.p_w_after or self.rate0 != other.rate0 or self.rate1 != other.rate1 or self.group != other.group or self.data_format != other.data_format or self.weights_layout != other.weights_layout or self.out_shapes != other.out_shapes or self.out_dtypes != other.out_dtypes or self.breadcrumbs != other.breadcrumbs or self.history != other.history or any([v != other.extended_attr[k] if not isinstance(v, float) else not np.allclose(v, other.extended_attr[k]) for k, v in self.extended_attr.items()]):
			return False
		return True

	def __hash__(self):
		return hash((self.name, self.k_h, self.k_w, self.s_h, self.s_w, self.p_h_before, self.p_h_after, self.p_w_before, self.p_w_after, self.rate0, self.rate1, self.group, self.data_format, self.weights_layout, tuple(self.out_shapes), tuple(self.out_dtypes), self.breadcrumbs, tuple(self.history), immute(self.extended_attr)))
