...
def pull_notebook(token, user_id, strategy_name, path, output_file_name=None, notebook_data=None):
    ...
    return {'success': True, 'file_path': file_path, 'updateTime': notebook_data['updateTime']}
...
def get_jupyter_url():
    process = subprocess.Popen(['jupyter', 'lab', 'list'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    while True:
...
def open_ipynb_file(WORKSPACE_PATH, file_name, envs, open_url=True):
    ...
    env = os.environ.copy()
    env.update(envs)
    command = """eval "$(conda shell.bash activate)" && jupyter lab --ServerApp.root_dir=\""""+WORKSPACE_PATH+"""\" --no-browser  --LabApp.tornado_settings=\"{'headers': {'Content-Security-Policy': \\"frame-ancestors 'self' http://localhost:5173 http://127.0.0.1:8888\\"}}" &"""
    print(command)
