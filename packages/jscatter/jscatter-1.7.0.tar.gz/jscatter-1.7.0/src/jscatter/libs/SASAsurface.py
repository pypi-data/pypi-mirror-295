...
def _surface_atoms(atoms, solvent_radius=0., point_density=258, vdwradii=None):
    global _pooldata
    # maxrad and atom_data[:,3] include +solvent_radius
    (maxrad, atom_data, center) = _get_atom_data(atoms, solvent_radius, vdwradii)
    ...
    _pooldata = {'point_density': point_density,
                 'center': center,
                 'neighbors': NeighborList(atoms,  maxrad + solvent_radius, atom_data),
                 'atom_data': atom_data,
                 'tess1': tess1}
...
def _asurf(i):
    ...
    # do the real calc
    (points1, points_unit) = _atom_surf(neighbors, i, atom_data, pos1, tot_rad, point_density, tess1, ret_fmt >= 2)
    res = _xlate_results(points1, points_unit, point_density, tot_rad, pos1, ret_fmt, center)
