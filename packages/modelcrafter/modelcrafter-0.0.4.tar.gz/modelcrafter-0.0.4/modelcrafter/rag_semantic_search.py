...
class RAGSemanticSearch:
    ...
    def _create_and_populate_collection(self):
        ...
        if collection_exists:
            logger.info(f"Collection '{self.collection_name}' already exists. Using the existing collection.")
            collection = persistent_client.get_collection(name=self.collection_name, embedding_function=ef)
        else:
