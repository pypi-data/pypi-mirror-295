...
from src.utils import get_current_time_ms, create_tmp_dir_, config


class LighthouseReport:
    def __init__(self, url: str, temp_dir, output_path: str = 'lighthouse_report.json', headless: bool = True,
    ...
    async def run_report(self, pbar: tqdm) -> dict:
        ...
        try:
            chrome_flags = f"--enable-logging --v=99 --log-path={self.data_dir}/logs --user-data-dir={self.data_dir}"
            if self.headless:
            ...
            command = [
                ...
                '--preset', self.preset,
                '--chrome-path', self.chrome_executable_path
            ]
