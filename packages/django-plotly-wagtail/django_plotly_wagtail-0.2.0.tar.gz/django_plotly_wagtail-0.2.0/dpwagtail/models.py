...
@register_snippet
class DashInstanceSnippet(models.Model):
    dash_instance = models.ForeignKey(DashInstance,
                                      null=True,
                                      unique=False,
                                      on_delete=models.SET_NULL)

    panels = [FieldPanel("dash_instance"),
              ]

    def __str__(self):
        return str(self.dash_instance)


class DjangoPlotlyDashBlogPage(Page):
    """Block page using one or more dash apps"""
    ...
    body = StreamField([
        ('heading', blocks.CharBlock(classname="full_title")),
        ('paragraph', blocks.RichTextBlock()),
        ('dash_instance', SnippetChooserBlock(DashInstanceSnippet)),
    ])
...
class DjangoPlotlyDashIndexPage(Page):
    """Index page for a collection of DjangoPlotlyDashPage and DjangoPlotlyDashBlogPage children"""

    def get_context(self, request, *args, **kwargs):
        context = super().get_context(request, *args, **kwargs)
        context['dpd_pages'] = DjangoPlotlyDashPage.objects.child_of(self).live()
        context['dpd_blog_pages'] = DjangoPlotlyDashBlogPage.objects.child_of(self).live()
        return context
