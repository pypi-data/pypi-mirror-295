...
class NamedPath(object):
    ...
    def solve(
        self, context: dict, skip_context_errors: bool = False, relative: bool = False, local: bool = False
    ) -> str:
        """
    ...
    def iter_path(
        ...
        solve: bool = True,
        dirs_only: bool = True,
        skip_context_errors: bool = False,
    ...
    def get_parts(
        self, context: dict = None, solve: bool = False, dirs_only: bool = False, skip_context_errors: bool = False
    ) -> list:
        context = self.get_context(context or {})
        if context:
            src_path, opts = self.__class__.remove_optional(self.path, context)
            try:
                _, context_variables = self.expand_attrs(src_path, context, skip_context_errors=skip_context_errors)
            except TypeError as e:
                logger.error(f"Error path resolve: {src_path}")
                raise
        else:
            src_path = self.path
            context_variables = {}
    ...
    def expand_variables(self, text: str, context: dict) -> str:
        ...
        text, opts = self.remove_optional(text, ctx)
    ...
    def expand_attrs(cls, text: str, context: dict, **kwargs) -> tuple[str, dict]:
        ...
        new_context = {}
        new_text = text
        text, opts = cls.remove_optional(text, context)
        ...
        def get_next_value(obj, name):
            if hasattr(obj, name):
                ...
                if name in obj:
                    ...
                    raise KeyError(f'No key "{name}" in {obj}')
            else:
                raise TypeError(f"Unsupported type for nested value in key {name}: {type(obj)}")
            ...
            if "." in var_name:
                ...
                if var_name not in context:
                    if not skip_context_errors:
                        raise PathContextError(f"Variable {var_name} not in context")
                else:
    ...
    def split_var_name_and_options(cls, var: str) -> tuple:
        products = re.split(r"([:|])", var, 1)
        if len(products) > 1:
            var_name, options = products[0], products[1:]
    ...
    def remove_optional(cls, text: str, context: dict):
        ...
        for var in pat.finditer(text):
            if var.group(1).split(":")[0].split(".")[0] not in context:
                ...
                var_name = var.group(0).strip("<>{}").split(".")[0]
                optionals.append(var_name)
            text = text.replace(var.group(0), var.group(0).strip("<>"))
        return text, optionals
    ...
    def get_short(self, custom_path=None) -> str:
        ...
        if custom_path is not None:
            ...
            path = self.path
        return Path((path).split("]", 1)[-1].lstrip("\\/")).as_posix()
...
class NamedPathDrive(NamedPath):  # TODO: Work in progress! dont use this class
    ...
    def update_owner(self, context, skip_context_errors=False, parents=False, skip_non_exists=False, **kwargs):
        if parents:
            parent = self.get_parent()
            if parent:
                parent.update_owner(context, skip_context_errors, parents, skip_non_exists)

        for path, user, group in zip(
            self.iter_path(context, dirs_only=False, skip_context_errors=True, full_path=True),
    ...
    def update_permissions(self, context, skip_context_errors=False, parents=False, skip_non_exists=False, **kwargs):
        if parents:
            parent = self.get_parent()
            if parent:
                parent.update_permissions(context, skip_context_errors, parents, skip_non_exists, **kwargs)

        for path, perm in zip(
            self.iter_path(context, dirs_only=False, skip_context_errors=True, full_path=True),
            ...
            perm = (
                perm
                or kwargs.get("default_permission")
                or (self.default_dir_permission if not os.path.splitext(path)[1] else self.default_file_permission)
            )
            chmod(path, perm)
    ...
    def makedirs(self, context, skip_context_errors=False, **kwargs):
        ...
        parts = list(
            zip(
                self.iter_path(
                    context, solve=True, dirs_only=True, skip_context_errors=skip_context_errors, full_path=True
                ),
        ...
        for i, (path, perm, group, user) in enumerate(parts):
            ...
            if not os.path.exists(path):
                ...
                if real_path != link_source:
                    raise IOError(
                        "Linked path {} referenced to different source: {}, correct source: {}".format(
...
class NamedPathTree:
    ...
    def __init__(
        self, root_path: str or Path, path_list: dict = None, default_context: dict = None, path_class=None, **kwargs
    ):
        self.kwargs = kwargs
    ...
    def update_patterns(self, path_list: dict):
        ...
        for path_name, options in path_list.items():  # type: str, str
            ...
            self._scope[path_name] = self.path_class(
                self.root, path_name, options, self._scope, default_context=self.default_context, **self.kwargs
            )
    ...
    def show_tree(self, **kwargs):
        ...
        def _show(products, print_path=True, print_name=True, max_name_width=0, indent=0, placeholder="-"):
                ...
                print(
                    "".join(
                        [
                            ...
                            (
                                ("{:>{}}|".format(prod["inst"].name, max_name_width + 2) if print_name else "")
                                if print_path
                                else prod["inst"].name
                            ),
                            placeholder * indent * 2 if print_path else "",
                            ("/" + prod["inst"].get_short()) if print_path else "",
                        ]
                    )
                )
                if prod["ch"]:
...
class NamedPathTreeDrive(NamedPathTree):  # TODO: Work in progress! dont use this class
    ...
    def transfer_to(
        ...
        return dict(remapped_paths=path_pairs, skipped_paths=skipped_paths)
