        ...
        if div_title:
            ...
            if title=="us_patent_references":
                ...
                for tr in tr_list:
                    td_list=tr.find_all("td")
                    for td in td_list:
                        res_list.append(td.get_text().strip())
                text=",".join(res_list).replace("|||","|")
            if title=="description":
                ...
                for heading in headings:
                    ...
                    while next_element and next_element.name not in ['h1']:
...
def getPatentInfo(fpo_search_result:FpoSearchResult):
...
def downloadPdfByUrl(pdf_url,save_path):
...
def downloadPdf(fpo_patent_info:FpoPatentInfo,save_dir):
    if os.path.basename(save_dir).find(".pdf")!=-1:
        downloadPdfByUrl(fpo_patent_info.pdf_url,save_dir)
    else:
        save_dir=os.path.join(save_dir,f"{fpo_patent_info.patent_id}.pdf")
        downloadPdfByUrl(fpo_patent_info.pdf_url,save_dir)
def autoSpider(query_txt,save_dir="data",save_pdf=True,num_in_search=1,id_is_query=True):
    ...
    for index,fpo_search_result in enumerate(fpo_search_result_list):
        if index+1>num_in_search:
            break
        fpo_patent_info=getPatentInfo(fpo_search_result)
        ...
        content_dict=fpo_patent_info.__dict__.pop("description")
        patent_info_file=os.path.join(base_patent_dir,f"info.json")
        with open(patent_info_file,"w",encoding="utf-8") as f:
            json.dump(fpo_patent_info.__dict__,f,ensure_ascii=False)

        patent_content_file = os.path.join(base_patent_dir, f"content.json")
        with open(patent_content_file, "w", encoding="utf-8") as f:
            json.dump(content_dict, f, ensure_ascii=False)
        if save_pdf:
            patent_pdf_file = os.path.join(base_patent_dir, f"patent.pdf")
            downloadPdfByUrl(fpo_patent_info.pdf_url,patent_pdf_file)