...
"""
    ...
    exp = result.expect([pexpect.EOF, pexpect.TIMEOUT])
    ...
    result = pexpect.spawn(
        sys.executable,[ "-m", "mbpy.cli", "uninstall", "requests"],
        cwd=tmp_path,
        timeout=30,
    )
    for line in result.readlines():
        print(line.decode())
    result = result.expect([pexpect.EOF, pexpect.TIMEOUT])
    updated_content = pyproject_path.read_text()
    assert "requests" not in updated_content, f"'requests' found in updated content: {updated_content}"
    result = pexpect.spawn(
        sys.executable,[ "-m", "mbpy.cli", "install", "requests"],
        cwd=tmp_path,
        timeout=5
