...
def create_project(
    ...
    src_dir = project_root / project_name.replace("-", "_")
    ...
    setup_readme(project_root, project_name, description)
...
"""
    ...
    return project_root  # Return the project path

def setup_gitignore(project_root: Path) -> None:
    gitignore_content = """
# Python specific
*.pyc
*.pyo
...
def setup_readme(project_root: Path, project_name: str, description: str) -> None:
    readme_content = f"""
# {project_name}
{description}
"""
    should_write = not (project_root / "README.md").exists()
    Path(project_root / "README.md").touch(exist_ok=True)
    if should_write:
