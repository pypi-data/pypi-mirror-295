...
class Application:
	...
	#--------------------------------------------------------------------------------
	@staticmethod
	def LogExceptionWithExit(message : Union[str, Exception], useTraceback : bool = True) -> None:
		isException = Builtins.IsInstanceType(message, Exception)
		if isException and useTraceback:
			traceback.print_exc()
