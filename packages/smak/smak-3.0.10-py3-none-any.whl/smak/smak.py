...
class Main:
    def __init__(self,master):
        ...
        self.plotColorPal=tkinter.StringVar()
        ...
        menubar.addmenuitem('File','command',label='Convert SSRL Mosaic - AdjStatic',command=self.createSSRLMosaicC)
        ...
        menubar.addmenuitem('File','command',label='Save Displays as OME Tiff',command=self.asksavemanyOMEdisplays)
        menubar.addmenuitem('File','command',label='Save Displays as Animated GIF',command=self.asksavemanytoGIF)
...
#        menubar.addmenuitem('File','command',label='Save CT sections',command=self.saveCTsections)
        ...
        menubar.addmenuitem('View','separator')
        menubar.addcascademenu('View','Plotter Color Palette')
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='husl',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='hls',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='colorblind',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='Accent',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='BrBG',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='Greys',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='Oranges',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='Paired',command=None,variable=self.plotColorPal)
        menubar.addmenuitem('Plotter Color Palette','radiobutton',label='Spectral',command=None,variable=self.plotColorPal)

        self.plotColorPal.set('husl')
        menubar.addmenuitem('View','separator')
...
#        menubar.addmenuitem('Process','separator') 
        ...
        menubar.addmenuitem('Process','command',label='Convert Camera Channels to LAB', command=self.convCamLAB)
        menubar.addmenuitem('Process','command',label='Convert Camera Channels to HSV', command=self.convCamHSV)
        menubar.addmenuitem('Process','command',label='Convert Camera Channels to YCrCb', command=self.convCamYCrCb)
        menubar.addmenuitem('Process','command',label='Convert Camera Channels to XYZ', command=self.convCamXYZ)
        ...
        menubar.addmenuitem('Analyze','command',label='Plot ROI Spectra',command=self.plotFFSpectra)
        ...
        menubar.addmenuitem('MCA View Options','radiobutton',label='Spectrum',command=self.setMCAspectrum,variable=self.MCAXAXIS)
        menubar.addmenuitem('MCA Spectra','separator')        
...
#        menubar.addmenuitem('MCA Spectra','separator') 
        ...
        menubar.addmenuitem('FTIR Support','command',label='Convert %T to Abs',command=self.IRconvertTtoA)        
...
#more?
        ...
        self.lastMCApixno=None
        self.lastMCAbinselect=None
        self.PCAzoomstack=[]
...
#        self.contoursOn.set(0)
    ...
    def createSSRLMosaic(self,stype='MV',adj=False):
        globalfuncs.setstatus(self.status,"Ready")
        #get file name
        ...
        for impfn in multfn:
            ...
            while not done:
                if adj==True:
                    #get factors
                    xa=tkinter.simpledialog.askfloat(title='Mosaic',prompt='Enter adjustment factor XFOV',initialvalue=xa)
                    if xa is None: xa=1.0
                    ya=tkinter.simpledialog.askfloat(title='Mosaic',prompt='Enter adjustment factor FFOV',initialvalue=ya)
                    if ya is None: ya=1.0
                ...
                if stype=='MV': 
                    ...
                    mosaicfn=stitch_hdf_mod.stitch_hdf_man(impfn,left=self.mosaicCropLeft,right=self.mosaicCropRight,xfvx=xa,yfvx=ya)
                else:
                ...
                if adj:
                    if tkinter.messagebox.askyesno(title="Mosaic Builder",message="Adjust moasic params?"):
                        done=False
                    else:
                        done=True
                else:
    ...
    def createSSRLMosaicC(self):
        self.createSSRLMosaic(stype='man',adj=True)
...
#################################  MCA viewing routines
    ...
    def setMCAspectrum(self):
        if not tkinter.messagebox.askyesno("MCA X-axis Properties", "Use values in MCA data file (if present)"):
            self.setMCAslopeValue()
        else:
            self.MCAreplotBuffers()
        
    def setMCAslopeValue(self):
        mt='Enter number of ev per MCA bin - cancel to accept current values'
        nv=tkinter.simpledialog.askfloat(title='MCA X-axis pproperties',prompt=mt,initialvalue=self.currentMCAXraySlope)
        if nv==None or nv=='': 
            self.MCAreplotBuffers()
            return
    ...
    def updateMCAgraph(self,plot=True):
        ...
        if not self.MCAviewexist:
            ...
            menubar.addmenuitem('MCA','separator')
            menubar.addmenuitem('MCA','command',label='Fit Spectral Peak to Data',command=self.MCApeakfitdataStart)
            menubar.addmenuitem('MCA','command',label='Active Spectral Peak Fit',command=self.MCApeakfitdataActive)
            menubar.addcascademenu('MCA','Spectral Fit Type')
            self.MCASpectralfitType=tkinter.StringVar()
            menubar.addmenuitem('Spectral Fit Type','radiobutton',label='Gaussian',variable=self.MCASpectralfitType)
            menubar.addmenuitem('Spectral Fit Type','radiobutton',label='pVoigt',variable=self.MCASpectralfitType)            
            self.MCASpectralfitType.set('pVoigt')
...
##                    self.MCAgraph.element_delete(g)            
    ...
    def MCApeakfitdataActive(self):
        
        if self.lastMCApixno is None: 
            print ('no active spectra')
            return
        if self.lastMCAbinselect is None:
            print ('no bins selected')
            return
        self.spfitmask=None
        self.MCApeakfitfunction(self.lastMCAbinselect[0],self.lastMCAbinselect[1],None)

    def MCApeakfitdataStart(self):
        globalfuncs.setstatus(self.status,"Ready")
        if not self.hasdata:
            print('No Data')
            globalfuncs.setstatus(self.status,'No Data')
        ...
        if self.MCAfilename=='':
            ...
            return        
        #get data mask
        self.askmaskdialog=Pmw.SelectionDialog(self.imgwin,title="Select Fit Mask (cancel for all data)",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Channel',scrolledlist_items=self.mapdata.labels,
                                                   command=self.selectmaskchannelforfit)
        self.askmaskdialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.SINGLE)  
        
    def selectmaskchannelforfit(self,result):
        opt=self.askmaskdialog.getcurselection()[0]
        self.askmaskdialog.withdraw()
        if result=='Cancel':
            globalfuncs.setstatus(self.status,'Using Uniform Mask')
            self.spfitmask=None
        else:
            iind=self.mapdata.labels.index(opt)+2
            self.spfitmask=self.mapdata.data.get(iind)
        
        #get data range (spectral or bin)
        self.binrangewithPF=True
        if self.MCAXAXIS.get()=='Bins':
            self.MCArebindata()
        else:
            self.MCArebindataValue()
        
    def MCApeakfitfunction(self,bmin,bmax,basename):
        
        binmin=min(bmin,bmax)
        binmax=max(bmin,bmax)
        print (binmin,binmax)
        ...
        if ext!=".hdf5":
            ...
            self.binrangewithPF=False
            globalfuncs.setstatus(self.status,"format not supported")    
            return
        ...
        #create new arrays for I,h,FWHM,pos
        newdata=zeros((self.mapdata.nxpts*self.mapdata.nypts,4),dtype=np.float32)
        ...
        if self.MCAXAXIS.get()==('Bins'):
            ...
            if len(self.currentMCAXvalues) != len(self.MCArawdataBuffer[0]):
                ...
                MCAx=self.currentMCAXvalues
        MCAx=MCAx[binmin:binmax]

        #iterate on masked area
        if self.spfitmask is None:
            fitind = range(self.mapdata.nxpts*self.mapdata.nypts)
        else:
            fitind = np.ravel(self.spfitmask).nonzero()[0]
        if basename is None:
            fitind = [self.lastMCApixno]
        for fi in fitind:
            print (fi)
            fdata=mcadata[fi,binmin:binmax]
            fdata = fdata.astype(dtype=np.float64)
            MCAx = MCAx.astype(dtype=np.float64)
            
            minp=(MCAx[-1]-MCAx[0])*4/len(MCAx)
            initguess=(1,(MCAx[-1]-MCAx[0])/2+MCAx[0],minp,0,np.min(fdata),0.5)
            pbs=[(0,np.inf),(MCAx[0],MCAx[-1]),(minp,(MCAx[-1]-MCAx[0])),(-np.inf,np.inf),(-np.inf,np.inf),(0,1)]
            if self.MCASpectralfitType.get()=='Gaussian':
                fe=globalfuncs.gausslineFit
                fe2=globalfuncs.gausseqnline
                initguess=initguess[:-1]
                pbs=pbs[:-1]
            else:
                fe=globalfuncs.psVogtFit
                fe2=globalfuncs.psVogteqn

            #print("fitting psVogt+linear...",initguess)  
            try:
                bs=[[],[]]
                for i in pbs:
                    bs[0].append(i[0])
                    bs[1].append(i[1])
                bs=[tuple(bs[0]),tuple(bs[1])]
                result,cov=scipy.optimize.curve_fit(fe,MCAx,fdata,p0=initguess,bounds=bs,max_nfev=5000)
                fp=result
            ...
            bigL = 2.35482*fp[2]
            if self.MCASpectralfitType.get()=='Gaussian':
                h=fp[0]/bigL*0.398942
                fp=list(fp)
                fp.append(0.0)
            else:
                h=2*fp[0]/3.14156/bigL*(1+fp[5]*0.47566)
            #print('FWHM: ',bigL)
            #print('Ht: ',h)
            newdata[fi,0]=fp[0]
            newdata[fi,1]=h
        ...
        if basename is None:
            ...
            self.MCAgraph.cleargraphs()
            gfit=fe2(fp,MCAx)
            self.MCAgraph.plot(tuple(MCAx),tuple(fdata),text='MCA',color='green')
            self.MCAgraph.plot(tuple(MCAx),tuple(gfit),text='fit',color='red')
            self.MCAgraph.draw()
            self.newMCAplot.show()            
            return
        #add data!
        ...
        for ty in adds:
            ...
            while not valid:
                ...
                if newname not in self.mapdata.labels:
                    ...
                    i+=1                    
            self.addchannel(newdata[:,adds.index(ty)].reshape(self.mapdata.data.get(0).shape),newname)
...
#                    chsigma=fresult['uncertainties'][ch][::-1,:]
    ...
    def makeMCAdialog(self,itype="Bins"):
        ...
        if self.binrangewithPF:
            text = 'MCA PeakFit Spectrum'
            sctext = 'New Channel Basename: '
        else:
            text = 'MCA Spectrum Rebin'
            sctext = 'New Channel Name: '
        self.MCAbindialog=Pmw.Dialog(self.imgwin,title=text,buttons=('OK','Cancel'),
        ...
        self.E1.setvalue(0)
        self.E2.setvalue(0)
        self.MCAnewchannel=Pmw.EntryField(h,labelpos='w',label_text=sctext,entry_width=15,hull_background='#d4d0c8',label_background='#d4d0c8')
    ...
    def MCAdotherebin(self,result):        
        ...
        if result=='Cancel' or self.MCAnewchannel.get()=='':
            if self.binrangewithPF:
                self.binrangewithPF=False
                print('MCA peakfit cancelled')
                globalfuncs.setstatus(self.status,'MCA peakfit cancelled')
                return                
            else:
        ...
        self.lastMCAbinselect=[binmin,binmax]

        if self.binrangewithPF:
            globalfuncs.setstatus(self.status,"Peakfitting MCA data...")
            self.MCApeakfitfunction(binmin,binmax,newname)
        else:
...
#################################  FTIR Analysis
    ...
    def IRconvertTtoA(self):
        ...
        resulty = 2-np.log10(mcadata)
        
        newfn=os.path.splitext(self.MCAfilename)[0]+"_irabs.hdf5"
...
#################################  Spectrum Maker
    ...
    def displaySpectrumGraph(self,pixno,dataover=None):
        if dataover is None:
...
##                                gtype.element_delete(g)      
        self.maindisp.xyplotcolorind=0     
        if dataover is None:
            #order data
        ...
        #make graphs
        palette=sblite.color_palette(self.plotColorPal.get(), n_colors=8)
        ...
        if len(syv.shape)==1:
            ...
            for self.maindisp.xyplotcolorind in range(syv.shape[0]):
                colorplot=colors[self.maindisp.xyplotcolorind%len(colors)]
                self.maindisp.graphx2.plot(tuple(xv),tuple(syv[self.maindisp.xyplotcolorind]),color=colorplot,text='XV'+str(self.maindisp.xyplotcolorind))                
        self.maindisp.graphx2.draw()
        self.maindisp.newlineplot2.show()

    def plotFFSpectra(self):
        self.exportFFSpectra(plotonly=True)
    ...
    def exportFFSpectra(self,PCA=False,plotonly=False):
        self.exportFFPCAflag=PCA
        self.exportFFplotonly=plotonly
        globalfuncs.setstatus(self.status,"Ready")
        if not self.hasdata:
            print('No Data')
            globalfuncs.setstatus(self.status,'No Data')
    ...
    def exportFFSpectraNext(self,result,maskchan):
        ...
        if self.exportFFplotonly:
            syv=transpose(syv)
            print (syv.shape)
            self.displaySpectrumGraph(0,dataover=syv)
            return
...
#################################  PCA routines
    ...
    def pcamininext(self,result):
        ...
        self.PCAdataStruct = PCAAnalysisMathClass.PCADataStructure(pcarawdata,self.PCAcompMAXNO,self.imgwin,
                                                                   ...
                                                                   chdialog = self.getChannelNameViaDialog,
                                                                   classifier = self.PCA_cluster_classifier)
...
#################################  Save data routines
    ...
    def asksavemanyOMEdisplays(self):
        self.savemultiType='ome' 
        self.genericsavemanydisplays()

    def asksavemanytoGIF(self):
        self.savemultiType='gif'     
        self.genericsavemanydisplays()

    def asksavemanydisplays(self):
        self.savemultiType='def' 
        self.genericsavemanydisplays()
        
    def genericsavemanydisplays(self):
        ...
        if not self.hasdata:
            ...
            globalfuncs.setstatus(self.status, 'No Data')
            return
        if self.savemultiType=='def':
            bs=('Standard','HighDef', 'Cancel')
        elif self.savemultiType=='ome':
            bs=('OME Tiff', 'Cancel')
        else:
            bs=('AnimatedGIF', 'Cancel')
        self.multisavedialog=Pmw.SelectionDialog(self.imgwin, title="Save Multiple Displays",
                                                     buttons=bs, defaultbutton='Cancel',
        ...
        if result in ['Standard','HighDef']:
            ...
            for going in goner:
                ...
                if result=='AnimatedGIF':
                    self.domapimage(datind=datind)
                    imgstack.append(ImageTk.getimage(self.maindisp.image))
                else:
                    imgstack.append(self.mapdata.data.get(datind))
            if result=='AnimatedGIF':
                fo=imgstack[0]
                fo.save(fb+"_animated.gif", format="GIF", append_images=imgstack,save_all=True, duration=100,loop=0)
                globalfuncs.setstatus(self.status,"Image display animation saved in: "+fn+"_animated.gif")
            else:
                curXres=int(abs(self.mapdata.xvals[2]-self.mapdata.xvals[1])*100000)/100000.
                curYres=int(abs(self.mapdata.yvals[2]-self.mapdata.yvals[1])*100000)/100000.
                metadata = {
                ...
                for going in goner:
                    nd={"Name":going,
                        "SamplesPerPixel":1,
                        "ExcitationWavelength":self.mapdata.energy,
                ...
                writer.write()                
                globalfuncs.setstatus(self.status,"Image OME data saved in: "+fn)
...
################################# UV-MCA Colorspaces
    ...
    def colorConvertVars(self,var=None):
        self.colorspaceChange = {
            'lab':cv.COLOR_RGB2LAB,
            'hsv':cv.COLOR_RGB2HSV,
            'ycc':cv.COLOR_RGB2YCrCb,
            'xyz':cv.COLOR_RGB2XYZ}
        self.colorspaceBasenames= {
            'lab':['L','A','B'],
            'hsv':['H','S','V'],
            'ycc':['Y','Cr','Cb'],
            'xyz':['cX','cY','cZ']}
        if var is not None: self.masterColorChangeKey=var
    
    def convCamLAB(self):
        self.masterColorChangeKey='lab'
        self.convCamGeneric()

    def convCamHSV(self):
        self.masterColorChangeKey='hsv'
        self.convCamGeneric()

    def convCamYCrCb(self):
        self.masterColorChangeKey='ycc'
        self.convCamGeneric()

    def convCamXYZ(self):        
        self.masterColorChangeKey='xyz'
        self.convCamGeneric()

    def convCamGeneric(self):
        #ask for color channels...
        pass
        self.chancolconvdict={}
        self.chancolCCSdialog=Pmw.Dialog(self.imgwin,title='Correlate Color RGB Map Channels',buttons=('OK','Cancel'),defaultbutton='OK',
                                     command=self.chancoldone)
        inter = self.chancolCCSdialog.interior()
        self.colcd={}
       
        for q in ['RED','GREEN','BLUE']:
            cb=Pmw.ComboBox(inter,label_text=q,labelpos='w',history=0,scrolledlist_items=self.mapdata.labels,dropdown=1)
            cb.pack(side=tkinter.TOP,padx=5,pady=5)
            if q in self.mapdata.labels:
                cb.selectitem(q,setentry=1)
            self.colcd[q]=cb
        alist = list(self.colcd.values())  
        Pmw.alignlabels(alist)
        self.chancolCCSdialog.show()
       
    def chancoldone(self,result):
        if result=='Cancel':
            print('Color calculation cancelled')
            self.chancolCCSdialog.withdraw()
            return    
        #check validity
        for n in list(self.colcd.keys()):
            if self.colcd[n].get()=='':
                print('Need all channels correlated')
                return
            else:
                self.chancolconvdict[n]=self.colcd[n].get()       
        self.chancolCCSdialog.withdraw()
        self.chanCamColorConvert()
        
    def chanCamColorConvert(self,over=None,typeover=None):
        if over is not None:
            self.chancolconvdict=None
        self.colorConvertVars(typeover)
            
        #convert
        globalfuncs.setstatus(self.status,'Doing color conversion analysis...')  
        
        
        rgbimg = np.zeros((self.mapdata.data.get(0).shape[0],self.mapdata.data.get(0).shape[1],3))
        rgbimg[:,:,0] = self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['RED'])+2)
        rgbimg[:,:,1] = self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['GREEN'])+2)
        rgbimg[:,:,2] = self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['BLUE'])+2)
    
        newlab = cv.cvtColor(rgbimg.astype(dtype=np.uint8), self.colorspaceChange[self.masterColorChangeKey])
    
        
#        newlab = np.zeros((self.mapdata.data.get(0).shape[0]*self.mapdata.data.get(0).shape[1],3))
#        rraw = np.ravel(self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['RED'])+2))
#        graw = np.ravel(self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['GREEN'])+2))
#        braw = np.ravel(self.mapdata.data.get(self.mapdata.labels.index(self.chancolconvdict['BLUE'])+2))
#       
#        for i in range(len(rraw)):
#        #first convert to xyz, then to lab        
#            xyz = colormodels.xyz_from_rgb((rraw[i],graw[i],braw[i]))
#            lab = colormodels.lab_from_xyz(xyz)
#            newlab[i,0]=lab[0]
#            newlab[i,1]=lab[1]
#            newlab[i,2]=lab[2]
#        newlab=newlab.reshape((self.mapdata.data.get(0).shape[0],self.mapdata.data.get(0).shape[1],3))

        globalfuncs.setstatus(self.status,'Color conversion analysis complete...')  
        #save
        #basenames=['L','A','B']
        for basename in self.colorspaceBasenames[self.masterColorChangeKey]:
            newname=globalfuncs.fixlabelname(basename)                
            ind=1
            ok=False            
            while not ok:    
                if newname in self.mapdata.labels:
                    ...
                    ok=True
            #add the channel
            self.addchannel(newlab[:,:,self.colorspaceBasenames[self.masterColorChangeKey].index(basename)],newname)          
        globalfuncs.setstatus(self.status,'Color conversion analysis saved...')  
...
#################################  View colormaps
    ...
    def getChannelNameViaDialog_b(self,result):
        ...
        print (self.PCA_cluster_classifier)
        self.channamedialog.deactivate()
