...
class Key(object):
    ...
    def _send_file_internal(self, fp, method="PUT", headers=None, cb=None, num_cb=10,
        ...
        if self.bucket.connection.local_encrypt and crypt_context:
            ...
            if self.bucket.connection.enable_crc:
                ...
                if not utils.check_crc(self.client_crc, resp.getheader(self.provider.checksum_crc64ecma_header)):
    ...
    async def download_file_async(self, filename, byte_range=None, headers=None,
        ...
        # 使用线程池执行同步函数，实现异步效果
        with ThreadPoolExecutor() as pool:
            ret = await loop.run_in_executor(pool, lambda: self.get_contents_as_string(
    ...
    def restore_object(self, headers=None):
        return self.bucket.restore_object(self.name, headers=headers)
