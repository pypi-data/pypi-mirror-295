...
class MyFeed2(bt.feeds.DataBase):
    params = (
        ('volume', 'vol'),
        ('openinterest', None),
    ...
    def __init__(self):
        ...
        super(MyFeed2, self).__init__(fromdata=datetime.strptime(self.p.start_time, '%Y-%m-%d %H:%M:%S').replace(second=0, microsecond=0),
    ...
    def _load(self):
        ...
        print(f"_load, start_time_str: {start_time_str}, end_time_str: {end_time_str}")
        ...
        if key not in self.cache:
            ...
            if self.p.debug:
                print(f"MyFeed2, fetch data params: {params}")

            response = requests.post(self.p.url, params=params).json()
            if self.p.debug:
                print(f"MyFeed2, fetch data response: {response}")
