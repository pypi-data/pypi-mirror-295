...
async def setup_agent(settings):
    ...
    new_model = settings["model_selector"]
    
    if current_model != new_model:
        cl.user_session.set("current_model", new_model)
        await cl.Message(content=f"模型已切換至 {new_model}").send()
    
    current_audio_setting = cl.user_session.get("audio_reply_enabled", False)
    new_audio_setting = settings["audio_reply_enabled"]
    
    if current_audio_setting != new_audio_setting:
        cl.user_session.set("audio_reply_enabled", new_audio_setting)
        if new_audio_setting:
            await cl.Message(content="語音回覆功能已啟用。系統將以語音形式回覆您的訊息。").send()
        else:
            await cl.Message(content="語音回覆功能已停用。系統將以文字形式回覆您的訊息。").send()
