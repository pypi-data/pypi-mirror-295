...
async def test_cairo():
    try:
        ...
        file_png, t = await text_layout(f"{text[:256]}..")
        print(file_png, t)
        image = Image.open(file_png)
        image = await correct_orientation(image)
        file_name_part = datetime.datetime.utcnow().strftime(f'%d-%m-%Y_%H-%M-%S-%f.webp')
...
Мы предлагаем уникальную возможность встретиться с единомышленниками и обсудить самые актуальные темы."""
        ...
        # county for man. Of it up he still court alone widow seems. Suspected he remainder rapturous my sweetness. All vanity regard sudden nor simple can. World mrs and vexed china since after often.
        #
