...
class WeDroid:
    ...
    def __init__(self, config_path=None, translation_path=None, store_ini='none'):
        ...
        mod_err_msg = ('When used as a module/package `wedroid_settings.json` must either be present in the current '
                       'directory, in the users home directory, or passed in manually with `-c /path/to/wedroid_settings.json`')

        # Setup a default configuration from wedroid_settings should be in: <project-root>
        if not config_path:
            config_path = os.path.abspath(self.run_path + '/../wedroid_settings.json')

        # if not config in current directory?
        if not os.path.isfile(config_path):
            home = expanduser("~")
            config_path = os.path.abspath(home + '/wedroid_settings.json')
        ...
        if os.path.isfile(config_path):
            self.logger.cli_log(f"Config loaded from: {config_path}", 'debug')
        ...
        self.announce_humidity = self.wedroid_settings['announce_humidity']
        ...
        self.log_gps_coords = True
        if 'log_gps_coords' in self.wedroid_settings:
            self.log_gps_coords = self.wedroid_settings['log_gps_coords']
    ...
    def get_location(self):
        ...
        log_current_long = current_long
        log_current_lat = current_lat

        # If log_gps_coords is False in wedroid_settings.json
        if not self.log_gps_coords:
            log_current_long = "hidden"
            log_current_lat = "hidden"

        # Log location data source
        msg = f'{msg} using {self.location_source} location data: latitude: {log_current_lat} longitude: {log_current_long}'
        self.logger.cli_log(msg, 'info')

        # If last known GPS failed or device location prohibited get stored location data.
        if current_loc:
    ...
    def generate_weather_announcement(self):
        ...
        if lc is not False:
            ...
            # Humidity message addition
            if self.announce_humidity:
                report = report + msg + ". "
...
def main():
    ...
    parser.add_argument("-d", "--defaults", help="Generate a `wedroid_settings.json` at the current location",
                        default=False, type=bool, action=argparse.BooleanOptionalAction)
    ...
    default_config = args.defaults
    current_dir = os.getcwd()
    WEDROID_CONFIG = f"{current_dir}/wedroid_settings.json"

    # If default_config defined then generate template configuration and exit.
    if default_config:
        cfh = ConfigurationHelper()
        cfh.create_configuration(current_dir)
        logger.cli_log(f"Generated new configuration file at: `{current_dir}/wedroid_settings.json`")
        exit()
