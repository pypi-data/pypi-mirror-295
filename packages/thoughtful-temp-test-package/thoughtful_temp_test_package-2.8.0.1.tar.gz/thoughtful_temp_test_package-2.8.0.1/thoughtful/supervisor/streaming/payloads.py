...
class StartProcessPayload(Payload):
    """
    Payload for notifying the stream consumer of a request to start another process
    """

    def __init__(self, run_id: str, process_id: int, inputs: dict):
        self.process_id = process_id
        self.inputs = inputs
        super().__init__(run_id, action=Action.START_PROCESS)

    def __json__(self) -> dict:
        _json = super().__json__()
        _json["payload"]["process_id"] = self.process_id
        _json["payload"]["inputs"] = self.inputs
        return _json
