...
def forecaster(tags, api_key, lat=None, lon=None):
    ...
    def update_graph(tag):
        ...
        df = tags[tag]
        x = requests.post('http://127.0.0.1:5000/forecast', json={'df':df.to_json(orient='records', date_format='iso'), 'api_key':api_key, 'lat':lat, 'lon':lon})
        js = x.json()
...
def leak_detector(tags, unit, api_key):
    ...
    def loop(pathname):
        ...
        for tag in tags:
            df = tags[tag]
            x = requests.post('http://127.0.0.1:5000/leakage', json={'df': df.to_json(orient='records', date_format='iso'), 'unit':unit, 'api_key':api_key})
            js = x.json()
            # fig = plotly.io.from_json(json.dumps(js))
            fcst = pd.read_json(json.dumps(js), orient='records')
...
def blockage_detector(tags, api_key):
    ...
    def loop(pathname):
        ...
        for tag in tags:
            df = tags[tag]
            x = requests.post('http://127.0.0.1:5000/blockage',
...
def inflow_infiltration(tags, api_key, unit=None, population=None, personal_daily_volume=None):
    ...
    @app.callback(
        [Output('anomaly-plot', 'figure'), Output('volume-plot','figure')],
        [Input('tag-dropdown', 'value')]
    )
    def update_graph(tag):
        import requests
        ...
        df = tags[tag]
        x = requests.post('http://127.0.0.1:5000/inflow', json={'df':df.to_json(orient='records', date_format='iso'), 'api_key':api_key})
        js = x.json()
        fcst = pd.read_json(json.dumps(js), orient='records')

        if population is not None and personal_daily_volume is not None and unit is not None:
            u = unit.split('/')[-1]
            if u == 's':
