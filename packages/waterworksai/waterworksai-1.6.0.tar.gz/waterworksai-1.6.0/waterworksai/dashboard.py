...
def forecaster(tags, api_key, weather=None, production=None, jupyter_mode=None):
...
def leak_detector(tags, api_key, unit, production=None, jupyter_mode=None):
    ...
    def loop(pathname):
        ...
        for tag in tags:
            df = tags[tag]
            x = requests.post('https://www.waterworks.ai/api/leakage', json={'df': df.to_json(orient='records', date_format='iso'), 'unit':unit, 'api_key':api_key})
            js = x.json()
            # fig = plotly.io.from_json(json.dumps(js))
            fcst = pd.read_json(json.dumps(js), orient='records')
...
def inflow_infiltration(tags, api_key, unit=None, population=None, personal_daily_volume=None, production=None, jupyter_mode=None):
