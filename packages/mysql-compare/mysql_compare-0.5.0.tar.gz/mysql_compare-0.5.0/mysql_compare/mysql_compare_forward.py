...
class MysqlTableCompareForward:
    ...
    def run(self):
        ...
        with connect(**self.source_dsn) as source_con, connect(**self.target_dsn) as target_con:
            ...
            if source_table_struct != target_table_struct:
            ...
            if len(source_table_pkeys) == 0:
                source_table_pkeys = source_table_ukeys

            self.table_keys = source_table_pkeys.copy()

            if len(self.table_keys) == 0:
                self.logger.error("The primary keys or unique keys not exists.")
                raise Exception()

            if len(target_table_pkeys) == 0:
                target_table_pkeys = target_table_ukeys

            if self.table_keys != target_table_pkeys:
        ...
        with connect(**self.source_dsn) as source_con, connect(**self.target_dsn) as target_con:
            while True:
                ...
                if len(diff_rows) >= 1:
                    ...
                    if self.is_repair:
                        self.logger.debug(f"repair after sleep {self.repair_wait}s")
                        time.sleep(self.repair_wait)
                        _sleep_time = self.repair_wait
                ...
                for diff_row in diff_rows:
                    ...
                    if len(diff_rows_twice) == 1:
                        ...
                        if self.is_repair:
                            repair_row(target_con, self.dst_db, self.dst_tab, diff_rows_twice[0])
...
def parse_args(external_args=None):
    ...
    parser.add_argument("--repair", action="store_true", help="Repair the tables")
    parser.add_argument("--repair-wait-time", type=int, default=0, help="Repair the tables")
...
def run(args=None):
    ...
    arg_repair_wait: int = args.repair_wait_time
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=arg_parallel) as executor:
        for src_db, src_tab, dst_db, dst_tab in process_tables:
            compare_name = f"{src_db}.{src_tab}@{dst_db}.{dst_tab}.forward"

            if os.path.exists(f"{compare_name}.err.log.done"):
                os.remove(f"{compare_name}.err.log.done")
            if not os.path.exists(f"{compare_name}.log.done"):
                mtc = MysqlTableCompareForward(compare_name, src_dsn, dst_dsn, src_db, src_tab, dst_db, dst_tab, arg_batch_size, arg_repair, arg_repair_wait)
                future_to_task[executor.submit(mtc.run)] = f"{compare_name}"

        for future in concurrent.futures.as_completed(future_to_task):
            compare_name = future_to_task[future]
            compare_success += 1
            try:
                future.result()
