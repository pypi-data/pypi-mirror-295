...
def xml2d(e, attribute_prefix=None, group_lists=False, keep_tags=False):
    ...
    def _guess_str_to_native(val: str) -> object:
        try:
    ...
    def _cast_str_to_native(val: str, d: None | dict) -> object:
        if d is None:
            # d==None means attribute value; guess the native type
            return _guess_str_to_native(val)
    ...
    def _xml2d(e):
        ...
        if attribute_prefix is not None:
            kids = {
                f"{attribute_prefix}{k}": _cast_str_to_native(v, None) if k != "value" else v
                for k, v in e.attrib.items()
                if e.tag != "tag" or k != "name" or keep_tags
            }
        else:
            kids = {
                k: _cast_str_to_native(v, None) if k != "value" else v
                for k, v in e.attrib.items()
                if e.tag != "tag" or k != "name" or keep_tags
            }
        # map text
