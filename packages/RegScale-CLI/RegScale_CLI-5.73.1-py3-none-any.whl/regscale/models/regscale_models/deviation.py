...
class Deviation(RegScaleModel):
    ...
    def _get_additional_endpoints() -> ConfigDict:
        ...
        return ConfigDict(  # type: ignore
            get_by_issue="/api/{model_slug}/getbyIssue/{intParentID}",
            export_deviation_by_ssp="/api/{model_slug}/getAllByPart/{intParentID}/{strModule}/{strType}/{strPart}",
        )

    @classmethod
    def get_by_issue(cls, issue_id: int) -> Optional["Deviation"]:
        """
        ...
        """
        response = cls._api_handler.get(endpoint=cls.get_endpoint("get_by_issue").format(intParentID=issue_id))
