...
MSTOpt::MSTOpt(const vector<vector<double> > & _pair_wise_distances, int _num_bins, int nested_level) 
    ...
    if (!(pair_wise_distances.size() == number_of_bins)) {
        throw std::runtime_error("Assertion failed: pair_wise_distances.size() == number_of_bins");
    }
...
double MSTOpt::calculate_MST()
{
    ...
    for (int ii = 0 ; ii < number_of_bins ; ii++)
    {
        ...
        */
        ...
        if (!(closest_vertex != -1)) {
            throw std::runtime_error("Assertion failed: closest_vertex != -1");
        }
...
bool MSTOpt::dis_locate()
{
    ...
    pair<int,int> * crt_order = new pair<int,int>[number_of_bins];
    ...
    if (!(crt_cost < current_upper_bound + ZERO_PLUS)) {
        throw std::runtime_error("Assertion failed: crt_cost < current_upper_bound + ZERO_PLUS");
    }
...
MSTOpt::Block_Chain MSTOpt::break_into_blocks(){
    ...
    if (!(total_bins == number_of_bins)) {
        throw std::runtime_error("Assertion failed: total_bins == number_of_bins");
    }
...
void MSTOpt::block_fix_orientation(Block_Chain& bc) {
    ...
    if (!(new_cost + total_cost_reduction < initial_cost + ZERO_PLUS)) {
        throw std::runtime_error("Assertion failed: new_cost + total_cost_reduction < initial_cost + ZERO_PLUS");
    }
...
bool MSTOpt::block_optimize_iteration(Block_Chain& bc){
    ...
    if (!(new_cost < original_cost + total_incremental_cost + ZERO_PLUS)) {
        throw std::runtime_error("Assertion failed: new_cost < original_cost + total_incremental_cost + ZERO_PLUS");
    }
...
void MSTOpt::copy_over_order(const Block_Chain& bc){
    ...
    if (!(crt_marker_id == number_of_bins)) {
        throw std::runtime_error("Assertion failed: crt_marker_id == number_of_bins");
    }
...
void MSTOpt::sanity_check(){
    ...
    for (int ii = 0; ii < number_of_bins; ii++)
    {
        if (tmp[ii] != 1 )
        {
            throw std::runtime_error("ERROR, not a valid order. " + to_string(ii) + " appeared " + to_string(tmp[ii]) + " times");
    ...
    if (not ((tmp_upper_bound - current_upper_bound < epsilon) and (tmp_upper_bound - current_upper_bound > -epsilon))){
        ...
        throw std::runtime_error("ERROR, current_upper_bound is not correct. tmp_upper_bound:" + to_string(tmp_upper_bound) + " upper_bound:" + to_string(current_upper_bound));
    ...
    if (not ((tmp_lower_bound - MST_lower_bound < epsilon) and (tmp_lower_bound - MST_lower_bound > -epsilon))) {
        ...
        throw std::runtime_error("ERROR, the lowerbound is not correct. tmp_lower_bound:" + to_string(tmp_lower_bound) + " lower_bound:" + to_string(MST_lower_bound));
