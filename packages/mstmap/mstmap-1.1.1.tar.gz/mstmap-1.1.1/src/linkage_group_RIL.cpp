...
void RIL_dist_cal::count_class(){
    ...
    for (int ii = 0; ii < num_of_individuals_; ii++) {
        ...
        if (!(marker1_total < 1 + epsilon)) {
            throw std::runtime_error("Assertion failed: marker1_total < 1 + epsilon");
        }

        // assert(marker2_total < 1 + epsilon);
        if (!(marker2_total < 1 + epsilon)) {
            throw std::runtime_error("Assertion failed: marker2_total < 1 + epsilon");
        }
    ...
    if (!(total < num_of_eff_individuals_ + epsilon)) {
        throw std::runtime_error("Assertion failed: total < num_of_eff_individuals_ + epsilon");
    }
...
void RIL_dist_cal::expected_CDEFG(double delta,
    ...
    if (!(generation_index_ > 1)) {
        throw std::runtime_error("Assertion failed: generation_index_ > 1");
    }
    double CC = 0.0;
    double DD = 0.0;
    double EE = 0.0;
    ...
    if (!(total < 1 + epsilon)) {
        throw std::runtime_error("Assertion failed: total < 1 + epsilon");
    }
...
double RIL_dist_cal::squared_error(double delta) const {
    ...
    if (!(CC1 + DD1 + EE1 + FG1 < 1 + epsilon)) {
        throw std::runtime_error("Assertion failed: CC1 + DD1 + EE1 + FG1 < 1 + epsilon");
    }
...
//      MSTOpt opt_iter(pair_wise_distances, number_of_bins);
        ...
        if ((order_changed) and (missing_data.size() > 0) and (current_upper_bound <  last_upper_bound + ZERO_MINUS )) {
            ...
            // Condition added by Amir
            if (!quiet) cout << "start a new iteration" << endl;
        }
    }
};
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
