...
from cyberdrop_dl.utils.utilities import FILE_FORMATS, DEBUG_VAR, log_with_color, purge_dir, log
...
class Sorter:
    ...
    async def sort(self) -> None:
        ...
        await log("Generating folder list", 20)
        for folder in download_folders:
            try:
                ...
                if e.__class__ == ValueError:
                    continue
                await log(f"Error: {e}\n\nfolder: {folder}\ndownload_dir: {self.download_dir}\nrelative_folder: {relative_folder}", 40)
                raise e
        ...
        await log("Sorting files", 20)
        for folder in self.download_dir.iterdir():
            if not folder.is_dir():
                await log(f"Skipping: {folder.name} - Not a folder", 20)
                continue
            if folder not in download_folders and self.sort_cdl_only:
                await log(f"Skipping: {folder.name} - Not downloaded by CDL", 20)
                continue
            
            await log(f"Started scanning folder: {folder.name}", 20)
            files = await self.find_files_in_dir(folder)
            await log(f"Finished scanning folder: {folder.name}", 20)
            if DEBUG_VAR:
                await log(f"Found {len(files)} files in folder: {folder.name}", 20)
            await log(f"Starting sorting files in folder: {folder.name}", 20)
            for file in files:
            ...
            await log(f"Finished sorting files in folder: {folder.name}", 20)
