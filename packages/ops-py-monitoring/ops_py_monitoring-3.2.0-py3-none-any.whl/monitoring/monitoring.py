...
class Monitoring(object):
    ...
    def write_md_report(self):
        ...
        out = self.kv.get_summary_markdown()
        if out:
            with open(self.md_kv_summary_output_file, 'w') as f:
                f.write(out)

        out = self.kv.get_report_markdown()
        if out:
    ...
    def prepare_and_post(self):
        ...
        # The slack_post function is called to post the 'Slack Workflow reports' instead
        ...
        if ADDITIONAL_INFO:
            if not str(ADDITIONAL_INFO).startswith("["):
                ...
                try:
                    ...
                    logging.error(f"'ADDITIONAL_INFO' not valid: {str(ADDITIONAL_INFO)}")

        # If Teams output, we add the additional info (if any) as facts
        if self.teams_output:
            payload = json.loads(self.payloads[0])
            if additional_info:
            ...
            self.success = post_payloads(self.msg_handler, [payload])
            return

        # If payloads not posted to team we proceed with posting to the Slack app webhook

        # Add the additional info (if any) to the summary post
        if additional_info:

            # Get the summary payload (the first one)
            summary = self.payloads[0]
...
def main():
    ...
    parser.add_argument("-W", "--ssl_warning_threshold", type=int, default=30,
