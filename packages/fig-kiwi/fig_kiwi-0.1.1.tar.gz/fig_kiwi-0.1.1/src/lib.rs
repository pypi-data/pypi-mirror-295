...
use std::{collections::HashMap, error::Error, fs::File, io::{Read, Seek, SeekFrom}};
...
fn read_fig<'a, R: Read>(py: Python<'a>, mut fig: R, type_converters: &'a PyDict) -> Result<PyObject, Box<dyn Error>> {
    ...
    // detect the content type of the segment
    let mut magic: [u8; 4] = [0; 4];
    fig.read_exact(&mut magic)?;
    
    let segment = magic.chain(fig.by_ref()).take(segment_len.into());
    let segment_reader: Box<dyn Read> = if magic == ZSTD_MAGIC {        
        Box::new(zstd::stream::read::Decoder::new(segment)?)
    } else {
        Box::new(flate2::read::DeflateDecoder::new(segment))
    };

    let kiwi = KiwiReader::new(segment_reader.bytes());
    ...
    let mut kiwi = KiwiReader::new(segment_reader.bytes());
