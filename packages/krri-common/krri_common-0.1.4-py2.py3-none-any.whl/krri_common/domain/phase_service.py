...
class PhaseService:
    ...
    def batch_process_phase(self, data_batch: List[Tuple[int, bytes]], expiration_time: int):
        """
        ...
        zadd_data = {phase_data: timestamp for timestamp, phase_data in data_batch}

        # Use pipeline to execute multiple commands in a batch
        with self.client.pipeline() as pipe:
            # Add multiple elements to the sorted set
            pipe.zadd("phase", zadd_data)

            # Set expiration time for each element in the sorted set
            for timestamp, phase_data in data_batch:
                pipe.expireat("phase", timestamp + expiration_time)

            # Execute all commands in the pipeline
            pipe.execute()

    def get_phase(self, start_timestamp: int, end_timestamp: int) -> (int, np.ndarray):
        """
        ...
        """
        group_of_phase_data = self.client.zrangebyscore("phase", start_timestamp, end_timestamp)  # List[key]
        start_of_phase_data = group_of_phase_data[0]
        start_timestamp_of_phase_data = self.client.zscore("phase", start_of_phase_data)
