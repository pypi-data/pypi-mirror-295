...
from ..data import (Surface, EdgeNetwork, Marker, Text2D, Text3D, Container,
                    ...
                    Ellipsoid, FilledPolygon, ChangeReasons, BlockModelDefinition)
...
class Project:
  ...
  def __init__(
    ...
    try:
      ...
      if isinstance(existing_mcpd, McpdConnection):
        self.backend.use_existing_connection(existing_mcpd)

      self.__load_dlls_with_dataengine_types()
      self.backend.connect_to_project()
  ...
  @property
  def _data_engine_api(self) -> capi.DataEngine:
    """Access the DataEngine C API."""
    return capi.DataEngine()

  @property
  def _modelling_api(self) -> capi.Modelling:
    """Access the Modelling C API."""
    return capi.Modelling()

  @property
  def _selection_api(self) -> capi.Selection:
    """Access the Selection C API."""
    return capi.Selection()

  @property
  def _scan_api(self) -> capi.Scan:
    """Access the Scan C API."""
    return capi.Scan()

  @property
  def _viewer_api(self) -> capi.Viewer:
    """Access the Viewer C API."""
    return capi.Viewer()

  @property
  def _vulcan_api(self) -> capi.Vulcan:
    """Access the Vulcan C API."""
    return capi.Vulcan()

  @property
  def _drillhole_model_api(self) -> capi.DrillholeModel:
  ...
  def add_objects(
      ...
      new_objects: Iterable[tuple[str, ObjectID[DataObject]]] |
    ...
    try:
      object_ids_to_add = [
        ...
        for name, new_object in new_objects
      ]
  ...
  def copy_object(
    ...
    with ReadLock(old_handle, self._data_engine_api) as r_lock:
      ...
      if not copyobj:
        last_error = self._data_engine_api.ErrorMessage().decode(
          "utf-8")
  ...
  def rename_object(
    ...
    if old_parent and old_path:
      ...
      if old_parent.is_orphan:
        ...
        except HiddenObjectPermissionError as error:
  ...
  def _delete(
    ...
    success = self._data_engine_api.DeleteObject(mdf_object)
    if not success:
      error = self._data_engine_api.ErrorMessage().decode("utf-8")
  ...
  def _type_for_object(
    ...
    object_type = self._data_engine_api.ObjectDynamicType(object_handle)
  ...
  def is_recycled(self, mdf_object: DataObject | ObjectID[DataObject]) -> bool:
    ...
    """
    mdf_object = self.__get_obj_handle(mdf_object)
    return self._data_engine_api.ObjectHandleIsInRecycleBin(mdf_object)
  ...
  def project_path(self) -> pathlib.Path:
    ...
    """
    path_string = self._data_engine_api.ProjectPath(self.backend.index)
    if path_string:
  ...
  def _get_before_state(
    ...
    # Note that oid.is_a(Container) is almost always True.
    # pylint: disable=protected-access
    if (oid.is_a((VisualContainer, StandardContainer))
