...
from .colour_scheme import _ColourScheme
...
if typing.TYPE_CHECKING:
  from mapteksdk.capi import Modelling, Sdp
...
class PrimitiveAttributes(MutableMapping[AttributeKey, np.ndarray]):
  ...
  def __getitem__(self, name: str | AttributeKey) -> np.ndarray:
    ...
    if not isinstance(name, AttributeKey):
      ...
      key = name
    return self.__attributes[key].values
  ...
  def __setitem__(self, name: str | AttributeKey, value: npt.ArrayLike):
    ...
    try:
      self.__attributes[key].values = value
    except KeyError:
      try:
        ...
        # There are no attributes with the same name, so it should
        # be safe to add it.
        pass
  ...
  def __contains__(self, name: str | AttributeKey) -> bool:
  ...
  def _apply_colour_scheme(
    self,
    attribute: AttributeKey | str,
    colour_scheme: _ColourScheme
  ):
    ...
    if not isinstance(colour_scheme, _ColourScheme):
      raise TypeError(
  ...
  def _modelling_api(self) -> Modelling:
    # pylint: disable=protected-access
    return self.owner._modelling_api()

  def _sdp_api(self) -> Sdp:
