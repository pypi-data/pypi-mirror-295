...
class Container(DataObject):
  ...
  def remove(self, name: str) -> ObjectID[DataObject]:
    ...
    self._data_engine_api().ContainerRemove(
      self._lock.lock, name.encode('utf-8'))
  ...
  def _children(self, include_hidden: bool):
    ...
    iterator = self._data_engine_api().ContainerBegin(lock)
    end = self._data_engine_api().ContainerEnd(lock)
    while iterator.value != end.value:
      buf_size = self._data_engine_api().ContainerElementName(
        lock,
        iterator,
        None,
        0)
      name_buffer = ctypes.create_string_buffer(buf_size)
      self._data_engine_api().ContainerElementName(lock,
                                        iterator,
                                        name_buffer,
                                        buf_size)
      ...
      if _should_include_child(name):
        object_id = ObjectID(
          self._data_engine_api().ContainerElementObject(lock, iterator))
