...
class PSO(object):
    ...
    def initialize_pop(self, feedin_solutions=None):
        ...
        np.savetxt(self.save_path + 'initial_pop.txt',
                   self.current_pop)
        self.record()
    ...
    def save_current_best(self):
        ...
        np.savetxt(self.save_path + 'current_best_solution.txt',
                   self.current_pop[i_best, :])
        np.savetxt(self.save_path + 'current_hist.txt',
                   self.hist)
        with open(self.save_path + 'current_info.txt', 'w') as f:
            f.write('gen: {0}\n'.format(self.i_gen)  + 
