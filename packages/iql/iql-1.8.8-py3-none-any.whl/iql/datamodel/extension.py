...
class IqlExtension:
    ...
    def apply_pandas_operations_postpivot(self, working_df, options: Dict[str, object]) -> DataFrame:
        ...
        ewma_opt: Optional[Tuple[str, str, int, int]] = options.get("ewma")
        if ewma_opt is not None:
            src, dest, halflife, min_periods = ewma_opt
            working_df[dest] = working_df[src].ewm(halflife=halflife, min_periods=min_periods).mean()
