...
class NyxClient:
    ...
    def update_subscriptions(self, refresh=True):
        ...
        if not purchases:
            ...
            return
        self._subscribed_products = tuple(k["product_name"] for k in purchases)
    ...
    @property
    def subscribed_products(self) -> tuple[str]:
    ...
    def get_subscribed_datasets(self) -> list[NyxProduct]:
        ...
        Returns:
            A list of `NyxProduct` instances.
        """
        query = f"""
        {_SELECT}
        WHERE {{
    ...
    def get_subscribed_datasets_for_categories(self, categories: list[str]) -> list[NyxProduct]:
        ...
        Returns:
            A list of `NyxProduct` instances matching the specified categories.
        """
        query = f"""
        {_SELECT}
        WHERE {{
    ...
    def get_subscribed_datasets_for_genres(self, genres: list[str]) -> list[NyxProduct]:
        ...
        Returns:
            A list of NyxProduct instances matching the specified genres.
        """
        query = f"""
        {_SELECT}
        WHERE {{
    ...
    def get_subscribed_datasets_for_creators(self, creators: list[str]) -> list[NyxProduct]:
        ...
        Returns:
            A list of NyxProduct instances matching the specified creators.
        """
        query = f"""
        {_SELECT}
        WHERE {{
    ...
    def create_product(
        ...
        Args:
            name: The unique identifier for the product.
            title: The display title of the product.
            description: A detailed description of the product.
            size: The size of the product, typically in bytes.
            genre: The genre or category of the product.
            categories: A list of categories the product belongs to.
            download_url: The URL where the product can be downloaded.
            content_type: The mime type of the data located at download_url.
            lang: The language of the product. Defaults to "en".
            status: The publication status of the product. Defaults to "published".
            preview: A preview or sample of the product. Defaults to an empty string.
            price: The price of the product in cents. If 0, the product is free. Defaults to 0.
            license_url: The URL of the license for the product. Defaults to Creative Commons Zero.

        Returns:
            A `NyxProduct`, containing the download URL and title.
    ...
    def delete_product(self, product: NyxProduct) -> None:
