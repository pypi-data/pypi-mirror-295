...
class Parser:
    ...
    def dataset_as_db(
        ...
        Args:
            twins: A list of NyxProduct instances to process.
            ...
            if content:
                try:
                    if twin.content_type == "csv":
                        ...
                        log.warning("%s is unsupported type %s", twin.title, twin.content_type)
                        continue
                    ...
                    if twin.content_type in ["csv", "json", *Parser._excel_mimes]:
                        log.warning("%s could not be processed as a %s", twin.title, twin.content_type)
                except Exception as e:
                    log.warning("Unexpected error for %s: %s", twin.title, e)
