...
class NyxLangChain(NyxClient):
    ...
    def query(
        ...
        Args:
            ...
            include_own (bool): Include your own products, created in Nyx, in the query.
        ...
        if include_own:
            products.extend(self.get_datasets_for_creators(creators=[self.config.org]))
        log.debug("using products: %s", [p.title for p in products])
