...
class EasyDF:
    ...
    def append_from_dict(self, record: dict) -> 'EasyDF':
        # TODO: unionByName
        self._df = self._df.unionByName(EasyDF._spark.createDataFrame([Row(**record)]),
                                        allowMissingColumns=True)
        return self

    def append_from_row(self, row: Row) -> 'EasyDF':
        # TODO: unionByName
        self._df = self._df.unionByName(EasyDF._spark.createDataFrame([row]), allowMissingColumns=True)
