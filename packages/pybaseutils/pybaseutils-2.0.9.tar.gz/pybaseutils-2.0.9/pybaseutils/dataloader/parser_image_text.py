...
class TextDataset(Dataset):
    ...
    def __init__(self, filename, data_root=None, class_name=None, transform=None, shuffle=False, use_rgb=False,
        ...
        :param disp:
        :param check:
        :param kwargs: crop_scale,resample,save_info
        """
        self.kwargs = kwargs
    ...
    def __getitem__(self, index):
        ...
        image = self.crop_image(image, bbox=bbox, scale=self.kwargs.get("crop_scale", [1.2, 1.2])) if bbox else image
        if self.transform:
    ...
    def crop_image(self, image, bbox, scale=[1.2, 1.2]):
        """裁剪图片"""
        if not bbox: return image
        boxes = image_utils.get_square_boxes([bbox], use_max=True)
