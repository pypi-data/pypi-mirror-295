...
class TestAccount(BaseTest):
    ...
    def test_add_collaborator(self):
        response = self.sdk.accounts.add_collaborator(self.collaborator_email)
        assert response['member'] == self.collaborator_email
    ...
    def test_delete_collaborator(self):
        response = self.sdk.accounts.delete_collaborator(self.collaborator_email)
        assert response['member'] == self.collaborator_email
        accounts, _ = self.sdk.accounts.list()
        assert len(accounts) > 0
        assert any(a['member'] != self.collaborator_email for a in accounts)
