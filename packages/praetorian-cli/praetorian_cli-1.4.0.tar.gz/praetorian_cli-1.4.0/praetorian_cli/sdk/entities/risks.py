class Risks:
    ...
    def add(self, asset_key, name, status, comment=None):
        ...
        comment: str
            Optional comment
        """
        return self.api.update('risk', dict(key=asset_key, name=name, status=status, comment=comment))['risks'][0]

    def get(self, key, get_attributes=False):
        ...
        get_attributes: bool
            Specify whether to also retrieve the attributes associated with this risk. This will
            make one more API call.
        """
        return self.api.search.by_exact_key(key, get_attributes)
    ...
    def update(self, key, status=None, comment=None):
        ...
        return self.api.update('risk', params)
