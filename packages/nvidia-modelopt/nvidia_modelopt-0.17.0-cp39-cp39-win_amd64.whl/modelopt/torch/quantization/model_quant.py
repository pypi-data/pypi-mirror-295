...
def auto_quantize(
    ...
    Args:
        ...
        constraints: Constraints for the search. Currently we support ``weight_compression``.
            ``weight_compression`` constraint specifies the total weight size as a ratio of the un-quantized
            fp16/bf16 weight size.

            Here is an example for valid ``constraints`` argument:
        ...
        quantization_formats: A list of the string names of the quantization formats to search for.
            ...
            .. note::
                ...
                Currently `TensorRT-LLM <https://github.com/NVIDIA/TensorRT-LLM>`_ deployment does not support
                ``AutoQuantize`` with mixed quantization formats.

                For deployment to TensorRT-LLM, please specify a single format and None (for skipping quantization)
                as ``quantization_formats`` arguments. For example:

                .. code-block:: python

                    # A valid quantization_formats argument for deployment to TensorRT-LLM with `AutoQuantize`
