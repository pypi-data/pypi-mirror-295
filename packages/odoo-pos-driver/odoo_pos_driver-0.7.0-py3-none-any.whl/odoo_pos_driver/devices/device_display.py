...
class DeviceDisplay(DeviceAbstractWithQueue):
    ...
    def _convert_task_to_ascii_lines(self, task):
        ...
        if action == "display_lines":
            ...
            # First Line
            left_part = unidecode(task.data.get("product_name"))
            ...
            lines_ascii = [
                _("Deleting Line ..."),
                self._prepare_line(unidecode(task.data.get("product_name"))),
            ]
    ...
    def _send_message(self, serial, message_list, action_description=""):
        ...
        if not message_list:
            return
        message = b"".join([x.encode("ascii") for x in message_list])
