...
def merge_numeric_sublists(word_list):
    merged_list = []
    for word in word_list:
        if len(merged_list) == 0 or not isinstance(word, str):
            merged_list.append(word)
        else:
            last_word = merged_list[-1]
            if isinstance(last_word, str) and last_word[
                -1] not in string.punctuation and word not in string.punctuation:
                merged_list[-1] += word
            else:
                merged_list.append(word)
    return merged_list
...
def flatten_list(lst):
    new_list = ["".join(sublist) if isinstance(sublist, list) else sublist for sublist in lst]
    return new_list


def sort_word_box_list(word_box_list):
    sorted_word_box_list = sorted(word_box_list, key=lambda box: box[0])
    return sorted_word_box_list


def cn_cal_ocr_word_box(rec_str, box, rec_word_info):
    col_num, word_list, word_col_list, state_list = rec_word_info
    box = box.tolist()
    bbox_x_start = box[0][0]
    bbox_x_end = box[1][0]
    ...
    cn_col_list = []
    merged_word = ''
    merge_punctuation = False
    for idx, (word, word_col, state) in enumerate(zip(word_list, word_col_list, state_list)):
        if state == 'cn':
            if len(word_col) > 0:
                char_seq_length = (word_col[-1] - word_col[0] + 1) * cell_width
                char_width = char_seq_length / (len(word_col) - 1) if len(word_col) > 1 else char_seq_length
                cn_width_list.append(char_width)
                cn_col_list += word_col
                word_box_content_list += word
        else:
            if isinstance(word, str) and (word.isdigit() or word in ['-', '+', '.', ',', '，']):
                merged_word += word
                merge_punctuation = True
            else:
                if merge_punctuation:
                    word_box_content_list.append(merged_word)
                    merged_word = ''
                    merge_punctuation = False
                ...
                if len(word_col) > 0:
                    ...
                    cell = ((cell_x_start, bbox_y_start), (cell_x_end, bbox_y_start), (cell_x_end, bbox_y_end),
                            (cell_x_start, bbox_y_end))
                    word_box_list.append(cell)
                    word_box_content_list.append(word)

    if len(merged_word) > 0:
        word_box_content_list.append(merged_word)
