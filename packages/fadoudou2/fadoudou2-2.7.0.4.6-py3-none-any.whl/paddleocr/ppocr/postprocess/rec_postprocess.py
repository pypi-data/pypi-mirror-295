    ...
    def get_word_info(self, text, selection):
        ...
        for c_i, char in enumerate(text):
            if '\u4e00' <= char <= '\u9fff':
                ...
                c_state = state  # 标点符号与前一个字符同组
            else:
                c_state = 'splitter'

            if char == '.' and state == 'en&num' and c_i + 1 < len(text) and bool(
                    re.search('[0-9]', text[c_i + 1])):  # grouping floating number
                c_state = 'en&num'
    ...
    def __call__(self, preds, label=None, return_word_box=False, *args, **kwargs):
        ...
        if return_word_box:
            for rec_idx, rec in enumerate(text):
                ...
                if 'wh_ratio_list' in kwargs and 'max_wh_ratio' in kwargs:
                    wh_ratio_list = kwargs['wh_ratio_list']
                    max_wh_ratio = kwargs['max_wh_ratio']
                    if rec_idx < len(wh_ratio_list):  # 检查索引是否越界
                        wh_ratio = wh_ratio_list[rec_idx]
                        rec[2][0] = rec[2][0] * (wh_ratio / max_wh_ratio)
                    else:
                        print("Warning: rec_idx exceeds length of wh_ratio_list.")
                        pass
        if label is None:
            return text
