...
class TextRecognizer(object):
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            for i, rno in enumerate(range(len(rec_result))):
                if beg_img_no + i < len(indices):
                    rec_res[indices[beg_img_no + i]] = rec_result[rno]
                else:
                    print("Warning: Index out of bounds for rec_res.")
                    pass
            if self.benchmark:
...
def remove_punctuation(input_string):
    r = "[_.!+=\—,$%^，”。？、~@#￥%……&*《》<>「」{}【】（）()/\\\[\]'\"]"
    sentence = re.sub(r, '', input_string)
    return sentence
...
def main(args):
    ...
    # ******************************************************************************
    files = []
    with open(args.valid_dir, "r", encoding="utf-8") as f:
        for line in f:
            content = line.strip("\n").split()
            tup = (content[0], content[1])
            files.append(tup)
    ...
    for ino in range(len(img_list)):
        filename = os.path.basename(valid_image_file_list[ino])

        context = [val for fname, val in files if fname == filename]

        if not context:
            logger.info("File: {} not found!".format(filename))
            continue

        rec = remove_punctuation(rec_res[ino][0])
        answer = remove_punctuation(context[0])
        diff = dmp.diff_main(rec, answer)
        if len(answer) == 0 and len(diff) == 0:
            avg_f += fscore
            continue
        dmp.diff_cleanupSemantic(diff)
        fscore = diff_eval(diff)
        avg_f += fscore
        if fscore != 1.0:
            logger.info(f"Predicts of {valid_image_file_list[ino]}:{rec}, F-1 score: {fscore}, answer {answer}")
    logger.info("Average F-1 score: {}".format(avg_f / len(img_list)))
    # ******************************************************************************
