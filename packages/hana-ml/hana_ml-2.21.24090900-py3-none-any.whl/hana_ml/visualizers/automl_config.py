...
def save_code_by_python(frame_id, config_str):
    EmbeddedUI.generate_file(EmbeddedUI.get_resource_temp_dir_path() + os.sep + frame_id + '_config.json', config_str)
...
class AutoMLConfig(EmbeddedUI):
    ...
    def __init__(self, config_dict: Union[str, dict] = 'classification', iframe_height: int = 500) -> None:
        ...
        self.html_str = self.get_resource_template('automl_config.html').render(frameId=self.frame_id, operator_metadata=OPERATOR_METADATA, parameter_metadata=PARAMETER_METADATA, input_config_str=input_config_str, config_template_dict_str=config_template_dict_str)
        self.render_html_str(self.get_iframe_str(self.html_str, self.frame_id, iframe_height))

        self.runtime_platform = self.get_runtime_platform()
        time.sleep(2)
    ...
    def generate_html(self, file_name):
        ...
        file_name : str
            HTML file name.
        """
        self.generate_file('{}_automl_config.html'.format(file_name), self.html_str)
    ...
    def get_config_dict(self):
        ...
        # 1. send cmd to frontend
        if self.runtime_platform[1] in ['jupyter', 'vscode', 'bas']:
            js_str = "for (let i = 0; i < window.length; i++) {const targetWindow = window[i];if(targetWindow['frameId']){if(targetWindow['frameId'] === '" + self.frame_id + "'){" + "targetWindow['save_code_by_ui']();" + "}}}"
            self.execute_js_str("{};".format(js_str))
            time.sleep(1)

        # 2. send cmd to vscode extension
        if self.runtime_platform[1] in ['vscode', 'bas']:
            print('AutoML Config Temp Flag: {}: {}'.format(self.get_resource_temp_dir_path() + os.sep, self.frame_id))
            time.sleep(1)

        # 3. get code from temp file
        if self.runtime_platform[1] in ['jupyter', 'vscode', 'bas']:
            file_path = self.get_resource_temp_dir_path() + os.sep + self.frame_id + '_config.json'
            retry_count = 1
            while retry_count <= 3:
                if os.path.exists(file_path):
                    with open(file_path, 'r') as config_file:
                        code = json.load(config_file)
                        break
                else:
                    retry_count = retry_count + 1
                    time.sleep(5)

        # 4. handle code or print warnings
        if code:
            del code['frameId']
            ...
            if self.runtime_platform[1] in ['jupyter', 'vscode', 'bas']:
                ...
                if self.runtime_platform[1] in ['vscode', 'bas']:
                    print('\nIn order to successfully call get_config_dict on the VSCode or BAS platform, you must import the VSCode extension package manually.')
                    print('VSCode extension package path: \n{}'.format(self.get_resource_root_dir_path() + os.sep + 'hanamlapi-monitor-1.2.0.vsix'))
            else:
                print('Please click the button on the UI to get the latest config code!')
