...
class _AutoMLBase(PALBase):
    ...
    def _gen_predict_args_json(self, predict_args, model):
        ...
        if predict_args:
            ...
            elif all(all(x not in key for x in estimators_all) for key in list(predict_args.keys())):#estimator name missing, infer it from pipeline model info
                pipeline_head = model.sort('ROW_INDEX').collect()['MODEL_CONTENT'][0]
                p_estimator = (pipeline_head.split(':')[1]).strip()[2:-1]
                if p_estimator == 'TimestampConvertor':#possible with timestamp convertor
                    p_estimator = ((pipeline_head.split('"data":{')[1]).split(':')[0]).strip()[1:-1]
                effective_args = predict_args
                predict_args_json = '{"' + p_estimator + '":{'
                predict_args_map = self._predict_kwargs_map[p_estimator]
                for var_key, var_val in effective_args.items():
                    if var_key not in predict_args_map:
                        msg = "Variable '{}' is not supported, ".format(var_key) +\
                        "it is simply ignored."
                        logger.warning(msg)
...
#            self.progress_indicator_id = "AutoML-" + str(self.gen_id)
    ...
    def _predict(self, data, key=None, features=None, model=None,
        ...
        predict_args_json, no_valid_args = self._gen_predict_args_json(predict_args, model)
        if no_valid_args > 0:
