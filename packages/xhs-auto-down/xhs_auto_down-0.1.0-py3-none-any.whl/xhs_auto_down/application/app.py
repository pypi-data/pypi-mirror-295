...
####处理小红书作品链接
    async def extract(
        ...
        ######提取链接
        logging(log, self.message("##########链接提取--开始："+url), WARNING)
        urls = await self.__extract_links(url, log)
        logging(log, self.message("##########链接提取--完成："+str(urls)), WARNING)
        if not urls:
            logging(log, self.message("##########链接提取--失败："+str(urls)), WARNING)
            logging(log, self.message("提取小红书作品链接失败"), WARNING)
        else:
            logging(log, self.message("##########链接提取--成功："+str(urls)), WARNING)
            logging(
    ...
    async def __extract_links(self, url: str, log) -> list:
        urls = []
        for i in url.split():
            logging(log, self.message("url解析-----SHORT"+str(url.split())), WARNING)
            if u := self.SHORT.search(i):
                logging(log, self.message("url解析-----SHORT"+str(u.group())), WARNING)
            ...
            if u := self.SHARE.search(i):
                logging(log, self.message("url解析-----SHARE"), WARNING)
                urls.append(u.group())
            elif u := self.LINK.search(i):
                logging(log, self.message("url解析-----LINK"), WARNING)
                urls.append(u.group())
        logging(log, self.message("url解析完成"+str(urls)), WARNING)
        return urls
