...
class DataHandler:
    ...
    @staticmethod
    def mean_std_normalize_images(data_dir: str, read_by_txt: bool = True, debug_channel: bool = False):
        """
        获得所有图像总的均值和方差，默认读取txt文件，如果不存在则计算并保存到txt文件中。
        :param data_dir: 总数据集路径
        :param read_by_txt: 是否读取mean_std.txt文件，如果存在则直接读取，否则计算并保存到文件中，默认读取
        :param debug_channel: 在进行数组相加时，是否捕捉到异常，默认不捕捉
        :return:
        """
        ...
        for root, dirs, files in os.walk(data_dir):
            for file in files:
                if file.endswith(('.jpg', '.png', '.jpeg', '.JPG', '.PNG', '.JPEG', 'bmp', 'BMP')):
                    file_path = os.path.join(root, file)
                    img = Image.open(file_path)
                    img_array = np.array(img)
                    normalized_img_array = img_array / 255.0
                    total_pixels += normalized_img_array.size
                    try:
                        sum_normalized_pixels += np.sum(normalized_img_array, axis=(0, 1))
                    except:
                        if debug_channel:
                            print(f'捕捉到图像{file_path}异常！')
