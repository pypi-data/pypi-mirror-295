...
1. Example
    ...
    # Trade from optimism-USDC to USDC on a random chain
    cctt run --password abc --source optimism-USDC --destination-policy fixed:USDC

    # Explicitly set the source to nothing to force a random source token to be chosen. Set destination policy to random to randomly choose destination.
