...
class Order:
    ...
    def get_order_index_by_hash(self, tx_hash):
        ...
        if not order:
            self.logger.info(f"order {tx_hash} not received")
            return None
        else:
            order = self.get_order_by_hash(tx_hash)
            if order:
    ...
    def wait_order_received(self, tx_hash, timeout=60, poll_interval=1):
        ...
        self.logger.error(f"order {tx_hash} not received in {timeout}s")
        return False

    def wait_order_confirmed(self, tx_hash, timeout=60, poll_interval=1):
        start_time = time.time()
        while time.time() - start_time < timeout:
            order = self.get_order_by_hash(tx_hash)
        ...
        self.logger.error(f"order {tx_hash} not confirmed in {timeout}s")
        return False
