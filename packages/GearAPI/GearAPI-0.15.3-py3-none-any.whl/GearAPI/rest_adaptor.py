...
from models.rest_adaptor_models import Result
from utilis.utilis import retry
...
class RestAdaptor:
    ...
    def __init__(self, user:str, password:str, ssl_verify: bool = True):
        ...
        self.websocket_url = 'wss://t21092648.cumulocity.com'
    ...
    def _do(self, http_method: str, endpoint: str, ep_params: Dict = None, data: Dict = None, stream=None, headers=None, timeout=None) -> Result:
        ...
        try:
            response = requests.request(method=http_method, url=full_url, verify=self._ssl_verify,
                                        auth=HTTPBasicAuth(self.user, self.password), params=ep_params, json=data, stream=stream, headers=headers, timeout=timeout)
    ...
    # def base64_encoded_credential(self):
    #     auth = base64.b64encode(f"{self.tenant_id}/{self.user}:{self.password}".encode()).decode()
    #     return auth
