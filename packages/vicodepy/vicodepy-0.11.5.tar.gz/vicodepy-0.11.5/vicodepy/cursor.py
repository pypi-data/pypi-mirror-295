...
class Cursor(QGraphicsItem):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.pressed:
            time = int(
                pos.x() * self.time_pane.duration / self.time_pane.scene.width()
            )
            ...
            self.time_pane.window.set_video_position(time)

            if pos.x() < 0:
                self.setPos(0, 0)
            elif pos.x() > self.time_pane.scene.width():
                self.setPos(self.time_pane.scene.width(), 0)
            else:
                self.setPos(pos.x(), 0)
