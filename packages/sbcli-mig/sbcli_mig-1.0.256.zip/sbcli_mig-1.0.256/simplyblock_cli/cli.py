...
from simplyblock_core import grafana_ops
...
class CLIWrapper:
    ...
    def __init__(self):
        ...
        #----------------- Org -------------------
        #
        subparser = self.add_command('organization', 'oraganisation commands', aliases=['org'])
        sub_command = self.add_sub_command(subparser,"create","Create a grafana organization")
        sub_command.add_argument("--name", help='organization name/id')
        sub_command.add_argument("--cluster-id", help='cluster-id to map with organization')
        sub_command.add_argument("--api-endpoint", help='cluster api-gw endpoint')
        sub_command.add_argument("--cluster-secret",help='cluster secret')
    ...
    def run(self):
        ...
        if args.command in ['storage-node', 'sn']:
            ...
            if sub_command == 'create':
                ret = self.organization_create(args)
    ...
    def organization_create(self, args):
        org_name = args.name
        org_cluster = args.cluster_id
        api_endpoint= args.api_endpoint
        cluster_secret = args.cluster_secret
        return grafana_ops.create_grafana_organization(
            org_name,org_cluster,api_endpoint,cluster_secret
        )
