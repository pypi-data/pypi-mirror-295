...
def easy_tsne(data, labels, oids=None, point_mode: Literal["point", "digital", "id", "both"] = "both", show=True,
              save_path=None, font_size=5, figsize=(8, 8)):
    """
    ...
    labels_type_count = len(torch.unique(labels))
    data_size = len(data)
    # 将数据展平 (10000, 28*28)
    ...
    # 绘制 t-SNE 结果
    if point_mode == "id":
        scatter = plt.scatter(images_tsne[:, 0], images_tsne[:, 1], c=labels, cmap='rainbow', alpha=0.7, s=0)

        for i in range(data_size):
            plt.text(images_tsne[i, 0], images_tsne[i, 1], str(oids[i].item()),
                     color=plt.cm.rainbow(labels[i].item() / labels_type_count),
        ...
        scatter = plt.scatter(images_tsne[:, 0], images_tsne[:, 1], c=labels, cmap='rainbow', alpha=0.7)
