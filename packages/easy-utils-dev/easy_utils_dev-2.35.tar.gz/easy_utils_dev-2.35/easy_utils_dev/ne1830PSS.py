...
class PSS1830 :
    ...
    def connect(self , mode='cli' , neip=None , port=22  , user = 'admin', pw ='admin' , rootpw='ALu12#' ,  jumpserver = None , resetRequired = False, tmpPassword = None) :
    ...
    def cli_execute(self , command , wait=True ,ifPassword=False,expect=None) :
        '''
        ...
        if wait :
            result = self.wait_result(command,expect)
            return result
        # else :
        #     sleep(.75)
        #     try :
        #         self.screenBuffer+=self.channel.recv(99999999).decode('utf-8')
        #     except :
        #         pass
    ...
    def wait_result(self , command,expect, autoError = True) :
        ...
        while True:
            ...
            if new_data :
                ...
                if expect in data or self.prompt in data:   
                    ...
                    return_data = ''
                    for line in data2 :
                        if autoError and 'error' in line.lower():
    ...
    def getUsers(self) :
        self.cli_execute(f'show admin user *' , wait=False)
        sleep(3)
        a = self.buffer().splitlines()
        users=[]
        for line in a :
