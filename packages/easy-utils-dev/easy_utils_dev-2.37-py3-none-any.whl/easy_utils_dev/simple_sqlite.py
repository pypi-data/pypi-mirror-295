...
print(f"Working Version: {easy_utils_dev.version}")

class initDB :
    def __init__(self, db , logger=None,useDefaultLogger=False,enable_log=True,id=getRandomKey(n=5)) :
        self.db_file = db
    ...
    def db_connect(self, timeout=5):
        # self.logger.debug(f'DATABASE : Connecting to {self.db_file}')
        ...
        # if self.applyDatabaseParams :
        #     self.cur.execute('PRAGMA journal_mode=WAL')
        #     self.con.commit()
        #     self.applyDatabaseParams=False
    ...
    def insert_to_table(self,table_name , table_headers , table_values , autocommit= True) :
        if not self.lastCon :
            ...
            self.lastCon = con
            self.lastCur = cur
        cur ,  con = self.lastCur , self.lastCon
        wait_job=True
        ...
        while wait_job :
            try :
                ...
                print(f"Insertion failed. try again.")
                wait_job=True
                sleep(.5)
                if i == tries :
