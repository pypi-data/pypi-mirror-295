...
def _get_endpoint_for_login(endpoint: Optional[str]) -> str:
    ...
    if _unionmeta_byoc_is_installed():
        ...
        if endpoint is not None:
            return _clean_endpoint(endpoint)
        return getenv(_UNION_SERVERLESS_ENDPOINT_ENV_VAR, default=_DEFAULT_GCP_SERVERLESS_ENDPOINT)
...
def _check_unionmeta_byoc(config: Optional[str]):
    ...
    if config is not None or _UNION_CONFIG.config_obj is not None:
...
def _get_config_obj(config: Optional[str] = None, default_to_union_semantics: bool = False) -> Config:
    ...
    if _UNION_CONFIG.is_direct_union_cli_call or default_to_union_semantics:
        ...
        if cfg_file is not None:
        ...
        elif _unionmeta_byoc_is_installed():
            ...
            # Use _UNION_SERVERLESS_ENDPOINT_ENV_VAR for backward compatibility
            config = Config.for_endpoint(endpoint=getenv(_UNION_SERVERLESS_ENDPOINT_ENV_VAR))
        return config
