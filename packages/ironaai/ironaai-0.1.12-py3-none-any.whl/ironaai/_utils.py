...
def _module_check(module_name: str, attr_name: str, provider_name: str = None):
    ...
    if provider_name:
        ...
        err_msg = (
            f"`ironaai` requires `{parent_module_name}` to generate responses from {provider_title} models. "
            "Please install via `pip install ironaai[create]`."
        )
    else:
        err_msg = (
            f"`ironaai` requires `{parent_module_name}` to generate responses. Please install via "
            "`pip install ironaai[create]`."
        )
    ...
    # a JSON schema with title and description
        ...
        if not format_tool_to_openai_function:
            raise ValueError(
                ...
                "Please run pip install ironaai[create] to install langchain."
            )
        ...
        if not convert_python_function_to_openai_function:
            raise ValueError(
                ...
                "Tool calling of functions is only available with langchain. Please run pip install ironaai[create] "
...
def _default_headers(
    ironaai_api_key: str, user_agent: str
) -> Dict[str, str]:
    return {
        "content-type": "application/json",
        "Authorization": f"Bearer {ironaai_api_key}",
