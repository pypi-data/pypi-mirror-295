...
from ironaai._utils import _module_check
from ironaai.exceptions import (
    ApiError,
    CreateUnavailableError,
    MissingLLMConfigs,
)
from ironaai.llms.config import LLMConfig
from ironaai.llms.request import (
    amodel_select,
    create_preference_id,
    model_select,
    report_latency,
)
from ironaai.metrics.metric import Metric
from ironaai.prompts import _curly_escape, inject_system_prompt
...
def _ndllm_factory(import_target: _NDClientTarget = None):
    _invoke_error_msg_tmpl = (
        "{fn_name} is not available. `ironaai` can generate LLM responses after "
        "installing additional dependencies via `pip install ironaai[create]`."
    )
    ...
    class _NDRouterClient(BaseModel):
        ...
        def __init__(
            ...
            nd_api_url: Optional[str] = settings.IRONAAI_API_URL,
            user_agent: str | None = None,
            **kwargs,
        ):
            if api_key is None:
                api_key = settings.IRONAAI_API_KEY
            IAIApiKeyValidator(api_key=api_key)
            ...
            ), "API key is not set. Please set a Irona AI API key."
        ...
        async def amodel_select(
            ...
            **kwargs,
            ...
            """
            This function calls the IronaAI backend to fetch the most suitable model for the given prompt,
            ...
            Parameters:
                ...
                hash_content (Optional[bool]): Flag for hashing content before sending to IronaAI API.
                ...
                preference_id (Optional[str]): The ID of the router preference that was configured via the Dashboard.
                                                Defaults to None.
                metric (Metric, optional): Metric used by IronaAI router to choose the best LLM.
                ...
                timeout (int): The number of seconds to wait before terminating the API call to Irona AI backend.
                                Default to 5 seconds.
                nd_api_url (Optional[str]): The URL of the IronaAI API. Defaults to settings.IRONAAI_API_URL.
        ...
        def default_llm(self) -> LLMConfig:
            """
            Return the default LLM that's set on the IronaAI client class.
            """
    ...
    try:
        ...
        msg = _invoke_error_msg_tmpl.format(fn_name="IronaAI creation")
    ...
    class _NDInvokerClient(_NDRouterClient, LLM):
        """
        Implementation of IronaAI class, the main class responsible for creating and invoking LLM prompts.
        ...
        it will check for IRONAAI_API_KEY in the .env file.

        Raises:
            MissingLLMProviders: you must specify at least one LLM provider for the router to work
            ApiError: error raised when the IronaAI API call fails.
        ...
        def create(
            ...
            **kwargs,
            ...
            Raises:
                ApiError: when the IronaAI API fails
            ...
            Returns:
                tuple[Union[AIMessage, BaseModel], str, LLMConfig]:
                    ...
                    str: session_id returned by the IronaAI API
        ...
        def invoke(
            ...
            **kwargs,
            ...
            1. API call to IronaAI backend to get the most suitable LLM for the given prompt
            ...
            try:
                ...
                if best_llm.provider == "google":
                    LOGGER.warning(
                        ...
                        "If you see this message, `ironaai` has returned a Google model as the best option, "
                        "but the LLM call will fail. If possible, `ironaai` will fall back to a non-Google model."
                    )
...
class IronaAI(_NDClient):
    api_key: str
    """
    API key required for making calls to IronaAI.
    You can get an API key via our dashboard: https://app.ironaai.ai
    If an API key is not set, it will check for IRONAAI_API_KEY in .env file.
    """
    ...
    as for example IronaAI API call fails, your code won't break and you have
    ...
    latency_tracking: bool
    """
    Tracking and sending latency of LLM call to IronaAI server as feedback, so we can improve our router.
    By default this is turned on, set it to False to turn off.
    """

    hash_content: bool
    """
    Hashing the content before being sent to the IronaAI API.
    ...
    nd_api_url: Optional[str]
    """The URL of the IronaAI API. Defaults to settings.IRONAAI_API_URL."""
