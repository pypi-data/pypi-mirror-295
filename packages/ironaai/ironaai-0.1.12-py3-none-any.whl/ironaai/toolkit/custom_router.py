...
from ironaai.llms.client import IronaAI
from ironaai.llms.config import LLMConfig
from ironaai.settings import IRONAAI_API_KEY, IRONAAI_API_URL, VERSION
...
class CustomRouter:
    ...
    Attributes:
        ...
        api_key (Optional[str], optional): The IronaAI API key. If not specified, will try to
            find it in the environment variable IRONAAI_API_KEY.
    """
    ...
    def _prepare_joint_dataset(
        ...
        llm_configs = IronaAI._parse_llm_configs_data(llm_configs)
    ...
    def fit(
        ...
        nd_api_url: Optional[str] = IRONAAI_API_URL,
    ) -> str:
        """
        ...
        Parameters:
            ...
            nd_api_url (Optional[str], optional): The URL of the IronaAI API. Defaults to prod.
    ...
    def _eval_custom_router(
        ...
        eval_results["ironaai/score"] = []
        eval_results["ironaai/response"] = []
        eval_results["ironaai/recommended_provider"] = []
        ...
        for _, row in tqdm(joint_df.iterrows(), total=len(joint_df)):
            ...
            for provider in llm_configs:
                ...
                if (
                    ...
                    eval_results["ironaai/score"].append(provider_score)
                    eval_results["ironaai/response"].append(
                        provider_response
                    )
                    eval_results["ironaai/recommended_provider"].append(
        ...
        nd_average_score = eval_results_df["ironaai/score"].mean()
        ...
        eval_stats["Irona AI Average Score"] = [nd_average_score]
        ...
        Returns:
            Tuple[pandas.DataFrame, pandas.DataFrame]:
                eval_results_df: A DataFrame containing all the prompts, responses of each provider
                    ...
                    (indicated by column <provider>/<model>/score), and ironaai custom router
                    response and score (indicated by column ironaai/response and ironaai/score).
                eval_stats_df: A DataFrame containing the "Best Average Provider" computed from the
                    ...
                    and the "Irona AI Average Score" achieved through custom router.
        """
