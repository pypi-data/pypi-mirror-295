...
class EMS(object):
	...
	def import_config(self, conf_name):
		...
		try:
			with open(f"/usr/conf/%s" % conf_name, "r") as config_file:
				config = json.load(config_file)

				# Write config via API
				for category in config:
					response = requests.put(f"http://{self.__base_url}/{category}/settings", json=config[category], auth=EMSAPIAuth())
	...
	def start(self):
		if self.__state in ["Off", "Stopped"]:
			response = requests.post(f"http://{self.__base_url}/starts", json={}, auth=EMSAPIAuth())
	...
	def stop(self):
		if self.__state not in ["Off", "Stopped"]:
			response = requests.post(f"http://{self.__base_url}/stops", json={}, auth=EMSAPIAuth())
			return response.status_code == 201
		else:
			return False

	def restart(self):
		response = requests.post(f"http://{self.__base_url}/restarts", json={}, auth=EMSAPIAuth())
		return response.status_code == 201

	def accept_calibration(self):
		response = requests.post(f"http://{self.__base_url}/calibration/confirmations", json={}, auth=EMSAPIAuth())
		return response.status_code == 201
	...
	def retrieve_layout(self):
		response = requests.get(f"http://{self.__base_url}/mgs/layout", auth=EMSAPIAuth())
		layout = {}
	...
	def to_global(self, x, y):
		...
		response = requests.post(f"http://{self.__base_url}/positioning/conversions/global", json={"x":x, "y":y}, auth=EMSAPIAuth())
	...
	def configure_sensors(self):
		response = requests.get(f"http://{self.__base_url}/calibration/sensors", auth=EMSAPIAuth())
		...
		for sensor in sensors['sensors']:
			requests.put(f"http://{self.__base_url}{sensor['self']}", json=configuration, auth=EMSAPIAuth())
