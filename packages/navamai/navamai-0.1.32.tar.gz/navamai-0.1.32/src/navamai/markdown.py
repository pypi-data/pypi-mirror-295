...
def count_tokens(file_path):
    ...
    try:
        ...
        # console.print(f"Detected encoding for {file_path}: {encoding}")
        
        with open(file_path, 'r', encoding=encoding) as file:
            content = file.read()
    except Exception as e:
        console.print(f"[bold red]Error reading file {file_path}: {str(e)}[/bold red]")
        return 0  # Return 0 tokens if we can't read the file

    word_tokens = word_count(content)
    punctuation_tokens = punctuation_count(content)
    subword_factor = int(0.1 * (word_tokens + punctuation_tokens))  # Assume 10% subword tokens

    total_tokens = word_tokens + punctuation_tokens + subword_factor
