...
def extend_linestring(line, near_pt, length):
    ...
    dx, dy = np.diff(
        np.vstack([coords[nearest_end[0]], coords[nearest_end[1]]]), axis=0
    )[0]
...
class RasterPart:
    ...
    def from_bounds(cls, f, bnds):
        idxs = (
            list(f.index(bnds[0], bnds[1]))[::-1]
            + list(f.index(bnds[2], bnds[3]))[::-1]
        )
        return cls(
            f,
            min(idxs[0], idxs[2]),
