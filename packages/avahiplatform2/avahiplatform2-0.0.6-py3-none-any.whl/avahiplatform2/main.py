...
def get_instance_rag_semantic_search(path, mode="local", region_name=None, huggingface_api_token=None):
    return RAGSemanticSearch(path, mode, region_name, huggingface_api_token)
...
def perform_semantic_search(question, path, mode="local", region_name=None, huggingface_api_token=None):
    """
    Perform semantic search on the given question using either local documents or AWS S3.

    Parameters:
    question (str): The question to be used for semantic search.
    path (str): The path where the documents are stored (local directory or S3 path).
    mode (str): Either "local" or "aws".
    region_name (str, optional): AWS region name when using AWS mode.
    huggingface_api_token (str, optional): API token for HuggingFace when using local mode.

    Returns:
    dict: A dictionary containing the search results or error information.
    """
    try:
        instance = get_instance_rag_semantic_search(path, mode, region_name, huggingface_api_token)
        ...
        logger.error(f"Error in perform_semantic_search: {str(e)}")
        return {
            "status": "error",
            "message": instance._get_user_friendly_error(e)
        }
