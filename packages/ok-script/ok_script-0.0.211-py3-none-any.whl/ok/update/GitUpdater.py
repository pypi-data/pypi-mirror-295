...
class GitUpdater:
    ...
    def do_update_launcher(self):
        ...
        self.do_list_all_versions()
        self.set_start_success()
        self.kill_launcher()
        if self.version != self.launcher_config.get('launcher_version'):
            logger.info(
            ...
            if self.install_dependencies('launcher_env'):
                ...
                self.launcher_config['launcher_version'] = self.version

            copy_exe_files(os.path.join('repo', self.launcher_config['launcher_version']), os.getcwd())
        clean_repo('repo', [self.version] + self.all_versions)
    ...
    def read_launcher_config(self, path):
        launch_profiles = self.do_read_launcher_config(path)
        if not launch_profiles:
            launch_profiles = self.do_read_launcher_config(os.getcwd())
            logger.error(f'read launcher config from {path} failed, now try read cwd for dev mode {launch_profiles}')
        return launch_profiles

    def do_read_launcher_config(self, path):
        launch_profiles = []
        full_version = find_line_in_requirements(os.path.join(path, 'requirements.txt'), 'ok-script')
