...
if __name__ == "__main__":
    ...
    try:
        ...
        shutil.copytree(os.path.join(mini_python, 'Python_3.11.9'), python_dir)

        shutil.copytree(os.path.join(mini_python, 'git_2.46.0_win_64'), os.path.join(python_dir, 'git'))

        git_location = get_git_exe_location()
        shutil.copy(git_location, python_dir)

        logger.info(f'copied {git_location} to {python_dir}')
