...
class GitUpdater:
    ...
    def __init__(self, app_config, exit_event):
        add_to_path(os.path.join(os.getcwd(), 'python', 'git'))
...
def add_to_path(folder_path):
    ...
    if folder_path not in current_path:
        ...
        logger.info(f"Added {folder_path} to PATH for the current script. {current_path}")
    else:
        logger.info(f"{folder_path} is already in the PATH for the current script. {current_path}")
