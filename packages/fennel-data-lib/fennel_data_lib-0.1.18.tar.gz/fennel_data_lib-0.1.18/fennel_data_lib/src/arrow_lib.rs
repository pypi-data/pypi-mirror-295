...
lazy_static! {
    static ref ARROW_FLOAT_INT_CONVERSION_CTR: CounterVec = register_counter_vec!(
        "arrow_float_int_conversion_ctr",
        "Counter to measure the number of times a float was converted to Int as a hack",
        &["type"]
    )
    .unwrap();
}
...
pub fn from_arrow_array(array: &ArrayRef, dtype: &Type, nullable: bool) -> Result<Vec<Value>> {
    ...
    match (dtype, array.data_type()) {
        ...
        (Type::List(t), List(d)) | (Type::List(t), LargeList(d)) => {
            // assert that the types match
            // TODO: https://linear.app/fennel-ai/issue/DEV-3231/unify-df-arrow-datatype-and-value-conversions.
            let expected_dtype = to_arrow_dtype(t.as_ref(), t.is_nullable());
            let actual_dtype = (**d).data_type();
            if expected_dtype != *actual_dtype {
                return Err(anyhow!(
                    "expected list of type: {:?} to match, found: {:?}",
                    t,
                    d
                ));
            }
        ...
        (Type::Struct(s), Struct(arrow_fields)) => {
            if arrow_fields.len() != s.num_fields() {
                return Err(anyhow!("field count mismatch"));
            }
...
fn from_arrow_large_list_array(
    ...
    for elem in array.iter() {
        if let Some(elem) = elem {
            let elem_vals = from_arrow_array(&elem, dtype, false)?;
            let list_val = Value::List(Arc::new(List::new(dtype.clone(), &elem_vals)?));
...
fn from_arrow_list_array(array: &ArrayRef, dtype: &Type, nullable: bool) -> Result<Vec<Value>> {
...
mod tests {
    ...
    fn test_list_type_invalid_type() {
        // array type
        {
            ...
            from_arrow_array(&list_array, &Type::List(Box::new(Type::Float)), true).unwrap_err();
