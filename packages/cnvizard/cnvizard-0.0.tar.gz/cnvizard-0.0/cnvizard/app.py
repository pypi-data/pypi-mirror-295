...
def validate_env_file(env_file):
    ...
    optional_keys = ["APPSETTING_IGV_OUTLINK"]
    missing_keys = []
    invalid_paths = []
...
def load_and_select_env(unique_id="1"):
    ...
    if env_choice == "Use Default Bundled Resources":
        ...
        if omim_file:
            ...
            if st.button("Confirm and use default Resources"):
                try:
                    ...
                    # Write environment file dynamically
                    ...
                    st.success("Using default resources with the uploaded OMIM file.")

                    return env_file_path  # Return to indicate success
                except Exception as e:
                    st.error(f"Error loading OMIM file: {e}")
                    return None  # Return None in case of error
        else:
            st.warning("Please upload an OMIM file to proceed.")
            return None  # Ensure None is returned if OMIM file isn't uploaded
...
def main(env_file_path):
    ...
    st.markdown(
        "This is a Streamlit web app providing analysis tools for genetic copy number variants."
    )
    ...
    reference_files_dir = Path(
        os.getenv("REFERENCE_FILES_DIR", "./resources/references")
    )
    annotsv_format_path = Path(
        os.getenv("ANNOTS_SV_FORMAT_PATH", "./resources/annotsv_format.txt")
    )
