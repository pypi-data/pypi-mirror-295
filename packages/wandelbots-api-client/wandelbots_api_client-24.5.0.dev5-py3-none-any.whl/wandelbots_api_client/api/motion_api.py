...
class MotionApi:
    ...
    async def stream_move_backward(self, cell: Annotated[str, Field(strict=True, description="Unique identifier addressing a cell in all API calls. Since the cell identifier is used as Kubernetes namespace, the following rules apply: * Contain only lowercase alphanumeric characters or '-'. * Start with an alphanumeric character. * End with an alphanumeric character. * A maximum length of 63 characters. * The identifier \"kube-system\", \"default\" and \"wandelbots\" are reserved and cannot be used. ")], motion: Annotated[StrictStr, Field(description="This represents the UUID of a motion. Every executable or partially executable motion is cached and an UUID is returned. Indicate the UUID to execute the motion or retrieve information on the motion.")], playback_speed_in_percent: Annotated[StrictInt, Field(description="Set the velocity for executed movements of the motion in percent. A percentage of 100% means that the robot moves as fast as possible without violating the limits. Setting this value does not affect the overall shape of the velocity profile. Everything is slowed down by the same factor. Therefore, this should only be used for teaching and trajectory evaluation purposes. If the process requires a certain velocity, the respective limits should be set when planning the motion. This will not change the velocity override of the controller. The controller velocity override value shall be 100% to ensure controllability of the motion group. ")], response_rate: Annotated[Optional[StrictInt], Field(description="Update rate for the response message in milliseconds (ms). Default is 200 ms. We recommend to use the step rate of the controller or a multiple of the step rate as Nova updates the state in the controller's step rate as well. Minimal response rate is the step rate of controller.")] = None, response_coordinate_system: Annotated[Optional[StrictStr], Field(description="Unique identifier addressing a coordinate system to which the cartesian data of the responses should be converted. Default is the world coordinate system. ")] = None, set_ios: Annotated[Optional[List[SetIO]], Field(description="Define an arb I/O that is listened to during the motion. If the defined comparator evaluates to true the execution is started. ")] = None, start_on_io: Annotated[Optional[StartOnIO], Field(description="Defines an I/O that is listened to during the motion. If the defined comparator evaluates to true the execution is started. ")] = None, pause_on_io: Annotated[Optional[PauseOnIO], Field(description="Defines an I/O that is listened to during the motion. If the defined comparator evaluates to true the execution is paused. ")] = None) -> AsyncGenerator[StreamMoveResponse, None]:  # noqa: E501
        ...
        if locals().get("set_ios") is not None:
            url_schema = append_parameter_connector(url_schema)
            if isinstance(set_ios, list):
              url_schema += "&".join(["set_ios=" + s for s in set_ios])
            else:
              url_schema += "set_ios="+str(set_ios)
        if locals().get("start_on_io") is not None:
            url_schema = append_parameter_connector(url_schema)
            if isinstance(start_on_io, list):
              url_schema += "&".join(["start_on_io=" + s for s in start_on_io])
            else:
              url_schema += "start_on_io="+str(start_on_io)
        if locals().get("pause_on_io") is not None:
            url_schema = append_parameter_connector(url_schema)
            if isinstance(pause_on_io, list):
              url_schema += "&".join(["pause_on_io=" + s for s in pause_on_io])
            else:
              url_schema += "pause_on_io="+str(pause_on_io)
