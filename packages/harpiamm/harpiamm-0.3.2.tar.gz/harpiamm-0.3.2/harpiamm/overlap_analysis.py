    ...
    try:
        ...
        for ind, file_name in enumerate(file_names):
            ...
            try:
                ...
                # If there's a runtime warning, set fit values to NaN and move
                # on to other images
                print("A runtime warning occurred when processing "
                      "file {:s}".format(file_name))
                fit_result = [np.nan, np.nan]
            except Exception as excpt:
                # If there's an expection, set fit values to NaN as well and
                # move on to other images
...
def _make_overlap_report(
    ...
    try:
        ...
        warn("Pump data not available, pump metrics will be skipped. "
             "Reason: ", excpt)
    ...
    try:
        ...
        warn("Probe data not available, probe metrics will be skipped. "
             "Reason: ", excpt)
        overlap_in_spec = False
        par_pu = None
