...
def cam_focus_test(
    ...
    """
    print("=== HARPIA pump-probe XYZ overlap camera scan ===")

    if cam_sn is None:
        cam_sn = read_setup('cam_sn')
        if cam_sn is None:
            print("Sample plane camera SN not specified, using the first "
                  "available camera")
            cam_id = 0
        else:
    ...
    if use_cam:
        ...
        if cam_id is not None:
            ...
            if flir_sn_vid_bug:
                print("WARNING: USB VID returned instead of camera SN. This is"
                      " a known FLIR Spinaker API bug. The requested camera "
                      "will likely be not detected even if the provided SN is "
                      "corrected. You can pass an explicit camera index using "
                      "'cam_id' if you can recognize the camera you need in "
                      "the list above. If there are only two FLIR cameras, "
                      "e.g. a PC running only a HARPIA-MM pass cam_id=1 if the"
                      " default behaviour opens the incorrect camera. "
                      "RepluggingIf all cameras or rebooting the PC usually "
                      "fixes the issue and correct SN strings are reuturned.")

        if cam is None:
            cam = cam_list[0]
            print("WARNING: Camera with SN {:s} not found. Using the first "
                  "available camera with SN {:s}".format(
                      cam_sn, cam.GetUniqueID()))
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("Could not connect to HARPIA Service App") \
                from excpt
