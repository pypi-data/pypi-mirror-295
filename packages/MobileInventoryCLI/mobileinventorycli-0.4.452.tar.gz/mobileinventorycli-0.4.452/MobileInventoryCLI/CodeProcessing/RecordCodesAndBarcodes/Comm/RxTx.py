...
class RxTx:
	htext=f"""Please Note Commands can be guessed from the first match of a partial so a doWhat == 'new c' will EQUAL doWhat == 'new contact'\n"""
	def print_help(self):
		print(self.htext)
				...
				if dateSearch in [None,]:
					...
					if search in [None,]:
						...
						with Session(ENGINE) as session:
							results=session.query(MailBox)
							results=results.filter(or_(
								MailBoxContacts.Title.icontains(search.lower()),
								MailBoxContacts.MsgText.icontains(search.lower()),
								MailBoxContacts.Addressed_From_email.icontains(search.lower()),
								MailBoxContacts.Addressed_To_email.icontains(search.lower()),
								)
							).all()
					ct=len(results)
					for num,i in enumerate(results):
