                        ...
                        while completed < ct:
                            ...
                            for num, i in enumerate(rout):
                                ...
                                if old == completed_reps:
                                    nxt=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"(ELAPSED:{datetime.now()-start})({completed_reps+1}/{total_reps}|{round(((completed_reps+1)/total_reps)*100,2)})Next?",helpText="Go to next exercise, just hit enter",data="boolean")
                                    #print(nxt)
