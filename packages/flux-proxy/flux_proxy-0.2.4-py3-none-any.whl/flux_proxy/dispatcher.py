...
from flux_proxy.protocol import (RPCBatchRequest, RPCBatchResponse, RPCRequest,
                                 RPCResponse)
...
class RPCDispatcher:
    ...
    def __init__(
        self, plugin_name: str = "", required_packages: list = []
    ) -> None:
        self.plugin_name = plugin_name
        self.method_map: dict[str, MappedMethod] = {}
        self.context_map = {}
        self.subdispatchers = {}
    ...
    def create(self, name=None, pass_context=False):
        ...
        def _(f):
            self.add_method(f, name=name, pass_context=pass_context)
    ...
    def add_plugin(self, dispatcher: RPCDispatcher):
        ...
        """
        self.subdispatchers.setdefault(dispatcher.plugin_name, []).append(
            dispatcher
        )
    ...
    def add_method(
        self, f: Callable, *, name: str = None, pass_context: bool = False
    ) -> None:
        ...
        self.method_map[name] = MappedMethod(f, pass_context)

    def get_method(self, name: str) -> MappedMethod:
    ...
    async def dispatch(
        ...
        if hasattr(request, "create_batch_response"):
            ...
            return await self._dispatch(context, request, caller)

    async def _dispatch(self, context, request, caller):
        try:
            mapped_method = self.get_method(request.method)
        ...
        method = mapped_method.f

        if mapped_method.pass_context:
            name = method.__name__
            method = functools.partial(method, context)
