...
class OmniParse:
    ...
    def _add_docs_to_memory(self):
        ...
        try:
            ...
            logger.error(f"Error adding documents to memory: {str(e)}")
            raise
    ...
    def _add_single_document(self, document_path: str):
        ...
        try:
            ...
            logger.error(f"Error processing {document_path}: {str(e)}")
    ...
    def run(self, query: str) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Retrieved context with {token_count} tokens")
            
            chunked_context = chunk_text_dynamic(
                context, limit_tokens=self.limit_tokens
            )
            logger.info(f"Chunked context into {len(chunked_context)} parts")
