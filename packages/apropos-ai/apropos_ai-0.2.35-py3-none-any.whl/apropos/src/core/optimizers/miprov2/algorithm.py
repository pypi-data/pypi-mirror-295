...
from apropos.src.core.optimizers.miprov2.search_backends import (
    PromptDelta,
...
class MIPrO_V2p1_DAG(DAGOptimizer):
    ...
    def curate_questions_for_bootstrapping(self, learnable_questions: List[Question], successes: List[List[DagRecord]], failures: List[List[DagRecord]], pass_at_ks: List[float]) -> Tuple[List[Question], List[Question], List[Question]]:
        question_curation_response: QuestionCurationResponse = question_curation_prompt.run(
            inputs = {
                "<<<HIGHLY_LEARNABLE_QUESTIONS>>>": "\n".join([f"### Question {i}\n{question}" for i, question in enumerate(learnable_questions, 1)]),
                "<<<SUCCESS_RATES>>>": str(pass_at_ks),
                "<<<SUCCESS_TRACES>>>": str([random.choice(successes)] if successes else []),
                "<<<FAILURE_TRACES>>>": str([random.choice(failures)] if failures else [])
            },
        ...
        # See what happens?
        demo_questions = [
            learnable_questions[i] for i in question_curation_response.bootstrapping_question_indices if i in range(len(learnable_questions))
        ]
        train_questions = [
            learnable_questions[i] for i in question_curation_response.train_question_indices if i in range(len(learnable_questions))
        ]
        val_questions = [
            learnable_questions[i] for i in question_curation_response.val_question_indices if i in range(len(learnable_questions))
        ]
    ...
    async def get_learnable_questions(self) -> Tuple[List[Question], List[List[DagRecord]], List[List[DagRecord]], List[float]]:
        import asyncio
        ...
        async def process_question(question_index):
            ...
            temp_schedule = [self.cfg["learnable_questions"]["base_temp"] + 0.01 * i for i in range(self.cfg["learnable_questions"]["k_for_pass_at_k"])]
            ...
            correctnesses_with_records = await asyncio.gather(
                *[question.compute_and_score_attempt(program) for program in temp_scheduled_programs]
            )
            ...
            if sum(correctnesses) not in [0, len(correctnesses)]:
                return (
                    question,
                    [record for correctness, record in correctnesses_with_records if correctness],
                    [record for correctness, record in correctnesses_with_records if not correctness],
        ...
        for batch_start in tqdm(range(0, max_questions, batch_size), desc="Sampling questions for batch"):
            batch_end = min(batch_start + batch_size, max_questions)
            batch_results = await asyncio.gather(*[process_question(i) for i in range(batch_start, batch_end)])
        ...
        return list(learnable_questions), list(successes), list(failures), list(pass_at_ks)

    # Propose prompt instruction components (Simple => Plan Search)
    async def propose_prompt_instruction_components_async(self, learnable_questions: List[Question], successes: List[List[DagRecord]], failures: List[List[DagRecord]], pass_at_ks: List[float], bootstrapped_demos_by_node: Dict[str, Dict[str, List[Demonstration]]]):
        random.seed(42)
        ...
        nodes_responsible_by_name = [node for sublist in problems_response.nodes_responsible_by_name for node in sublist]
        assert set(nodes_responsible_by_name).issubset(set(self.student_program.nodes.keys())), f"Nodes responsible by name: {nodes_responsible_by_name} not found in student program: {self.student_program.nodes.keys()}"
        assert min([len(examples) for examples in problems_response.example_question_indices]) >= 1, f"Example question indices: {problems_response.example_question_indices} have less than 1 example question"
        ...
        problems = [{
            ...
            "example_questions": [learnable_questions[j] for j in problems_response.example_question_indices[i] if j in range(len(learnable_questions))],
            "nodes_responsible_by_name": problems_response.nodes_responsible_by_name[i],
        ...
        for problem in problems:
            ...
            intervention_concepts_response: InterventionConceptsResponse = await get_intervention_concepts_prompt.arun(
                inputs = {
                    "<<<FAILURE_MODE>>>": problem["problem_name"],
                    "<<<FAILURE_DESCRIPTION>>>": problem["problem_description"],
                    "<<<EXAMPLE_QUESTIONS>>>": "\n".join([f"### Question {i}\n{question}" for i, question in enumerate(problem["example_questions"], 1)]),
            ...
            program_string = str({node_name: self.student_program.to_dict()["nodes"][node_name]['runnables']['transform']['prompt'] for node_name in problem["nodes_responsible_by_name"]})
            sys, user = get_prompt_delta_prompt.compile(
                inputs = {
                    "<<<INTERVENTION_CONCEPTS>>>": str(intervention_concepts_response.intervention_concepts_by_node),
                    "<<<FAILURE_DESCRIPTION>>>": problem["problem_description"],
            ...
            for node_name in nodes_dictified:
                ...
                for message in unique_subcomponents_by_message:
                    for subcomponent in unique_subcomponents_by_message[message]:
                        valid_topics_by_subcomponent_by_node[(message, subcomponent)] = [t["topic_name"] for t in prompt[subcomponent]]
            prompt_delta_response: PromptDeltasResponse = await get_prompt_delta_prompt.arun(
                inputs = {
                    "<<<INTERVENTION_CONCEPTS>>>": str(intervention_concepts_response.intervention_concepts_by_node),
                    "<<<CURRENT_PROGRAM>>>": program_string,
                    "<<<VALID_TOPICS_BY_SUBCOMPONENT>>>": "",
                    "<<<FAILURE_DESCRIPTION>>>": problem["problem_description"],
                },
                lm=LLM(model_name="gpt-4o-mini"),
            ...
            print("---")
            print(prompt_delta_response)
            print("---")
            assert all(prompt_delta_response.subcomponents[i] in ["premise", "objective", "constraints", "user"] for i in range(len(prompt_delta_response.subcomponents)))
            core_prompt_deltas = [
                {
                    ...
                    "prompt_delta": PromptDelta(
                        ...
                        instructions_fields_edits=prompt_delta_response.instructions_fields_edits[i],
                    )
                }
                for i in range(len(prompt_delta_response.descriptions))
            ]
            for core_prompt_delta in core_prompt_deltas:
                core_prompt_delta["prompt_delta"].validate(self.student_program.nodes[core_prompt_delta["node_name"]].transform.prompt)
                core_prompt_delta["prompt_delta"].apply(self.student_program.nodes[core_prompt_delta["node_name"]].transform.prompt)
            ...
            for i, core_prompt_delta in tqdm(enumerate(core_prompt_deltas)):
                variations_response: PromptDeltasVariationsResponse = await get_prompt_delta_variations_prompt.arun(
                    inputs = {
                        "<<<CORE_DELTA>>>": str(core_prompt_delta["prompt_delta"].to_dict()),
                        "<<<CURRENT_PROGRAM>>>": program_string,
                ...
                for j, variation in enumerate(variations_response.messages):
                    prompt_delta = PromptDelta(
                        ...
                        instructions_fields_edits=variations_response.instructions_fields_edits[j],
                    )
                    prompt_delta.validate(self.student_program.nodes[core_prompt_delta["node_name"]].transform.prompt)
                    prompt_delta.apply(self.student_program.nodes[core_prompt_delta["node_name"]].transform.prompt)
                ...
                if not problem["problem_name"] in deltas_by_problem_by_node:
                    deltas_by_problem_by_node[problem["problem_name"]] = {}
                if not core_prompt_delta["node_name"] in deltas_by_problem_by_node[problem["problem_name"]]:
                    deltas_by_problem_by_node[problem["problem_name"]][core_prompt_delta["node_name"]] = []
                deltas_by_problem_by_node[problem["problem_name"]][core_prompt_delta["node_name"]].extend(prompt_delta_variations[i])

            bootstrapped_demos_for_node = bootstrapped_demos_by_node[core_prompt_delta["node_name"]]
            ...
            demos_by_problem_by_node[problem["problem_name"]][core_prompt_delta["node_name"]] = bootstrapped_demos_for_node
        ...
        for problem, nodes in deltas_by_problem_by_node.items():
            ...
            for node, deltas in nodes.items():
                assert isinstance(deltas, list), f"Deltas for problem {problem} and node {node} are not a list: {deltas}"
                assert all(isinstance(delta, PromptDelta) for delta in deltas), f"Deltas for problem {problem} and node {node} are not all PromptDelta: {deltas}"
    ...
    async def optimize_program(self) -> LM_DAG:
        learnable_questions, successes, failures, pass_at_ks = await self.get_learnable_questions()
        ...
        print("Chosen lengths: ", len(questions_for_bootstrapping), len(questions_for_train), len(questions_for_val))
        bootstrapped_demos_by_node = await self.bootstrap_demonstrations_on_subset(questions_for_bootstrapping)
        instruction_deltas_by_problem_by_node, demos_by_problem_by_node = await self.propose_prompt_instruction_components_async(questions_for_train, successes, failures, pass_at_ks, bootstrapped_demos_by_node)
        self.search_space = SearchSpace(
