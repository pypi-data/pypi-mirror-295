...
class PromptTemplate(BaseModel):
        ...
        for topic in self.user.user:
            ...
            for field in topic.input_fields:
                ...
                try:
                    ...
                    raise Exception(f"Inputs must be strings, instead got: {type(inputs[field])} for input field: {field}")
    ...
    def run(
        ...
        if response_model:
            response = lm.sync_respond(system_prompt=system, user_prompt=user, response_model=response_model, multi_threaded=multi_threaded)
        else:
            response = lm.sync_respond(system_prompt=system, user_prompt=user, multi_threaded=multi_threaded)
        return response
