...
class OpenAIStandardProvider(BaseProvider):
    ...
    def hit_cache(self, messages, model, temperature, response_model):
        if self.multi_threaded:
            return threaded_cache.hit_cache(messages, model, temperature, response_model)
        else:
    ...
    def add_to_cache(self, messages, model, temperature, response_model, content):
        if self.multi_threaded:
            return threaded_cache.add_to_cache(messages, model, temperature, response_model, content)
        else:
            return cache.add_to_cache(messages, model, temperature, response_model, content)
    ...
    @backoff.on_exception(
        ...
        max_tries=BACKOFF_TOLERANCE,
        on_backoff=lambda details: print(f"Backing off: {details['tries']} tries, {details['wait']} seconds") if details['tries'] > 3 else None,
    )
