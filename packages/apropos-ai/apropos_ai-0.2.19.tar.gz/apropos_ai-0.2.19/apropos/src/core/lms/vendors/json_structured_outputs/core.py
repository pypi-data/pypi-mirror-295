...
def add_json_instructions_to_prompt(
    system_prompt, user_prompt, response_model: Optional[Type[BaseModel]] = None, reminder: Optional[str] = None
) -> Tuple[str, str]:
    if response_model:
...
def add_json_instructions_to_messages(
    messages: List[Dict[str, str]], response_model: Optional[Type[BaseModel]] = None, reminder: Optional[str] = None
) -> List[Dict[str, str]]:
    ...
    try:
        ...
        try:
            ...
            final = await json_debugger_async(
                response_prepared, response_model=None, provider_name="groq"
            )
            if final == "ESCALATE":
