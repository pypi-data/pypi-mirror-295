...
from wmain.m3u8.paser import WParser, M3U8VideoList
...
class WDownloader:
    ...
    def flush_tasks(self, filter_func: Union[None, callable] = None):
        ...
        for video_list in self.res.video_lists:
            ...
            if video_list.key_url:
                video_list.key = self.res.session.get(video_list.key_url).content
                if len(video_list.key) != 16:
                    raise Exception(f"key的长度应为16, 实际为{len(video_list.key)}")
