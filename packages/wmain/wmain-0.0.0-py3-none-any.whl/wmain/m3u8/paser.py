...
class M3U8Union:
    ...
    def __init__(self, m3u8_line: str):
        ...
        self.attrs_dict = {
            attr.split("=")[0]: attr.split("=")[1] for attr in self.attrs if "=" in attr
        }
...
class WParser:
    ...
    def parse_m3u8_by_session(self, session: Union[WSession, None] = None):
        if session is None:
            self._m3u8_parse(self.session.get(self.m3u8_url).text)
        else:
            self._m3u8_parse(session.get(self.m3u8_url).text)
    ...
    @property
    def video_lists(self) -> List[M3U8VideoList]:
        r = []
        sequence = "0"
        video_list: M3U8VideoList = M3U8VideoList()
        for union in self.unions:
            if union.basic_tag == EXT_KEY:
                ...
                if "URI" in attrs:
                    video_list.key_url = self.m3u8_url.join(attrs["URI"])
                if "IV" in attrs:
                    video_list.iv = (
