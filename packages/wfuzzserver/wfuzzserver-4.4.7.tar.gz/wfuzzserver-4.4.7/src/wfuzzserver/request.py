...
class Request:
    ...
    def reBuild(self,fuzzparam,pos):
       ...
       if pos=="url":
           ...
           if self.jsonbody:
               ...
               temp = json.loads(self.body)
               temp[fuzzparam] = temp[fuzzparam]+"FUZZ"
               body = dicttoxml(temp, root=self.xmlroot, attr_type=False)
    ...
    def reBuildAll(self):
       variations = []
       for param in self.baseParams["url"]:
           variations.append(self.reBuild(param,"url"))
       for param in self.baseParams["body"]:
           variations.append(self.reBuild(param,"body"))
       for header in self.baseParams["headers"]:
           variations.append(self.reBuild(header,"headers"))
       for up in self.baseParams["urlparams"]:
           variations.append(self.reBuild(up,"urlparams"))
       return variations
