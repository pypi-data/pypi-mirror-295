...
def rec_record(
    report: list[ReportRecord] | None, record: ReportRecord | None, relative_root: PosixPath | None = None
) -> None:
...
def process_folder(
    ...
    report: list[ReportRecord] | None = None,
) -> list[ReportRecord] | None:

    if write_report and report is None:
        report = list()
    if skip_symlinks and root.is_symlink():
        rec_record(
            report,
            ReportRecord(root, depth=current_depth, folder=root.is_dir(), symlink=True),
    ...
    if skip_missing and is_broken_path(root):
        rec_record(
            report,
            ReportRecord(root, depth=current_depth, folder=root.is_dir(), missing=True),
            relative_root=relative_root,
        )
        return report
    ...
    if skip_hidden and is_hidden_path(root):
        rec_record(
            report,
            ReportRecord(root, depth=current_depth, folder=root.is_dir(), hidden=True),
            relative_root=relative_root,
        )
        return report
    ...
    if root.is_dir():
        ...
        if folder_handler:
            if skip_root:
                rec_record(
                    report,
                    ReportRecord(root, depth=current_depth, folder=root.is_dir(), root=True),
                    relative_root=relative_root,
                )
            else:
                rec_record(report, folder_handler(root, current_depth), relative_root=relative_root)
        ...
        if max_depth == 0 or current_depth < max_depth:
            for entry in root.iterdir():
                ...
                if entry.is_dir():
                    ...
                    if file_patterns is None:
                        if file_handler:
                            rec_record(report, file_handler(entry, current_depth), relative_root=relative_root)
                    else:
            ...
            rec_record(report, ReportRecord(root, depth=current_depth), relative_root=relative_root)
    elif write_report:
        rec_record(report, ReportRecord(root, depth=current_depth), relative_root=relative_root)
