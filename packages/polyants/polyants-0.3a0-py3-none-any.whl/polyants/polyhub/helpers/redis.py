...
from polyants.polyhub.enums import ObjectType, GenericState
...
def get_permission_key(user_id, object_type, object_state=None):
    object_type = object_type or ObjectType.UNKNOWN
    object_state = object_state or GenericState.UNKNOWN
    return f'{REDIS_PREFIX}{user_id}:permission:{ObjectType(object_type).value}:{GenericState(object_state).value}'
