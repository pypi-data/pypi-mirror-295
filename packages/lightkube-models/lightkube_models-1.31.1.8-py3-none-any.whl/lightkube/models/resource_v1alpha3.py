...
class AllocationResult(DictMixin):
    r"""AllocationResult contains attributes of an allocated resource.
      ...
      * **controller** ``Optional[str]`` - Controller is the name of the DRA driver which handled the allocation. That
        driver is also responsible for deallocating the claim. It is empty when the
        claim can be deallocated without involving a driver.
        A driver may allocate devices provided by other drivers, so this driver name
        here can be different from the driver names listed for the results.
...
class DeviceClassSpec(DictMixin):
    r"""DeviceClassSpec is used in a [DeviceClass] to define what can be allocated and
      ...
      * **suitableNodes** ``Optional[core_v1.NodeSelector]`` - Only nodes matching the selector will be considered by the scheduler when
        trying to find a Node that fits a Pod when that Pod uses a claim that has not
        been allocated yet *and* that claim gets allocated through a control plane
        controller. It is ignored when the claim does not use a control plane
...
class ResourceClaimSpec(DictMixin):
    r"""ResourceClaimSpec defines what is being requested in a ResourceClaim and how
      ...
      * **controller** ``Optional[str]`` - Controller is the name of the DRA driver that is meant to handle allocation of
        this claim. If empty, allocation is handled by the scheduler while scheduling
        a pod.
        Must be a DNS subdomain and should end with a DNS domain owned by the vendor
        of the driver.
        This is an alpha field and requires enabling the DRAControlPlaneController
...
class ResourceClaimStatus(DictMixin):
    r"""ResourceClaimStatus tracks whether the resource has been allocated and what
      ...
      * **deallocationRequested** ``Optional[bool]`` - Indicates that a claim is to be deallocated. While this is set, no new
        consumers may be added to ReservedFor.
        This is only used if the claim needs to be deallocated by a DRA driver. That
        driver then must deallocate this claim and reset the field together with
