...
class DataHandler:
    ...
    def eeg_acc_reshape(self):
        if self.device == 101:
            self.acc, self.eeg = eeg_acc_reshape(self.acc, self.eeg)
        elif self.device == 102 or self.device == 44:
            self.acc, self.eeg0, self.eeg1 = eeg_acc_reshape(self.acc, self.eeg0, self.eeg1)
            (self.ecg, self.emg) = ecg_emg_reshape(self.ecg, self.emg) if self.ecg is not None else (None, None)
        else:
            pass
