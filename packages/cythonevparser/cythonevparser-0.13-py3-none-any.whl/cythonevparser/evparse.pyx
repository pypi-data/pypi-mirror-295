...
configstuff.cache_regexnumber={}
configstuff.cache_hierachyregex={}
configstuff.cache_finchi={}
configstuff.cache_stripped_strings={}
configstuff.cache_number_zero={}
configstuff.struct_dicts_temp={}
configstuff.cached_script_files={}
config_settings=sys.modules[__name__]
config_settings.running_uiautomators=[]
config_settings.running_parsing_threads=[]
...
cdef class MyDict:
    ...
    def add_to_struct_array(self,np.ndarray structarray, print_counter):
        cdef:
            strmap.iterator  begin = self.c_dict.begin()
            strmap.iterator  end = self.c_dict.end()
        while (begin!=end):
...
cdef openmp.omp_lock_t globallocker
openmp.omp_init_lock(&globallocker)
    ...
    try:
        while not config_settings.stop:
            try:
                ...
                openmp.omp_set_lock(&globallocker)
                while not my_parsing_queue.empty():
                    ...
                    try:
                        ...
                        if stop_or_continue==2:
                            openmp.omp_unset_lock(&globallocker)
                            return 1
...
class EvParse:
    def __init__(
        ...
        self.uiautomator_cmd = (
            uiautomator_cmd
            if isinstance(uiautomator_cmd, bytes)
            else uiautomator_cmd.encode()
        )
    ...
    def callback_function(self, **kwargs):
        try:
            ...
            regex_flags = kwargs.get("regex_flags", re.IGNORECASE)
            ...
            for x in re.finditer(
                ...
                try:
                    ...
                    column = all_categories_eventparser_utf8[
                        xstart
