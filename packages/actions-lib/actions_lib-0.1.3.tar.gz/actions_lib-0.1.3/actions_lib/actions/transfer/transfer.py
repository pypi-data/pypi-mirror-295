...
token_mapping = {
    ...
    "base": {
        "usdc": "0xd9f944a5c0c8ebBf5c72e50bd48b8C6dc76A07F4",  # USDC on Base
    },
}
...
def transfer(receiver, amount, step, run_mode = 'auto', chain = "base", token = "0xBfcB0b8956cE46e561C05f3cd75B1370311B7DAa", **kwargs):
    """
    ...
    if run_mode == 'auto':
      ...
      allowance = token_contract.functions.allowance(Web3.to_checksum_address(executor), control_address).call()
      ...
      if active_chain.lower() != chain.lower():
          ...
          action_data = ActionData(func='', group='', params=params)
          action = Action(msg=None, type='setting', data=action_data)
          ...
          next_action_data = ActionData(func='transfer', group='transfer', params=params)
          next_action = Action(msg=None, type='backend', data=next_action_data)
          ...
          action = Action(msg=None, type='backend', data=action_data)
          return {
            'result': None,
            'action': action.__dict__,
            'next_action': None
