...
class Dataframe:
    ...
    def __init__(self, df: pd.DataFrame):
        self.df = deepcopy(df)
        self.crypto_cols = ['Symbol', 'Date', 'Name', 'Open', 'High', 'Low', 'Close', 'Volume']
        self.equity_cols = ['Ticker', 'Date', 'Open', 'High', 'Low', 'Close', 'Volume']

        if not self.df.empty:
            self.rename_columns()
            self.filter_columns()
            self.convert_to_floats()
            has_columns, missing = self.check_columns(self.crypto_cols, self.equity_cols)
            if not has_columns:
    ...
    def filter_columns(self):
        set1 = self.crypto_cols
        set2 = self.equity_cols
        columns_to_keep = [col for col in set1 if col in self.df.columns] + [col for col in set2 if col in self.df.columns and col not in set1]
    ...
    def check_columns(self, crypto_cols, equity_cols):
        if 'Symbol' in self.df.columns:
            if 'Name' not in self.df.columns:
                return False, [['Name']]
            missing_columns = [col for col in crypto_cols if col not in self.df.columns]
            if not missing_columns:
                return True, []
        elif 'Ticker' in self.df.columns:
            missing_columns = [col for col in equity_cols if col not in self.df.columns]
            if not missing_columns:
                return True, []
        else:
            return False, [['Ticker or Symbol']]
...
class DirectionalMovementIndex:
    """
    A class to calculate and analyze the Directional Movement Index (DMI) and Average Directional Index (ADX)
    ...
    -----------
    df : pd.DataFrame
        The input DataFrame containing financial data with columns like 'High', 'Low', 'Close', and 'Date'.
    period : int
    ...
    """
    def __init__(self, df, period=14, adx_threshold=25):
        self.dataframe = Dataframe(df)
        if not self.dataframe.verify_period_sufficiency(period):
            raise ValueError(f"Dataframe does not have enough unique dates for the specified period of {period} days.")

        # Assign the DataFrame after verification
        self.df = self.dataframe.df
        self.period = period
        self.adx_threshold = adx_threshold

        # Proceed with the calculation of indicators and signals
...
class OnBalanceVolume:
    """
    A class to calculate and analyze On-Balance Volume (OBV) for a given dataset.
    ...
    --------
        __init__(df):
            Initializes the OnBalanceVolume with a given DataFrame.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date', 'Close', and 'Volume' for financial data analysis.
    """	
    def __init__(self, df):
        self.df = Dataframe(df).df 
        self._calculate_obv()
...
class AccumulationDistributionLine:
    """
    A class to calculate and analyze the Accumulation/Distribution (A/D) Line for a given dataset.
    ...
    --------
        __init__(df):
            Initializes the AccumulationDistributionLine with a given DataFrame.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date', 'Close', 'High', 'Low', and 'Volume' for financial data analysis.
    """	
    def __init__(self, df):
        self.df = Dataframe(df).df
...
class AroonIndicator:
    """
    A class to calculate and analyze the Aroon Indicator for a given dataset.
    ...
    Attributes:
    -----------
        dataframe (Dataframe): An instance of the Dataframe class that preprocesses and validates the input data.
        df (pandas.DataFrame): DataFrame containing financial data with 'Date', 'High', 'Low', and 'Close' columns.
        period (int): The period over which to calculate the Aroon indicators (default is 25 days).

    Methods:
    --------
        __init__(df, period=25):
            Initializes the AroonIndicator with a given DataFrame and period.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date', 'High', 'Low', and 'Close' for financial data analysis.
        period (int, optional): The period over which to calculate the Aroon indicators. Default is 25 days.

    Raises:
        ValueError: If the DataFrame does not contain enough unique dates for the specified period.
    """	
    def __init__(self, df, period=25):
...
class MACD:
    """
    A class to calculate and analyze the Moving Average Convergence Divergence (MACD) for a given dataset.
    ...
    Attributes:
    -----------
        df (pandas.DataFrame): DataFrame containing financial data with 'Date' and 'Close' columns.
    ...
    Methods:
    --------
        __init__(df, short_window=12, long_window=26, signal_window=9):
            Initializes the MACD with a given DataFrame and window periods for calculation.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date' and 'Close' for financial data analysis.
        short_window (int, optional): The period for the short-term EMA. Default is 12 days.
        long_window (int, optional): The period for the long-term EMA. Default is 26 days.
        signal_window (int, optional): The period for the Signal line EMA. Default is 9 days.
    """	
...
class RelativeStrengthIndex:
    """
    A class to calculate and analyze the Relative Strength Index (RSI) for a given dataset.
    ...
    Attributes:
    -----------
        dataframe (Dataframe): An instance of the Dataframe class for preprocessing and validating data.
        df (pandas.DataFrame): DataFrame containing financial data with 'Date' and 'Close' columns.
        period (int): The period for calculating RSI, default is 14 days.

    Methods:
    --------
        __init__(df, period=14):
            Initializes the RSI with a given DataFrame and period for calculation.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date' and 'Close' for financial data analysis.
        period (int, optional): The period for calculating RSI. Default is 14 days.
    """	
...
class FastStochasticOscillator:
    """
    A class to calculate and analyze the Fast Stochastic Oscillator for a given dataset.
    ...
    Methods:
    --------
        __init__(df, k_period=14, d_period=3):
            Initializes the Fast Stochastic Oscillator with a given DataFrame and periods for %K and %D lines.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date', 'High', 'Low', and 'Close' for financial data analysis.
        k_period (int, optional): The period for calculating the %K line. Default is 14 days.
        d_period (int, optional): The period for calculating the %D line. Default is 3 days.
    """	
    def __init__(self, df, k_period=14, d_period=3):
        self.dataframe = Dataframe(df)
        
        # The verification should account for the largest of the two periods to ensure all calculations can be performed
        max_period = max(k_period, d_period)
        if not self.dataframe.verify_period_sufficiency(max_period):
            raise ValueError(f"Dataframe does not have enough unique dates for the specified period of {max_period} days.")
...
class MovingAveragesAndBollingerBands:
    ...
    Methods:
    --------
        __init__(df, sma_period=20, ema_period=20, bb_period=20, bb_std=2):
            Initializes the MovingAveragesAndBollingerBands with a given DataFrame and periods for SMA, EMA, and Bollinger Bands.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date' and 'Close' for financial data analysis.
        sma_period (int, optional): The period for calculating the Simple Moving Average. Default is 20 days.
        ema_period (int, optional): The period for calculating the Exponential Moving Average. Default is 20 days.
        bb_period (int, optional): The period for calculating the Bollinger Bands. Default is 20 days.
        bb_std (float, optional): The number of standard deviations for the Bollinger Bands. Default is 2.
    """	
    def __init__(self, df, sma_period=20, ema_period=20, bb_period=20, bb_std=2):
        self.dataframe = Dataframe(df)
        
        # The verification should account for the largest of the three periods to ensure all calculations can be performed
        max_period = max(sma_period, ema_period, bb_period)
        if not self.dataframe.verify_period_sufficiency(max_period):
            raise ValueError(f"Dataframe does not have enough unique dates for the specified period of {max_period} days.")        
...
class AverageTrueRange:
    ...
    Methods:
    --------
        __init__(df, atr_period=14):
            Initializes the AverageTrueRange with a given DataFrame and ATR period.
    ...
    Parameters:
        df (pandas.DataFrame): DataFrame with columns 'Date', 'High', 'Low', and 'Close' for financial data analysis.
        atr_period (int, optional): The period over which to calculate the Average True Range. Default is 14 days.
    """	
    def __init__(self, df, atr_period=14):
        self.dataframe = Dataframe(df)
        if not self.dataframe.verify_period_sufficiency(atr_period):
            raise ValueError(f"Dataframe does not have enough unique dates for the specified atr_period of {atr_period} days.")        
...
def __dir__():
    return ['DirectionalMovementIndex', 'OnBalanceVolume', 'AccumulationDistributionLine', 'AroonIndicator', 'MACD', 'RelativeStrengthIndex', 'FastStochasticOscillator', 'MovingAveragesAndBollingerBands', 'AverageTrueRange']

__all__ = ['DirectionalMovementIndex', 'OnBalanceVolume', 'AccumulationDistributionLine', 'AroonIndicator', 'MACD', 'RelativeStrengthIndex', 'FastStochasticOscillator', 'MovingAveragesAndBollingerBands', 'AverageTrueRange']
