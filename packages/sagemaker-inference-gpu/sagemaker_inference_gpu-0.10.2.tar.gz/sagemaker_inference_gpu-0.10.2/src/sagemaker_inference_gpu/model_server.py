...
import sagemaker_inference_gpu
from sagemaker_inference_gpu import default_handler_service, environment, logging, utils
from sagemaker_inference_gpu.environment import code_dir
...
DEFAULT_MMS_CONFIG_FILE = pkg_resources.resource_filename(
    sagemaker_inference_gpu.__name__, "/etc/default-mms.properties"
)
MME_MMS_CONFIG_FILE = pkg_resources.resource_filename(
    sagemaker_inference_gpu.__name__, "/etc/mme-mms.properties"
)
DEFAULT_MMS_LOG_FILE = pkg_resources.resource_filename(
    sagemaker_inference_gpu.__name__, "/etc/log4j2.xml"
)
...
def start_model_server(handler_service=DEFAULT_HANDLER_SERVICE):
    ...
    Args:
        handler_service (str): python path pointing to a module that defines
            ...
            Defaults to ``sagemaker_inference_gpu.default_handler_service``.
