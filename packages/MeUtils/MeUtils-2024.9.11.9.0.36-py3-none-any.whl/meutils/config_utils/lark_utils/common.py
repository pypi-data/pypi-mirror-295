...
async def get_next_token_for_polling(feishu_url, check_token: Optional[Callable] = None, max_size: int = 10):  # 轮询
    df = await aget_spreadsheet_values(feishu_url=feishu_url, to_dataframe=True)
    api_keys = df[0]
    # logger.debug(df)
    api_keys = [k for k in api_keys if k]  # 过滤空值
    api_keys = np.random.choice(api_keys, size=max_size)

    num = len(api_keys)
    logger.debug(f"重复数：{num - len(set(api_keys))}")
