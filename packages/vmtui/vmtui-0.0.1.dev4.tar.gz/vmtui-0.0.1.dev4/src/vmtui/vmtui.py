...
class Domain:
    ...
    def set_graphics_password(self, password):
        self._read_xml()
        graphics = self.xmldesc.find('devices/graphics')
        if graphics is not None:                                
...
class ISOSelection(Screen):
    ...
    def read_images(self):
        # TODO: make robust against missing path/ini
...
class VMTUI(App):
    ...
    CSS_PATH = pkg_resources.resource_filename('vmtui', 'static/vmtui.css')
    ...
    def compose(self) -> ComposeResult:
        # called before on_mount
        self.log("VMTUI::compose()")
    ...
    def on_mount(self):
        # called after compose
        self.log("VMTUI::on_mount()")
    ...
    def update_domain_selection(self, message: VMList.Selected):
        self.log("update_status", message.domain.name())
            ...
            try:
                {
                    ...
                    "vnc": lambda : self.push_screen("pwin"),
                    "iso": lambda : self.push_screen("iso"),
                    "boot": dom.boot_iso,
