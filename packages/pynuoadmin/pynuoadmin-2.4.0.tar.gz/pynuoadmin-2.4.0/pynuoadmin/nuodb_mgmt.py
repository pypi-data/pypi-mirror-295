# (C) Copyright 2016-2023 Dassault Systemes SE.  All Rights Reserved.
...
class AdminConnection(object):
    ...
    def _send_request(self, method, *resource_path, **kwargs):
        ...
        # We must use an integer in request timeouts
        request_kwargs['timeout'] = int(kwargs.get('timeout', self.response_timeout))
    ...
    def hotcopy_database_simple(
            ...
            max_rate=None, test_mode=None,
            timeout_sec=None, check_contains_all_sgs=None):
        """
        ...
        :param str default_journal_dir: the default journal location to use if
                                        one is not specified in `journal_dirs`;
                                        `default_backup_dir` must be specified
        ...
        :param int test_mode: If specified, put hot copy into "test mode."
        :param int timeout_sec: if specified, the number of seconds to wait for
                                the hot-copy request to complete
        :param bool check_contains_all_sgs:
                verify that all storage groups configured in the database are
                covered by the supplied archive IDs; enabled by default
        ...
        if test_mode is not None:
            payload['testMode'] = test_mode
        # specify hotcopy timeout and bump HTTP response timeout by that value
        if timeout_sec is not None:
            payload['timeoutSec'] = int(timeout_sec)
            response_timeout = int(timeout_sec) + self.response_timeout
        else:
            response_timeout = self.response_timeout
        if check_contains_all_sgs is not None:
            payload['checkContainsAllSgs'] = check_contains_all_sgs
    ...
    def hotcopy_database(self, db_name, type, shared=False,
                         ...
                         max_rate=None, test_mode=None, timeout_sec=None,
                         check_contains_all_sgs=None):
        """
    ...
    def get_hotcopy_statuses(self, db_name, timeout_sec=None, min_seconds_running=None):
        ...
        # Wait for response from admin for the admin's timeout plus response_timeout
        if timeout_sec is not None:
            params['timeoutSec'] = int(timeout_sec)
            response_timeout = int(timeout_sec) + self.response_timeout
        else:
            response_timeout = self.response_timeout
    ...
    def update_data_encryption(self, db_name, new_password,
        ...
        if timeout_sec is None:
            timeout_sec = 10
        params = dict(configUpdateTimeoutSec=int(timeout_sec))
    ...
    def get_admin_logs(self, dump_threads=False, include_core_files=False,
        ...
        return Downloader(self._get(
            ...
            params=dict(includeThreadDump=dump_threads,
                        ...
                        ignore=ignore,
                        excludeSensitiveFiles=not include_sensitive_files),
            stream=True))
    ...
    def _add_trusted_certificate(self, alias, certificate_pem=None,
        ...
        response_timeout = self.response_timeout
        if timeout is not None:
            params['waitFullyReplicatedSec'] = int(timeout)
    ...
    def stream_async_request(self, start_id, request_type, connect_timeout=5.0,
                             read_timeout=None):
        # Returns a Generator that yields bytes buffers.
    ...
    def _stream_receive(session):
        """Returns a generator that yields bytes buffers."""
        # In pynuodb >=3.x, Session.stream_recv() will do this
        if hasattr(session, 'stream_recv'):
            return session.stream_recv()
        # No?  We have to do it ourselves
        return AdminConnection.__pynuo2_stream_generator(session)

    @staticmethod
    def __pynuo2_stream_generator(session):
        # stream bytes from socket until it is closed using pynuodb 2.x
        try:
            ...
            while len(msg) != 0:
                if session._Session__cipherIn is not None:
                    ...
                    if sys.version[0] == '3':
                        # In pynuodb 2.x, transform() returns a latin-1 str
                        # Convert it back into bytes in Python 3.
    ...
    @staticmethod
    def _liveness_threshold_generator(wait_for_acks=False, limit=None):
        if wait_for_acks:
    ...
    def check_database(self, db_name, check_running=False, check_syncing=False, num_processes=None,
        ...
        :param str db_name: The name of the database to check
        :param bool check_running: Check that the database is in the RUNNING state
        :param bool check_syncing: Check that all processes in the database are in the SYNCING or RUNNING state
        :param int num_processes: Check that the database contains the specified number of processes
        :param float check_liveness: Check that all processes have responded within the specified number of seconds
        :param float timeout: How long (in seconds) to wait for database check to pass
        :param bool wait_forever: Wait forever for database check to pass. Overriden by an explicit value for `timeout`
        :param bool wait_for_acks: If specified along with `timeout`, wait for messages to be received from all
        processes while the command is running
        :param fail_on_process_exit: If specified along with `timeout`, fail-fast if a process exits while the command
        is running

        :return: The database object, if all checks succeeded

        :raises RuntimeError: A check failed for a reason that may self recover
        :raises UnrecoverableException: A check failed for a reason that is not expected to self recover
        """
        if timeout is None and wait_forever:
            timeout = AdminConnection._FOREVER
        if timeout is not None:
            liveness_gen = self._liveness_threshold_generator(wait_for_acks, check_liveness)
            expected_incarnation = None
            if fail_on_process_exit:
                database = self.get_database(db_name)
                expected_incarnation = database.incarnation
                if database.state == 'NOT_RUNNING':
                    expected_incarnation = (expected_incarnation[0] + 1, 0)
            return retry(
                lambda: self._check_database(
                    db_name, check_running, check_syncing, num_processes,
                    check_liveness=next(liveness_gen),
                    expected_incarnation=expected_incarnation),
                timeout)
        return self._check_database(
            db_name, check_running, check_syncing, num_processes, check_liveness)

    def _check_database(self, db_name, check_running=False, check_syncing=False, num_processes=None,
                        check_liveness=None, expected_incarnation=None):
        database = self.get_database(db_name)
        if check_running and database.state != 'RUNNING':
            raise RuntimeError('Database {} is not RUNNING: {}'.format(
                db_name, database))
        if expected_incarnation is not None and expected_incarnation != database.incarnation:
            raise UnrecoverableException(
                'Process failures detected due to unexpected incarnation for database {}: expected={}, actual={}'.format(
                    db_name, expected_incarnation, database.incarnation))

        # check that all processes have expected state
        processes = list(self.get_processes(db_name))

        if check_syncing:
            non_running_or_syncing = [str(p) for p in processes
                                      if p.durable_state != 'MONITORED'
                                      or p.engine_state not in ('RUNNING', 'SYNCING')]
            if len(non_running_or_syncing) != 0:
                raise RuntimeError(
                    'Database {} has processes that are not syncing or running: \n{}'.format(
                        db_name, '\n'.join(non_running_or_syncing)))
        elif check_running or num_processes:
        ...
        if check_liveness is not None:
            check_liveness = float(check_liveness)
    ...
    def check_process(self, start_id, check_running=False, check_exited=False,
        ...
        :param str start_id: The Start ID of the process to check
        :param bool check_running: Check that the process is in the RUNNING state
        :param bool check_exited: Check that the process has exited
        :param float check_liveness: Check that the process has responded within the specified number of seconds
        :param float timeout: How long (in seconds) to wait for process check to pass
        :param bool wait_forever: Wait forever for process check to pass. Overriden by an explicit value for `timeout`
        :param bool wait_for_ack: If specified along with `timeout`, wait for a message to be received from the process
        while the command is running

        :return: The process object, if all checks succeeded
        ...
        proc = None
        if timeout is not None:
            proc = retry(lambda: self.get_process(start_id), timeout)
        if not check_exited and type(proc) is ExitedProcess:
            raise RuntimeError(
                'Process with Start ID {} has already exited: \'{}\''
                .format(start_id, proc.reason))
        if timeout is not None:
            liveness_gen = self._liveness_threshold_generator(wait_for_ack, check_liveness)
            return retry(
                lambda: self._check_process(
                    start_id, check_running, check_exited, next(liveness_gen)),
                timeout)
    ...
    def _check_process(self, start_id, check_running, check_exited,
        ...
        if not check_exited and type(proc) is ExitedProcess:
            raise UnrecoverableException(
                'Process with Start ID {} has already exited: \'{}\''
                .format(start_id, proc.reason))
        if check_exited and type(proc) is not ExitedProcess:
            raise RuntimeError(
        ...
        if check_liveness is not None:
            if proc.last_ack is None:
                # TODO: This error is misleading if the process has never successfully acked
                raise RuntimeError(
    ...
    def check_server(self, check_active=False, check_connected=False, timeout=None, wait_forever=False,
                     check_converged=False):
        """
        Check the status of the API server

        :param bool check_active: Check that server is ACTIVE
        :param bool check_connected: Check that server is CONNECTED, to verify that it is reachable by its advertised
        address
        :param float timeout: How long (in seconds) to wait for server check to pass
        :param bool wait_forever: Wait forever for server check to pass. Overriden by an explicit value for `timeout`
        :param bool check_converged: Check that server has the same commit index as the leader

        :return: The server object, if all checks succeeded
        ...
        if timeout is not None:
            return retry(
                lambda: self._check_server(
                    check_active, check_connected, check_converged),
                timeout)
        return self._check_server(
            check_active, check_connected, check_converged)

    def _check_server(self, check_active=False, check_connected=False,
                      check_converged=False):
        ...
        for server in servers:
            if server.is_local:
                api_server = server
            if server.role == 'LEADER' and server.connected_state == "Connected":
                leader = server
                leader_id = server.id

        if api_server is None:
            raise RuntimeError('API server info not found')

        msg = ''
        # if `check_active`, check that server is ACTIVE
        if check_active and api_server.raft_state != 'ACTIVE':
            msg += api_server.id + ' is not ACTIVE\n'
        # if `check_connected`, check that server is CONNECTED
        if check_connected and api_server.connected_state != 'Connected':
            msg += api_server.id + ' is not reachable by its advertised address\n'
        # if `check_converged`, check that server is caught up with leader
        if check_converged:
            if api_server.leader != leader_id:
                msg += '{} has inconsistent leader: expected={}, actual={}\n'.format(
                    api_server.id, api_server.leader, leader_id)
            elif leader is None:
                msg += api_server.id + ' has no leader\n'
            elif api_server.commit_index < leader.commit_index:
                msg += '{} has lower commit index than the leader {}:\n'.format(
                    api_server.id, leader.id)
                msg += '  {}->{}\n'.format(api_server.id, api_server.commit_index)
                msg += '  {}->{}\n'.format(leader.id, leader.commit_index)
            elif api_server.commit_index is None:
                msg += '{} and {} have unknown commit indexes\n'.format(
                    api_server.id, leader.id)
            elif api_server == leader:
                # if the API server is the leader, make sure it is connected to
                # a majority of servers; this is meant to detect the case where
                # the API server would trivially pass the `check_converged`
                # test because it thinks it is the leader while being
                # partitioned from the majority of the domain (leaders do not
                # step down until they receive a Raft message with a higher log
                # term, which may not happen until a partition is healed)

                # exclude evicted servers from consideration, since their
                # absence does not affect quorum
                voters = [s for s in servers if not s.is_evicted]
                disconnected_voters = [s.id for s in voters
                                       if s.connected_state != 'Connected']
                if len(disconnected_voters) >= len(voters) / 2.0:
                    # display all disconnected servers, including evicted
                    disconnected = [s.id for s in servers
                                    if s.connected_state != 'Connected']
                    msg += 'Servers not connected to leader {}: {}\n'.format(
    ...
    def check_servers(self, check_active=False, check_connected=False,
        ...
        :param bool check_active: Check that all servers are ACTIVE
        :param bool check_connected: Check that all servers are CONNECTED to the server performing check
        :param bool check_leader: Check that all servers agree on a leader
        :param int num_servers: Check that membership size equals specified number
        :param float timeout: How long (in seconds) to wait for server check to pass
        :param bool wait_forever: Wait forever for server check to pass. Overriden by an explicit value for `timeout`
        :param float check_liveness: Check that all servers have responded within the specified number of seconds
        :param bool wait_for_acks: If specified along with `timeout`, wait for messages to be received from all servers
        while the command is running
        :param check_converged: Check that all servers have the same commit index. This check will only work if
        check_leader is also True.

        :return: The list of servers, if all checks succeeded
    ...
    def _check_servers(self, check_active=False, check_connected=False,
        ...
        for server in servers:
            ...
            if server.connected_state != 'Connected' or (
                    server.last_ack is not None and
                    check_liveness is not None and
                    server.last_ack > check_liveness):
            ...
            commit_indexes[server.commit_index] = commit_indexes.get(
                server.commit_index, set())
            commit_indexes[server.commit_index].add(server.id)
        ...
        if (check_connected or check_liveness is not None) and len(disconnected) != 0:
        ...
        elif check_leader and check_converged and (
                len(commit_indexes) != 1 or None in commit_indexes):
            # unpack unique (leader, term) from key set
            leader_term, = list(leader_terms.keys())
            leader, _ = leader_term
            msg += 'Non-unique commit indexes:\n'
            for commit_index, server_ids in commit_indexes.items():
                # denote leader with brackets
                server_list = ','.join(
                    ('<{}>'.format(s) if s == leader else s)
                    for s in sorted(server_ids))
                msg += '  (commit={})->[{}]\n'.format(commit_index, server_list)
...
def retry(check_fn, timeout, initial_delay=0.25, max_delay=5):
    """
     Try to run a function until it succeeds. The function is retried, with exponential delay, until it either:
         1. The function returns without raising an exception
         2. The function raises :py:class:`UnrecoverableException`
         3. The `timeout` seconds have passed

     :param callable check_fn: A no argument function to call
     :param float timeout: How long (in seconds) to keep calling the function
     :param float initial_delay: The minimum amount of time to wait between retries
     :param float max_delay: The maximum amount of time to wait between retries

     :return: The return value of the function, if it succeeded

     :raises Exception: If the function did not succeed, the exception raised by it on its final iteration
     """
    delay = initial_delay
    start_time = time.time()
    while True:
...
class Server(Entity):
    ...
    def labels(self):
        """
        Get any admin labels applied to the server.
...
class Process(Entity):
    ...
    def journal_recovery_progress(self):
        """
        Fraction of journal that has been recovered.
    ...
    @property
    def archive_save_progress(self):
        """
        Fraction of objects modified by journal recovery that have been saved.

        :returns float:
        """

        return self.get_nested('archiveSaveProgress')
