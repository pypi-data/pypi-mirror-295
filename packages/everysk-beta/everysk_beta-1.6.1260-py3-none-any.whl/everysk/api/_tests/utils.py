...
class UtilsTestCase(TestCase):
    ...
    def test_negative_sleep_value(self, mock_sleep):
        mock_sleep.side_effect = lambda x: exec('raise ValueError("sleep length must be non-negative")') if x < 0 else None
        with self.assertRaises(ValueError):
            sleep(-1)

    @mock.patch("time.sleep")
    def test_invalid_sleep_value(self, mock_sleep):
        mock_sleep.side_effect = lambda x: exec('raise TypeError("str object cannot be interpreted as an integer")') if isinstance(x, str) else None
