...
class Anomaly_Insertion(object):
    ...
    def perlin_noise_pattern(self, image, anom_source_img=None):
        """
        ...
        Args:
            image (PIL.Image, mode=RGB): The original image.
    ...
    def superpixel_anomaly(self, image, anom_source_img=None):
        ...
        uniq_v                  =       np.random.choice(no_of_seg, int(np.ceil(len(no_of_seg)*0.1))) # uniq_v = np.random.choice(no_of_seg)   
    ...
    def perlin_with_roi_anomaly(self, img, anom_source_img=None, use_mask= True, transparency_range=[0.15, 1.]):
        ...
        while (np.sum(mask)==0):
            ...
            if use_mask:
                ...
                target_foreground_mask  = np.ones((img.shape[0], img.shape[1]))
            ...
            # step 3. blending image and anomaly source
            anomaly_source_img = (((- mask_expanded + 1) * img) + anomaly_source_img).astype(np.uint8)
            mask               = np.expand_dims((mask*255).astype(np.uint8),axis=-1)
            _check+=1
            if _check>=3: use_mask = False 
        ...
        if in_fg_region:
            ...
            if (np.sum(intersect_mask) > int(2 / 3 * np.sum(aug_mask == 255))):
                ...
                if len(widths) == 0 or len(heights) == 0:  # no contours
                    ...
                    return n_image, np.expand_dims(aug_mask_shifted,axis=-1)
    ...
    def simplex_noise_anomlay(self, image):
        ...
        h_noise = np.random.randint(10, int(img.shape[0]//2))
        w_noise = np.random.randint(10, int(img.shape[0]//2))
        start_h_noise = np.random.randint(1, img.shape[0] - h_noise)
        start_w_noise = np.random.randint(1, img.shape[0] - w_noise)
    ...
    def fract_aug(self, image, initial_point=(0,0), iterations=10000, anom_source_img=None):
        ...
        while (np.sum(mask_img)<=0.2*(np.max(mask_img.shape))):
            ...
            plt.axis('off')
            ''' os.makedirs('pail/test_img', exist_ok=True)
            plt.savefig('pail/test_img/fac_img.png') '''
            fig             = plt.gcf() 
            ...
            #os.remove('pail/test_img/fac_img.png')
            #x_vals_abs, y_vals_abs  = abs_val_norm(x_vals), abs_val_norm(y_vals) 
            #### ############
