...
class ChangesApplyAction(BaseAction):
    ...
    def run(self, request, context):
        ...
        if change.source == ChangeSource.UPDATE_APPLICATION:
            # Льготы в изменениях
            privileges = json.loads(change.data).get('DeclarationPrivilege')
            ...
            if (
                change.state == ChangeStatus.ACCEPT and privileges and
                [priv for priv in privileges
                 if priv.get('conc_unit') or priv.get('conc_privilege')]
                and change.declaration.status.code in (
                    DSS.status_queue_privilege())
            ):
                change.declaration.change_status(
        ...
        # (например послали заявку с другими данными ФИО и с тем же externalID,
            ...
            change.declaration.status.code in (DSS.REFUSED, DSS.ARCHIVE)
        ):
            ...
            change.declaration.date = change.create.date()
            change.declaration.save()
