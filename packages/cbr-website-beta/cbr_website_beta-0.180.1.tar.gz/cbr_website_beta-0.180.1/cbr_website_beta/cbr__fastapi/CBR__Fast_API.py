...
class CBR__Fast_API(Fast_API):
    ...
    def s3_db_server_requests(self):
        ...
        s3_db_server_requests = S3_DB__Server_Requests(**kwargs)    # set up tasks, including creating target bucket if it doesn't exist
        s3_db_server_requests.setup()
        return s3_db_server_requests
    ...
    def background_task__log_request_data(self,request: Request, response: Response):
        ...
        try:                                                                                        # we need to capture this error since an exception here will bring the server down
            ...
            s3_db              = cbr_fast_api.s3_db_server_requests()
