...
class GammaExposure:
    ...
    @classmethod
    def run(cls, client, symbol, num_strikes=200):
        """
        Creates an instance and directly calculate the gamma exposure in one step.
        :param client: Initialized Schwab API client
        ...
        """
        instance = cls(client)
        return instance.get_gamma_exposure(symbol, num_strikes)

    def get_option_chains(self, symbol, num_strikes=200):
        """
        Fetchs option chains for the given symbol.
        :param symbol: Stock ticker symbol (e.g., 'AAPL')
        :param num_strikes: Number of strikes closest to the money to fetch
