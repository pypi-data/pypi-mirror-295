...
from pyeqx.common import FunctionExecuteResult
...
class Process(ProcessBase):
    ...
    def _write_bronze_ingestion_log(self, name: str, data: dict[str, Any]):
        try:
            log_data = create_dataframe(
                spark=self.operation.get_current_spark_session(),
                schema=BRONZE_INGESTION_LOG_SCHEMA,
                datas=[data],
            )
    ...
    def _read_destination(
        self, name: str, schema: StructType = None, options: dict = {}
    ):
        """
    ...
    def _write_destination(self, name: str, data: DataFrame, options: dict = {}):
        """
        ...
        return self.operation.write_destination(
            data=data, data_module=data_config.dest, options=options
        )
