...
class ProdDataManager:
    ...
    def setTransformationFileStatus(self, lfn, status):
        if status == "PROCESSED":
            self.transClient.setFileStatus(
                self.TransformationID, lfn, TransformationFilesStatus.PROCESSED
            )
        elif status == "UNUSED":
            self.transClient.setFileStatus(
                self.TransformationID, lfn, TransformationFilesStatus.UNUSED
            )
        else:
            DIRAC.gLogger.warn(f"Not allowed File Status {status}")
