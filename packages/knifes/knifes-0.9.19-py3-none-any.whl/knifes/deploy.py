...
def autostart_at_boot(project_dir, log_dir, app_name):
    ...
    ret = subprocess.run(f'systemctl daemon-reload && systemctl enable {app_name}.gunicorn.service', shell=True, executable='/bin/bash', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if ret.returncode != 0:
        print_err(ret.stderr.decode())
        print_err('设置开机自启动失败')
        raise SystemExit
