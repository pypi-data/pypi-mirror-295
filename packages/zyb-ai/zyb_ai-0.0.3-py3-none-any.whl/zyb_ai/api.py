...
def img_transfer(img, style, dst='./'):
    ...
    image_name = os.path.split(img)[-1].split('.')[0] + '_transfer_' + str(int(time.time())) + '.png'
...
def text2image(text, name=None, dst='./'):
    ...
    if name is None:
        name = 'text2image_' + str(int(time.time())) + '.png'
...
def img_cartoon(img, dst='./'):
    ...
    if not data:
        raise ValueError("获取数据失败")
    image_data = base64.b64decode(data.get("image"))
    image_name = os.path.split(img)[-1].split('.')[0] + '_cartoon_' + str(int(time.time())) + '.png'
    filename = os.path.join(dst, image_name)
    with open(filename, 'wb') as fp:
        fp.write(image_data)
    return image_name
...
def face_aging(img, age, dst='./'):
    ...
    image_name = os.path.split(img)[-1].split('.')[0] + '_aging_' + str(int(time.time())) + '.png'
    filename = os.path.join(dst, image_name)
    with open(filename, 'wb') as fp:
        fp.write(image_data)
    return image_name
