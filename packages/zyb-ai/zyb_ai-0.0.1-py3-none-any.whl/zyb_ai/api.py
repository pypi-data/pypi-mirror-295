...
def img_transfer(img, style, dst='./'):
    ...
    with open(img, 'rb') as fp:
        content = fp.read()
    res = requests.post(url, data={"image": base64.b64encode(content), "option": style})
    result = res.json()
    ...
    filename = os.path.join(dst, os.path.split(img)[-1].split('.')[0] + '_transfer_' + str(int(time.time())) + '.png')
...
def text2image(text, name=None, dst='./'):
    ...
    err_no = result.get("errNo")
    if err_no > 0:
        raise ValueError(result.get("errMsg", "接口调用错误"))
    data = result.get("data")
    if not data:
        raise ValueError("获取数据失败")
    pid = data[0]
...
def img_cartoon(img, dst='./'):
    ...
    with open(img, 'rb') as fp:
        content = fp.read()
    res = requests.post(url, json={"image": base64.b64encode(content)})
    result = res.json()
    err_no = result.get("errNo")
    if err_no > 0:
        raise ValueError("%s: %s" % (err_no, result.get("errMsg", "接口调用错误")))
    ...
    if not data:
        raise ValueError("获取数据失败")
    image_data = base64.b64decode(data.get("image"))
    filename = os.path.join(dst, os.path.split(img)[-1].split('.')[0] + '_cartoon_' + str(int(time.time())) + '.png')
    with open(filename, 'wb') as fp:
        fp.write(image_data)
    return filename
...
def face_aging(img, age, dst='./'):
    ...
    with open(img, 'rb') as fp:
        content = fp.read()
    res = requests.post(url, json={"image": base64.b64encode(content), "age": age})
    result = res.json()
    err_no = result.get("errNo")
    if err_no > 0:
        raise ValueError("%s: %s" % (err_no, result.get("errMsg", "接口调用错误")))
    ...
    filename = os.path.join(dst, os.path.split(img)[-1].split('.')[0] + '_aging_' + str(int(time.time())) + '.png')
    with open(filename, 'wb') as fp:
        fp.write(image_data)
    return filename
