...
class LMNN:
    ...
    def mahalanobis_distance(self, x1, x2, L):
        """
        Compute the Mahalanobis distance between two points x1 and x2 with a transformation matrix L.
        ...
        """
        diff = self.vector_subtract(x1, x2)
        transformed_diff = self.transform_point(diff, L)
        return sum(d**2 for d in transformed_diff)
...
class Visualization:
    ...
    def plot_similarity_matrix(self, dataset):
        """
        ...
        for i in range(n):
            for j in range(n):
                similarity_matrix[i][j] = 1 / (1 + self.distance_function(dataset[i], dataset[j]))
