...
class Distance(ABC):
	...
	def checkData(self,data1,data2):
		str_raise=f'In {self.__class__.__name__} class, '
		if self.__class__.__name__=='Euclidean'or self.__class__.__name__=='L2'or self.__class__.__name__=='Manhattan'or  self.__class__.__name__=='L1'or  self.__class__.__name__=='Minkowski' :
			if len(data1) != len(data2):
				raise ValueError(str_raise+'points must have the same dimensions')
		if self.__class__.__name__=='Levenshtein':
			pass
		if self.__class__.__name__=='Hamming':
			if len(data2) != len(data2):
				raise ValueError(str_raise+'strings must be of the same length')
		if self.__class__.__name__=='InverseTanimoto':
