...
class FilterDateTime:
    ...
    def to_datetime(self, exact_reference: Optional[datetime] = None) -> datetime:
        """
        ...
        if self.time_reference:
            if (
                ...
                if exact_reference is None:
                    err = "exact_reference must be provided when using MATERIALIZATION_START_TIME or MATERIALIZATION_END_TIME"
                    raise ValueError(err)
                return exact_reference + self.offset
                ...
                return datetime(2, 1, 1).replace(tzinfo=timezone.utc)
            elif self.time_reference == TectonTimeConstant.UNBOUNDED_FUTURE:
                return datetime(2500, 1, 1).replace(tzinfo=timezone.utc)
            else:
