...
def get_time_window_from_data_source_node(
    ...
    if using_select_range:
        ...
        ftl_start = feature_time_limits.start if feature_time_limits else None
        ftl_end = feature_time_limits.end if feature_time_limits else None

        new_start = start_date_time.to_datetime(ftl_start).replace(tzinfo=timezone.utc)
        new_end = end_date_time.to_datetime(ftl_end).replace(tzinfo=timezone.utc)
