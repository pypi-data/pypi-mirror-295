...
from tecton.repo_utils import maybe_load_repo_config_or_default
...
def process_files(
    py_files: List[Path], repo_root: Path, repo_config_path: Optional[Path]
) -> Dict[str, List[Dict[str, any]]]:
    maybe_load_repo_config_or_default(repo_config_path)
...
def print_migration_progress(all_objects: Dict[str, BaseTectonObject]):
    ...
    safe_print(f"  Step {len(steps) + 1}: Migrate all other imports.")
...
def print_interactive_migration_guidance(all_objects: Dict[str, List[Dict[str, any]]], repo_root: str) -> None:
    ...
    while step <= len(OBJECT_TO_MIGRATE_TO_GUIDANCE_STEPS):
        ...
        # Display options
        if index > 0 and index < len(objects_to_migrate) - 1:
            options = f"Press n for next {obj_type}, p for previous {obj_type}, q to quit."
        elif index == 0:
            options = f"Press n for next {obj_type}, q to quit."
        elif index == len(objects_to_migrate) - 1:
            options = f"Press p for previous {obj_type}, q to quit."
        else:
        ...
        # Handle user input with Click
        user_input = click.prompt("Your choice", type=str, default="n").lower()
        ...
        if user_input == "n":
            if index < len(objects_to_migrate) - 1:
                ...
                safe_print(
                    f"Step {step}: Migrate {_fco_to_pretty_print(obj_type, len(objects_to_migrate))} complete. Run `tecton plan` to ensure your changes do not trigger unintended changes, then run `tecton upgrade` for the next set of steps."
                )
                return
...
def upgrade(path: Optional[Path] = None, repo_config_path: Optional[Path] = None, non_interactive: bool = False):
    ...
    If REPO_CONFIG_PATH is provided, use the repo config file at the specified path.
    If REPO_CONFIG_PATH is not provided, use the repo config file at the base of the repo.
    """
    ...
    with conf._temporary_set("TECTON_SKIP_OBJECT_VALIDATION", True), conf._temporary_set(
        ...
        all_objects = process_files(py_files, Path(repo_root), repo_config_path)
