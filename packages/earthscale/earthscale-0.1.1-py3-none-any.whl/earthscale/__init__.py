...
def _load_dataset(
    name: str, domain: DatasetDomain | None, version: str | None
) -> Dataset:
    supabase_client = get_supabase_client()
    dataset_repo = DatasetRepository(supabase_client, domain=domain, version=version)
    return dataset_repo.get_by_name(name)
