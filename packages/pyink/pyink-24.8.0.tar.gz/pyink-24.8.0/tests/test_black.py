...
class FakeContext(click.Context):
    ...
    def __init__(self) -> None:
        ...
        self.command: click.Command = pyink.main
...
class BlackTestCase(BlackBaseTestCase):
    ...
    def check_features_used(self, source: str, expected: Set[Feature]) -> None:
        ...
        actual = pyink.get_features_used(node)
    ...
    def test_reformat_one_with_stdin_filename(self) -> None:
        with patch(
            ...
            fsts.assert_called_once_with(
                fast=True, write_back=pyink.WriteBack.YES, mode=DEFAULT_MODE, lines=()
            )
    ...
    def test_line_ranges_with_stdin(self) -> None:
        ...
        runner = BlackRunner()
        result = runner.invoke(
            pyink.main, ["--line-ranges=1-1", "-"], input=BytesIO(code.encode("utf-8"))
        )
    ...
    def test_line_ranges_in_pyproject_toml(self) -> None:
        ...
        result = BlackRunner().invoke(
            pyink.main, ["--code", "print()", "--config", str(config)]
        )
...
class TestASTSafety(BlackBaseTestCase):
    def check_ast_equivalence(
        ...
        pyink.parse_ast(source)
        pyink.parse_ast(dest)
        if should_fail:
            with self.assertRaises(ASTSafetyError):
                pyink.assert_equivalent(source, dest)
        else:
            pyink.assert_equivalent(source, dest)
